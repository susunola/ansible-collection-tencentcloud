...
It prints one row per item with its age, its response time, and a status:
`OK` (answered in time), `WAITING` (unanswered, still inside the window), or
`BREACH` (unanswered and overdue, or answered late). Authentication is
optional — `GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token` — but
unauthenticated requests are rate-limited to 60 per hour, which a repo with a
few hundred issues will exhaust in one run.

`--check` is the CI form. It fails only on *open* breaches, so a historical
breach does not keep the job red forever: answering, closing, or labelling
the item clears it.
...
| --- | --- | --- |
| `triage` | Nobody has looked at it yet. This is the queue. | automatic on open, removed at first response |
...
| `dependencies` | A dependency version bump, usually from dependabot. | automatic |
| `sla-breach` | First response is overdue. | maintainer, when `--check` goes red |
