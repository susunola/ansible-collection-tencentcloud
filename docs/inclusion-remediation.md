...
1,025 modules, not "1000+" against a 524-module self-check, and the 519 lines
...
| "zero ... markers across all 524 modules" | 1,025 modules: 456 write + 569 `_info`. |
...
That last one hid the actual defect: **4,559 `validate-modules` errors**.
...
| Zero `validate-modules` errors (was 4,559) | `ansible-test sanity --test validate-modules` |
| The ignore files carry no must-not-ignore code and every entry is justified | `python scripts/check_sanity_ignore.py` |
| Ignore entries are down from 2,076 to 204 (519 to 51 per file) | `python scripts/check_sanity_ignore.py --print` |
| No module documents options as single-line flow mappings (was 643) | `python scripts/normalize_module_docs.py --check` |
| All 1,025 modules document check mode and idempotency | `python scripts/add_module_attributes.py --check` |
...
- **1,511 option descriptions (26%) are still under 25 characters**
  ("ALB ID.", "Desired state."). They are readable now, not informative.
  1,401 of them are in write modules, spread over 442 modules at about three
