# 维护一个 991 模块的腾讯云 Ansible 集合：一个月里可复用的经验

> 结论先行：集合大到跑不了、又不能被 CI 跑的时候，能救命的不是更多测试，而是一层"不执行也能查"的静态门禁。本文把这套做法和踩过的坑写成可复用经验，给后续接手或做同类集合的人。

## 背景

`susunola.tencentcloud` 是一个面向腾讯云的 Ansible 集合：991 个插件（456 个写模块 + 535 个 `_info` 只读模块）、68 个 role、从官方 SDK 自动生成。问题不在"写不出模块"，而在"模块一多，引用关系就管不住"——文档、示例 playbook、移植指南里写死的模块名、选项名、role 变量，任何一次重命名都会让它们静默失效，而 CI 全绿。
