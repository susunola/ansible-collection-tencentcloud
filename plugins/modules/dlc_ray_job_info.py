...
def id_request(cls, job_id):
    request = cls(); request.Id = job_id; return request


def history_request(models, p, page):
    request = id_request(models.GetRayJobHistoryRequest, p["ray_job_id"]); request.Page, request.PageSize = page, p["page_size"]; return request


def pods_request(models, p, page):
    request = id_request(models.GetRayJobPodsRequest, p["ray_job_id"]); request.Page, request.PageSize = page, p["page_size"]
...
def event_request(models, p, context=None):
    request = id_request(models.GetRayJobEventRequest, p["ray_job_id"]); request.PageSize = p["page_size"]
    if context: request.Context = context
    if p.get("start_time") is not None: request.StartTime = p["start_time"]
    if p.get("end_time") is not None: request.EndTime = p["end_time"]
    if p.get("event_type") is not None: request.EventType = p["event_type"]
    return request
...
def page_number_read(module, function, request_fn, item_field, p):
    ...
    for page in range(1, p["max_pages"] + 1):
        response = module.sdk_call(function, request_fn(page)); items = getattr(response, item_field) or []
...
def event_read(module, client, models, p):
    ...
    for _ in range(p["max_pages"]):
        ...
        if response.ListOver: break
        context = response.Context
        if not context: break
        if context in seen: module.fail_json(msg="DLC Ray job event pagination repeated a context token", ray_job_id=p["ray_job_id"], context=context)
        seen.add(context)
    else: truncated = True
    return values, truncated
...
def run_module():
    spec = {
        "ray_job_id": {"required": True}, "include_history": {"type": "bool", "default": False},
        "include_events": {"type": "bool", "default": False}, "include_pods": {"type": "bool", "default": False},
        "include_yaml": {"type": "bool", "default": False}, "start_time": {"type": "int"}, "end_time": {"type": "int"},
        "event_type": {}, "page_size": {"type": "int", "default": 100}, "max_pages": {"type": "int", "default": 100},
    }
    ...
    if p.get("event_type") is not None and not re.match(r"^[A-Za-z]+$", p["event_type"]): module.fail_json(msg="event_type must contain ASCII letters only")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        response = module.sdk_call(client.GetRayJob, id_request(models.GetRayJobRequest, p["ray_job_id"])); detail = response._serialize(allow_none=True); request_id = detail.pop("RequestId", None)
        history, events, pods, yaml = [], [], [], None; truncated = {"history": False, "events": False, "pods": False}
        if p["include_history"]: history, truncated["history"] = page_number_read(module, client.GetRayJobHistory, lambda page: history_request(models, p, page), "Items", p)
        if p["include_events"]: events, truncated["events"] = event_read(module, client, models, p)
        if p["include_pods"]: pods, truncated["pods"] = page_number_read(module, client.GetRayJobPods, lambda page: pods_request(models, p, page), "Items", p)
        if p["include_yaml"]: yaml = module.sdk_call(client.GetRayJobYaml, id_request(models.GetRayJobYamlRequest, p["ray_job_id"])).Yaml
