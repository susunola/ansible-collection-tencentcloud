...
RETURN = r'''
cloud_native_api_gateway_canary_rules:
  description: Matching TSE cloud native api gateway canary rules.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeCloudNativeAPIGatewayCanaryRules, request),
        lambda response: response.Result.CanaryRuleList if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    cloud_native_api_gateway_canary_rules = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, cloud_native_api_gateway_canary_rules=cloud_native_api_gateway_canary_rules,
