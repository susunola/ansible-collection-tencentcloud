...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.trabbit_serverless_vhost_info
    description: Gather information about Tencent Cloud TRABBIT serverless vhosts.
...
RETURN = r"""virtual_host:
  ...
  sample:
    ...
    Description: Renamed production workloads
...
def describe_request(models, p, offset=0):
    r = models.DescribeRabbitMQServerlessVirtualHostRequest()
    r.InstanceId, r.VirtualHost, r.Offset, r.Limit = p["instance_id"], p["name"], offset, 100
    return r


def create_request(models, p):
    r = models.CreateRabbitMQServerlessVirtualHostRequest()
    r.InstanceId, r.VirtualHost, r.Description = p["instance_id"], p["name"], p["description"]
    r.TraceFlag = p.get("trace_enabled") if p.get("trace_enabled") is not None else False
    r.MirrorQueuePolicyFlag = p["mirror_queue_policy"]
    return r


def update_request(models, p):
    r = models.ModifyRabbitMQServerlessVirtualHostRequest()
    r.InstanceId, r.VirtualHost, r.Description = p["instance_id"], p["name"], p["description"]
    r.TraceFlag = p.get("trace_enabled") if p["apply_trace"] else None
    return r


def delete_request(models, p):
    r = models.DeleteRabbitMQServerlessVirtualHostRequest()
    r.InstanceId, r.VirtualHost = p["instance_id"], p["name"]
    return r
...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRabbitMQServerlessVirtualHost, describe_request(models, p, offset))
...
def desired(p):
    return {"VirtualHost": p["name"], "Description": p["description"], "MirrorQueuePolicyFlag": p["mirror_queue_policy"]}
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "apply_trace": {"type": "bool", "default": False},
        },
        required_if=[("apply_trace", True, ["trace_enabled"])],
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteRabbitMQServerlessVirtualHost, delete_request(models, p))
        ...
        if before == target and not p["apply_trace"]:
            module.exit_json(changed=False, virtual_host=current)
        diff = maybe_diff(module, before, target)
        if current:
            require_immutable_unchanged(module, before, target, ("MirrorQueuePolicyFlag",), "RabbitMQ Serverless virtual host")
        if not module.check_mode:
            module.sdk_call(
                client.ModifyRabbitMQServerlessVirtualHost if current else client.CreateRabbitMQServerlessVirtualHost,
