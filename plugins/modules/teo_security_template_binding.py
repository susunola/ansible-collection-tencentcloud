...
def describe_request(models, p):
    request = models.DescribeSecurityTemplateBindingsRequest(); request.ZoneId, request.TemplateId = p["zone_id"], [p["template_id"]]; return request


def bind_request(models, p, domains):
    request = models.BindSecurityTemplateToEntityRequest(); request.ZoneId, request.TemplateId = p["zone_id"], p["template_id"]
    request.Entities, request.Operate, request.OverWrite = domains, "bind", p["overwrite"]; return request


def unbind_request(models, p, domain):
    request = models.BindSecurityTemplateToEntityRequest(); request.ZoneId, request.TemplateId = p["zone_id"], p["template_id"]
    request.Entities, request.Operate = [domain], "unbind-" + p["unbind_policy"]; return request


def get_bindings(module, client, models, p):
    response = module.sdk_call(client.DescribeSecurityTemplateBindings, describe_request(models, p)); result = []
    for binding in response.SecurityTemplate or []:
        if binding.TemplateId != p["template_id"]: continue
        for scope in binding.TemplateScope or []:
            if scope.ZoneId != p["zone_id"]: continue
...
def run_module():
    module = TencentCloudModule(argument_spec={"zone_id": {"required": True}, "template_id": {"required": True}, "domains": {"type": "list", "elements": "str", "required": True}, "overwrite": {"type": "bool", "default": True}, "unbind_policy": {"choices": ["keep-policy", "use-default"], "default": "keep-policy"}}, supports_check_mode=True)
    p = module.params
    if len(p["domains"]) != len(set(p["domains"])): module.fail_json(msg="domains must not contain duplicates")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TeoClient, "teo.tencentcloudapi.com")
    try:
        bindings = get_bindings(module, client, models, p); before = active_domains(bindings); target = sorted(p["domains"])
        if before == target: module.exit_json(changed=False, bindings=bindings)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            missing, extra = sorted(set(target) - set(before)), sorted(set(before) - set(target))
            if missing: module.sdk_call(client.BindSecurityTemplateToEntity, bind_request(models, p, missing))
            for domain in extra: module.sdk_call(client.BindSecurityTemplateToEntity, unbind_request(models, p, domain))
