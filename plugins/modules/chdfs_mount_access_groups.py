...
options:
  file_system_id:
    description:
      - Parent file system ID used to read the mount point.
    type: str
    required: true
  mount_point_id:
    description:
  ...
  access_group_ids:
    description:
      - Exact desired access-group ID set.
    type: list
    required: true
    elements: str
...
def associate_request(models, mount_point_id, ids):
    r = models.AssociateAccessGroupsRequest()
    r.MountPointId, r.AccessGroupIds = mount_point_id, ids
    return r


def disassociate_request(models, mount_point_id, ids):
    r = models.DisassociateAccessGroupsRequest()
    r.MountPointId, r.AccessGroupIds = mount_point_id, ids
    return r
...
def current_ids(module, client, models, p):
    ...
    matches = [x for x in response.MountPoints or [] if x.MountPointId == p["mount_point_id"]]
    if not matches:
        module.fail_json(msg="CHDFS mount point was not found", mount_point_id=p["mount_point_id"])
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "mount_point_id": {"required": True},
            "access_group_ids": {"type": "list", "elements": "str", "required": True},
        },
    ...
    try:
        target = sorted(set(p["access_group_ids"]))
        before = current_ids(module, client, models, p)
        if before == target:
            module.exit_json(changed=False, access_group_ids=before)
        diff = maybe_diff(module, before, target)
        add = sorted(set(target) - set(before))
        remove = sorted(set(before) - set(target))
        if not module.check_mode:
            if remove:
                module.sdk_call(client.DisassociateAccessGroups, disassociate_request(models, p["mount_point_id"], remove))
            if add:
                module.sdk_call(client.AssociateAccessGroups, associate_request(models, p["mount_point_id"], add))
            before = current_ids(module, client, models, p)
