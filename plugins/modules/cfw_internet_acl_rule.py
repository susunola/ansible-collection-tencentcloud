...
def describe_request(models, offset=0):
    request = models.DescribeAclRuleRequest(); request.Offset, request.Limit = offset, 100; return request
...
def _rule(models, p, rule_uuid=None):
    ...
    item.Direction, item.OrderIndex, item.Enable = DIRECTIONS[p["direction"]], p["order_index"] if p["order_index"] is not None else -1, "true" if p["enabled"] else "false"
...
def create_request(models, p):
    request = models.AddAclRuleRequest(); request.Rules = [_rule(models, p)]; return request


def update_request(models, p, rule_uuid):
    request = models.ModifyAclRuleRequest(); request.Rules = [_rule(models, p, rule_uuid)]; return request


def delete_request(models, p, rule_uuid):
    request = models.RemoveAclRuleRequest(); request.RuleUuid, request.Direction = [rule_uuid], DIRECTIONS[p["direction"]]; return request


def find_rule(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeAclRule, describe_request(models, offset)); values = list(response.Data or [])
        for value in values:
            item = value._serialize(allow_none=True)
            uuid = item.get("Uuid")
            if p.get("rule_uuid") is not None and uuid == p["rule_uuid"]: matches.append(item)
    ...
    if len(matches) > 1: module.fail_json(msg="Multiple internet ACL rules matched; specify rule_uuid")
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "protocol": {"choices": ["ANY", "TCP", "UDP", "ICMP"], "default": "ANY"}, "ports": {"default": "-1/-1"}, "action": {"choices": list(ACTIONS), "default": "accept"},
            "direction": {"choices": list(DIRECTIONS), "default": "outbound"}, "enabled": {"type": "bool", "default": True}, "order_index": {"type": "int"},
            "scope": {}, "parameter_template_id": {},
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.RemoveAclRule, delete_request(models, p, current["Uuid"]))
