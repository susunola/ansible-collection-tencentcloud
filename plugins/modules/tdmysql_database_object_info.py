...
def database_request(models, p, offset):
    ...
    request.InstanceId, request.Offset, request.Limit = p["instance_id"], offset, p["page_size"]
    if p.get("database_regexp") is not None:
        request.DatabaseRegexp = p["database_regexp"]
    return request


def object_request(models, p, offset):
    request = models.DescribeDatabaseObjectsRequest()
    request.InstanceId, request.DbName = p["instance_id"], p["database"]
    request.Offset, request.Limit = offset, p["page_size"]
    if p.get("table_regexp") is not None:
        request.TableRegexp = p["table_regexp"]
    return request
...
def read_databases(module, client, models, p):
    ...
    for _ in range(p["max_pages"]):
        response = module.sdk_call(client.DescribeDatabases, database_request(models, p, offset))
        items = response.Databases or []
...
def read_objects(module, client, models, p):
    values = {"Tables": [], "Views": [], "Procs": [], "Funcs": []}
    offset, request_id, truncated = 0, None, False
    for _ in range(p["max_pages"]):
        response = module.sdk_call(client.DescribeDatabaseObjects, object_request(models, p, offset))
        page_count = 0
        for key in values:
            items = getattr(response, key, None) or []
            page_count = max(page_count, len(items))
            values[key].extend(item._serialize(allow_none=True) for item in items)
        request_id = response.RequestId
        offset += page_count
        if page_count < p["page_size"]:
            break
    else:
        truncated = True
...
def run_module():
    ...
    if p.get("database") and p.get("database_regexp"):
        module.fail_json(msg="database_regexp is only valid in database-list mode")
    if not p.get("database") and p.get("table_regexp"):
        module.fail_json(msg="table_regexp requires database")
    module.require_sdk()
    models, cm = _load()
    client = module.create_client(cm.TdmysqlClient, "tdmysql.tencentcloudapi.com")
    try:
        if p.get("database"):
            values, truncated, request_id = read_objects(module, client, models, p)
            module.exit_json(changed=False, objects=values, truncated=truncated, request_id=request_id)
