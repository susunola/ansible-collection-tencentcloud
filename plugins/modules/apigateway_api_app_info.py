...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.apigateway_api_app
    description: Create or delete a Tencent Cloud API Gateway application.
...
RETURN = r'''
api_apps:
  description: Matching APIGATEWAY api apps.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeApiAppsStatus, request),
        lambda response: response.Result.ApiAppSet if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    api_apps = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, api_apps=api_apps,
