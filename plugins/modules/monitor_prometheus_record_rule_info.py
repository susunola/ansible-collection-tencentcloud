...
options:
  instance_id:
    description: Prometheus instance ID whose recording rules are returned.
    type: str
    required: true
  page_size:
...
prometheus_record_rules:
  description: Matching Prometheus recording rules.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribePrometheusRecordRules, request),
        lambda response: response.Records,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    prometheus_record_rules = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, prometheus_record_rules=prometheus_record_rules,
