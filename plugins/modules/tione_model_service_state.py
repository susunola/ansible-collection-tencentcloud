...
def wait_service(module, client, models, p, accepted):
    def poll():
        ...
        if actual in FAILED:
            module.fail_json(msg="TIONE model service entered a failed state", service=current)
        return actual
...
def run_module():
    ...
    p = module.params
    if p["waiter_delay"] < 1 or p["waiter_timeout"] < 1:
        module.fail_json(msg="waiter_delay and waiter_timeout must be positive")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if current is None:
            module.fail_json(msg="TIONE model service does not exist; create it before managing operational state", service_id=p["service_id"])
        actual = service_status(current)
        if actual in FAILED:
            module.fail_json(msg="TIONE model service is in a failed state", service=current)
        ...
        if actual in TRANSITIONAL and not p["wait"]:
            module.fail_json(msg="TIONE model service is transitioning; enable wait before changing state", service=current)
        ...
        if not module.check_mode:
            ...
            if actual != target_status:
                ...
                if p["wait"]:
                    wait_service(module, client, models, p, [target_status])
                current = get(module, client, models, p)
