...
FIELDS = {
    ...
    "enable_memory_overcommit": "EnableMemoryOverCommit",
    "cpu_hard_limit": "CpuHardLimit",
    "min_cpu_percent": "MinCpuPercent",
    "min_memory_percent": "MinMemoryPercent",
    "max_concurrency": "MaxConcurrencyNum",
...
def group_request(models, params):
    ...
    request.WorkloadGroup = models.WorkloadGroupConfig()
    request.WorkloadGroup.from_json_string(json.dumps(desired(params)))
    return request
...
def update_request(models, instance_id, payload):
    ...
    request.InstanceId = instance_id
    request.WorkloadGroup = models.WorkloadGroupConfig()
    request.WorkloadGroup.from_json_string(json.dumps(payload))
    return request
...
def delete_request(models, params):
    ...
    request.InstanceId = params["instance_id"]
    request.WorkloadGroupName = params["name"]
    return request
...
def status_request(models, instance_id, enabled):
    ...
    request.InstanceId = instance_id
    request.OperationType = "open" if enabled else "close"
    return request
...
def run_module():
    spec = {
        ...
        "enable_memory_overcommit": {"type": "bool"},
        "cpu_hard_limit": {},
        "min_cpu_percent": {"type": "int"},
        "min_memory_percent": {"type": "int"},
        "max_concurrency": {"type": "int"},
        "max_queue_size": {"type": "int"},
        "queue_timeout": {"type": "int"},
        "workload_groups_enabled": {"type": "bool"},
    }
    ...
    try:
        ...
        if not module.check_mode:
            if current is None:
                ...
                response = None
            if response is not None and response.ErrorMsg:
                module.fail_json(msg=response.ErrorMsg)
            if status_changed:
        ...
        module.exit_json(
            changed=group_changed or status_changed,
            **(diff or {}),
            workload_group=current if not module.check_mode else target,
