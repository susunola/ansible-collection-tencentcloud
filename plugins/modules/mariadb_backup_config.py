...
def modify_request(models, p):
    request = models.ModifyBackupConfigsRequest(); request.InstanceId, request.Days = p["instance_id"], p["retention_days"]
...
def normalize(value):
    ...
    return {"retention_days": value.get("Days"), "start_time": value.get("StartBackupTime"), "end_time": value.get("EndBackupTime"), "weekdays": sorted(value.get("WeekDays") or [], key=WEEKDAYS.index), "archive_after_days": value.get("ArchiveDays")}


def desired(p):
    return {"retention_days": p["retention_days"], "start_time": p["start_time"], "end_time": p["end_time"], "weekdays": sorted(set(p["weekdays"]), key=WEEKDAYS.index), "archive_after_days": p["archive_after_days"]}


def run_module():
    module = TencentCloudModule(argument_spec={"instance_id": {"required": True}, "retention_days": {"type": "int", "default": 7}, "start_time": {"default": "22:00"}, "end_time": {"default": "23:59"}, "weekdays": {"type": "list", "elements": "str", "choices": WEEKDAYS, "default": WEEKDAYS}, "archive_after_days": {"type": "int", "default": -1}}, supports_check_mode=True)
    p = module.params
    if p["retention_days"] < 1 or p["retention_days"] > 3650: module.fail_json(msg="retention_days must be between 1 and 3650")
    if p["archive_after_days"] != -1 and p["archive_after_days"] < 1: module.fail_json(msg="archive_after_days must be -1 or a positive number")
    ...
    try:
        ...
        if not module.check_mode:
            module.sdk_call(client.ModifyBackupConfigs, modify_request(models, p)); current = normalize(module.sdk_call(client.DescribeBackupConfigs, describe_request(models, p["instance_id"])))
