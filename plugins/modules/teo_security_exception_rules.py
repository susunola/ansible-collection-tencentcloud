...
options:
  ...
  rules:
    ...
    suboptions:
      ...
      skip_option: {type: str, choices: [SkipOnAllRequestFields, SkipOnSpecifiedRequestFields], default: SkipOnAllRequestFields, description: Request-field bypass mode for managed rules.}
...
def _normalize(values, sdk=False):
    ...
    for value in values or []:
        ...
        for field in (value.get("RequestFieldsForException") if sdk else value["request_fields"]) or []:
            fields.append({"field_scope": field.get("Scope") if sdk else field["field_scope"], "condition": (field.get("Condition") if sdk else field["condition"]) or "", "target_field": field.get("TargetField") if sdk else field["target_field"]})
        result.append({"name": value.get("Name") if sdk else value["name"], "condition": value.get("Condition") if sdk else value["condition"], "enabled": (value.get("Enabled") == "on") if sdk else value["enabled"], "skip_scope": value.get("SkipScope") if sdk else value["skip_scope"], "skip_option": (value.get("SkipOption") or "SkipOnAllRequestFields") if sdk else value["skip_option"], "web_security_modules": sorted((value.get("WebSecurityModulesForException") if sdk else value["web_security_modules"]) or []), "managed_rule_ids": sorted((value.get("ManagedRulesForException") if sdk else value["managed_rule_ids"]) or []), "managed_rule_group_ids": sorted((value.get("ManagedRuleGroupsForException") if sdk else value["managed_rule_group_ids"]) or []), "request_fields": sorted(fields, key=lambda item: (item["field_scope"], item["target_field"], item["condition"]))})
...
def update_request(models, p, current=None):
    ...
    for value in p["rules"]:
        item = models.ExceptionRule(); item.Name, item.Condition, item.Enabled = value["name"], value["condition"], "on" if value["enabled"] else "off"
        ...
        item.ManagedRuleGroupsForException = value["managed_rule_group_ids"]; item.RequestFieldsForException = []
        for field in value["request_fields"]:
            request_field = models.RequestFieldsForException(); request_field.Scope, request_field.Condition, request_field.TargetField = field["field_scope"], field["condition"], field["target_field"]; item.RequestFieldsForException.append(request_field)
        ...
        output.append(item)
    rules = models.ExceptionRules(); rules.Rules = output
    policy = models.SecurityPolicy(); policy.ExceptionRules = rules
...
def get_rules(module, client, models, p):
    ...
    if not policy or not policy.ExceptionRules: return [], []
    raw = [item._serialize(allow_none=True) for item in policy.ExceptionRules.Rules or []]; return raw, _normalize(raw, True)


def run_module():
    module = TencentCloudModule(argument_spec={"zone_id": {"required": True}, "scope": {"choices": ["zone", "template", "host"], "default": "zone"}, "template_id": {}, "host": {}, "rules": {"type": "list", "elements": "dict", "required": True, "options": {"rule_id": {}, "name": {"required": True}, "condition": {"required": True}, "enabled": {"type": "bool", "default": True}, "skip_scope": {"choices": ["WebSecurityModules", "ManagedRules"], "required": True}, "skip_option": {"choices": ["SkipOnAllRequestFields", "SkipOnSpecifiedRequestFields"], "default": "SkipOnAllRequestFields"}, "web_security_modules": {"type": "list", "elements": "str", "default": [], "choices": ["websec-mod-managed-rules", "websec-mod-rate-limiting", "websec-mod-custom-rules", "websec-mod-adaptive-control", "websec-mod-bot"]}, "managed_rule_ids": {"type": "list", "elements": "str", "default": []}, "managed_rule_group_ids": {"type": "list", "elements": "str", "default": []}, "request_fields": {"type": "list", "elements": "dict", "default": [], "options": {"field_scope": {"choices": ["body.json", "cookie", "header", "uri.query", "uri", "body"], "required": True}, "condition": {"default": ""}, "target_field": {"required": True}}}}}}, supports_check_mode=True)
    p = module.params
    ...
    if len(names) != len(set(names)): module.fail_json(msg="exception rule names must be unique")
    for rule in p["rules"]:
        if rule["skip_scope"] == "WebSecurityModules" and not rule["web_security_modules"]: module.fail_json(msg="WebSecurityModules exceptions require web_security_modules")
        if rule["skip_scope"] == "ManagedRules" and bool(rule["managed_rule_ids"]) == bool(rule["managed_rule_group_ids"]): module.fail_json(msg="ManagedRules exceptions require exactly one of managed_rule_ids or managed_rule_group_ids")
        if rule["skip_option"] == "SkipOnSpecifiedRequestFields" and not rule["request_fields"]: module.fail_json(msg="SkipOnSpecifiedRequestFields requires request_fields")
