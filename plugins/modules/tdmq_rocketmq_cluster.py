...
def describe_request(models, p, offset=0):
    ...
    if p.get("cluster_id"):
        request.ClusterIdList = [p["cluster_id"]]
    else:
        request.NameKeyword = p["name"]
    return request


def create_request(models, p):
    request = models.CreateRocketMQClusterRequest()
    request.Name, request.Remark = p["name"], p["remark"]
    return request


def update_request(models, p, cluster_id):
    request = models.ModifyRocketMQClusterRequest()
    request.ClusterId, request.ClusterName, request.Remark = cluster_id, p["name"], p["remark"]
    return request
...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRocketMQClusters, describe_request(models, p, offset))
        items = list(response.ClusterList or [])
        matches = []
        for item in items:
            raw = item._serialize(allow_none=True)
            value = sanitize(raw.get("Info") or {})
            if (p.get("cluster_id") and value.get("ClusterId") == p["cluster_id"]) or (not p.get("cluster_id") and value.get("ClusterName") == p["name"]):
                value["Status"] = raw.get("Status")
                matches.append(value)
        if matches:
            if len(matches) > 1:
                module.fail_json(msg="multiple RocketMQ clusters matched name; specify cluster_id")
            return matches[0]
        offset += len(items)
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteRocketMQCluster, delete_request(models, current["ClusterId"]))
        ...
        if not module.check_mode:
            if current:
                ...
                module.sdk_call(client.CreateRocketMQCluster, create_request(models, p))
            current = find(module, client, models, p)
