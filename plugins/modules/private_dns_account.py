...
def account_object(models, uin, account):
    value = models.PrivateDNSAccount(); value.Uin, value.Account = uin, account; return value


def mutation_request(models, class_name, uin, account):
    value = getattr(models, class_name)(); value.Account = account_object(models, uin, account); return value


def list_request(models, offset):
    value = models.DescribePrivateDNSAccountListRequest(); value.Offset, value.Limit = offset, 100; return value


def find(module, client, models, uin):
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribePrivateDNSAccountList, list_request(models, offset)); items = list(response.AccountSet or [])
        matches.extend(item._serialize(allow_none=True) for item in items if str(item.Uin) == str(uin)); offset += len(items)
        if not items or offset >= int(response.TotalCount or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple Private DNS account relationships have the same UIN", uin=uin)
...
def wait(module, client, models, uin, present):
    ...
    while True:
        ...
        if time.time() >= deadline: module.fail_json(msg="Timed out waiting for Private DNS account relationship", uin=uin, expected="present" if present else "absent")
        time.sleep(module.params["waiter_delay"])


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "uin": {"required": True}, "account": {"required": True}}, supports_check_mode=True)
    p = module.params; module.require_sdk(); models, cm = _load(); client = module.create_client(cm.PrivatednsClient, "privatedns.tencentcloudapi.com")
    try:
        current = find(module, client, models, p["uin"])
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, account_binding=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeletePrivateDNSAccount, mutation_request(models, "DeletePrivateDNSAccountRequest", p["uin"], p["account"])); current = wait(module, client, models, p["uin"], False)
            module.exit_json(changed=True, **(diff or {}), account_binding=current)
        target = {"Uin": p["uin"], "Account": p["account"]}
        if current:
            if current.get("Account") != p["account"]: module.fail_json(msg="account name for an existing UIN cannot be updated in place; remove the relationship first", current=current)
            module.exit_json(changed=False, account_binding=current)
        diff = maybe_diff(module, None, target)
        if not module.check_mode: module.sdk_call(client.CreatePrivateDNSAccount, mutation_request(models, "CreatePrivateDNSAccountRequest", p["uin"], p["account"])); current = wait(module, client, models, p["uin"], True)
