...
def list_request(models, p, page=1):
    request = models.ListModelVersionsRequest(); request.ModelUid, request.Page, request.PageSize = p["model_uid"], page, 200; return request


def find(module, client, models, p):
    page, matches = 1, []
    while True:
        response = module.sdk_call(client.ListModelVersions, list_request(models, p, page)); items = response.Items or []
        ...
        page += 1
    if len(matches) > 1: module.fail_json(msg="Multiple DLC model versions matched the exact label", model_uid=p["model_uid"], version=p["version"])
    return matches[0] if matches else None


def desired(p, current=None):
    result = dict(current or {}); result["Version"] = p["version"]
...
def create_request(models, p):
    request = models.CreateModelVersionRequest(); request.ModelUid, request.ModelVersion = p["model_uid"], p["version"]
    for source, target in (("description", "Description"), ("storage_uri", "StorageUri"), ("use_custom_storage", "UseCustomStorage"), ("storage_type", "StorageType")):
        if p.get(source) is not None: setattr(request, target, p[source])
    if p.get("goosefs_config") is not None: request.GooseFSConfig = _model(models.GooseFSConfig, p["goosefs_config"])
    return request


def wait_version(module, client, models, p):
    def poll(): return "ready" if find(module, client, models, p) else "absent"
    wait_for_state(module, poll, ["ready"], timeout=p["waiter_timeout"], delay=p["waiter_delay"])


def run_module():
    spec = {
        "model_uid": {"required": True}, "version": {"required": True}, "description": {}, "storage_uri": {}, "use_custom_storage": {"type": "bool"},
        "storage_type": {"choices": ["LOCAL", "CFS", "COS", "CFSTurbo", "GooseFS"]}, "goosefs_config": {"type": "dict"},
    ...
    if p.get("storage_type") in ("CFS", "COS", "CFSTurbo") and not p.get("storage_uri"): module.fail_json(msg="storage_uri is required for CFS, COS and CFSTurbo versions")
    if p.get("storage_type") == "LOCAL" and p.get("goosefs_config") is not None: module.fail_json(msg="goosefs_config cannot be used with LOCAL storage")
    ...
    try:
        ...
        if current:
            ...
            if changes: module.fail_json(msg="DLC model versions are immutable and the requested label already has conflicting metadata", model_version=current, immutable_drift=changes)
        ...
        if not module.check_mode:
            response = module.sdk_call(client.CreateModelVersion, create_request(models, p)); version_id = response.VersionId
            if p["wait"]: wait_version(module, client, models, p)
            current = find(module, client, models, p)
        module.exit_json(changed=True, **(diff_value or {}), model_version=current if not module.check_mode else target, version_id=(current or {}).get("VersionId") or version_id)
