...
def desired(p):
    ...
    if p.get("remark") is not None: value["Remark"]=p["remark"]
    return value
...
def describe(module,client,models,p,version=None):
    offset=0; values=[]
    while True:
        r=models.DescribeResourceConfigsRequest(); r.ResourceId,r.WorkSpaceId,r.Offset,r.Limit=p["resource_id"],p["workspace_id"],offset,100
        if version is not None: r.ResourceConfigVersions=[version]
        response=module.sdk_call(client.DescribeResourceConfigs,r); page=response.ResourceConfigSet or []; values.extend(x._serialize(allow_none=True) for x in page); offset+=len(page)
        if version is not None or not page or offset>=int(response.TotalCount or 0): break
...
def references(module,client,models,p,version):
    offset=0; result=[]
    while True:
        r=models.DescribeResourceRelatedJobsRequest(); r.ResourceId,r.ResourceConfigVersion,r.WorkSpaceId=p["resource_id"],version,p["workspace_id"]; r.Offset,r.Limit=offset,100
...
def run_module():
    ...
    try:
        if p["state"]=="absent":
            ...
            if not current: module.exit_json(changed=False,resource_config=None)
