...
def describe_request(models, instance_id): request = models.DescribeInstanceSSLStatusRequest(); request.InstanceId = instance_id; return request
def modify_request(models, instance_id, enabled): request = models.ModifyInstanceSSLStatusRequest(); request.InstanceId, request.Enabled = instance_id, enabled; return request
...
def get(module, client, models, instance_id):
    response = module.sdk_call(client.DescribeInstanceSSLStatus, describe_request(models, instance_id)); return {"InstanceId": instance_id, "SSLStatus": response.SSLStatus}
...
def wait_ssl(module, client, models, p, flow_id, desired):
    ...
    def poll():
        if not flow_done[0]:
            ...
            if status in ("failed", "paused"): module.fail_json(msg="TDSQL MySQL SSL operation failed", flow_id=flow_id, status=status)
...
def run_module():
    spec = {"instance_id": {"required": True}, "enabled": {"type": "bool", "required": True}, "wait": {"type": "bool", "default": True}, "waiter_delay": {"type": "int", "default": 5}, "waiter_timeout": {"type": "int", "default": 600}}
    ...
    try:
        current = get(module, client, models, p["instance_id"]); desired = "enabled" if p["enabled"] else "disabled"
        if str(current["SSLStatus"] or "").lower() == desired: module.exit_json(changed=False, ssl=current)
        target = {"InstanceId": p["instance_id"], "SSLStatus": desired.capitalize()}; diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            response = module.sdk_call(client.ModifyInstanceSSLStatus, modify_request(models, p["instance_id"], p["enabled"]))
            if p["wait"]: wait_ssl(module, client, models, p, response.FlowId, desired)
