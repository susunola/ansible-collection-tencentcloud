...
short_description: Gather Tencent Cloud Cloud Firewall NAT ACL rules
version_added: "1.4.0"
description:
  - Returns NAT firewall access-control rules from Cloud Firewall.
options:
  rule_uuid:
...
EXAMPLES = r'''
- name: List Cloud Firewall NAT ACL rules
...
RETURN = r'''
rules:
  description: Matching NAT ACL rules.
  returned: always
  type: list
  elements: dict
rule:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeNatAcRule, request),
