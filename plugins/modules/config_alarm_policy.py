...
options:
  ...
  notice_period: {type: list, elements: int, choices: [1, 2, 3, 4, 5, 6, 7], default: [1, 2, 3, 4, 5, 6, 7], description: Exact weekdays on which notifications are sent.}
...
def list_request(models, offset=0):
    request = models.ListAlarmPolicyRequest(); request.Offset, request.Limit = offset, 100; return request


def _apply(request, p):
    request.Name = p["name"]
    if hasattr(type(request), "Type"): request.Type = p["event_type"]
...
def update_request(models, p, policy_id):
    request = _apply(models.UpdateAlarmPolicyRequest(), p); request.AlarmPolicyId = policy_id; return request


def delete_request(models, policy_id):
    request = models.DeleteAlarmPolicyRequest(); request.AlarmPolicyId = policy_id; return request


def find_policy(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.ListAlarmPolicy, list_request(models, offset)); values = list(response.AlarmPolicyList or [])
        for value in values:
            item = value._serialize(allow_none=True)
            if p.get("alarm_policy_id") is not None and item.get("AlarmPolicyId") == p["alarm_policy_id"]: matches.append(item)
            elif p.get("alarm_policy_id") is None and p.get("name") and item.get("Name") == p["name"]: matches.append(item)
        offset += len(values)
        if offset >= int(response.Total or 0) or not values: break
    if len(matches) > 1: module.fail_json(msg="Multiple Config alarm policies matched; specify alarm_policy_id")
    return matches[0] if matches else None


def desired(p):
    return {"Name": p["name"], "Type": p["event_type"], "EventScope": sorted(set(p["event_scopes"])), "RiskLevel": sorted(set(p["risk_levels"])), "NoticeTime": p["notice_time"], "NotificationMechanism": p["notification_mechanism"], "Status": 1 if p["enabled"] else 2, "NoticePeriod": sorted(set(p["notice_period"])), "Description": p["description"]}


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "alarm_policy_id": {"type": "int"}, "name": {}, "event_type": {"type": "int", "choices": [1], "default": 1}, "event_scopes": {"type": "list", "elements": "int", "choices": [1, 2], "default": [1]}, "risk_levels": {"type": "list", "elements": "int", "choices": [1, 2, 3], "default": [1, 2, 3]}, "notice_time": {}, "notification_mechanism": {}, "enabled": {"type": "bool", "default": True}, "notice_period": {"type": "list", "elements": "int", "choices": [1, 2, 3, 4, 5, 6, 7], "default": [1, 2, 3, 4, 5, 6, 7]}, "description": {"default": ""}}, required_one_of=[("alarm_policy_id", "name")], supports_check_mode=True)
    p = module.params
    if p["state"] == "present" and any(not p.get(key) for key in ("name", "notice_time", "notification_mechanism")): module.fail_json(msg="name, notice_time and notification_mechanism are required when state=present")
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, alarm_policy=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteAlarmPolicy, delete_request(models, current["AlarmPolicyId"]))
        ...
        if before:
            before["EventScope"], before["RiskLevel"], before["NoticePeriod"] = sorted(set(before["EventScope"] or [])), sorted(set(before["RiskLevel"] or [])), sorted(set(before["NoticePeriod"] or []))
            before["Description"] = before["Description"] or ""
        if before == target: module.exit_json(changed=False, alarm_policy=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current: module.sdk_call(client.UpdateAlarmPolicy, update_request(models, p, current["AlarmPolicyId"]))
            else: p["alarm_policy_id"] = module.sdk_call(client.AddAlarmPolicy, create_request(models, p)).AlarmPolicyId
            current = find_policy(module, client, models, p)
