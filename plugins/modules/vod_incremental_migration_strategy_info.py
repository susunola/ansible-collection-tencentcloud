...
options:
  filters:
    description: VOD API filter names mapped to lists of values.
    type: dict
    default: {}
  page_size:
...
RETURN = r'''
incremental_migration_strategies:
  description: Matching VOD incremental migration strategies.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.vod.v20240718 import models, vod_client
    except ImportError:
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeIncrementalMigrationStrategyInfos, request),
        lambda response: response.StrategyInfoSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    incremental_migration_strategies = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, incremental_migration_strategies=incremental_migration_strategies, total_count=total_count)
