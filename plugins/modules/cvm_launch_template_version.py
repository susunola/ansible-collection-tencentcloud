...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cvm_launch_template_version_info
    description: Gather information about Tencent Cloud CVM launch template versions.
...
def describe_request(models, p, offset=0):
    ...
    request.LaunchTemplateId, request.Offset, request.Limit = p["template_id"], offset, 100
    if p.get("version") is not None:
        request.LaunchTemplateVersions = [p["version"]]
    return request


def create_request(models, p):
    request = models.CreateLaunchTemplateVersionRequest()
    request._deserialize(p["template_data"])
    request.LaunchTemplateId, request.LaunchTemplateVersionDescription = p["template_id"], p["description"]
    request.LaunchTemplateVersion = p.get("base_version")
    return request


def default_request(models, p, version):
    request = models.ModifyLaunchTemplateDefaultVersionRequest()
    request.LaunchTemplateId, request.DefaultVersion = p["template_id"], version
    return request


def delete_request(models, p, version):
    request = models.DeleteLaunchTemplateVersionsRequest()
    request.LaunchTemplateId, request.LaunchTemplateVersions = p["template_id"], [version]
    return request


def _subset(current, target):
    if isinstance(target, dict):
        return isinstance(current, dict) and all(k in current and _subset(current[k], v) for k, v in target.items())
...
def find(module, client, models, p):
    response = module.sdk_call(client.DescribeLaunchTemplateVersions, describe_request(models, p))
    matches = []
    for item in response.LaunchTemplateVersionSet or []:
        value = item._serialize(allow_none=True)
        if (p.get("version") is not None and int(value.get("LaunchTemplateVersion") or 0) == p["version"]) or (
            p.get("version") is None and value.get("LaunchTemplateVersionDescription") == p.get("description")
        ):
            matches.append(value)
    if len(matches) > 1:
        module.fail_json(msg="Multiple launch-template versions matched; specify version")
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "make_default": {"type": "bool", "default": False},
            "force_replace": {"type": "bool", "default": False},
        },
        required_one_of=[("version", "description")],
        supports_check_mode=True,
    )
    p = module.params
    if p["state"] == "present" and (not p.get("description") or not p.get("template_data")):
        module.fail_json(msg="description and template_data are required when state=present")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, launch_template_version=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteLaunchTemplateVersions, delete_request(models, p, int(current["LaunchTemplateVersion"])))
        ...
        if data_matches and default_matches:
            module.exit_json(changed=False, launch_template_version=current)
        if current and not data_matches and not p["force_replace"]:
            module.fail_json(
                msg="launch-template versions are immutable; set force_replace=true to replace this version", version=current.get("LaunchTemplateVersion")
            )
        target = {"LaunchTemplateVersionDescription": p["description"], "LaunchTemplateVersionData": p["template_data"], "IsDefaultVersion": p["make_default"]}
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if current and not data_matches:
                old_version = int(current["LaunchTemplateVersion"])
                preserve_default = bool(current.get("IsDefaultVersion"))
                p["version"] = None
                p["version"] = int(module.sdk_call(client.CreateLaunchTemplateVersion, create_request(models, p)).LaunchTemplateVersionNumber)
                current = find(module, client, models, p)
                if preserve_default or p["make_default"]:
                    module.sdk_call(client.ModifyLaunchTemplateDefaultVersion, default_request(models, p, p["version"]))
                    current = find(module, client, models, p)
            ...
            if p["make_default"] and not current.get("IsDefaultVersion"):
                module.sdk_call(client.ModifyLaunchTemplateDefaultVersion, default_request(models, p, int(current["LaunchTemplateVersion"])))
                current = find(module, client, models, p)
