...
RETURN = r'''
backup_plans:
  description: Matching DBS backup plans.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.dbs.v20211108 import models, dbs_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-dbs package is required.")

    client = dbs_client.DbsClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "dbs.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.DescribeBackupPlans, request),
        lambda response: response.Items,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    backup_plans = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, backup_plans=backup_plans, total_count=total_count)
