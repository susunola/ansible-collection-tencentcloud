...
def describe_request(models, p, offset=0):
    request = models.DescribeRabbitMQPermissionRequest(); request.InstanceId, request.User, request.VirtualHost = p["instance_id"], p["user"], p["virtual_host"]
    request.Offset, request.Limit = offset, 100; return request


def modify_request(models, p):
    request = models.ModifyRabbitMQPermissionRequest(); request.InstanceId, request.User, request.VirtualHost = p["instance_id"], p["user"], p["virtual_host"]
    request.ConfigRegexp, request.WriteRegexp, request.ReadRegexp = p["configure_regex"], p["write_regex"], p["read_regex"]; return request


def delete_request(models, p):
    request = models.DeleteRabbitMQPermissionRequest(); request.InstanceId, request.User, request.VirtualHost = p["instance_id"], p["user"], p["virtual_host"]; return request


def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRabbitMQPermission, describe_request(models, p, offset)); items = list(response.RabbitMQPermissionList or [])
        for item in items:
...
def comparable(value):
    return {"User": value.get("User"), "VirtualHost": value.get("VirtualHost"), "ConfigRegexp": value.get("ConfigRegexp"), "WriteRegexp": value.get("WriteRegexp"), "ReadRegexp": value.get("ReadRegexp")}


def desired(p):
    return {"User": p["user"], "VirtualHost": p["virtual_host"], "ConfigRegexp": p["configure_regex"], "WriteRegexp": p["write_regex"], "ReadRegexp": p["read_regex"]}
...
def run_module():
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteRabbitMQPermission, delete_request(models, p))
        ...
        if not module.check_mode: module.sdk_call(client.ModifyRabbitMQPermission, modify_request(models, p)); current = find(module, client, models, p)
