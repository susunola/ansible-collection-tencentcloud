...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.config_recorder_info
    description: Gather Tencent Cloud Config recorder state.
...
def update_request(models, resource_types):
    ...
    request.ResourceTypes = sorted(set(resource_types))
    return request
...
def run_module():
    module = TencentCloudModule(
        argument_spec={"enabled": {"type": "bool", "default": True}, "resource_types": {"type": "list", "elements": "str", "default": []}},
    ...
    try:
        ...
        if before == desired:
            module.exit_json(changed=False, recorder=current)
        diff = maybe_diff(module, before, desired)
        if not module.check_mode:
            if before["ResourceTypes"] != desired["ResourceTypes"]:
                module.sdk_call(client.UpdateConfigRecorder, update_request(models, desired["ResourceTypes"]))
            if before["Enabled"] != desired["Enabled"]:
                if desired["Enabled"]:
                    module.sdk_call(client.OpenConfigRecorder, open_request(models))
                else:
                    module.sdk_call(client.CloseConfigRecorder, close_request(models))
            current = find_recorder(module, client, models)
