...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.tdcpg_cluster_info
    description: Gather information about Tencent Cloud TDCPG clusters.
...
def describe_request(models, p, page=1):
    ...
    r.PageNumber, r.PageSize = page, 100
    if p.get("cluster_id") or p.get("name"):
        f = models.Filter()
        f.Name, f.Values = ("ClusterId", [p["cluster_id"]]) if p.get("cluster_id") else ("ClusterName", [p["name"]])
        r.Filters = [f]
    return r
...
def instances_request(models, cluster_id, page=1):
    ...
    r.ClusterId, r.PageNumber, r.PageSize = cluster_id, page, 100
    return r


def create_request(models, p):
    r = models.CreateClusterRequest()
    r.Zone, r.MasterUserPassword, r.CPU, r.Memory = p["zone"], p["master_password"], p["cpu"], p["memory"]
    ...
    r.AutoRenewFlag = None if p.get("auto_renew") is None else int(p["auto_renew"])
    r.StoragePayMode, r.Storage = p.get("storage_pay_mode"), p.get("storage")
    return r


def create_instances_request(models, p, cluster_id, count):
    r = models.CreateClusterInstancesRequest()
    r.ClusterId, r.CPU, r.Memory, r.InstanceCount = cluster_id, p["cpu"], p["memory"], count
    return r


def resize_request(models, p, cluster_id, instance_ids):
    r = models.ModifyClusterInstancesSpecRequest()
    r.ClusterId, r.InstanceIdSet, r.CPU, r.Memory, r.OperationTiming = cluster_id, instance_ids, p["cpu"], p["memory"], "IMMEDIATE"
    return r


def delete_instances_request(models, cluster_id, instance_ids):
    r = models.DeleteClusterInstancesRequest()
    r.ClusterId, r.InstanceIdSet = cluster_id, instance_ids
    return r
...
def renew_request(models, cluster_id, enabled):
    r = models.ModifyClustersAutoRenewFlagRequest()
    r.ClusterIdSet, r.AutoRenewFlag = [cluster_id], int(enabled)
    return r
...
def run_module():
    spec = {
        ...
        "db_major_version": {},
        "db_kernel_version": {},
        "pay_mode": {"choices": ["PREPAID", "POSTPAID_BY_HOUR"], "default": "POSTPAID_BY_HOUR"},
    ...
    module = TencentCloudModule(argument_spec=spec, required_one_of=[("cluster_id", "name")], required_together=[("cpu", "memory")], supports_check_mode=True)
    p = module.params
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if p["purge"]:
                if status != "isolated":
                    module.fail_json(msg="purge requires an already isolated TDSQL-C PostgreSQL cluster", current_status=status)
            ...
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.IsolateCluster, isolate_request(models, cluster_id))
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a new TDSQL-C PostgreSQL cluster", missing=missing)
            target = {
                "ClusterName": p["name"],
                "Zone": p["zone"],
                "DBVersion": p.get("db_version"),
                "InstanceCount": p.get("instance_count") or 1,
                "StorageLimit": p.get("storage"),
            }
            diff = maybe_diff(module, None, target)
            if not module.check_mode:
                module.sdk_call(client.CreateCluster, create_request(models, p))
        ...
        endpoints = current.get("EndpointSet") or []
        networks = {(x.get("VpcId"), x.get("SubnetId")) for x in endpoints}
        immutable = {
            "Zone": p.get("zone"),
            "DBVersion": p.get("db_version"),
            "DBMajorVersion": p.get("db_major_version"),
        ...
        if drift:
            module.fail_json(msg="TDSQL-C PostgreSQL placement and database versions are immutable", immutable_drift=drift)
        cluster_id = current["ClusterId"]
        members = instances(module, client, models, cluster_id)
        desired_count = p.get("instance_count") if p.get("instance_count") is not None else len(members)
        if desired_count < len(members) and not p["allow_scale_in"]:
            module.fail_json(msg="set allow_scale_in=true to delete excess cluster instances", current_count=len(members), desired_count=desired_count)
        changed = False
        before = {"ClusterName": current.get("ClusterName"), "InstanceCount": len(members), "AutoRenewFlag": current.get("AutoRenewFlag")}
        desired = {
            "ClusterName": p.get("name") or current.get("ClusterName"),
            "InstanceCount": desired_count,
            "AutoRenewFlag": int(p["auto_renew"]) if p.get("auto_renew") is not None else current.get("AutoRenewFlag"),
        }
        ...
        if before != desired or spec_drift:
            ...
            if not module.check_mode:
                ...
                if spec_drift:
                    module.sdk_call(client.ModifyClusterInstancesSpec, resize_request(models, p, cluster_id, [x["InstanceId"] for x in members]))
                if desired_count > len(members):
                    module.sdk_call(client.CreateClusterInstances, create_instances_request(models, p, cluster_id, desired_count - len(members)))
                elif desired_count < len(members):
                    module.sdk_call(
                        client.DeleteClusterInstances, delete_instances_request(models, cluster_id, [x["InstanceId"] for x in members[desired_count:]])
                    )
                if before["AutoRenewFlag"] != desired["AutoRenewFlag"]:
                    module.sdk_call(client.ModifyClustersAutoRenewFlag, renew_request(models, cluster_id, p["auto_renew"]))
