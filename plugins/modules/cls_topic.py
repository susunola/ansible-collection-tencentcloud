...
options:
  state:
    description:
      - C(present) creates the topic with V(CreateTopic) when it does not exist and updates it with V(ModifyTopic)
        when it differs. C(absent) deletes it with V(DeleteTopic).
