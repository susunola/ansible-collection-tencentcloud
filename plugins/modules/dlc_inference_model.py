...
def normalize(value):
    ...
    for key in ("Tags", "Tasks", "SupportedEngines"):
        if result.get(key) is not None: result[key] = sorted(result[key])
    result["ResourceTags"] = resource_tags(result.get("ResourceTags"))
    if result.get("StorageType") is not None: result["StorageType"] = str(result["StorageType"]).upper()
    return result


def list_request(models, page=1):
    request = models.ListInferenceModelsRequest(); request.Page, request.PageSize = page, 200; return request


def find(module, client, models, p):
    page, matches = 1, []
    while True:
        response = module.sdk_call(client.ListInferenceModels, list_request(models, page)); items = response.Items or []
        for item in items:
        ...
        page += 1
    if len(matches) > 1: module.fail_json(msg="Multiple DLC inference models matched the requested identity", name=p["name"], model_uid=p.get("model_uid"))
    current = normalize(matches[0]) if matches else None
    if current and current.get("Name") != p["name"]: module.fail_json(msg="DLC model_uid exists with a different name", requested_name=p["name"], model=current)
    return current


def desired(p, current=None):
    result = dict(current or {}); result["Name"] = p["name"]
    if p.get("model_uid") is not None: result["ModelUid"] = p["model_uid"]
    for source, target in (("model_type", "ModelType"), ("provider", "Provider"), ("description", "Description"), ("parameter_size", "ParameterSize"), ("storage_type", "StorageType"), ("use_custom_storage", "HasCustomStorage")):
        if p.get(source) is not None: result[target] = str(p[source]).upper() if source == "storage_type" else p[source]
    if p.get("tags") is not None: result["Tags"] = sorted(p["tags"])
    if p.get("tasks") is not None: result["Tasks"] = sorted(p["tasks"])
    if p.get("resource_tags") is not None: result["ResourceTags"] = resource_tags(p["resource_tags"])
    return result
...
def immutable_drift(p, current):
    target, changes = desired(p, current), {}
    for source, key in (("model_type", "ModelType"), ("provider", "Provider"), ("tasks", "Tasks"), ("storage_type", "StorageType"), ("use_custom_storage", "HasCustomStorage")):
        if p.get(source) is not None and current.get(key) != target.get(key): changes[key] = (current.get(key), target.get(key))
    return changes


def create_request(models, p):
    request = models.CreateInferenceModelRequest(); request.Name, request.ModelType, request.InitialVersion = p["name"], p["model_type"], p["initial_version"]
    mapping = {"model_uid": "ModelUid", "provider": "Provider", "description": "Description", "parameter_size": "ParameterSize", "storage_uri": "StorageUri", "use_custom_storage": "UseCustomStorage", "storage_type": "StorageType", "goosefs_config": "GooseFSConfig"}
    for source, target in mapping.items():
        if p.get(source) is not None: setattr(request, target, p[source] if source != "goosefs_config" else _model(models.GooseFSConfig, p[source]))
    if p.get("tags") is not None: request.Tags = sorted(p["tags"])
    if p.get("tasks") is not None: request.Tasks = sorted(p["tasks"])
    if p.get("resource_tags") is not None: request.ResourceTags = [_model(models.Tag, x) for x in resource_tags(p["resource_tags"])]
    return request
...
def update_request(models, p, model_uid):
    request = models.UpdateInferenceModelRequest(); request.ModelUid = model_uid
    if p.get("description") is not None: request.Description = p["description"]
    if p.get("parameter_size") is not None: request.ParameterSize = p["parameter_size"]
    if p.get("tags") is not None: request.Tags = sorted(p["tags"])
    if p.get("resource_tags") is not None: request.ResourceTags = [_model(models.Tag, x) for x in resource_tags(p["resource_tags"])]
    return request
...
def run_module():
    ...
    spec = {
        "name": {"required": True}, "model_uid": {}, "model_type": {}, "initial_version": {}, "provider": {}, "description": {}, "parameter_size": {},
        "tags": {"type": "list", "elements": "str"}, "tasks": {"type": "list", "elements": "str"}, "storage_uri": {}, "use_custom_storage": {"type": "bool"},
        "storage_type": {"choices": ["Local", "COS", "CFS", "CFSTurbo", "GooseFS"]}, "goosefs_config": {"type": "dict"},
        "resource_tags": {"type": "list", "elements": "dict", "options": tag}, "wait": {"type": "bool", "default": True},
    ...
    if len(p["name"]) > 256: module.fail_json(msg="name must not exceed 256 characters")
    ...
    try:
        ...
        if not current:
            ...
            if missing: module.fail_json(msg="creation parameters are required for a DLC inference model", missing=missing)
            target, diff_value, uid, model_id = desired(p), maybe_diff(module, None, desired(p)), p.get("model_uid"), None
            if not module.check_mode:
                response = module.sdk_call(client.CreateInferenceModel, create_request(models, p)); uid, model_id = response.ModelUid, response.ModelId
                if not p.get("model_uid"): p["model_uid"] = uid
                if p["wait"]: wait_model(module, client, models, p)
                current = find(module, client, models, p)
            module.exit_json(changed=True, **(diff_value or {}), model=current if not module.check_mode else target, model_uid=(current or {}).get("ModelUid") or uid, model_id=(current or {}).get("ModelId") or model_id)
        fixed = immutable_drift(p, current)
        if fixed: module.fail_json(msg="DLC inference model type, provider, tasks and storage settings are immutable", model=current, immutable_drift=fixed)
        changes = mutable_drift(p, current)
        if not changes: module.exit_json(changed=False, model=current, model_uid=current.get("ModelUid"), model_id=current.get("ModelId"))
        ...
        if not module.check_mode:
            ...
            if p["wait"]: wait_model(module, client, models, p, expected={k: v[1] for k, v in changes.items()})
            current = find(module, client, models, p)
        module.exit_json(changed=True, **(diff_value or {}), model=current if not module.check_mode else after, model_uid=current.get("ModelUid"), model_id=current.get("ModelId"))
