...
def normalize(value):
    ...
    return {"Id": root.get("Id") or "default", "Status": root.get("Status"), "Tiering": {"AccessTier": tiering.get("AccessTier"), "Days": int(tiering["Days"]), "RequestFrequent": int(tiering["RequestFrequent"])}}


def get_rule(client, bucket):
    try: return normalize(client.get_bucket_intelligenttiering_v2(Bucket=bucket, Id="default"))
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "transition_days": {"type": "int", "choices": [30, 60, 90], "default": 30}, "request_frequent": {"type": "int", "default": 1}}, supports_check_mode=True)
    p = module.params
    if p["request_frequent"] < 1: module.fail_json(msg="request_frequent must be positive")
    ...
    try:
        ...
        if p["state"] == "absent":
            if current is None: module.exit_json(changed=False, intelligent_tiering=None)
        ...
        target = {"Id": "default", "Status": "Enabled", "Tiering": {"AccessTier": "INFREQUENT", "Days": p["transition_days"], "RequestFrequent": p["request_frequent"]}}
        if current == target: module.exit_json(changed=False, intelligent_tiering=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode: client.put_bucket_intelligenttiering_v2(Bucket=bucket, Id="default", IntelligentTieringConfiguration=target)
