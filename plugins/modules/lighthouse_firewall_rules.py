...
def _rule(models, value):
    item = models.FirewallRule()
    item._deserialize({key: value.get(key) for key in FIELDS if value.get(key) is not None})
    return item


def normalize_rule(value):
    return {key: value.get(key) or "" for key in FIELDS}


def rule_key(value):
    return tuple(normalize_rule(value)[key] for key in FIELDS)


def normalize_rules(values):
    return sorted((normalize_rule(value) for value in values or []), key=rule_key)


def describe_request(models, instance_id, offset=0):
    request = models.DescribeFirewallRulesRequest()
    request.InstanceId, request.Offset, request.Limit = instance_id, offset, 100
    return request


def create_request(models, instance_id, rules, version):
    request = models.CreateFirewallRulesRequest()
    request.InstanceId, request.FirewallRules, request.FirewallVersion = instance_id, [_rule(models, value) for value in rules], version
    return request
...
def describe(module, client, models, instance_id):
    ...
    version = None
    while True:
        response = module.sdk_call(client.DescribeFirewallRules, describe_request(models, instance_id, offset))
        values = list(response.FirewallRuleSet or [])
        version = response.FirewallVersion
        result.extend(item._serialize(allow_none=True) for item in values)
        offset += len(values)
...
def run_module():
    module = TencentCloudModule(
        argument_spec={"instance_id": {"required": True}, "rules": {"type": "list", "elements": "dict", "default": []}}, supports_check_mode=True
    )
    p = module.params
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        target = normalize_rules(p["rules"])
        current_set, target_set = {rule_key(v): v for v in current}, {rule_key(v): v for v in target}
        remove = [current_set[key] for key in sorted(set(current_set) - set(target_set))]
        add = [target_set[key] for key in sorted(set(target_set) - set(current_set))]
        if not remove and not add:
            module.exit_json(changed=False, rules=current, firewall_version=version)
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if remove:
                module.sdk_call(client.DeleteFirewallRules, delete_request(models, p["instance_id"], remove, version))
                current, version = describe(module, client, models, p["instance_id"])
            if add:
                module.sdk_call(client.CreateFirewallRules, create_request(models, p["instance_id"], add, version))
            current, version = describe(module, client, models, p["instance_id"])
