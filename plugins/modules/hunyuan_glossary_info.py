...
RETURN = r'''
glossaries:
  description: Matching HUNYUAN glossaries.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        ...
        module.fail_json(msg="The tencentcloud-sdk-python-hunyuan package is required.")

    client = hunyuan_client.HunyuanClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "hunyuan.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.ListGlossary, request),
        lambda response: response.Glossaries,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    glossaries = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, glossaries=glossaries, total_count=total_count)
