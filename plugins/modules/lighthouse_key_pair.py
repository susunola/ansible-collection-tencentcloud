...
def describe_request(models, p, offset=0):
    ...
    if p.get("key_id"):
        request.KeyIds = [p["key_id"]]
    return request


def import_request(models, p):
    request = models.ImportKeyPairRequest()
    request.KeyName, request.PublicKey = p["name"], p["public_key"].strip()
    return request
...
def associate_request(models, p, key_id, instance_ids):
    ...
    request.KeyIds, request.InstanceIds, request.AssociateType = [key_id], sorted(instance_ids), p["association_type"]
    if p.get("username") and p["association_type"] == "ONLINE":
...
def disassociate_request(models, p, key_id, instance_ids):
    ...
    request.KeyIds, request.InstanceIds, request.DisassociateType = [key_id], sorted(instance_ids), p["association_type"]
    if p.get("username") and p["association_type"] == "ONLINE":
        request.Username = p["username"]
    return request
...
def _instances(value):
    return sorted(
        value.get("AssociatedInstanceIds") or [item.get("InstanceId") for item in (value.get("AssociatedInstanceSet") or []) if item.get("InstanceId")]
    )


def comparable(value):
    return {"KeyName": value.get("KeyName"), "PublicKey": (value.get("PublicKey") or "").strip(), "InstanceIds": _instances(value)}


def desired(p):
    return {"KeyName": p["name"], "PublicKey": p["public_key"].strip(), "InstanceIds": sorted(p["instance_ids"])}
...
def find(module, client, models, p):
    ...
    while True:
        ...
        values = list(response.KeyPairSet or [])
        for item in values:
            value = item._serialize(allow_none=True)
            if (p.get("key_id") and value.get("KeyId") == p["key_id"]) or (not p.get("key_id") and value.get("KeyName") == p.get("name")):
                matches.append(value)
        offset += len(values)
        if offset >= int(response.TotalCount or 0) or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple Lighthouse key pairs matched; specify key_id")
...
def remove(module, client, models, p, current):
    ...
    if bound:
        module.sdk_call(client.DisassociateInstancesKeyPairs, disassociate_request(models, p, current["KeyId"], bound))
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "association_type": {"choices": ["ONLINE", "OFFLINE"], "default": "ONLINE"},
            "username": {},
            "force_replace": {"type": "bool", "default": False},
            "force_delete": {"type": "bool", "default": False},
        },
        required_one_of=[("key_id", "name")],
        supports_check_mode=True,
    )
    p = module.params
    if p["state"] == "present" and (not p["name"] or not p["public_key"]):
        module.fail_json(msg="name and public_key are required when state=present")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, key_pair=None)
            bound = _instances(current)
            if bound and not p["force_delete"]:
                module.fail_json(msg="key pair is associated with instances; set force_delete=true to disassociate and delete", instance_ids=bound)
            diff = maybe_diff(module, comparable(current), None)
            if not module.check_mode:
                remove(module, client, models, p, current)
        ...
        replace = bool(current and (before["KeyName"] != target["KeyName"] or before["PublicKey"] != target["PublicKey"]))
        if replace and not p["force_replace"]:
            module.fail_json(msg="name and public_key are immutable; set force_replace=true to disassociate and re-import")
        if before == target:
            module.exit_json(changed=False, key_pair=current)
        ...
        if not module.check_mode:
            if replace:
                ...
                current = None
            if not current:
                p["key_id"] = module.sdk_call(client.ImportKeyPair, import_request(models, p)).KeyId
                current = find(module, client, models, p)
            old_ids, new_ids = set(_instances(current)), set(p["instance_ids"])
            if old_ids - new_ids:
                module.sdk_call(client.DisassociateInstancesKeyPairs, disassociate_request(models, p, current["KeyId"], old_ids - new_ids))
            if new_ids - old_ids:
                module.sdk_call(client.AssociateInstancesKeyPairs, associate_request(models, p, current["KeyId"], new_ids - old_ids))
            current = find(module, client, models, p)
