...
def build_request(models, p, page):
    request = models.ListRayJobsRequest(); request.Page, request.PageSize = page, p["page_size"]
...
def read(module, client, models, p):
    ...
    for page in range(1, p["max_pages"] + 1):
        response = module.sdk_call(client.ListRayJobs, build_request(models, p, page)); items = response.Items or []
        jobs.extend(x._serialize(allow_none=True) for x in items); total_pages, request_id = int(response.TotalPages or 0), response.RequestId
