...
RETURN = r'''
auto_scaler_resource_strategy_binding_groups:
  description: Matching TSE auto scaler resource strategy binding groups.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeAutoScalerResourceStrategyBindingGroups, request),
        lambda response: response.Result.GroupInfos if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    auto_scaler_resource_strategy_binding_groups = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, auto_scaler_resource_strategy_binding_groups=auto_scaler_resource_strategy_binding_groups,
