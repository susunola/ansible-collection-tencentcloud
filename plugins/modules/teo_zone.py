...
def describe_request(models, p, offset=0):
    request = models.DescribeZonesRequest(); request.Offset, request.Limit = offset, 100
    value = p.get("zone_id") or p.get("name")
    if value:
        item = models.AdvancedFilter(); item.Name = "zone-id" if p.get("zone_id") else "zone-name"; item.Values = [value]; request.Filters = [item]
    return request


def create_request(models, p):
    request = models.CreateZoneRequest(); request.Type, request.ZoneName, request.Area = p["zone_type"], p["name"], p["area"]
    if p.get("plan_id"): request.PlanId = p["plan_id"]
    if p.get("alias_name"): request.AliasZoneName = p["alias_name"]
    return request


def update_request(models, p, zone_id):
    request = models.ModifyZoneRequest(); request.ZoneId, request.Type = zone_id, p["zone_type"]
    request.ZoneName, request.Area = p["name"], p["area"]
    if p.get("alias_name") is not None: request.AliasZoneName = p["alias_name"]
    return request


def status_request(models, zone_id, enabled):
    request = models.ModifyZoneStatusRequest(); request.ZoneId, request.Paused = zone_id, not enabled; return request


def delete_request(models, zone_id):
    request = models.DeleteZoneRequest(); request.ZoneId = zone_id; return request


def find_zone(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeZones, describe_request(models, p, offset)); values = list(response.Zones or [])
        for value in values:
            item = value._serialize(allow_none=True)
            if p.get("zone_id") and item.get("ZoneId") == p["zone_id"]: matches.append(item)
            elif not p.get("zone_id") and p.get("name") and item.get("ZoneName") == p["name"]: matches.append(item)
        offset += len(values)
        if offset >= int(response.TotalCount or 0) or not values: break
    if len(matches) > 1: module.fail_json(msg="Multiple EdgeOne zones matched; specify zone_id")
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "zone_id": {}, "name": {}, "zone_type": {"choices": ["partial", "full", "noDomainAccess", "dnsPodAccess", "ai"], "default": "partial"}, "area": {"choices": ["global", "mainland", "overseas"], "default": "overseas"}, "alias_name": {}, "plan_id": {}, "enabled": {"type": "bool", "default": True}}, required_one_of=[("zone_id", "name")], supports_check_mode=True)
    p = module.params
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, zone=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteZone, delete_request(models, current["ZoneId"]))
        ...
        if before: before["AliasZoneName"] = before["AliasZoneName"] or ""
        if before == target: module.exit_json(changed=False, zone=current)
        ...
        if not module.check_mode:
            if not current:
                ...
                if config_changed: module.sdk_call(client.ModifyZone, update_request(models, p, current["ZoneId"]))
            if bool(current and current.get("Paused")) != (not p["enabled"]): module.sdk_call(client.ModifyZoneStatus, status_request(models, p["zone_id"] or current["ZoneId"], p["enabled"]))
            current = find_zone(module, client, models, p)
