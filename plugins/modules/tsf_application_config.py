...
options:
  ...
  config_id:
    description:
      - Existing configuration version ID; application, name and version are used when omitted.
    type: str
  application_id:
    description:
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.tsf_application_config_info
    description: Gather information about Tencent Cloud TSF application configs.
...
def checked(module, response, action):
    if response.Result is False:
        module.fail_json(msg="Tencent Cloud rejected the TSF application configuration %s" % action, request_id=response.RequestId)
...
def run_module():
    ...
    try:
        ...
        if current:
            require_immutable_unchanged(module, current, target, list(target), "TSF application configuration version")
            module.exit_json(changed=False, config=current)
        if params.get("value") is None:
            module.fail_json(msg="value is required when creating a TSF application configuration")
