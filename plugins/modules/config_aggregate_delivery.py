...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.config_aggregate_delivery_info
    description: Gather Tencent Cloud Config aggregate delivery settings.
...
def find_delivery(module, client, models, group_id):
    response = module.sdk_call(client.DescribeAggregateConfigDeliver, describe_request(models, group_id))
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "delivery_type": {"choices": ["COS", "CLS"], "required": True},
            "delivery_uin": {"type": "int", "default": 0},
    ...
    try:
        ...
        if not module.check_mode:
            module.sdk_call(client.UpdateAggregateConfigDeliver, update_request(models, p))
            current = find_delivery(module, client, models, p["account_group_id"])
