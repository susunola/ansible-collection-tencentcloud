...
def normalize(value):
    ...
    root = value.get("OriginConfiguration", value); rules = root.get("OriginRule") or []
...
def get_origin(client, bucket):
    try: return normalize(client.get_bucket_origin(Bucket=bucket))
...
def run_module():
    ...
    try:
        current = get_origin(client, bucket); target = normalize({"OriginRule": p.get("rules") or []}) if p["state"] == "present" else None
        if current == target: module.exit_json(changed=False, origin=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if target is None: client.delete_bucket_origin(Bucket=bucket)
            else: client.put_bucket_origin(Bucket=bucket, OriginConfiguration=target)
