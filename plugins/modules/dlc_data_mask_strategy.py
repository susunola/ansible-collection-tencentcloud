...
def describe_request(models, p, offset=0):
    request = models.DescribeDataMaskStrategiesRequest(); request.Offset, request.Limit = offset, 100
    if p.get("name"):
        item = models.Filter(); item.Name, item.Values = "strategy-name", [p["name"]]; request.Filters = [item]
    return request
...
def normalize_groups(values):
    ...
    if len(keys) != len(set(keys)): raise ValueError("duplicate DLC masking work-group rules are not allowed")
...
def normalize(value):
    return {"StrategyId": value.get("StrategyId"), "StrategyName": value.get("StrategyName") or "",
            "StrategyType": value.get("StrategyType") or "", "StrategyDesc": value.get("StrategyDesc") or "",
            "Groups": normalize_groups(value.get("Groups")), "Users": normalize_users(value.get("Users"))}


def find(module, client, models, p):
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeDataMaskStrategies, describe_request(models, p, offset)); page = response.Strategies or []
        for item in page:
            value = item._serialize(allow_none=True)
            if value.get("State") == 0: continue
            if (p.get("strategy_id") and value.get("StrategyId") == p["strategy_id"]) or (not p.get("strategy_id") and value.get("StrategyName") == p.get("name")): matches.append(value)
        offset += len(page)
        if not page or offset >= int(response.TotalCount or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple DLC masking strategies matched; specify strategy_id")
    return matches[0] if matches else None


def desired(p, current=None):
    value = normalize(current or {})
    if p.get("strategy_id"): value["StrategyId"] = p["strategy_id"]
    ...
    if p.get("groups") is not None: value["Groups"] = normalize_groups(p["groups"])
    if p.get("users") is not None: value["Users"] = normalize_users(p["users"])
    return value


def strategy_model(models, value):
    payload = dict(value); payload["Users"] = ";".join(value.get("Users") or [])
    item = models.DataMaskStrategyInfo(); item.from_json_string(json.dumps(payload)); return item


def create_request(models, target):
    request = models.CreateDataMaskStrategyRequest(); request.Strategy = strategy_model(models, target); return request


def update_request(models, target):
    request = models.UpdateDataMaskStrategyRequest(); request.Strategy = strategy_model(models, target); return request


def delete_request(models, strategy_id):
    request = models.DeleteDataMaskStrategyRequest(); request.StrategyId = strategy_id; return request


def wait_strategy(module, client, models, p, target=None):
    def poll():
        current = find(module, client, models, p)
        if target is None: return "absent" if current is None else "pending"
...
def run_module():
    ...
    spec = {"state": {"choices": ["present", "absent"], "default": "present"}, "strategy_id": {}, "name": {},
            "strategy_type": {"choices": types}, "description": {}, "groups": {"type": "list", "elements": "dict"},
            "users": {"type": "list", "elements": "str"}, "allow_delete": {"type": "bool", "default": False},
            "wait": {"type": "bool", "default": True}, "waiter_delay": {"type": "int", "default": 5}, "waiter_timeout": {"type": "int", "default": 120}}
    module = TencentCloudModule(argument_spec=spec, required_one_of=[("strategy_id", "name")], supports_check_mode=True); p = module.params
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, strategy=None, strategy_id=None)
            if not p["allow_delete"]: module.fail_json(msg="set allow_delete=true to authorize deleting the DLC masking strategy", strategy=normalize(current))
            diff_value = maybe_diff(module, normalize(current), None)
            if not module.check_mode:
                p["strategy_id"] = current["StrategyId"]; module.sdk_call(client.DeleteDataMaskStrategy, delete_request(models, p["strategy_id"]))
                if p["wait"]: wait_strategy(module, client, models, p)
        ...
        if not current:
            ...
            if missing: module.fail_json(msg="creation parameters are required for a DLC masking strategy", missing=missing)
            target = desired(p); diff_value = maybe_diff(module, None, target); strategy_id = None
            if not module.check_mode:
                strategy_id = module.sdk_call(client.CreateDataMaskStrategy, create_request(models, target)).StrategyId
                p["strategy_id"] = strategy_id; target["StrategyId"] = strategy_id
                if p["wait"]: wait_strategy(module, client, models, p, target)
                current = find(module, client, models, p)
        ...
        if before == target: module.exit_json(changed=False, strategy=before, strategy_id=before["StrategyId"])
        diff_value = maybe_diff(module, before, target)
        if not module.check_mode:
            module.sdk_call(client.UpdateDataMaskStrategy, update_request(models, target)); p["strategy_id"] = before["StrategyId"]
            if p["wait"]: wait_strategy(module, client, models, p, target)
            current = find(module, client, models, p)
