...
options:
  nat_gateway_id:
    description: ID of the NAT gateway whose SNAT rules are returned.
    type: str
    required: true
  filters:
...
RETURN = r'''
snat_rules:
  description: Matching SNAT rules.
  returned: always
  type: list
  elements: dict
total_count:
...
def build_request(models, nat_gateway_id, filters, offset, limit):
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda offset, limit: build_request(
            models,
            module.params["nat_gateway_id"],
            module.params["filters"],
            offset,
            limit),
        lambda request: sdk_call(module, client.DescribeNatGatewaySourceIpTranslationNatRules, request),
        lambda response: response.SourceIpTranslationNatRuleSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    snat_rules = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, snat_rules=snat_rules,
