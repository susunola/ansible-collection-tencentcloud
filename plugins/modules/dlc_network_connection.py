...
def describe_request(models, p, offset=0):
    ...
    if p.get("vpc_id") is not None: request.DatasourceConnectionVpcId = p["vpc_id"]
    if p.get("connection_type") is not None: request.NetworkConnectionType = p["connection_type"]
    return request
...
def find(module, client, models, p):
    ...
    while True:
        ...
        for item in page:
            ...
            if value.get("DatasourceConnectionName") != p["name"]: continue
            if p.get("data_engine_name") is not None and value.get("HouseName") != p["data_engine_name"]: continue
            if p.get("vpc_id") is not None and value.get("DatasourceConnectionVpcId") != p["vpc_id"]: continue
            if p.get("connection_type") is not None and value.get("NetworkConnectionType") != p["connection_type"]: continue
            matches.append(value)
        offset += len(page)
        if not page or offset >= int(response.TotalCount or 0): break
    if not matches: module.fail_json(msg="DLC network connection was not found", name=p["name"])
    if len(matches) > 1: module.fail_json(msg="Multiple DLC network connections matched; provide data_engine_name, vpc_id or connection_type", name=p["name"])
    return matches[0]


def update_request(models, name, description):
    request = models.UpdateNetworkConnectionRequest(); request.NetworkConnectionName, request.NetworkConnectionDesc = name, description; return request


def wait_connection(module, client, models, p):
    def poll(): return "ready" if (find(module, client, models, p).get("NetworkConnectionDesc") or "") == p["description"] else "pending"
    wait_for_state(module, poll, ["ready"], timeout=p["waiter_timeout"], delay=p["waiter_delay"])


def run_module():
    spec = {
        "name": {"required": True}, "description": {"required": True}, "data_engine_name": {}, "vpc_id": {}, "connection_type": {"type": "int"},
    ...
    try:
        ...
        if (current.get("NetworkConnectionDesc") or "") == p["description"]: module.exit_json(changed=False, network_connection=current)
        target = dict(current); target["NetworkConnectionDesc"] = p["description"]; diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            module.sdk_call(client.UpdateNetworkConnection, update_request(models, p["name"], p["description"]))
            if p["wait"]: wait_connection(module, client, models, p)
            current = find(module, client, models, p)
