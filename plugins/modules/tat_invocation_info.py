...
def filters(models, values):
    ...
    for name, items in values:
        if items:
            item = models.Filter(); item.Name, item.Values = name, items if isinstance(items, list) else [items]; result.append(item)
    return result


def invocation_request(models, p, offset):
    request = models.DescribeInvocationsRequest(); request.Offset, request.Limit = offset, p["page_size"]
    if p.get("invocation_id"): request.InvocationIds = [p["invocation_id"]]
    else: request.Filters = filters(models, (("command-id", p.get("command_id")), ("instance-kind", p.get("instance_kind"))))
    return request


def task_request(models, p, offset):
    request = models.DescribeInvocationTasksRequest(); request.Offset, request.Limit, request.HideOutput = offset, p["page_size"], not p["include_output"]
    request.Filters = filters(models, (("invocation-id", p["invocation_id"]),)); return request


def scrub(value, include_output=False):
    result = dict(value); result["Parameters"], result["DefaultParameters"], result["CommandContent"] = "<redacted>", "<redacted>", "<redacted>"
    result.pop("CommandDocument", None)
    if not include_output and result.get("TaskResult"): result["TaskResult"] = {"Output": "<redacted>"}
    return result
...
def pages(module, operation, build, item_name, p):
    ...
    for _ in range(p["max_pages"]):
        response = module.sdk_call(operation, build(offset)); items = list(getattr(response, item_name) or [])
...
def run_module():
    module = TencentCloudModule(argument_spec={"invocation_id": {}, "command_id": {}, "instance_kind": {"choices": ["CVM", "LIGHTHOUSE"]}, "include_tasks": {"type": "bool", "default": True}, "include_output": {"type": "bool", "default": False}, "page_size": {"type": "int", "default": 100}, "max_pages": {"type": "int", "default": 1000}}, supports_check_mode=True, mutually_exclusive=[("invocation_id", "command_id"), ("invocation_id", "instance_kind")])
    p = module.params
    ...
    if p["include_output"] and not p.get("invocation_id"): module.fail_json(msg="include_output requires exact invocation_id mode")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TatClient, "tat.tencentcloudapi.com")
    try:
        values, total, truncated, request_id = pages(module, client.DescribeInvocations, lambda offset: invocation_request(models, p, offset), "InvocationSet", p); values = [scrub(value) for value in values]
        if p.get("invocation_id"):
            invocation = values[0] if values else None; tasks = []
            if invocation and p["include_tasks"]: tasks, _, _, request_id = pages(module, client.DescribeInvocationTasks, lambda offset: task_request(models, p, offset), "InvocationTaskSet", p); tasks = [scrub(value, p["include_output"]) for value in tasks]
