...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.api_gateway_usage_plan
    description: Manage Tencent Cloud API Gateway usage plans.
...
usage_plans:
  ...
  elements: dict
total_count:
  description: Number of usage plans reported by the API.
  returned: always
  type: int
request_id:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeUsagePlansStatus, request),
        lambda response: response.Result.UsagePlanStatusSet if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    usage_plans = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, usage_plans=usage_plans,
