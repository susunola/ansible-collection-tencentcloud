...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cfs_permission_group_info
    description: Gather information about Tencent Cloud CFS permission groups.
...
def describe_request(models):
    return models.DescribeCfsPGroupsRequest()


def create_request(models, p):
    request = models.CreateCfsPGroupRequest()
    request.Name, request.DescInfo = p["name"], p["description"]
    return request


def update_request(models, p, group_id):
    request = models.UpdateCfsPGroupRequest()
    request.PGroupId, request.Name, request.DescInfo = group_id, p["name"], p["description"]
    return request
...
def find(module, client, models, p):
    response = module.sdk_call(client.DescribeCfsPGroups, describe_request(models))
    matches = []
    for item in list(response.PGroupList or []):
        value = item._serialize(allow_none=True)
        if (p.get("permission_group_id") and value.get("PGroupId") == p["permission_group_id"]) or (
            not p.get("permission_group_id") and value.get("Name") == p.get("name")
        ):
            matches.append(value)
    if len(matches) > 1:
        module.fail_json(msg="Multiple CFS permission groups matched; specify permission_group_id")
    return matches[0] if matches else None


def desired(p):
    return {"Name": p["name"], "DescInfo": p["description"]}


def comparable(value):
    return {"Name": value.get("Name"), "DescInfo": value.get("DescInfo") or ""}
...
def run_module():
    module = TencentCloudModule(
        ...
        required_one_of=[("permission_group_id", "name")],
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, permission_group=None)
            diff = maybe_diff(module, comparable(current), None)
            if not module.check_mode:
                module.sdk_call(client.DeleteCfsPGroup, delete_request(models, current["PGroupId"]))
        ...
        if before == target:
            module.exit_json(changed=False, permission_group=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current:
                module.sdk_call(client.UpdateCfsPGroup, update_request(models, p, current["PGroupId"]))
            else:
                p["permission_group_id"] = module.sdk_call(client.CreateCfsPGroup, create_request(models, p)).PGroupId
            current = find(module, client, models, p)
