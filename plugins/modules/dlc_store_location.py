...
def create_request(models, location):
    request = models.CreateStoreLocationRequest(); request.StoreLocation = location; return request


def modify_request(models, enabled, location):
    request = models.ModifyAdvancedStoreLocationRequest(); request.Enable, request.StoreLocation = 1 if enabled else 0, location; return request


def desired(p, current=None):
    result = dict(current or {}); result["StoreLocation"] = p["store_location"]
    if p.get("advanced_enabled") is not None: result["AdvancedEnabled"] = p["advanced_enabled"]
    if p.get("advanced_store_location") is not None: result["AdvancedStoreLocation"] = p["advanced_store_location"]
    return result
...
def run_module():
    spec = {
        "store_location": {"required": True}, "advanced_enabled": {"type": "bool"}, "advanced_store_location": {},
    ...
    if not p["store_location"].startswith("cosn://"): module.fail_json(msg="store_location must use a cosn:// path")
    if p.get("advanced_store_location") is not None and not p["advanced_store_location"].startswith("cosn://"): module.fail_json(msg="advanced_store_location must use a cosn:// path")
    if p.get("advanced_enabled") is True and not p.get("advanced_store_location"): module.fail_json(msg="advanced_store_location is required when advanced_enabled=true")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = read(module, client, models); original = dict(current); changed = False
        if current["StoreLocation"] and current["StoreLocation"] != p["store_location"]:
            module.fail_json(msg="DLC base store location is immutable after initialization", store_location_config=current, immutable_drift={"StoreLocation": (current["StoreLocation"], p["store_location"])})
        if not current["StoreLocation"]:
            changed = True
            if not module.check_mode:
                module.sdk_call(client.CreateStoreLocation, create_request(models, p["store_location"]))
                if p["wait"]: wait_config(module, client, models, p, {"StoreLocation": p["store_location"]})
                current = read(module, client, models)
            else: current["StoreLocation"] = p["store_location"]
        ...
        if changes:
            ...
            if not module.check_mode:
                ...
                if p["wait"]: wait_config(module, client, models, p, {k: v[1] for k, v in changes.items()})
                current = read(module, client, models)
            else: current.update({k: v[1] for k, v in changes.items()})
