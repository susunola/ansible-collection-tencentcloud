...
def desired(params):
    ...
    if params.get("cooldown_ttl") is not None: value["CooldownTtl"] = params["cooldown_ttl"]
    if params.get("cooldown_datetime") is not None: value["CooldownDatetime"] = params["cooldown_datetime"]
    return value


def describe(module, client, models, instance_id, name):
    request = models.DescribeCoolDownPoliciesRequest(); request.InstanceId = instance_id
    ...
    if len(matches) > 1: module.fail_json(msg="Multiple Doris cooldown policies matched", name=name)
...
def request_for(models, params, update=False):
    ...
    request.InstanceId = params["instance_id"]; request.PolicyName = params["name"]
    request.CoolDownTtl = params.get("cooldown_ttl"); request.CoolDownDatetime = params.get("cooldown_datetime")
    return request
...
def run_module():
    module = TencentCloudModule(argument_spec={
        ...
        "cooldown_ttl": {}, "cooldown_datetime": {},
    }, required_one_of=[("cooldown_ttl", "cooldown_datetime")], mutually_exclusive=[("cooldown_ttl", "cooldown_datetime")], supports_check_mode=True)
    ...
    try:
        current = describe(module, client, models, params["instance_id"], params["name"]); target = desired(params)
