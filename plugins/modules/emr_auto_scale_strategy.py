...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.emr_auto_scale_strategy_info
    description: Gather information about Tencent Cloud EMR auto scale strategies.
...
def update(module, client, models, params, values, current, target):
    ...
    replacement["StrategyId"] = current.get("StrategyId")
    merged = [replacement if item.get("StrategyName") == params["name"] else item for item in values]
    payload = {"InstanceId": params["cluster_id"], "GroupId": params.get("group_id"), "StrategyType": 1 if params["strategy_type"] == "load" else 2}
...
def run_module():
    module = TencentCloudModule(
        ...
        required_if=[("state", "present", ["strategy"])],
    ...
    try:
        p = module.params
        values = describe(module, client, models, p["cluster_id"], p.get("group_id"), p["strategy_type"])
        current = find(values, p["name"])
        if p["state"] == "absent":
            if current is None:
                module.exit_json(changed=False, strategy=None)
            if not p["allow_node_termination"]:
                module.fail_json(msg="allow_node_termination=true is required to delete an EMR scaling strategy", strategy=current)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                delete(module, client, models, p, current)
            module.exit_json(changed=True, **(diff or {}), strategy=current if module.check_mode else None)
        target = wanted(p)
        if current is not None and not changed(subset(current, target), target):
            module.exit_json(changed=False, strategy=current)
        diff = maybe_diff(module, subset(current, target) if current else None, target)
        if not module.check_mode:
            if current is None:
                add(module, client, models, p, target)
            else:
                update(module, client, models, p, values, current, target)
