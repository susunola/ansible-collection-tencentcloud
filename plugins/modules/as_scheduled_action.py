...
options:
  ...
  state:
    description:
      - C(present) creates the scheduled action with V(CreateScheduledAction) when it does not exist and updates
        it with V(ModifyScheduledAction) when it differs. C(absent) deletes it with V(DeleteScheduledAction).
    type: str
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.as_scheduled_action_info
    description: Gather information about Tencent Cloud AUTOSCALING scheduled actions.
