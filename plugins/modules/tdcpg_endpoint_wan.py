...
def describe_request(models,cluster_id): r=models.DescribeClusterEndpointsRequest(); r.ClusterId=cluster_id; return r
def update_request(models,p): r=models.ModifyClusterEndpointWanStatusRequest(); r.ClusterId,r.EndpointId,r.WanStatus=p["cluster_id"],p["endpoint_id"],"OPEN" if p["state"]=="open" else "CLOSE"; return r
def find(module,client,models,p):
    ...
    if not matches: module.fail_json(msg="TDSQL-C PostgreSQL endpoint was not found",endpoint_id=p["endpoint_id"])
    return matches[0]
def is_open(value): return bool(value.get("WanIp") or value.get("WanDomain"))
def wait(module,client,models,p,target):
    deadline=time.time()+p["waiter_timeout"]
    while True:
        value=find(module,client,models,p)
        if is_open(value)==target: return value
        if time.time()>=deadline: module.fail_json(msg="Timed out waiting for endpoint public access convergence",endpoint=value,expected_open=target)
        time.sleep(p["waiter_delay"])


def run_module():
    module=TencentCloudModule(argument_spec={"cluster_id":{"required":True},"endpoint_id":{"required":True},"state":{"choices":["open","closed"],"default":"closed"}},supports_check_mode=True); p=module.params; module.require_sdk(); models,cm=_load(); client=module.create_client(cm.TdcpgClient,"tdcpg.tencentcloudapi.com")
    try:
        current=find(module,client,models,p); target=p["state"]=="open"
        if is_open(current)==target: module.exit_json(changed=False,endpoint=current)
        diff=maybe_diff(module,{"open":is_open(current)},{"open":target})
        if not module.check_mode: module.sdk_call(client.ModifyClusterEndpointWanStatus,update_request(models,p)); current=wait(module,client,models,p,target)
