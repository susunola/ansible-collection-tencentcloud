...
def describe_request(models, p, offset=0):
    request = models.DescribeOwaspWhiteRulesRequest(); request.Domain, request.Offset, request.Limit = p["domain"], offset, 100; return request
def _apply(request, models, p):
    ...
    return request
def create_request(models, p): return _apply(models.CreateOwaspWhiteRuleRequest(), models, p)
def update_request(models, p, rule_id):
    request = _apply(models.ModifyOwaspWhiteRuleRequest(), models, p); request.RuleId = rule_id; return request
def delete_request(models, p, rule_id):
    request = models.DeleteOwaspWhiteRuleRequest(); request.Domain, request.Ids = p["domain"], [rule_id]; return request
...
def comparable(v): return {"Name": v.get("Name"), "Ids": sorted(int(x) for x in (v.get("Ids") or [])), "Type": int(v.get("Type") or 0), "Strategies": _sorted(v.get("Strategies")), "LogicalOp": v.get("LogicalOp") or "and", "Status": int(v.get("Status") or 0)}
def desired(p): return {"Name": p["name"], "Ids": sorted(p["owasp_ids"]), "Type": p["allow_type"], "Strategies": _sorted(p["strategies"]), "LogicalOp": p["logical_operator"], "Status": 1 if p["enabled"] else 0}
def find(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeOwaspWhiteRules, describe_request(models, p, offset)); values = list(response.List or [])
        for item in values:
    ...
    if len(matches) > 1: module.fail_json(msg="Multiple WAF OWASP allowlist rules matched; specify rule_id")
    return matches[0] if matches else None


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "domain": {"required": True}, "rule_id": {"type": "int"}, "name": {}, "allow_type": {"type": "int", "choices": [0, 1], "default": 0}, "owasp_ids": {"type": "list", "elements": "int", "default": []}, "strategies": {"type": "list", "elements": "dict", "default": []}, "logical_operator": {"choices": ["and", "or"], "default": "and"}, "expire_time": {"type": "int", "default": 0}, "enabled": {"type": "bool", "default": True}}, required_one_of=[("rule_id", "name")], supports_check_mode=True)
    p = module.params
    if p["state"] == "present" and (not p.get("name") or not p["owasp_ids"]): module.fail_json(msg="name and owasp_ids are required when state=present")
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteOwaspWhiteRule, delete_request(models, p, int(current["RuleId"])))
        ...
        if not module.check_mode:
            if current: module.sdk_call(client.ModifyOwaspWhiteRule, update_request(models, p, int(current["RuleId"])))
            else: p["rule_id"] = int(module.sdk_call(client.CreateOwaspWhiteRule, create_request(models, p)).RuleId)
            current = find(module, client, models, p)
