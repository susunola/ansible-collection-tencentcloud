...
def describe_request(models, p, offset=0):
    ...
    item = models.Filter()
    item.Name, item.Values = ("db-instance-id", [p["instance_id"]]) if p.get("instance_id") else ("db-instance-name", [p["name"]])
...
def create_request(models, p):
    ...
    request.SpecCode, request.Storage, request.InstanceCount, request.Period = p["spec_code"], p["storage"], 1, p["period_months"]
    ...
    request.SecurityGroupIds, request.DeletionProtection = p["security_group_ids"], p["deletion_protection"]
    return request


def rename_request(models, instance_id, name):
    request = models.ModifyDBInstanceNameRequest()
    request.DBInstanceId, request.InstanceName = instance_id, name
    return request


def resize_request(models, p, instance_id):
    request = models.ModifyDBInstanceSpecRequest()
    request.DBInstanceId, request.Cpu, request.Memory, request.Storage = instance_id, p.get("cpu"), p.get("memory"), p.get("storage")
    return request


def renew_request(models, instance_id, flag):
    request = models.SetAutoRenewFlagRequest()
    request.DBInstanceIdSet, request.AutoRenewFlag = [instance_id], flag
    return request


def isolate_request(models, instance_id):
    request = models.IsolateDBInstancesRequest()
    request.DBInstanceIdSet = [instance_id]
    return request
...
def _wait(module, client, models, p, states):
    wait_for_state(
        module,
        lambda: (find(module, client, models, p) or {}).get("DBInstanceStatus"),
        states,
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "memory": {"type": "int"},
            "major_version": {},
            "charset": {"choices": ["UTF8", "LATIN1"], "default": "UTF8"},
            "admin_name": {"default": "dbadmin"},
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            status = current.get("DBInstanceStatus")
            instance_id = current["DBInstanceId"]
            if p["purge"]:
                if status not in ("isolated", "recycled", "offline"):
                    module.fail_json(msg="purge requires an already isolated PostgreSQL instance", current_status=status)
                diff = maybe_diff(module, current, None)
                if not module.check_mode:
                    module.sdk_call(client.DestroyDBInstance, destroy_request(models, instance_id))
                module.exit_json(changed=True, **(diff or {}), instance=None)
            if status in ("isolating", "isolated", "recycling", "recycled", "offline"):
                module.exit_json(changed=False, instance=current)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.IsolateDBInstances, isolate_request(models, instance_id))
                p["instance_id"] = instance_id
                _wait(module, client, models, p, ["isolated", "recycled"])
                current = find(module, client, models, p)
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a new PostgreSQL instance", missing=missing)
            target = {
                "DBInstanceName": p["name"],
                "Zone": p["zone"],
                "VpcId": p["vpc_id"],
                "SubnetId": p["subnet_id"],
                "DBInstanceStorage": p["storage"],
                "DBMajorVersion": p["major_version"],
            }
            diff = maybe_diff(module, None, target)
            if not module.check_mode:
                ids = module.sdk_call(client.CreateInstances, create_request(models, p)).DBInstanceIdSet or []
                p["instance_id"] = ids[0]
        ...
        if drift:
            module.fail_json(msg="PostgreSQL network placement and major version are immutable", immutable_drift=drift)
        desired = {
            "DBInstanceName": p.get("name") or current.get("DBInstanceName"),
            "DBInstanceCpu": p.get("cpu") if p.get("cpu") is not None else current.get("DBInstanceCpu"),
            "DBInstanceMemory": p.get("memory") if p.get("memory") is not None else current.get("DBInstanceMemory"),
            "DBInstanceStorage": p.get("storage") if p.get("storage") is not None else current.get("DBInstanceStorage"),
            "AutoRenew": p.get("auto_renew") if p.get("auto_renew") is not None else current.get("AutoRenew"),
        }
        ...
        instance_id = current["DBInstanceId"]
        if not module.check_mode:
            if before["DBInstanceName"] != desired["DBInstanceName"]:
                module.sdk_call(client.ModifyDBInstanceName, rename_request(models, instance_id, desired["DBInstanceName"]))
            if any(before[k] != desired[k] for k in ("DBInstanceCpu", "DBInstanceMemory", "DBInstanceStorage")):
                module.sdk_call(client.ModifyDBInstanceSpec, resize_request(models, p, instance_id))
                p["instance_id"] = instance_id
                _wait(module, client, models, p, ["running"])
            if before["AutoRenew"] != desired["AutoRenew"]:
                module.sdk_call(client.SetAutoRenewFlag, renew_request(models, instance_id, desired["AutoRenew"]))
            current = find(module, client, models, p)
        module.exit_json(changed=True, **(diff or {}), instance=current)
