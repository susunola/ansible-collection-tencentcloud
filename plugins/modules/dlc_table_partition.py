...
def kv(value):
    ...
    else: items = ((x.get("Key", x.get("key")), x.get("Value", x.get("value"))) for x in (value or []))
    return [{"Key": key, "Value": item} for key, item in sorted(items)]


def storage(value):
    if value is None: return None
    mapping = {"location": "Location", "input_format": "InputFormat", "output_format": "OutputFormat", "num_buckets": "NumBuckets", "compressed": "Compressed", "stored_as_sub_directories": "StoredAsSubDirectories", "serde_lib": "SerdeLib", "serde_name": "SerdeName", "bucket_cols": "BucketCols", "serde_params": "SerdeParams", "params": "Params", "cols": "Cols", "sort_columns": "SortColumns"}
    result = {}
    for source, target in mapping.items():
        current = value.get(target, value.get(source))
        if current is not None: result[target] = kv(current) if target in ("SerdeParams", "Params") else current
    return result


def normalize(value):
    result = dict(value or {}); result["Params"] = kv(result.get("Params"))
    if result.get("Sds") is not None: result["Sds"] = storage(result["Sds"])
    return result


def identity(p): return {"database_name": p["database_name"], "table_name": p["table_name"], "values": p["values"], "schema_name": p.get("schema_name"), "datasource_connection_name": p["datasource_connection_name"]}


def list_request(models, p, offset=0):
    request = models.DescribeDMSPartitionsRequest(); request.DatabaseName, request.TableName = p["database_name"], p["table_name"]
    ...
    if p.get("schema_name") is not None: request.SchemaName = p["schema_name"]
    return request
...
def find(module, client, models, p):
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeDMSPartitions, list_request(models, p, offset)); items = response.Partitions or []
        matches.extend(x._serialize(allow_none=True) for x in items if list(x.Values or []) == p["values"] and x.DatabaseName == p["database_name"] and x.TableName == p["table_name"])
        offset += len(items)
        if not items or offset >= int(response.Total or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple DLC partitions matched the exact values", partition_identity=identity(p))
    return normalize(matches[0]) if matches else None


def desired(p, current=None):
    result = dict(current or {}); result.update({"DatabaseName": p["database_name"], "TableName": p["table_name"], "Values": p["values"], "DatasourceConnectionName": p["datasource_connection_name"]})
    if p.get("schema_name") is not None: result["SchemaName"] = p["schema_name"]
    if p.get("name") is not None: result["Name"] = p["name"]
    if p.get("params") is not None: result["Params"] = kv(p["params"])
    if p.get("storage") is not None: result["Sds"] = storage(p["storage"])
    return result
...
def add_request(models, p):
    request = models.AddDMSPartitionsRequest(); request.Partitions = [_model(models.DMSPartition, desired(p))]; return request


def alter_request(models, p, current):
    request = models.AlterDMSPartitionRequest(); request.CurrentDbName, request.CurrentTableName = p["database_name"], p["table_name"]
    request.CurrentValues = current.get("Name") or "/".join(p["values"]); request.Partition = _model(models.DMSPartition, desired(p, current))
    request.DatasourceConnectionName = p["datasource_connection_name"]; return request


def drop_request(models, p):
    request = models.DropDMSPartitionsRequest(); request.DatabaseName, request.TableName, request.Values = p["database_name"], p["table_name"], p["values"]
    request.DatasourceConnectionName, request.DeleteData = p["datasource_connection_name"], p["delete_data"]
    if p.get("schema_name") is not None: request.SchemaName = p["schema_name"]
    if p.get("name") is not None: request.Name = p["name"]
    return request
...
def run_module():
    spec = {
        "state": {"choices": ["present", "absent"], "default": "present"}, "database_name": {"required": True}, "table_name": {"required": True},
        "values": {"type": "list", "elements": "str", "required": True}, "schema_name": {}, "name": {}, "datasource_connection_name": {"default": "DataLakeCatalog"},
        "params": {"type": "dict"}, "storage": {"type": "dict"}, "allow_delete": {"type": "bool", "default": False}, "delete_data": {"type": "bool", "default": False},
        "wait": {"type": "bool", "default": True}, "waiter_delay": {"type": "int", "default": 3}, "waiter_timeout": {"type": "int", "default": 180},
    }
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if not p["values"]: module.fail_json(msg="values must contain at least one partition value")
    if p["delete_data"] and (p["state"] != "absent" or not p["allow_delete"]): module.fail_json(msg="delete_data requires state=absent and allow_delete=true")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, partition=None, partition_identity=identity(p))
            if not p["allow_delete"]: module.fail_json(msg="set allow_delete=true to authorize dropping the DLC partition", partition=current)
            diff_value = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DropDMSPartitions, drop_request(models, p))
                if p["wait"]: wait_partition(module, client, models, p, absent=True)
        ...
        if not current:
            ...
            if not module.check_mode:
                ...
                if p["wait"]: wait_partition(module, client, models, p, expected={k: v for k, v in target.items() if k in ("Name", "Params", "Sds")})
                current = find(module, client, models, p)
        ...
        if not changes: module.exit_json(changed=False, partition=current, partition_identity=identity(p))
        ...
        if not module.check_mode:
            ...
            if p["wait"]: wait_partition(module, client, models, p, expected={k: v[1] for k, v in changes.items()})
            current = find(module, client, models, p)
