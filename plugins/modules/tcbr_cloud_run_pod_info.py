...
RETURN = r'''
cloud_run_pods:
  description: Matching TCBR cloud run pods.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.tcbr.v20220217 import models, tcbr_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-tcbr package is required.")

    client = tcbr_client.TcbrClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "tcbr.tencentcloudapi.com"),
    )
    paginator = Paginator(
    ...
    cloud_run_pods = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, cloud_run_pods=cloud_run_pods, total_count=total_count)
