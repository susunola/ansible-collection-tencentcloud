...
RETURN = r"""membership:
  description:
    - Requested group and effective member IDs.
  returned: always
  type: dict"""
...
def mutation_request(cls, p, consumer_ids):
    r = cls()
    r.GatewayId, r.ConsumerGroupId, r.ConsumerIds = p["gateway_id"], p["consumer_group_id"], consumer_ids
    return r
...
def list_values(module, client, request_factory, api, result_field):
    ...
    values = []
    while True:
        result = module.sdk_call(api, request_factory(offset)).Result
        page = getattr(result, result_field) if result else []
...
def resolve_names(module, client, models, p):
    if not p.get("consumer_group_id"):
        groups = list_values(
            module, client, lambda offset: _group_list_request(models, p, offset), client.DescribeCloudNativeAPIGatewayConsumerGroupList, "ConsumerGroups"
        )
        ids, missing = resolve_ids(groups, [p["consumer_group_name"]], "ConsumerGroupId")
        if missing:
            module.fail_json(msg="TSE gateway consumer group name was not found", consumer_group_name=missing[0])
        p["consumer_group_id"] = ids[0]
    if not p.get("consumer_ids"):
        consumers = list_values(
            module, client, lambda offset: _consumer_list_request(models, p, offset), client.DescribeCloudNativeAPIGatewayConsumerList, "Consumers"
        )
        ids, missing = resolve_ids(consumers, p["consumer_names"], "ConsumerId")
        if missing:
            module.fail_json(msg="TSE gateway consumer names were not found", consumer_names=missing)
        p["consumer_ids"] = ids
...
def inspect_members(module, client, models, p):
    ...
    for consumer_id in p["consumer_ids"]:
        ...
        if not result:
            module.fail_json(msg="TSE gateway consumer was not found", consumer_id=consumer_id)
        if p["consumer_group_id"] in group_ids(result._serialize(allow_none=True)):
            present.append(consumer_id)
    return present
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "consumer_group_id": {},
            "consumer_group_name": {},
            "consumer_ids": {"type": "list", "elements": "str"},
            "consumer_names": {"type": "list", "elements": "str"},
        },
        required_one_of=[("consumer_group_id", "consumer_group_name"), ("consumer_ids", "consumer_names")],
        mutually_exclusive=[("consumer_group_id", "consumer_group_name"), ("consumer_ids", "consumer_names")],
        supports_check_mode=True,
    )
    p = module.params
    supplied = p.get("consumer_ids") or p.get("consumer_names") or []
    if not 1 <= len(supplied) <= 10:
        module.fail_json(msg="consumer_ids or consumer_names must contain between 1 and 10 entries")
    if len(set(supplied)) != len(supplied):
        module.fail_json(msg="consumer_ids or consumer_names must not contain duplicates")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if not affected:
            module.exit_json(changed=False, membership={"ConsumerGroupId": p["consumer_group_id"], "ConsumerIds": before})
        diff = maybe_diff(module, {"ConsumerIds": before}, {"ConsumerIds": target})
        if not module.check_mode:
            if p["state"] == "present":
                api, cls = client.AddCloudNativeAPIGatewayConsumerInGroup, models.AddCloudNativeAPIGatewayConsumerInGroupRequest
            else:
                api, cls = client.RemoveCloudNativeAPIGatewayConsumerInGroup, models.RemoveCloudNativeAPIGatewayConsumerInGroupRequest
            module.sdk_call(api, mutation_request(cls, p, affected))
            effective = inspect_members(module, client, models, p)
        else:
            effective = target
        module.exit_json(
