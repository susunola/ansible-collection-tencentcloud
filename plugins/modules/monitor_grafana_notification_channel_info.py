...
options:
  instance_id:
    description: Grafana instance ID whose notification channels are returned.
    type: str
    required: true
  page_size:
...
grafana_notification_channels:
  description: Matching Grafana notification channels.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeGrafanaNotificationChannels, request),
        lambda response: response.NotificationChannelSet,
        lambda response: None,
    )
    item_set, total_count = paginator.fetch_all()
    grafana_notification_channels = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, grafana_notification_channels=grafana_notification_channels,
