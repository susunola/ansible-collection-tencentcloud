...
def statement_request(models, session_id, statement_id, task_id=None):
    request = models.DescribeNotebookSessionStatementRequest(); request.SessionId, request.StatementId = session_id, statement_id
    if task_id is not None: request.TaskId = task_id
    return request


def result_request(models, task_id, p, token=None):
    request = models.DescribeNotebookSessionStatementSqlResultRequest(); request.TaskId, request.MaxResults = task_id, p["max_results"]
    if token: request.NextToken = token
    if p.get("batch_id") is not None: request.BatchId = p["batch_id"]
    if p.get("data_field_cut_length") is not None: request.DataFieldCutLen = p["data_field_cut_length"]
    return request


def result_page(response):
    return {
        "TaskId": response.TaskId, "ResultSet": response.ResultSet,
        "ResultSchema": [x._serialize(allow_none=True) for x in (response.ResultSchema or [])],
        "NextToken": response.NextToken, "OutputPath": response.OutputPath, "UseTime": response.UseTime,
        "AffectRows": response.AffectRows, "DataAmount": response.DataAmount, "UiUrl": response.UiUrl,
...
def read_results(module, client, models, task_id, p):
    ...
    while True:
        ...
        page = result_page(response); pages.append(page); token = response.NextToken
        if not token: break
        if token in seen: module.fail_json(msg="DLC Notebook SQL-result pagination repeated a continuation token", task_id=task_id, next_token=token)
        seen.add(token)
    return pages
...
def run_module():
    spec = {
        "session_id": {"required": True}, "statement_id": {"required": True}, "task_id": {},
        "include_sql_result": {"type": "bool", "default": False}, "batch_id": {}, "max_results": {"type": "int", "default": 1000},
        "data_field_cut_length": {"type": "int"},
    }
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if not 1 <= p["max_results"] <= 1000: module.fail_json(msg="max_results must be between 1 and 1000")
    if p.get("data_field_cut_length") is not None and p["data_field_cut_length"] < 1: module.fail_json(msg="data_field_cut_length must be positive")
    ...
    try:
        ...
        if p["include_sql_result"]:
            ...
            if not task_id: module.fail_json(msg="task_id is required to retrieve SQL results and was not returned by the statement", statement=statement)
