...
def request(cls, name): value = cls(); value.SecretName = name; return value


def run_module():
    module = TencentCloudModule(argument_spec={"secret_name": {"required": True}, "include_history": {"type": "bool", "default": True}}, supports_check_mode=True); p = module.params
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.SsmClient, "ssm.tencentcloudapi.com")
    try:
        response = module.sdk_call(client.DescribeRotationDetail, request(models.DescribeRotationDetailRequest, p["secret_name"])); rotation = response._serialize(allow_none=True); request_id = rotation.pop("RequestId", response.RequestId)
        result = {"changed": False, "rotation": rotation, "request_id": request_id}
        if p["include_history"]:
            response = module.sdk_call(client.DescribeRotationHistory, request(models.DescribeRotationHistoryRequest, p["secret_name"])); history = response._serialize(allow_none=True); result["request_id"] = history.pop("RequestId", response.RequestId); result["history"] = history
