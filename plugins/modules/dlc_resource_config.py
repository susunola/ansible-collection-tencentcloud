...
NODE_FIELDS = {
    "name": "Name", "pod_cpu": "PodCpu", "pod_mem": "PodMem", "gpu_type": "GpuType", "gpu_num": "GpuNum",
    "envs": "Envs", "labels": "Labels", "resources_labels": "ResourcesLabels", "pod_num": "PodNum",
    "high_availability": "HighAvailability", "min_pod_num": "MinPodNum", "max_pod_num": "MaxPodNum",
    "enable_auto_scaling": "EnableAutoScaling", "resource_type": "ResourceType", "instance_type": "InstanceType",
    "spec": "Spec", "billing_item": "BillingItem",
}
...
def list_request(models, page=1, ray=False):
    ...
    request.Page, request.PageSize = page, 200; return request
...
def node(value):
    ...
    for source, target in NODE_FIELDS.items():
        ...
        if target in ("Envs", "Labels", "ResourcesLabels"):
            if current: result[target] = _pairs(current)
...
def normalize(value):
    ...
    if "Head" in result: result["Head"] = node(result["Head"])
    if "Worker" in result: result["Worker"] = workers(result["Worker"])
    return result
...
def find(module, client, models, name):
    ...
    while True:
        ...
        items = response.Items or []; matches.extend(x._serialize(allow_none=True) for x in items if x.Name == name)
        if not items or page >= int(response.TotalPages or 1): break
        page += 1
    if len(matches) > 1: module.fail_json(msg="Multiple DLC resource templates matched the exact name", name=name)
    return normalize(matches[0]) if matches else None


def payload(p, config_id=None):
    result = {"Name": p["name"]}
    if config_id: result["Id"] = config_id
    if p.get("description") is not None: result["Description"] = p["description"]
    if p.get("template_type") is not None: result["Type"] = p["template_type"]
    if p.get("head") is not None: result["Head"] = node(p["head"])
    if p.get("workers") is not None: result["Worker"] = workers(p["workers"])
    return result
...
def make_request(models, p, update=False, config_id=None):
    ...
    request.from_json_string(json.dumps(payload(p, config_id))); return request


def delete_request(models, config_id):
    request = models.DeleteResourceConfigRequest(); request.Id = config_id; return request
...
def scale_down(old_head, new_head, old_workers, new_workers):
    def lower(old, new): return any(old.get(k) is not None and new.get(k) is not None and new[k] < old[k] for k in CAPACITY)
    if old_head and new_head and lower(old_head, new_head): return True
    before = {x.get("Name"): x for x in old_workers or []}; after = {x.get("Name"): x for x in new_workers or []}
...
def references(module, client, models, config_id):
    ...
    for kind, method, ray in (("lab", client.ListLabs, False), ("ray_cluster", client.ListRayClusters, True)):
        ...
        while True:
            ...
            if not ray: request.Page, request.PageSize = page, 200
            response = module.sdk_call(method, request); items = response.Items or []
...
def run_module():
    spec = {
        ...
        "template_type": {"choices": ["Ray", "ray", "Spark", "spark"]}, "description": {}, "head": {"type": "dict"},
        "workers": {"type": "list", "elements": "dict"}, "allow_scale_down": {"type": "bool", "default": False},
        "allow_delete": {"type": "bool", "default": False}, "allow_delete_in_use": {"type": "bool", "default": False},
    ...
    if None in worker_names or len(worker_names) != len(set(worker_names)): module.fail_json(msg="each worker requires a unique name")
    for item in p.get("workers") or []:
        value = node(item)
        if value.get("MinPodNum") is not None and value.get("MaxPodNum") is not None and value["MinPodNum"] > value["MaxPodNum"]: module.fail_json(msg="worker min_pod_num must not exceed max_pod_num", worker=value)
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = find(module, client, models, p["name"])
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, resource_config=None, resource_config_id=None)
            if not p["allow_delete"]: module.fail_json(msg="set allow_delete=true to authorize deleting the DLC resource template", resource_config=current)
            refs = references(module, client, models, current["Id"])
            if refs and not p["allow_delete_in_use"]: module.fail_json(msg="set allow_delete_in_use=true to delete a DLC resource template referenced by Labs or Ray clusters", references=refs, resource_config=current)
            ...
            if not module.check_mode:
                ...
                if p["wait"]: wait_config(module, client, models, p, absent=True)
        ...
        if not current:
            ...
            if missing: module.fail_json(msg="creation parameters are required for a DLC resource template", missing=missing)
            after, diff_value = desired(p), maybe_diff(module, None, desired(p)); config_id = None
            if not module.check_mode:
                config_id = module.sdk_call(client.CreateResourceConfig, make_request(models, p)).Id
                if p["wait"]: wait_config(module, client, models, p, expected={k: v for k, v in after.items() if k != "Name"})
                current = find(module, client, models, p["name"])
            module.exit_json(changed=True, **(diff_value or {}), resource_config=current if not module.check_mode else after, resource_config_id=(current or {}).get("Id") or config_id)
        changes = drift(p, current)
        after = desired(p, current)
        if ("Head" in changes or "Worker" in changes) and scale_down(current.get("Head"), after.get("Head"), current.get("Worker"), after.get("Worker")) and not p["allow_scale_down"]:
            module.fail_json(msg="set allow_scale_down=true to authorize reducing DLC resource-template capacity", capacity_drift={k: v for k, v in changes.items() if k in ("Head", "Worker")})
        if not changes: module.exit_json(changed=False, resource_config=current, resource_config_id=current.get("Id"))
        ...
        if not module.check_mode:
            ...
            if p["wait"]: wait_config(module, client, models, p, expected={k: v[1] for k, v in changes.items()})
            current = find(module, client, models, p["name"])
