...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.monitor_prometheus_record_rule_info
    description: Gather information about Tencent Cloud Monitor Prometheus recording rules.
...
def build_describe(models, instance_id, name):
    ...
    item = models.Filter()
    item.Name, item.Values = "Name", [name]
...
def build_create(models, p):
    ...
    request.InstanceId, request.Name, request.Content = p["instance_id"], p["name"], p["content"]
    return request
...
def build_delete(models, p):
    ...
    request.InstanceId, request.Names = p["instance_id"], [p["name"]]
    return request


def find(module, client, models, p):
    response = module.sdk_call(client.DescribePrometheusRecordRules, build_describe(models, p["instance_id"], p["name"]))
    matches = [x._serialize(allow_none=True) for x in list(response.Records or []) if x.Name == p["name"]]
    if len(matches) > 1:
        module.fail_json(msg="Multiple Prometheus record rules have the requested name", name=p["name"])
...
def run_module():
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, record_rule=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeletePrometheusRecordRuleYaml, build_delete(models, p))
            module.exit_json(changed=True, **(diff or {}), record_rule=current if module.check_mode else None)
        target = {"Name": p["name"], "Content": p["content"]}
        before = {"Name": current.get("Name"), "Content": current.get("Content")} if current else None
        if before == target:
            module.exit_json(changed=False, record_rule=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            module.sdk_call(
                client.ModifyPrometheusRecordRuleYaml if current else client.CreatePrometheusRecordRuleYaml,
                build_update(models, p) if current else build_create(models, p),
            )
            current = find(module, client, models, p)
