...
def build_describe(models, group_id):
    request = models.DescribeMachineGroupConfigsRequest(); request.GroupId = group_id; return request


def build_apply(models, config_id, group_id):
    request = models.ApplyConfigToMachineGroupRequest(); request.ConfigId, request.GroupId = config_id, group_id; return request


def build_remove(models, config_id, group_id):
    request = models.DeleteConfigFromMachineGroupRequest(); request.ConfigId, request.GroupId = config_id, group_id; return request
...
def find(module, client, models, config_id, group_id):
    ...
    for item in list(response.Configs or []):
        if item.ConfigId == config_id: return {"ConfigId": config_id, "GroupId": group_id}
    return None


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "config_id": {"required": True}, "group_id": {"required": True}}, supports_check_mode=True)
    p = module.params; module.require_sdk(); models, cm = _load(); client = module.create_client(cm.ClsClient, "cls.tencentcloudapi.com")
    try:
        current = find(module, client, models, p["config_id"], p["group_id"]); target = {"ConfigId": p["config_id"], "GroupId": p["group_id"]}; present = p["state"] == "present"
