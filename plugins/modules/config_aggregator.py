...
def list_request(models, offset=0):
    request = models.ListAggregatorsRequest(); request.Offset, request.Limit = offset, 100; return request


def describe_request(models, group_id):
    request = models.DescribeAggregatorRequest(); request.AccountGroupId = group_id; return request


def create_request(models, p):
    request = models.CreateAggregatorRequest(); request.Name, request.Description, request.Type = p["name"], p["description"], p["aggregator_type"]
    request.AggregatorAccounts = []
    for value in p["accounts"]:
        item = models.AggregatorAccount(); item.MemberUin, item.MemberName = value["member_uin"], value["member_name"]; request.AggregatorAccounts.append(item)
    return request


def _accounts(values):
    return sorted(({"MemberUin": item.get("MemberUin") or item.get("member_uin"), "MemberName": item.get("MemberName") or item.get("member_name")} for item in values or []), key=lambda item: item["MemberUin"])


def find_aggregator(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.ListAggregators, list_request(models, offset)); values = list(response.Items or [])
        for value in values:
            item = value._serialize(allow_none=True)
            if p.get("account_group_id") and item.get("AccountGroupId") == p["account_group_id"]: matches.append(item)
            elif not p.get("account_group_id") and item.get("Name") == p["name"]: matches.append(item)
        offset += len(values)
        if offset >= int(response.Total or 0) or not values: break
    if len(matches) > 1: module.fail_json(msg="Multiple Config aggregators matched; specify account_group_id")
    if not matches: return None
    response = module.sdk_call(client.DescribeAggregator, describe_request(models, matches[0]["AccountGroupId"])); value = response._serialize(allow_none=True); value.pop("RequestId", None); value["AccountGroupId"] = matches[0]["AccountGroupId"]; return value


def run_module():
    module = TencentCloudModule(argument_spec={"account_group_id": {}, "name": {"required": True}, "description": {"default": ""}, "aggregator_type": {"required": True}, "accounts": {"type": "list", "elements": "dict", "default": [], "options": {"member_uin": {"type": "int", "required": True}, "member_name": {"required": True}}}}, supports_check_mode=True)
    p = module.params
    ...
    try:
        ...
        if current:
            before = {"Name": current.get("Name"), "Description": current.get("Description") or "", "Type": current.get("Type"), "AggregatorAccounts": _accounts(current.get("AggregatorAccounts"))}
            if before == desired: module.exit_json(changed=False, aggregator=current)
            module.fail_json(msg="Config aggregator attributes are immutable because the API exposes no update or delete operation", aggregator=current, desired=desired)
