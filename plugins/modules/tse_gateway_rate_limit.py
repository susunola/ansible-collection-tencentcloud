...
def request(cls,models,p,config=None):
    ...
    if config is not None: payload["LimitDetail"]=config
    r=cls(); r.from_json_string(json.dumps(payload)); return r
def apis(client,models,scope):
    if scope=="service": return (client.DescribeCloudNativeAPIGatewayServiceRateLimit,client.CreateCloudNativeAPIGatewayServiceRateLimit,client.ModifyCloudNativeAPIGatewayServiceRateLimit,client.DeleteCloudNativeAPIGatewayServiceRateLimit,models.DescribeCloudNativeAPIGatewayServiceRateLimitRequest,models.CreateCloudNativeAPIGatewayServiceRateLimitRequest,models.ModifyCloudNativeAPIGatewayServiceRateLimitRequest,models.DeleteCloudNativeAPIGatewayServiceRateLimitRequest)
    return (client.DescribeCloudNativeAPIGatewayRouteRateLimit,client.CreateCloudNativeAPIGatewayRouteRateLimit,client.ModifyCloudNativeAPIGatewayRouteRateLimit,client.DeleteCloudNativeAPIGatewayRouteRateLimit,models.DescribeCloudNativeAPIGatewayRouteRateLimitRequest,models.CreateCloudNativeAPIGatewayRouteRateLimitRequest,models.ModifyCloudNativeAPIGatewayRouteRateLimitRequest,models.DeleteCloudNativeAPIGatewayRouteRateLimitRequest)
...
def run_module():
    module=TencentCloudModule(argument_spec={"state":{"choices":["present","absent"],"default":"present"},"gateway_id":{"required":True},"scope":{"choices":["service","route"],"required":True},"resource":{"required":True},"config":{"type":"dict"}},required_if=[("state","present",("config",))],supports_check_mode=True); p=module.params; module.require_sdk(); models,cm=_load(); client=module.create_client(cm.TseClient,"tse.tencentcloudapi.com")
    try:
        describe,create,modify,delete,describe_cls,create_cls,modify_cls,delete_cls=apis(client,models,p["scope"]); before=current(module,describe,describe_cls,models,p)
        if p["state"]=="absent":
            if not before: module.exit_json(changed=False,rate_limit=None)
            diff=maybe_diff(module,before,None)
            if not module.check_mode: module.sdk_call(delete,request(delete_cls,models,p))
            module.exit_json(changed=True,**(diff or {}),rate_limit=None)
        target=p["config"]
        if before and contains(before,target): module.exit_json(changed=False,rate_limit=before)
        diff=maybe_diff(module,before,target)
        if not module.check_mode: module.sdk_call(modify if before else create,request(modify_cls if before else create_cls,models,p,target)); before=current(module,describe,describe_cls,models,p)
