...
def desired(params):
    return {"UserName": params["username"], "Host": params["host"], "Description": params["description"],
            "ReadOnly": params["read_only"], "DelayThresh": params["delay_threshold"],
            "SlaveConst": int(params["sticky_replica"]), "MaxUserConnections": params["max_user_connections"]}
...
def find(module, client, models, params):
    request = models.DescribeAccountsRequest(); request.InstanceId = params["instance_id"]
    response = module.sdk_call(client.DescribeAccounts, request)
    for item in response.Users or []:
        value = item._serialize(allow_none=True)
        if value.get("UserName") == params["username"] and (value.get("Host") or "%") == params["host"]: return value
    return None
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"},
        ...
        "password": {"no_log": True}, "rotate_password": {"type": "bool", "default": False}, "description": {"default": ""},
        "read_only": {"type": "int", "choices": [0, 1, 2, 3], "default": 0}, "delay_threshold": {"type": "int", "default": 10},
        "sticky_replica": {"type": "bool", "default": False}, "max_user_connections": {"type": "int", "default": 0}}, supports_check_mode=True)
    p = module.params
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteAccount, identity(models.DeleteAccountRequest(), p))
        ...
        if current: require_immutable_unchanged(module, before, target, ("ReadOnly", "DelayThresh", "SlaveConst", "MaxUserConnections"), "DCDB account")
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current is None:
                if not p["password"]: module.fail_json(msg="password is required when creating a DCDB account")
                request = identity(models.CreateAccountRequest(), p); request.Password, request.Description = p["password"], p["description"]
                request.ReadOnly, request.DelayThresh, request.SlaveConst, request.MaxUserConnections = p["read_only"], p["delay_threshold"], int(p["sticky_replica"]), p["max_user_connections"]
                module.sdk_call(client.CreateAccount, request)
            else:
                if before["Description"] != target["Description"]:
                    request = identity(models.ModifyAccountDescriptionRequest(), p); request.Description = p["description"]
                    module.sdk_call(client.ModifyAccountDescription, request)
                if p["rotate_password"]:
                    request = identity(models.ResetAccountPasswordRequest(), p); request.Password = p["password"]
