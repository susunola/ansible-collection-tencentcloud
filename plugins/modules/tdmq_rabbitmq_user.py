...
def describe_request(models, p, offset=0):
    ...
    request.InstanceId, request.User, request.Offset, request.Limit = p["instance_id"], p["name"], offset, 100
    return request


def apply_request(request, p, creating=False):
    request.InstanceId, request.User = p["instance_id"], p["name"]
    if creating or p.get("rotate_password"):
        request.Password = p.get("password")
    request.Description, request.Tags = p["description"], sorted(set(p["tags"]))
    if p.get("max_connections") is not None:
        request.MaxConnections = p["max_connections"]
    if p.get("max_channels") is not None:
        request.MaxChannels = p["max_channels"]
    request.EnableCamAuth = p["cam_auth_enabled"]
    return request


def create_request(models, p):
    return apply_request(models.CreateRabbitMQUserRequest(), p, True)


def update_request(models, p):
    return apply_request(models.ModifyRabbitMQUserRequest(), p)
...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRabbitMQUser, describe_request(models, p, offset))
        items = list(response.RabbitMQUserList or [])
        for item in items:
...
def comparable(value):
    return {
        ...
        "CamAuthEnabled": bool(value.get("CamAuthEnabled")),
    }
...
def desired(p, current=None):
    ...
    return {
        ...
        "MaxConnections": p["max_connections"] if p["max_connections"] is not None else current.get("MaxConnections"),
        "MaxChannels": p["max_channels"] if p["max_channels"] is not None else current.get("MaxChannels"),
        "CamAuthEnabled": p["cam_auth_enabled"],
    }
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "cam_auth_enabled": {"type": "bool", "default": False},
        },
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteRabbitMQUser, delete_request(models, p))
        ...
        if not module.check_mode:
            if not current and not p["password"]:
                module.fail_json(msg="password is required when creating a RabbitMQ user")
            module.sdk_call(
                client.ModifyRabbitMQUser if current else client.CreateRabbitMQUser, update_request(models, p) if current else create_request(models, p)
            )
            current = find(module, client, models, p)
