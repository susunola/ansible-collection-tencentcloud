...
def get_replication(client, bucket):
    try: return normalize(client.get_bucket_replication(Bucket=bucket))
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "role": {}, "rules": {"type": "list", "elements": "dict"}}, required_if=[("state", "present", ["role", "rules"])], supports_check_mode=True)
    ...
    try:
        current = get_replication(client, bucket); target = normalize({"Role": module.params.get("role"), "Rule": module.params.get("rules") or []})
        if module.params["state"] == "absent":
            if current is None: module.exit_json(changed=False, replication=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: client.delete_bucket_replication(Bucket=bucket)
            module.exit_json(changed=True, **(diff or {}), replication=current if module.check_mode else None)
        if current == target: module.exit_json(changed=False, replication=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode: client.put_bucket_replication(Bucket=bucket, ReplicationConfiguration=target)
