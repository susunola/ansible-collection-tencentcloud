...
def normalize_start(value):
    ...
    if not match: raise ValueError("start_time must use HH:MM or HH:MM:SS")
    return "%s:%s" % (match.group(1), match.group(2))


def expected_range(start, duration):
    start = normalize_start(start); hour, minute = (int(x) for x in start.split(":")); end_minutes = (hour * 60 + minute + duration * 60) % 1440
    return "%s-%02d:%02d" % (start, end_minutes // 60, end_minutes % 60)


def describe_request(models, instance_id): request = models.DescribeMaintenanceWindowRequest(); request.InstanceId = instance_id; return request


def modify_request(models, p):
    request = models.ModifyMaintenanceWindowRequest(); request.InstanceId = p["instance_id"]
    request.StartTime, request.Duration, request.WeekDays = normalize_start(p["start_time"]) + ":00", p["duration_hours"], sorted(p["week_days"], key=DAYS.index); return request
...
def run_module():
    spec = {"instance_id": {"required": True}, "start_time": {"required": True}, "duration_hours": {"type": "int", "choices": [1, 2, 3], "required": True}, "week_days": {"type": "list", "elements": "str", "required": True, "choices": DAYS}}
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    try: normalize_start(p["start_time"])
    except ValueError as exc: module.fail_json(msg=str(exc))
    if not p["week_days"] or len(p["week_days"]) != len(set(p["week_days"])): module.fail_json(msg="week_days must be non-empty and unique")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TdmysqlClient, "tdmysql.tencentcloudapi.com")
    try:
        current = get(module, client, models, p["instance_id"]); target = {"InstanceId": p["instance_id"], "MaintenanceWindow": expected_range(p["start_time"], p["duration_hours"]), "WeekDays": sorted(p["week_days"], key=DAYS.index)}
        if current == target: module.exit_json(changed=False, maintenance_window=current)
        diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            module.sdk_call(client.ModifyMaintenanceWindow, modify_request(models, p)); current = get(module, client, models, p["instance_id"])
