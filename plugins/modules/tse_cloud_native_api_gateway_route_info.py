...
RETURN = r'''
cloud_native_api_gateway_routes:
  description: Matching TSE cloud native api gateway routes.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeCloudNativeAPIGatewayRoutes, request),
        lambda response: response.Result.RouteList if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    cloud_native_api_gateway_routes = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, cloud_native_api_gateway_routes=cloud_native_api_gateway_routes,
