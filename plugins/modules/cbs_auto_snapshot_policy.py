...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cbs_auto_snapshot_policy_info
    description: Gather information about Tencent Cloud CBS automatic snapshot policies.
...
def describe_request(models, p, offset=0):
    ...
    if p.get("policy_id"):
        request.AutoSnapshotPolicyIds = [p["policy_id"]]
    return request


def create_request(models, p):
    request = models.CreateAutoSnapshotPolicyRequest()
    request.Policy, request.IsActivated, request.AutoSnapshotPolicyName = _policies(models, p["schedules"]), p["enabled"], p["name"]
    request.IsPermanent, request.RetentionDays = p["permanent"], p["retention_days"]
    return request


def update_request(models, p, policy_id):
    request = models.ModifyAutoSnapshotPolicyAttributeRequest()
    request.AutoSnapshotPolicyId, request.IsActivated, request.AutoSnapshotPolicyName = policy_id, p["enabled"], p["name"]
    request.IsPermanent, request.RetentionDays, request.Policy = p["permanent"], p["retention_days"], _policies(models, p["schedules"])
    return request


def delete_request(models, policy_id):
    request = models.DeleteAutoSnapshotPoliciesRequest()
    request.AutoSnapshotPolicyIds = [policy_id]
    return request


def bind_request(models, policy_id, disk_ids):
    request = models.BindAutoSnapshotPolicyRequest()
    request.AutoSnapshotPolicyId, request.DiskIds = policy_id, sorted(disk_ids)
    return request
...
def _schedule_key(value):
    return (tuple(value.get("Hour") or []), tuple(value.get("DayOfWeek") or []), tuple(value.get("DayOfMonth") or []), int(value.get("IntervalDays") or 0))


def _schedules(values):
    return sorted(values or [], key=_schedule_key)


def comparable(value):
    return {
        "AutoSnapshotPolicyName": value.get("AutoSnapshotPolicyName"),
        "Policy": _schedules(value.get("Policy")),
        "IsActivated": bool(value.get("IsActivated")),
        "IsPermanent": bool(value.get("IsPermanent")),
        "RetentionDays": int(value.get("RetentionDays") or 0),
        "DiskIds": sorted(value.get("DiskIdSet") or []),
    }


def desired(p):
    return {
        "AutoSnapshotPolicyName": p["name"],
        "Policy": _schedules(p["schedules"]),
        "IsActivated": p["enabled"],
        "IsPermanent": p["permanent"],
        "RetentionDays": p["retention_days"],
        "DiskIds": sorted(p["disk_ids"]),
    }
...
def find(module, client, models, p):
    ...
    while True:
        ...
        values = list(response.AutoSnapshotPolicySet or [])
        for item in values:
            value = item._serialize(allow_none=True)
            if (p.get("policy_id") and value.get("AutoSnapshotPolicyId") == p["policy_id"]) or (
                not p.get("policy_id") and value.get("AutoSnapshotPolicyName") == p.get("name")
            ):
        ...
        if offset >= int(response.TotalCount or 0) or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple CBS automatic snapshot policies matched; specify policy_id")
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "disk_ids": {"type": "list", "elements": "str", "default": []},
    ...
    p = module.params
    if p["state"] == "present" and (not p["name"] or not p["schedules"]):
        module.fail_json(msg="name and schedules are required when state=present")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if bound and not p["force_delete"]:
                module.fail_json(msg="policy is still bound to cloud disks; set force_delete=true to unbind and delete", disk_ids=bound)
        ...
        if not module.check_mode:
            if current:
                ...
                module.sdk_call(client.ModifyAutoSnapshotPolicyAttribute, update_request(models, p, policy_id))
            else:
            ...
            old_ids = set(current.get("DiskIdSet") or []) if current else set()
            new_ids = set(p["disk_ids"])
            if old_ids - new_ids:
