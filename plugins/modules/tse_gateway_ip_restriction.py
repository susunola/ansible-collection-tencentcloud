...
def get_current(module,client,models,p):
    response=module.sdk_call(client.DescribeCloudNativeAPIGatewayIPRestriction,request(models.DescribeCloudNativeAPIGatewayIPRestrictionRequest,p)); result=response.Result
...
def target_config(module,p,current):
    ...
    if not restriction or addresses is None: module.fail_json(msg="restriction_type and addresses are required when creating an IP restriction")
    return {"Enabled":p.get("enabled") if p.get("enabled") is not None else (current or {}).get("Enabled",True),"RestrictionType":restriction,"AddressList":addresses}
def run_module():
    module=TencentCloudModule(argument_spec={"state":{"choices":["present","absent"],"default":"present"},"gateway_id":{"required":True},"scope":{"choices":["service","route"],"required":True},"resource_id":{"required":True},"enabled":{"type":"bool"},"restriction_type":{"choices":["whiteList","blackList"]},"addresses":{"type":"list","elements":"str"}},supports_check_mode=True); p=module.params; module.require_sdk(); models,cm=_load(); client=module.create_client(cm.TseClient,"tse.tencentcloudapi.com")
    try:
        current=get_current(module,client,models,p)
        if p["state"]=="absent":
            if not current: module.exit_json(changed=False,ip_restriction=None)
            diff=maybe_diff(module,current,None)
            if not module.check_mode: module.sdk_call(client.DeleteCloudNativeAPIGatewayIPRestriction,request(models.DeleteCloudNativeAPIGatewayIPRestrictionRequest,p))
            module.exit_json(changed=True,**(diff or {}),ip_restriction=None)
        target=target_config(module,p,current); before={key:(current or {}).get(key) for key in target}
        if current and before==target: module.exit_json(changed=False,ip_restriction=current)
        diff=maybe_diff(module,before if current else None,target)
        if not module.check_mode: module.sdk_call(client.CreateOrModifyCloudNativeAPIGatewayIPRestriction,request(models.CreateOrModifyCloudNativeAPIGatewayIPRestrictionRequest,p,target)); current=get_current(module,client,models,p)
