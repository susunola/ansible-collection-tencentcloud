...
def list_request(models, p, offset):
    request = models.DescribeModelServiceGroupsRequest(); request.Offset, request.Limit = offset, p["page_size"]
...
def read_list(module, client, models, p):
    ...
    for _ in range(p["max_pages"]):
        response = module.sdk_call(client.DescribeModelServiceGroups, list_request(models, p, offset)); items = response.ServiceGroups or []
        values.extend(item._serialize(allow_none=True) for item in items)
        total, global_total, request_id = int(response.TotalCount or 0), int(response.GlobalTotalCount or 0), response.RequestId; offset += len(items)
...
def run_module():
    spec = {
        "service_id": {}, "service_group_id": {}, "project_id": {}, "filters": {"type": "dict", "default": {}},
        "tag_filters": {"type": "dict", "default": {}}, "order_field": {"choices": ["CreateTime", "UpdateTime"], "default": "UpdateTime"},
        "order": {"choices": ["ASC", "DESC"], "default": "DESC"}, "page_size": {"type": "int", "default": 100},
        "max_pages": {"type": "int", "default": 1000},
    }
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True, mutually_exclusive=[("service_id", "service_group_id"), ("service_id", "filters"), ("service_id", "tag_filters"), ("service_group_id", "filters"), ("service_group_id", "tag_filters")]); p = module.params
    ...
    try:
        if p.get("service_id"):
            response = module.sdk_call(client.DescribeModelService, service_request(models, p)); value = response.Service._serialize(allow_none=True) if response.Service else None
            module.exit_json(changed=False, service=value, request_id=response.RequestId)
        if p.get("service_group_id"):
            response = module.sdk_call(client.DescribeModelServiceGroup, group_request(models, p)); value = response.ServiceGroup._serialize(allow_none=True) if response.ServiceGroup else None
