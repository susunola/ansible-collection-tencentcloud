...
def column(value, position=None):
    ...
    for source, target in (("name", "Name"), ("type", "Type"), ("comment", "Comment"), ("precision", "Precision"), ("scale", "Scale")):
        ...
        if current is not None: result[target] = current
    if "Type" in result: result["Type"] = str(result["Type"]).lower()
    nullable = value.get("Nullable", value.get("nullable"))
    if nullable is not None: result["Nullable"] = str(nullable).lower() if isinstance(nullable, bool) else str(nullable).lower()
    if position is not None: result["Position"] = position
    return result
...
def pairs(values, fn): return [fn(item, index) if fn is column else fn(item) for index, item in enumerate(values or [])]


def normalize(value):
    result = dict(value or {}); base = dict(result.get("TableBaseInfo") or {})
    result["TableBaseInfo"] = {k: v for k, v in base.items() if k in ("DatabaseName", "TableName", "DatasourceConnectionName", "TableComment", "Type", "TableFormat", "PrimaryKeys") and v is not None}
    for key in ("Type", "TableFormat"):
        if key in result["TableBaseInfo"]: result["TableBaseInfo"][key] = str(result["TableBaseInfo"][key]).upper()
    if "PrimaryKeys" in result["TableBaseInfo"]: result["TableBaseInfo"]["PrimaryKeys"] = sorted(result["TableBaseInfo"]["PrimaryKeys"])
    result["Columns"] = pairs(result.get("Columns"), column)
    result["Partitions"] = pairs(result.get("Partitions"), partition)
    if result.get("InputFormatShort") is not None: result["InputFormatShort"] = str(result["InputFormatShort"]).upper()
    return result


def describe_request(models, p):
    request = models.DescribeTableRequest(); request.TableName, request.DatabaseName = p["name"], p["database_name"]
    request.DatasourceConnectionName = p["datasource_connection_name"]; return request


def find(module, client, models, p):
    try: response = module.sdk_call(client.DescribeTable, describe_request(models, p))
    except Exception as exc:
        if is_not_found(exc): return None
        raise
    if response.Table is None: return None
    value = normalize(response.Table._serialize(allow_none=True)); base = value.get("TableBaseInfo", {})
    return value if base.get("TableName") == p["name"] and base.get("DatabaseName") == p["database_name"] else None


def desired(p, current=None):
    result = dict(current or {}); base = dict(result.get("TableBaseInfo") or {})
    ...
    for source, target in (("comment", "TableComment"), ("table_type", "Type"), ("table_format", "TableFormat"), ("primary_keys", "PrimaryKeys")):
        if p.get(source) is not None:
            base[target] = sorted(p[source]) if source == "primary_keys" else (str(p[source]).upper() if source in ("table_type", "table_format") else p[source])
    result["TableBaseInfo"] = base
    if p.get("columns") is not None: result["Columns"] = pairs(p["columns"], column)
    if p.get("partitions") is not None: result["Partitions"] = pairs(p["partitions"], partition)
    if p.get("location") is not None: result["Location"] = p["location"]
    if p.get("data_format") is not None: result["InputFormatShort"] = p["data_format"].upper()
    return result
...
def generate_request(models, p):
    ...
    if p.get("comment") is not None: base["TableComment"] = p["comment"]
    if p.get("table_format") is not None: base["TableFormat"] = p["table_format"]
    if p.get("primary_keys") is not None: base["PrimaryKeys"] = p["primary_keys"]
    info = {"TableBaseInfo": base, "Columns": pairs(p["columns"], column), "Partitions": pairs(p.get("partitions"), partition), "DataFormat": {p["data_format"]: {}}}
    if p.get("location") is not None: info["Location"] = p["location"]
    request = models.CreateTableRequest(); request.TableInfo = _model(models.TableInfo, info); return request


def task_request(models, p, sql):
    request = models.CreateTasksRequest(); request.DatabaseName = p["database_name"]; request.DatasourceConnectionName = p["datasource_connection_name"]
    if p.get("data_engine_name") is not None: request.DataEngineName = p["data_engine_name"]
    if p.get("resource_group_name") is not None: request.ResourceGroupName = p["resource_group_name"]
    request.Tasks = _model(models.TasksInfo, {"TaskType": "SQLTask", "FailureTolerance": "Terminate", "SQL": base64.b64encode(sql.encode("utf-8")).decode("ascii")})
    return request


def task_status_request(models, task_id):
    request = models.DescribeTaskDetailRequest(); request.TaskInstanceId = task_id; return request
...
def wait_task(module, client, models, p, task_id):
    def poll():
        ...
        if info is None: return 0, "task not visible yet", None
...
def create(module, client, models, p):
    generated = module.sdk_call(client.CreateTable, generate_request(models, p)); sql = generated.Execution.SQL
    submitted = module.sdk_call(client.CreateTasks, task_request(models, p, sql)); task_ids = submitted.TaskIdSet or []
    if p["wait"]:
        for task_id in task_ids: wait_task(module, client, models, p, task_id)
        wait_table(module, client, models, p, True)
    return task_ids


def run_module():
    col = {"name": {"required": True}, "type": {"required": True}, "comment": {}, "precision": {"type": "int"}, "scale": {"type": "int"}, "nullable": {"type": "bool"}}
    part = {"name": {"required": True}, "type": {"required": True}, "comment": {}, "transform": {}, "transform_args": {"type": "list", "elements": "str"}}
    spec = {
        "state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "database_name": {"required": True},
        "datasource_connection_name": {"default": "DataLakeCatalog"}, "comment": {}, "table_type": {"default": "TABLE"}, "table_format": {},
        "data_format": {"choices": ["TextFile", "CSV", "Json", "Parquet", "ORC", "AVRO"], "default": "Parquet"}, "location": {},
        "primary_keys": {"type": "list", "elements": "str"}, "columns": {"type": "list", "elements": "dict", "options": col},
        "partitions": {"type": "list", "elements": "dict", "options": part}, "data_engine_name": {}, "resource_group_name": {},
        "allow_replace": {"type": "bool", "default": False}, "allow_delete": {"type": "bool", "default": False},
        "allow_delete_data": {"type": "bool", "default": False}, "wait": {"type": "bool", "default": True},
        "waiter_delay": {"type": "int", "default": 5}, "waiter_timeout": {"type": "int", "default": 900},
    }
    ...
    if len(names) != len(set(names)): module.fail_json(msg="column names must be unique")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, table=None, task_ids=[])
            if not p["allow_delete"]: module.fail_json(msg="set allow_delete=true to authorize deleting the DLC table", table=current)
            if has_data(current) and not p["allow_delete_data"]: module.fail_json(msg="DLC table contains data; set allow_delete_data=true to authorize deletion", table=current)
            diff_value = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteTable, delete_request(models, p))
                if p["wait"]: wait_table(module, client, models, p, False)
            module.exit_json(changed=True, **(diff_value or {}), table=None, task_ids=[])
        if not current:
            if not p.get("columns"): module.fail_json(msg="columns are required when creating a DLC table")
