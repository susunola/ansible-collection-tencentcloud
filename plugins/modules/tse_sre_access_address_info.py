...
def request(models,p):
    r=models.DescribeSREInstanceAccessAddressRequest(); r.InstanceId,r.VpcId,r.SubnetId,r.Workload,r.EngineRegion=p["instance_id"],p.get("vpc_id"),p.get("subnet_id"),p.get("workload"),p.get("engine_region"); return r
def serialize_response(response):
    value=response._serialize(allow_none=True) if hasattr(response,"_serialize") else {}
    value.pop("RequestId",None); return value
...
def run_module():
    ...
    try:
        response=module.sdk_call(client.DescribeSREInstanceAccessAddress,request(models,p)); module.exit_json(changed=False,access_address=serialize_response(response),request_id=getattr(response,"RequestId",None))
