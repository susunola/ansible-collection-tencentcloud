...
def describe_request(models, instance_id): request = models.DescribeDBParametersRequest(); request.InstanceId = instance_id; return request
...
def modify_request(models, instance_id, values):
    request = models.ModifyDBParametersRequest(); request.InstanceId = instance_id; request.Params = []
    for name, value in sorted(values.items()):
        item = models.DBParamValue(); item.Param, item.Value = name, str(value); request.Params.append(item)
    return request


def flow_request(models, task_id): request = models.DescribeFlowRequest(); request.FlowId = task_id; return request


def wait_task(module, client, models, task_id):
    def poll():
        response = module.sdk_call(client.DescribeFlow, flow_request(models, task_id)); status = str(response.Status or "").lower()
        if status in ("failed", "paused"): module.fail_json(msg="TDSQL MySQL parameter task failed", task_id=task_id, status=status, request_id=response.RequestId)
        return status
    wait_for_state(module, poll, ["success"], timeout=module.params["waiter_timeout"], delay=module.params["waiter_delay"])


def selected(current, names): return {name: current[name] for name in sorted(names) if name in current}


def run_module():
    spec = {"instance_id": {"required": True}, "parameters": {"type": "dict", "required": True}, "wait": {"type": "bool", "default": True}, "waiter_delay": {"type": "int", "default": 5}, "waiter_timeout": {"type": "int", "default": 600}}
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if not p["parameters"]: module.fail_json(msg="parameters must not be empty")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TdmysqlClient, "tdmysql.tencentcloudapi.com")
    try:
        all_current = read_parameters(module, client, models, p["instance_id"]); unknown = sorted(set(p["parameters"]) - set(all_current))
        if unknown: module.fail_json(msg="unknown TDSQL MySQL parameter names", unknown_parameters=unknown)
        current = selected(all_current, p["parameters"]); changes = {name: str(value) for name, value in p["parameters"].items() if str(current[name].get("Value")) != str(value)}
        restart_required = any(bool(current[name].get("NeedRestart")) for name in changes)
        if not changes: module.exit_json(changed=False, parameters=current, restart_required=False)
        target = {name: dict(value, Value=str(p["parameters"][name])) for name, value in current.items()}; diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            response = module.sdk_call(client.ModifyDBParameters, modify_request(models, p["instance_id"], changes))
            if p["wait"]: wait_task(module, client, models, response.TaskID)
