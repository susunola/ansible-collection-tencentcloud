#!/usr/bin/python
# -*- coding: utf-8 -*-
# Generated by scripts/generate_info_modules.py -- regenerate instead of editing.
# Copyright: (c) 2026, Tencent Cloud Ansible Collection Contributors
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)
from __future__ import absolute_import, division, print_function

__metaclass__ = type

DOCUMENTATION = r'''
---
module: iai_group_info
short_description: Gather information about Tencent Cloud IAI groups
version_added: "0.9.0"
description: Returns IAI groups visible in a Tencent Cloud region.
options:
  page_size:
    description: Number of results requested per API call.
    type: int
    default: 100
extends_documentation_fragment: tencentcloud.cloud.tencentcloud
author: Tencent Cloud Ansible Collection Contributors (@susunola)
'''

EXAMPLES = r'''
- name: List all groups
  tencentcloud.cloud.iai_group_info:
    region: ap-guangzhou
'''

RETURN = r'''
groups:
  description: Matching IAI groups.
  returned: always
  type: list
  elements: dict
total_count:
  description: Number of groups returned (the API reports no total count).
  returned: always
  type: int
'''

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.tencentcloud.cloud.plugins.module_utils.paging import Paginator
from ansible_collections.tencentcloud.cloud.plugins.module_utils.tencentcloud import (
    create_client_profile, create_credential, sdk_call, serialize_sdk_object,
    tencentcloud_argument_spec,
)


def build_request(models, offset, limit):
    request = models.GetGroupListRequest()
    request.Offset = offset
    request.Limit = limit
    return request


def run_module():
    argument_spec = tencentcloud_argument_spec()
    argument_spec.update({
        "page_size": {"type": "int", "default": 100},
    })
    module = AnsibleModule(
        argument_spec=argument_spec,
        supports_check_mode=True,
    )
    try:
        from tencentcloud.iai.v20200303 import models, iai_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-iai package is required.")

    client = iai_client.IaiClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "iai.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.GetGroupList, request),
        lambda response: response.GroupInfos,
        lambda response: None,
    )
    item_set, total_count = paginator.fetch_all()
    groups = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, groups=groups, total_count=total_count)


def main():
    run_module()


if __name__ == "__main__":
    main()
