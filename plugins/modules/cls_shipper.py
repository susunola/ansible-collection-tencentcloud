...
options:
  state:
    description:
      - C(present) creates the shipper with V(CreateShipper) when it does not exist and updates it with V(ModifyShipper)
        when it differs. C(absent) deletes it with V(DeleteShipper).
    type: str
