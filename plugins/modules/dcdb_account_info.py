...
options:
  instance_id:
    description: Sets the C(InstanceId) field of V(DescribeAccounts); omit it to leave that field unset.
    type: str
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.dcdb_account
    description: Manage Tencent Cloud DCDB accounts.
...
RETURN = r'''
accounts:
  description: Matching DCDB accounts.
  returned: always
  type: list
  elements: dict
  sample:
...
total_count:
  description: Number of accounts returned (the API reports no total count).
  returned: always
  type: int
request_id:
