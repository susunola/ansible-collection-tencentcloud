...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cos_bucket_policy_info
    description: Gather Tencent Cloud COS bucket policy.
...
def run_module():
    module = TencentCloudModule(
        argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "policy": {"type": "dict"}},
    ...
    try:
        current = get_policy(client, bucket)
        target = normalize(module.params.get("policy"))
        ...
        if not module.check_mode:
            client.put_bucket_policy(Bucket=bucket, Policy=json.dumps(target, sort_keys=True, separators=(",", ":")))
