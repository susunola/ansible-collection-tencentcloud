...
def instance_request(cls, instance_id):
    request = cls(); request.InstanceId = instance_id; return request
...
def describe(module, client, models, params):
    ...
    if params.get("security_group_ids") is not None:
        request = instance_request(models.DescribeDBSecurityGroupsRequest, params["instance_id"]); request.Product = "dcdb"
...
def desired(params):
    result = {}
    if params.get("encryption_enabled") is not None: result["encryption_enabled"] = params["encryption_enabled"]
    if params.get("ssl_enabled") is not None: result["ssl_enabled"] = params["ssl_enabled"]
    if params.get("security_group_ids") is not None: result["security_group_ids"] = sorted(set(params["security_group_ids"]))
    return result


def run_module():
    module = TencentCloudModule(argument_spec={"instance_id": {"required": True}, "encryption_enabled": {"type": "bool"},
        "ssl_enabled": {"type": "bool"}, "security_group_ids": {"type": "list", "elements": "str"}}, supports_check_mode=True)
    p = module.params
    if all(p.get(key) is None for key in ("encryption_enabled", "ssl_enabled", "security_group_ids")): module.fail_json(msg="at least one DCDB security control must be supplied")
    ...
    try:
        current, target = describe(module, client, models, p), desired(p); comparable = {key: current.get(key) for key in target}
        if current.get("encryption_enabled") and target.get("encryption_enabled") is False: module.fail_json(msg="DCDB data-at-rest encryption cannot be disabled", security_config=current)
        if comparable == target: module.exit_json(changed=False, security_config=current)
        diff = maybe_diff(module, comparable, target)
        if not module.check_mode:
            if target.get("encryption_enabled") and not current.get("encryption_enabled"):
                request = instance_request(models.ModifyDBEncryptAttributesRequest, p["instance_id"]); request.EncryptEnabled = 1
                module.sdk_call(client.ModifyDBEncryptAttributes, request)
                wait_for_state(module, lambda: describe(module, client, models, dict(p, ssl_enabled=None, security_group_ids=None)).get("encryption_enabled"), {True}, timeout=p["waiter_timeout"], delay=p["waiter_delay"])
            if "ssl_enabled" in target and current.get("ssl_enabled") != target["ssl_enabled"]:
                request = instance_request(models.ModifyInstanceSSLAttributesRequest, p["instance_id"]); request.SSLEnabled = int(target["ssl_enabled"])
                module.sdk_call(client.ModifyInstanceSSLAttributes, request)
                wanted_status = 2 if target["ssl_enabled"] else 3
                wait_for_state(module, lambda: describe(module, client, models, dict(p, encryption_enabled=None, security_group_ids=None)).get("ssl_status"), {wanted_status}, timeout=p["waiter_timeout"], delay=p["waiter_delay"])
            if "security_group_ids" in target and current.get("security_group_ids") != target["security_group_ids"]:
                request = instance_request(models.ModifyDBInstanceSecurityGroupsRequest, p["instance_id"]); request.Product = "dcdb"; request.SecurityGroupIds = target["security_group_ids"]
