...
def normalize(value):
    ...
    root = value.get("DomainCertificate", value); info = root.get("CertificateInfo") or {}
...
def get_certificate(client, bucket, domain_name):
    try: return normalize(client.get_bucket_domain_certificate(Bucket=bucket, DomainName=domain_name))
...
def run_module():
    module = TencentCloudModule(
        argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "domain_name": {"required": True}, "certificate_id": {}},
        required_if=[("state", "present", ["certificate_id"])], supports_check_mode=True)
    ...
    try:
        ...
        if current == target: module.exit_json(changed=False, domain_certificate=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if target is None:
                client.delete_bucket_domain_certificate(Bucket=bucket, DomainName=p["domain_name"])
            else:
                config = {"DomainList": {"DomainName": [p["domain_name"]]}, "CertificateInfo": {"CertType": "CustomCert", "CustomCert": {"CertID": p["certificate_id"]}}}
