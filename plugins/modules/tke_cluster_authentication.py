...
def build_describe(models, cluster_id): request = models.DescribeClusterAuthenticationOptionsRequest(); request.ClusterId = cluster_id; return request
def _model(models, name, value): item = getattr(models, name)(); item._deserialize(value); return item
def build_modify(models, p):
    request = models.ModifyClusterAuthenticationOptionsRequest(); request.ClusterId = p["cluster_id"]
    request.ServiceAccounts = _model(models, "ServiceAccountAuthenticationOptions", p["service_accounts"])
    request.OIDCConfig = _model(models, "OIDCConfigAuthenticationOptions", p["oidc"]); return request


def desired(p): return {"ServiceAccounts": p["service_accounts"], "OIDCConfig": p["oidc"]}
def find(module, client, models, cluster_id):
    ...
    return {"ServiceAccounts": response.ServiceAccounts._serialize(allow_none=True) if response.ServiceAccounts else {}, "OIDCConfig": response.OIDCConfig._serialize(allow_none=True) if response.OIDCConfig else {}}


def run_module():
    module = TencentCloudModule(argument_spec={"cluster_id": {"required": True}, "service_accounts": {"type": "dict", "required": True}, "oidc": {"type": "dict", "required": True}}, supports_check_mode=True)
    ...
    try:
        ...
        if current == target: module.exit_json(changed=False, authentication=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode: module.sdk_call(client.ModifyClusterAuthenticationOptions, build_modify(models, p)); current = find(module, client, models, p["cluster_id"])
