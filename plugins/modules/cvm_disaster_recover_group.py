...
def describe_request(models, p, offset=0):
    ...
    if p.get("group_id"):
        request.DisasterRecoverGroupIds = [p["group_id"]]
    elif p.get("name"):
        request.Name = p["name"]
    return request
...
def create_request(models, p):
    ...
    request.Name, request.Type = p["name"], p["placement_type"]
    request.Affinity = p["affinity"]
    return request


def update_request(models, p, group_id):
    request = models.ModifyDisasterRecoverGroupAttributeRequest()
    request.DisasterRecoverGroupId, request.Name, request.Affinity = group_id, p["name"], p["affinity"]
    return request


def delete_request(models, group_id):
    request = models.DeleteDisasterRecoverGroupsRequest()
    request.DisasterRecoverGroupIds = [group_id]
    return request


def find(module, client, models, p):
    response = module.sdk_call(client.DescribeDisasterRecoverGroups, describe_request(models, p))
    matches = []
    for item in response.DisasterRecoverGroupSet or []:
        value = item._serialize(allow_none=True)
        if (p.get("group_id") and value.get("DisasterRecoverGroupId") == p["group_id"]) or (not p.get("group_id") and value.get("Name") == p.get("name")):
            matches.append(value)
    if len(matches) > 1:
        module.fail_json(msg="Multiple CVM placement groups matched; specify group_id")
...
def comparable(v):
    return {
        "Name": v.get("Name"),
        "Type": v.get("Type"),
        "Affinity": int(v.get("Affinity") or 1),
    }


def desired(p):
    return {"Name": p["name"], "Type": p["placement_type"], "Affinity": p["affinity"]}
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "group_id": {},
            "name": {},
            "placement_type": {"choices": ["HOST", "SW", "RACK"], "default": "HOST"},
            "affinity": {"type": "int", "default": 1},
    ...
    if not 1 <= p["affinity"] <= 10:
        module.fail_json(msg="affinity must be between 1 and 10")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, placement_group=None)
            diff = maybe_diff(module, comparable(current), None)
            if not module.check_mode:
                module.sdk_call(client.DeleteDisasterRecoverGroups, delete_request(models, current["DisasterRecoverGroupId"]))
        ...
        immutable = bool(current and before["Type"] != target["Type"])
        ...
        if replace and not p["force_replace"]:
            module.fail_json(
                msg="immutable placement properties changed or affinity decreased; set force_replace=true to replace an empty group",
                current=before,
                desired=target,
            )
        if replace and current.get("InstanceIds"):
            module.fail_json(msg="cannot replace a non-empty placement group", instance_ids=current["InstanceIds"])
        if before == target:
            module.exit_json(changed=False, placement_group=current)
        ...
        if not module.check_mode:
            if replace:
                ...
                current = None
            if current:
                module.sdk_call(client.ModifyDisasterRecoverGroupAttribute, update_request(models, p, current["DisasterRecoverGroupId"]))
            else:
