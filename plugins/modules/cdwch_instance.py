...
def _spec(models, name, count, disk):
    if name is None and count is None and disk is None:
        return None
    item = models.NodeSpec()
    item.SpecName, item.Count, item.DiskSize = name, count, disk
    return item
...
def _secondary_zones(models, values):
    ...
    for value in values or []:
        item = models.SecondaryZoneInfo()
        item.SecondaryZone, item.SecondarySubnet = value["zone"], value["subnet_id"]
        item.UserIpNum, item.SecondaryUserSubnetIPNum = str(value.get("user_ip_count")) if value.get("user_ip_count") is not None else None, value.get(
            "user_ip_count"
        )
        result.append(item)
    return result
...
def describe_request(models, p, offset=0):
    r = models.DescribeInstancesNewRequest()
    r.Offset, r.Limit, r.IsSimple = offset, 100, False
    r.SearchInstanceId = p.get("instance_id")
    r.SearchInstanceName = p.get("name") if not p.get("instance_id") else None
    return r


def detail_request(models, instance_id):
    r = models.DescribeInstanceRequest()
    r.InstanceId, r.IsOpenApi = instance_id, True
    return r


def create_request(models, p):
    r = models.CreateInstanceNewRequest()
    r.Zone, r.HaFlag, r.UserVPCId, r.UserSubnetId = p["zone"], bool(p.get("high_availability")), p["vpc_id"], p["subnet_id"]
    r.ProductVersion, r.InstanceName = p["product_version"], p["name"]
    r.ChargeProperties = models.Charge()
    r.ChargeProperties.ChargeType = p.get("charge_type") or "POSTPAID_BY_HOUR"
    r.ChargeProperties.RenewFlag, r.ChargeProperties.TimeSpan = 1 if p["auto_renew"] else 0, p["period_months"]
    r.DataSpec = _spec(models, p["data_spec_name"], p["data_node_count"], p["data_disk_size"])
    r.CommonSpec = _spec(models, p.get("common_spec_name"), p.get("common_node_count"), p.get("common_disk_size"))
    r.TagItems, r.ClsLogSetId, r.CosBucketName = _tags(models, p.get("tags")), p.get("cls_logset_id"), p.get("cos_bucket_name")
    r.MountDiskType, r.HAZk = p.get("mount_disk_type"), p.get("zk_high_availability")
    r.SecondaryZoneInfo, r.CkDefaultUserPwd = _secondary_zones(models, p.get("secondary_zones")), p["password"]
    return r
...
def scale_nodes_request(models, p, instance_id, node_type, count):
    ...
    r.InstanceId, r.Type, r.NodeCount = instance_id, node_type, count
    r.ScaleOutCluster, r.UserSubnetIPNum, r.ScaleOutNodeIp = p.get("scale_out_cluster"), p.get("user_subnet_ip_count"), p.get("scale_out_node_ip")
    r.ReduceShardInfo = p.get("reduce_shard_info")
    return r


def scale_spec_request(models, instance_id, node_type, spec_name, rolling=True):
    r = models.ScaleUpInstanceRequest()
    r.InstanceId, r.Type, r.SpecName, r.ScaleUpEnableRolling = instance_id, node_type, spec_name, rolling
    return r


def resize_disk_request(models, instance_id, node_type, size):
    r = models.ResizeDiskRequest()
    r.InstanceId, r.Type, r.DiskSize = instance_id, node_type, size
    return r


def find(module, client, models, p):
    response = module.sdk_call(client.DescribeInstancesNew, describe_request(models, p))
    matches = []
    ...
    if len(matches) > 1:
        module.fail_json(msg="Multiple TCHouse-C instances matched; specify instance_id")
    if matches:
        response = module.sdk_call(client.DescribeInstance, detail_request(models, matches[0]["InstanceId"]))
        detail = response.InstanceInfo._serialize(allow_none=True) if response.InstanceInfo else {}
...
def _wait(module, client, models, p, predicate, label):
    def poll():
        ...
        if current is None:
            return "READY" if predicate(None) else "ABSENT"
        state = str(current.get("Status", ""))
        flow = current.get("InstanceStateInfo") or {}
        if state.lower() in ("failed", "error") or (flow.get("FlowMsg") and "fail" in str(flow.get("FlowMsg")).lower()):
            module.fail_json(msg="TCHouse-C asynchronous operation failed", operation=label, instance=current)
...
def _tag_dict(values):
    return {item.get("TagKey"): item.get("TagValue") for item in values or []}
...
def run_module():
    spec = {
        ...
        "zk_high_availability": {"type": "bool"},
        "data_spec_name": {},
        "data_node_count": {"type": "int"},
        "data_disk_size": {"type": "int"},
        "common_spec_name": {},
        "common_node_count": {"type": "int"},
        "common_disk_size": {"type": "int"},
        ...
        "cls_logset_id": {},
        "cos_bucket_name": {},
        "mount_disk_type": {"type": "int", "choices": [0, 1, 2]},
        "secondary_zones": {
            "type": "list",
            "elements": "dict",
            "options": {"zone": {"required": True}, "subnet_id": {"required": True}, "user_ip_count": {"type": "int"}},
        },
        "scale_out_cluster": {},
        "user_subnet_ip_count": {"type": "int"},
        "scale_out_node_ip": {},
        "reduce_shard_info": {"type": "list", "elements": "str"},
        "allow_scale_in": {"type": "bool", "default": False},
        "rolling_spec_change": {"type": "bool", "default": True},
    }
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DestroyInstance, destroy_request(models, instance_id))
                p["instance_id"] = instance_id
                _wait(module, client, models, p, lambda value: value is None or value.get("Status") == "Deleted", "destroy")
            module.exit_json(changed=True, **(diff or {}), instance=None)
        if not current:
            missing = [
                key
                for key in ("name", "zone", "vpc_id", "subnet_id", "product_version", "data_spec_name", "data_node_count", "data_disk_size", "password")
                if p.get(key) is None
            ]
            if missing:
                module.fail_json(msg="creation parameters are required for a new TCHouse-C instance", missing=missing)
            target = {
                "InstanceName": p["name"],
                "Zone": p["zone"],
                "VpcId": p["vpc_id"],
                ...
                "MasterSummary": {"Spec": p["data_spec_name"], "NodeSize": p["data_node_count"], "Disk": p["data_disk_size"]},
            }
            ...
            if not module.check_mode:
                ...
                _wait(module, client, models, p, lambda value: value is not None, "create")
                current = find(module, client, models, p)
            module.exit_json(changed=True, **(diff or {}), instance=current if not module.check_mode else target)
        master, common = current.get("MasterSummary") or {}, current.get("CommonSummary") or {}
        current_pay = str(current.get("PayMode", "")).lower()
        wanted_pay = {"PREPAID": "prepay", "POSTPAID_BY_HOUR": "hour"}.get(p.get("charge_type"))
        immutable = {
            "InstanceName": p.get("name"),
            "Zone": p.get("zone"),
            "VpcId": p.get("vpc_id"),
            "SubnetId": p.get("subnet_id"),
            "Version": p.get("product_version"),
            "HA": str(p["high_availability"]).lower() if p.get("high_availability") is not None else None,
            "HAZk": p.get("zk_high_availability"),
            "PayMode": wanted_pay,
            "Tags": p.get("tags"),
        }
        observed = {
            ...
            "HA": str(current.get("HA", "")).lower(),
            "HAZk": current.get("HAZk"),
            "PayMode": current_pay,
            "Tags": _tag_dict(current.get("Tags")),
        }
        drift = {key: (observed.get(key), value) for key, value in immutable.items() if value is not None and observed.get(key) != value}
        if drift:
            module.fail_json(msg="TCHouse-C identity, placement, version, billing, HA and tags are immutable", immutable_drift=drift)
        before = {
            "DataSpec": master.get("Spec"),
            "DataCount": master.get("NodeSize"),
            "DataDisk": master.get("Disk"),
            "CommonSpec": common.get("Spec"),
            "CommonCount": common.get("NodeSize"),
            "CommonDisk": common.get("Disk"),
        }
        desired = {
            "DataSpec": p.get("data_spec_name") or before["DataSpec"],
            "DataCount": p.get("data_node_count") if p.get("data_node_count") is not None else before["DataCount"],
            "DataDisk": p.get("data_disk_size") if p.get("data_disk_size") is not None else before["DataDisk"],
            "CommonSpec": p.get("common_spec_name") or before["CommonSpec"],
            "CommonCount": p.get("common_node_count") if p.get("common_node_count") is not None else before["CommonCount"],
            "CommonDisk": p.get("common_disk_size") if p.get("common_disk_size") is not None else before["CommonDisk"],
        }
        for key in ("DataDisk", "CommonDisk"):
            if desired[key] is not None and before[key] is not None and desired[key] < before[key]:
                module.fail_json(msg="TCHouse-C disks cannot be reduced", field=key)
        ...
        if shrinking and not p["allow_scale_in"]:
            module.fail_json(msg="set allow_scale_in=true to authorize reducing TCHouse-C node count")
        if shrinking and not p.get("reduce_shard_info"):
            module.fail_json(msg="reduce_shard_info is required for TCHouse-C scale-in")
        ...
        if growing and (p.get("user_subnet_ip_count") is None or not p.get("scale_out_node_ip")):
            module.fail_json(msg="user_subnet_ip_count and scale_out_node_ip are required for TCHouse-C scale-out")
        if before == desired:
        ...
        if not module.check_mode:
            for prefix, node_type, summary_key in (("Data", "DATA", "MasterSummary"), ("Common", "COMMON", "CommonSummary")):
                if before[prefix + "Spec"] != desired[prefix + "Spec"]:
                    module.sdk_call(
                        client.ScaleUpInstance, scale_spec_request(models, instance_id, node_type, desired[prefix + "Spec"], p["rolling_spec_change"])
                    )
                    _wait(
                        module,
                        client,
                        models,
                        p,
                        lambda value, k=summary_key, target=desired[prefix + "Spec"]: (value.get(k) or {}).get("Spec") == target,
                        "scale specification",
                    )
                if before[prefix + "Disk"] != desired[prefix + "Disk"]:
                    module.sdk_call(client.ResizeDisk, resize_disk_request(models, instance_id, node_type, desired[prefix + "Disk"]))
                    _wait(
                        module,
                        client,
                        models,
                        p,
                        lambda value, k=summary_key, target=desired[prefix + "Disk"]: (value.get(k) or {}).get("Disk") == target,
                        "resize disk",
                    )
                if before[prefix + "Count"] != desired[prefix + "Count"]:
                    module.sdk_call(client.ScaleOutInstance, scale_nodes_request(models, p, instance_id, node_type, desired[prefix + "Count"]))
                    _wait(
                        module,
                        client,
                        models,
                        p,
                        lambda value, k=summary_key, target=desired[prefix + "Count"]: (value.get(k) or {}).get("NodeSize") == target,
                        "scale nodes",
                    )
            current = find(module, client, models, p)
