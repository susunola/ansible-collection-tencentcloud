...
def get_domains(client, bucket):
    try:
        ...
        if cos.is_not_found(exc): return None, None
        raise
...
def run_module():
    ...
    try:
        current, verification = get_domains(client, bucket); target = normalize({"DomainRule": module.params.get("rules") or []})
        if module.params["state"] == "absent": target = None
        if current == target: module.exit_json(changed=False, domains=current, txt_verification=verification)
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if target is None: client.delete_bucket_domain(Bucket=bucket)
            else: client.put_bucket_domain(Bucket=bucket, DomainConfiguration=target)
