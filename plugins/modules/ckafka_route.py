...
def describe_request(models, instance_id, route_id=None):
    request = models.DescribeRouteRequest(); request.InstanceId, request.RouteId = instance_id, route_id; return request


def create_request(models, p):
    request = models.CreateRouteRequest(); request.InstanceId, request.VipType, request.AccessType = p["instance_id"], p["network_type"], p["access_type"]
...
def delete_request(models, instance_id, route_id):
    request = models.DeleteRouteRequest(); request.InstanceId, request.RouteId = instance_id, route_id; return request


def find(module, client, models, p):
    response = module.sdk_call(client.DescribeRoute, describe_request(models, p["instance_id"], p.get("route_id"))); items = list(response.Result.Routers or [])
    for item in items:
        value = item._serialize(allow_none=True)
        if p.get("route_id") and int(value.get("RouteId") or 0) == p["route_id"]: return value
        if not p.get("route_id") and value.get("VipType") == p["network_type"] and value.get("AccessType") == p["access_type"] and (value.get("VpcId") or None) == p.get("vpc_id") and (value.get("Subnet") or None) == p.get("subnet_id"): return value
    return None


def comparable(value):
    return {"VipType": value.get("VipType"), "AccessType": value.get("AccessType"), "VpcId": value.get("VpcId") or None, "Subnet": value.get("Subnet") or None, "Note": value.get("Note") or ""}
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "instance_id": {"required": True}, "route_id": {"type": "int"}, "network_type": {"type": "int", "choices": [1, 3, 7], "default": 3}, "access_type": {"type": "int", "choices": [0, 1, 3, 4, 5], "default": 0}, "vpc_id": {}, "subnet_id": {}, "public_bandwidth": {"type": "int"}, "note": {"default": ""}, "security_group_ids": {"type": "list", "elements": "str", "default": []}, "ip_whitelist": {"type": "list", "elements": "str", "default": []}}, supports_check_mode=True)
    p = module.params
    needs_identity = p["state"] == "present" or p["route_id"] is None
    if needs_identity and p["network_type"] == 3 and (not p["vpc_id"] or not p["subnet_id"]): module.fail_json(msg="vpc_id and subnet_id are required for VPC routes")
    if needs_identity and p["network_type"] == 1 and p["public_bandwidth"] is None: module.fail_json(msg="public_bandwidth is required for public routes")
    if p["public_bandwidth"] is not None and p["public_bandwidth"] % 3: module.fail_json(msg="public_bandwidth must be a multiple of 3")
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteRoute, delete_request(models, p["instance_id"], current["RouteId"]))
        ...
        if before == target: module.exit_json(changed=False, route=current)
        diff = maybe_diff(module, before, target)
        if current: require_immutable_unchanged(module, before, target, tuple(target), "CKafka route")
        if not module.check_mode: module.sdk_call(client.CreateRoute, create_request(models, p)); current = find(module, client, models, p)
