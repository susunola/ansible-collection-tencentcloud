...
def describe_request(models, p, offset=0):
    r = models.DescribeRabbitMQServerlessBindingsRequest(); r.InstanceId, r.VirtualHost, r.Offset, r.Limit, r.SourceExchange = p["instance_id"], p["virtual_host"], offset, 100, p["source_exchange"]
    if p["destination_type"] == "queue": r.QueueName = p["destination"]
    else: r.DestinationExchange = p["destination"]
    return r
def create_request(models, p):
    r = models.CreateRabbitMQServerlessBindingRequest(); r.InstanceId, r.VirtualHost, r.Source = p["instance_id"], p["virtual_host"], p["source_exchange"]; r.DestinationType, r.Destination, r.RoutingKey = p["destination_type"], p["destination"], p["routing_key"]; return r
def delete_request(models, p, binding_id):
    r = models.DeleteRabbitMQServerlessBindingRequest(); r.InstanceId, r.VirtualHost, r.BindingId = p["instance_id"], p["virtual_host"], binding_id; return r
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRabbitMQServerlessBindings, describe_request(models, p, offset)); items = list(response.BindingInfoList or [])
        for item in items:
...
def run_module():
    spec = {"state": {"choices": ["present", "absent"], "default": "present"}, "instance_id": {"required": True}, "virtual_host": {"required": True}, "binding_id": {"type": "int"}, "source_exchange": {"required": True}, "destination_type": {"choices": ["queue", "exchange"], "required": True}, "destination": {"required": True}, "routing_key": {"default": "", "no_log": False}}
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteRabbitMQServerlessBinding, delete_request(models, p, current["BindingId"]))
        ...
        if not module.check_mode: module.sdk_call(client.CreateRabbitMQServerlessBinding, create_request(models, p)); current = find(module, client, models, p)
