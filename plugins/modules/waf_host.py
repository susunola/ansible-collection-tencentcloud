...
def build_get(models, p):
    request = models.DescribeHostRequest()
    request.Domain, request.DomainId, request.InstanceID = p["domain"], p.get("domain_id"), p["instance_id"]
    return request


def _record(models, p):
    value = dict(p["host"] or {})
    value["Domain"] = p["domain"]
    if p.get("domain_id"):
        value["DomainId"] = p["domain_id"]
...
def build_create(models, p):
    ...
    request.Host, request.InstanceID, request.Tags = _record(models, p), p["instance_id"], _tags(models, p["tags"])
    return request


def build_update(models, p):
    request = models.ModifyHostRequest()
    request.Host, request.InstanceID = _record(models, p), p["instance_id"]
    return request
...
def build_delete(models, p):
    ...
    item = models.HostDel()
    item.Domain, item.DomainId, item.InstanceID = p["domain"], p.get("domain_id"), p["instance_id"]
    request.HostsDel = [item]
    return request
...
def desired(p):
    result = dict(p["host"] or {})
    result["Domain"] = p["domain"]
    if p.get("domain_id"):
        result["DomainId"] = p["domain_id"]
    return result
...
def run_module():
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, waf_host=None)
            p["domain_id"] = current.get("DomainId") or p.get("domain_id")
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteHost, build_delete(models, p))
        ...
        if current and comparable(current, target) == target:
            module.exit_json(changed=False, waf_host=current)
        diff = maybe_diff(module, comparable(current, target) if current else None, target)
        if not module.check_mode:
            if current:
                p["domain_id"] = current.get("DomainId") or p.get("domain_id")
                module.sdk_call(client.ModifyHost, build_update(models, p))
            else:
                module.sdk_call(client.CreateHost, build_create(models, p))
            current = find(module, client, models, p)
