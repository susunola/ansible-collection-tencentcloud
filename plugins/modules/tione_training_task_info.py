...
def detail_request(models, p):
    request = models.DescribeTrainingTaskRequest(); request.Id = p["task_id"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    if p.get("instance_id") is not None: request.InstanceId = p["instance_id"]
    return request


def list_request(models, p, offset):
    request = models.DescribeTrainingTasksRequest(); request.Offset, request.Limit = offset, p["page_size"]
...
def run_module():
    spec = {
        "task_id": {}, "instance_id": {}, "project_id": {}, "filters": {"type": "dict", "default": {}},
        "tag_filters": {"type": "dict", "default": {}}, "order_field": {"choices": ["CreateTime", "UpdateTime", "StartTime"], "default": "UpdateTime"},
        "order": {"choices": ["ASC", "DESC"], "default": "DESC"}, "page_size": {"type": "int", "default": 50},
        "max_pages": {"type": "int", "default": 1000},
    }
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True, required_by={"instance_id": "task_id"}, mutually_exclusive=[("task_id", "filters"), ("task_id", "tag_filters")]); p = module.params
    if not 1 <= p["page_size"] <= 50: module.fail_json(msg="page_size must be between 1 and 50")
