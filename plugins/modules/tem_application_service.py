...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.tem_application_service_info
    description: Gather information about Tencent Cloud TEM application services.
...
def _model(models, value):
    x = models.ServicePortMapping()
    x.from_json_string(json.dumps(value))
    return x


def describe_request(models, p):
    r = models.DescribeApplicationServiceListRequest()
    r.EnvironmentId, r.ApplicationId, r.SourceChannel = p["environment_id"], p["application_id"], p["source_channel"]
    return r


def payload(p):
    value = dict(p.get("service") or {})
    value["ServiceName"], value["Type"] = p["name"], p["access_type"]
    return value


def create_request(models, p):
    r = models.CreateApplicationServiceRequest()
    r.ApplicationId, r.EnvironmentId, r.SourceChannel = p["application_id"], p["environment_id"], p["source_channel"]
    r.Service = _model(models, payload(p))
    return r


def update_request(models, p):
    r = models.ModifyApplicationServiceRequest()
    r.ApplicationId, r.EnvironmentId, r.SourceChannel = p["application_id"], p["environment_id"], p["source_channel"]
    r.Data = _model(models, payload(p))
    return r


def delete_request(models, p):
    r = models.DeleteApplicationServiceRequest()
    r.ApplicationId, r.EnvironmentId, r.ServiceName, r.SourceChannel = p["application_id"], p["environment_id"], p["name"], p["source_channel"]
    return r
...
def describe(module, client, models, p):
    ...
    for item in items:
        ...
        if value.get("ServiceName") == p["name"] and value.get("Type") == p["access_type"]:
            matches.append(value)
    if len(matches) > 1:
        module.fail_json(msg="Multiple TEM application service mappings matched", name=p["name"], access_type=p["access_type"])
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteApplicationService, delete_request(models, p))
        ...
        if not module.check_mode:
            module.sdk_call(
                client.ModifyApplicationService if current else client.CreateApplicationService,
