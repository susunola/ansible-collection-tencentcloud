...
def get_logging(client, bucket):
    try: return normalize(client.get_bucket_logging(Bucket=bucket))
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "target_bucket": {}, "target_prefix": {"default": ""}}, required_if=[("state", "present", ["target_bucket"])], supports_check_mode=True)
    ...
    try:
        current = get_logging(client, bucket); target = normalize({"LoggingEnabled": {"TargetBucket": module.params.get("target_bucket"), "TargetPrefix": module.params.get("target_prefix") or ""}})
        if module.params["state"] == "absent": target = None
        if current == target: module.exit_json(changed=False, logging=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode: client.put_bucket_logging(Bucket=bucket, BucketLoggingStatus=target or {})
