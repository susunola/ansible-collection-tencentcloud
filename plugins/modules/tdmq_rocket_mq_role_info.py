...
RETURN = r'''
rocket_mq_roles:
  description: Matching TDMQ rocket mq roles.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeRocketMQRoles, request),
        lambda response: response.RoleSets,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    rocket_mq_roles = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, rocket_mq_roles=rocket_mq_roles,
