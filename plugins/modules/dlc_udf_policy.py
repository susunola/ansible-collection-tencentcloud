...
def normalize(values):
    ...
    for value in values or []:
        ...
        for access in set(value.get("Accesses", value.get("accesses", []))):
            ...
            entry["users"].update(users); entry["groups"].update(groups)
    return [
        {"Accesses": [access], "Users": sorted(value["users"]), "Groups": sorted(value["groups"])}
        for access, value in sorted(by_access.items())
    ]


def describe_request(models, p):
    request = models.DescribeUDFPolicyRequest(); request.Name, request.DatabaseName, request.CatalogName = p["name"], p["database_name"], p["catalog_name"]; return request
...
def update_request(models, p):
    request = models.UpdateUDFPolicyRequest(); request.Name, request.DatabaseName, request.CatalogName = p["name"], p["database_name"], p["catalog_name"]
    request.UDFPolicyInfos = []
    for value in normalize(p["policy_infos"]):
        item = models.UDFPolicyInfo(); item.from_json_string(json.dumps(value)); request.UDFPolicyInfos.append(item)
    return request


def wait_policy(module, client, models, p, target):
    def poll(): return "ready" if read(module, client, models, p) == target else "pending"
    wait_for_state(module, poll, ["ready"], timeout=p["waiter_timeout"], delay=p["waiter_delay"])


def run_module():
    info = {
        "accesses": {"type": "list", "elements": "str", "required": True}, "users": {"type": "list", "elements": "str", "default": []},
        "groups": {"type": "list", "elements": "str", "default": []},
    }
    spec = {
        "name": {"required": True}, "database_name": {"required": True}, "catalog_name": {"required": True},
        "policy_infos": {"type": "list", "elements": "dict", "required": True, "options": info}, "allow_empty": {"type": "bool", "default": False},
    ...
    if not target and not p["allow_empty"]: module.fail_json(msg="set allow_empty=true to authorize clearing all DLC UDF policy entries")
    for index, item in enumerate(target):
        if not item["Accesses"]: module.fail_json(msg="each UDF policy entry requires at least one access", index=index)
        if not item["Users"] and not item["Groups"]: module.fail_json(msg="each UDF policy entry requires users or groups", index=index)
    ...
    try:
        ...
        if current == target: module.exit_json(changed=False, udf_policy=current)
        diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            response = module.sdk_call(client.UpdateUDFPolicy, update_request(models, p))
            if p["wait"]: wait_policy(module, client, models, p, target)
            ...
            if p["wait"]: current = read(module, client, models, p)
