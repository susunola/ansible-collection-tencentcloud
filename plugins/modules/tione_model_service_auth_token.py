...
def tokens(module, client, models, p):
    response = module.sdk_call(client.DescribeModelServiceGroup, group_request(models, p)); group = response.ServiceGroup
    if not group: module.fail_json(msg="TIONE service group does not exist", service_group_id=p["service_group_id"])
...
def find(module, values, p):
    ...
    if len(matches) > 1: module.fail_json(msg="Multiple TIONE auth tokens matched the exact name", name=p.get("name"))
...
def desired(p, current=None):
    value = copy.deepcopy(current or {"Base": {}}); base = value.setdefault("Base", {})
    if p.get("name") is not None: base["Name"] = p["name"]
    if p.get("description") is not None: base["Description"] = p["description"]
    if p.get("limits") is not None: value["Limits"] = normalized_limits(p["limits"])
    return value


def sanitize(value, show=False):
    result = copy.deepcopy(value)
    if result and not show: (result.get("Base") or {}).pop("Value", None)
    return result


def create_request(models, p):
    request = models.CreateModelServiceAuthTokenRequest(); request.ServiceGroupId = p["service_group_id"]; request.Name = p["name"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
...
def modify_request(models, p, current, rotate):
    request = models.ModifyModelServiceAuthTokenRequest(); request.ServiceGroupId = p["service_group_id"]; request.NeedReset = rotate
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    request.AuthToken = _model(models.AuthToken, desired(p, current)); return request


def delete_request(models, p, current):
    request = models.DeleteModelServiceAuthTokenRequest(); request.ServiceGroupId = p["service_group_id"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    request.AuthTokenValue = (current.get("Base") or {}).get("Value"); return request


def run_module():
    spec = {
        "state": {"choices": ["present", "absent"], "default": "present"}, "service_group_id": {"required": True},
        "project_id": {}, "token_id": {}, "name": {}, "description": {}, "limits": {"type": "list", "elements": "dict"},
        "rotate_from_token_id": {"no_log": True}, "show_token_value": {"type": "bool", "default": False},
    ...
    if not (p.get("token_id") or p.get("name")): module.fail_json(msg="token_id or name is required")
    if p["state"] == "absent" and not p.get("token_id"): module.fail_json(msg="token_id is required for safe deletion")
    ...
    try:
        ...
        if p["state"] == "absent":
            if current is None: module.exit_json(changed=False, auth_token=None, token_id=p["token_id"])
            if not p["allow_delete"]: module.fail_json(msg="allow_delete=true is required to delete a TIONE auth token", auth_token=sanitize(current))
            if not (current.get("Base") or {}).get("Value"): module.fail_json(msg="TIONE did not return the token value required by its delete API", token_id=p["token_id"])
            diff_value = maybe_diff(module, sanitize(current), None)
            if not module.check_mode: module.sdk_call(client.DeleteModelServiceAuthToken, delete_request(models, p, current))
            module.exit_json(changed=True, **(diff_value or {}), auth_token=None, token_id=p["token_id"])
        if current is None:
            if p.get("token_id"): module.fail_json(msg="requested token_id does not exist", token_id=p["token_id"])
            target = desired(p); diff_value = maybe_diff(module, None, sanitize(target)); token_id = None
            if not module.check_mode:
                module.sdk_call(client.CreateModelServiceAuthToken, create_request(models, p))
                current = find(module, tokens(module, client, models, p), p)
                if current is None: module.fail_json(msg="created TIONE auth token was not returned by service-group discovery", name=p["name"])
                token_id = (current.get("Base") or {}).get("Id")
                if p.get("limits") is not None:
                    p = dict(p, token_id=token_id); module.sdk_call(client.ModifyModelServiceAuthToken, modify_request(models, p, current, False))
                    current = find(module, tokens(module, client, models, p), p)
            module.exit_json(changed=True, **(diff_value or {}), auth_token=sanitize(current if not module.check_mode else target, p["show_token_value"]), token_id=token_id)
        current_id = (current.get("Base") or {}).get("Id"); rotate = p.get("rotate_from_token_id") == current_id
        target = desired(p, current); config_drift = sanitize(current) != sanitize(target)
        if not config_drift and not rotate: module.exit_json(changed=False, auth_token=sanitize(current, p["show_token_value"]), token_id=current_id)
        ...
        if not module.check_mode:
            ...
            lookup = dict(p, token_id=None, name=(target.get("Base") or {}).get("Name")); current = find(module, tokens(module, client, models, lookup), lookup)
            if current is None: module.fail_json(msg="updated TIONE auth token was not returned by service-group discovery")
            current_id = (current.get("Base") or {}).get("Id")
        module.exit_json(changed=True, **(diff_value or {}), auth_token=sanitize(current if not module.check_mode else target, p["show_token_value"]), token_id=current_id)
