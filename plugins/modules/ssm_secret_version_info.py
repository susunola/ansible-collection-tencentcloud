...
def list_request(models, name): request = models.ListSecretVersionIdsRequest(); request.SecretName = name; return request
def value_request(models, p):
    request = models.GetSecretValueRequest(); request.SecretName, request.VersionId = p["secret_name"], p["version_id"]
    if p.get("encryption_public_key") is not None: request.EncryptionPublicKey = p["encryption_public_key"]
    if p.get("encryption_algorithm") is not None: request.EncryptionAlgorithm = p["encryption_algorithm"]
    return request


def run_module():
    spec = {"secret_name": {"required": True}, "version_id": {}, "include_secret_value": {"type": "bool", "default": False}, "encryption_public_key": {}, "encryption_algorithm": {}}
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if p["include_secret_value"] and not p.get("version_id"): module.fail_json(msg="version_id is required when include_secret_value=true")
    if (p.get("encryption_public_key") or p.get("encryption_algorithm")) and not p["include_secret_value"]: module.fail_json(msg="encryption response options require include_secret_value=true")
    ...
    try:
        if p["include_secret_value"]:
            response = module.sdk_call(client.GetSecretValue, value_request(models, p)); value = response._serialize(allow_none=True); request_id = value.pop("RequestId", response.RequestId)
            module.exit_json(changed=False, secret_value=value, request_id=request_id)
        response = module.sdk_call(client.ListSecretVersionIds, list_request(models, p["secret_name"])); values = [item._serialize(allow_none=True) for item in response.Versions or []]
        if p.get("version_id") is not None: values = [item for item in values if item.get("VersionId") == p["version_id"]]
