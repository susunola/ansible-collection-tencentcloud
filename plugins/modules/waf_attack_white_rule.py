...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.waf_attack_white_rule_info
    description: Gather information about Tencent Cloud WAF attack-whitelist rules.
...
def create_request(models, p):
    return _apply(models.AddAttackWhiteRuleRequest(), models, p)


def update_request(models, p, rule_id):
    return _apply(models.ModifyAttackWhiteRuleRequest(), models, p, rule_id)
...
def _sorted_rules(values):
    return sorted(
        values or [],
        key=lambda item: (item.get("MatchField") or "", item.get("MatchParams") or "", item.get("MatchMethod") or "", item.get("MatchContent") or ""),
    )
...
def comparable(value):
    return {
        ...
        "SignatureIds": sorted(value.get("SignatureIds") or ([value["SignatureId"]] if value.get("SignatureId") else [])),
        "TypeIds": sorted(value.get("TypeIds") or ([value["TypeId"]] if value.get("TypeId") else [])),
        "Rules": _sorted_rules(value.get("MatchInfo") or []),
    }
...
def desired(p):
    return {
        ...
        "Mode": p["mode"],
        "SignatureIds": sorted(p["signature_ids"]),
        "TypeIds": sorted(p["type_ids"]),
        "Rules": _sorted_rules(p["rules"]),
    }
...
def find(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeAttackWhiteRule, describe_request(models, p, offset))
        values = list(response.List or [])
        for item in values:
            value = item._serialize(allow_none=True)
            if (p.get("rule_id") is not None and int(value.get("WhiteRuleId") or 0) == p["rule_id"]) or (
        ...
        if offset >= int(response.Total or 0) or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple WAF attack allow rules matched; specify rule_id")
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "signature_ids": {"type": "list", "elements": "str", "default": []},
    ...
    if p["state"] == "present" and p["mode"] == 0 and not p["signature_ids"]:
        module.fail_json(msg="signature_ids is required when mode=0")
    if p["state"] == "present" and p["mode"] == 1 and not p["type_ids"]:
        module.fail_json(msg="type_ids is required when mode=1")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                ...
                if response.FailIds:
                    module.fail_json(msg="Tencent Cloud WAF did not delete the attack allow rule", failed_rule_ids=response.FailIds)
        ...
        if not module.check_mode:
            if current:
                module.sdk_call(client.ModifyAttackWhiteRule, update_request(models, p, int(current["WhiteRuleId"])))
            else:
                p["rule_id"] = int(module.sdk_call(client.AddAttackWhiteRule, create_request(models, p)).RuleId)
            current = find(module, client, models, p)
