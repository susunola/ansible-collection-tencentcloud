...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cfw_vpc_acl_rule_info
    description: Gather information about Tencent Cloud CFW vpc ac rules.
...
def _rule(models, p, rule_uuid=None):
    ...
    if rule_uuid is not None:
        item.Uuid = rule_uuid
    if p.get("firewall_group_id") is not None:
        item.FwGroupId = p["firewall_group_id"]
...
def delete_request(models, p, rule_uuid):
    ...
    request.RuleUuids, request.IpVersion = [rule_uuid], p["ip_version"]
    return request
...
def find_rule(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeVpcAcRule, describe_request(models, offset))
        values = list(response.Data or [])
        for value in values:
            item = value._serialize(allow_none=True)
            if p.get("rule_uuid") is not None and item.get("Uuid") == p["rule_uuid"]:
                matches.append(item)
            elif p.get("rule_uuid") is None and item.get("EdgeId") == p["edge_id"] and p.get("description") and item.get("Description") == p["description"]:
                matches.append(item)
        offset += len(values)
        if offset >= int(response.Total or 0) or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple inter-VPC ACL rules matched; specify rule_uuid")
...
def desired(p):
    ...
    if p.get("firewall_group_id") is not None:
        result["FwGroupId"] = p["firewall_group_id"]
    if p.get("parameter_template_id") is not None:
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "rule_uuid": {"type": "int"},
            "description": {},
            "edge_id": {"required": True},
            "source": {},
            "destination": {},
            "destination_type": {"choices": ["net", "domain", "dnsparse"], "default": "net"},
            "protocol": {
                "choices": ["ANY", "TCP", "UDP", "ICMP", "HTTP", "HTTPS", "HTTP/HTTPS", "SMTP", "SMTPS", "SMTP/SMTPS", "FTP", "DNS", "TLS/SSL"],
                "default": "ANY",
            },
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.RemoveVpcAcRule, delete_request(models, p, current["Uuid"]))
