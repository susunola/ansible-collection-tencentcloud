...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.api_gateway_api_key
    description: Manage Tencent Cloud API Gateway API keys.
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeApiKeysStatus, request),
        lambda response: response.Result.ApiKeySet if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    api_keys = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, api_keys=api_keys,
