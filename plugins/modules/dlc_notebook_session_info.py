...
def build_request(models, p, offset=0):
    ...
    request.Offset, request.Limit, request.Asc = offset, p["page_size"], p["ascending"]
    if p.get("data_engine_name") is not None:
        request.DataEngineName = p["data_engine_name"]
    if p.get("states") is not None:
        request.State = p["states"]
    if p.get("sort_fields") is not None:
        request.SortFields = p["sort_fields"]
    filters = []
...
def read(module, client, models, p):
    ...
    while True:
        response = module.sdk_call(client.DescribeNotebookSessions, build_request(models, p, offset))
        page = response.Sessions or []
        sessions.extend(x._serialize(allow_none=True) for x in page)
        total = int(response.TotalElements or 0)
...
def run_module():
    ...
    spec = {
        "data_engine_name": {},
        "states": {"type": "list", "elements": "str", "choices": states},
        "keyword": {"no_log": False},
        "engine_generation": {"choices": ["supersql", "native"]},
        "sort_fields": {"type": "list", "elements": "str"},
