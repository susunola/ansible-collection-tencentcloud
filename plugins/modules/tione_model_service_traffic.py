...
def authorization_request(models, p):
    ...
    request.ServiceGroupId = p["service_group_id"]
    request.AuthorizationEnable = p["authorization_enable"]
    return request
...
def wait_weights(module, client, models, p):
    def poll():
        group = get(module, client, models, p)
        state = str((group or {}).get("WeightUpdateStatus") or "").lower()
        if state == "update_failed":
            module.fail_json(msg="TIONE service-group weight update failed", service_group=group)
        if current_weights(group) == normalized_weights(p["weights"]) and state not in ("updating",):
            return "converged"
        return state
...
def validate_weights(module, values):
    if values is None:
        return
    ids = [item.get("ServiceId") for item in values]
    weights = [item.get("Weight") for item in values]
    if not values or any(value in (None, "") for value in ids) or any(value is None for value in weights):
        module.fail_json(msg="each weight requires ServiceId and Weight")
    if len(ids) != len(set(ids)):
        module.fail_json(msg="weight ServiceId values must be unique")
    if any(not isinstance(value, int) or isinstance(value, bool) or value < 0 for value in weights):
        module.fail_json(msg="weights must be non-negative integers")
    if sum(weights) != 100:
        module.fail_json(msg="weights must total exactly 100", total=sum(weights))
...
def run_module():
    spec = {
        ...
        "weights": {"type": "list", "elements": "dict"},
    ...
    p = module.params
    if p.get("authorization_enable") is None and p.get("weights") is None:
        module.fail_json(msg="authorization_enable or weights is required")
    ...
    try:
        ...
        if not auth_drift and not weight_drift:
            module.exit_json(changed=False, service_group=current)
        target = dict(current)
        if p.get("authorization_enable") is not None:
            target["AuthorizationEnable"] = p["authorization_enable"]
        if p.get("weights") is not None:
            target["Services"] = normalized_weights(p["weights"])
        diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            if auth_drift:
                module.sdk_call(client.ModifyModelServiceAuthorization, authorization_request(models, p))
            if weight_drift:
                module.sdk_call(client.ModifyServiceGroupWeights, weights_request(models, p))
                if p["wait"]:
                    wait_weights(module, client, models, p)
            current = get(module, client, models, p)
