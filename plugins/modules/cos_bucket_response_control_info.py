...
DOCUMENTATION = r'''
---
module: cos_bucket_response_control_info
short_description: Gather Tencent Cloud COS response-header controls
version_added: "1.4.0"
description: Returns the normalized response query parameters allowed for a COS bucket.
options:
...
'''
EXAMPLES = r'''
- susunola.tencentcloud.cos_bucket_response_control_info:
    region: ap-guangzhou
    name: downloads
'''
RETURN = r'''
...
from ansible_collections.susunola.tencentcloud.plugins.modules.cos_bucket_response_control import get_control
...
def run_module():
    ...
    try:
        value = get_control(cos.create_cos_client(module), bucket)
        module.exit_json(changed=False, response_controls=[value] if value else [], response_control=value)
