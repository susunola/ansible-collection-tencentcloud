...
def normalize_policies(values):
    ...
    if len(keys) != len(set(keys)): raise ValueError("duplicate DLC user policies are not allowed")
...
def describe_request(models, user_id, account_source, offset=0):
    request = models.DescribeUsersRequest(); request.UserId, request.AccountType = user_id, account_source
    request.Offset, request.Limit = offset, 100; return request


def current_user(module, client, models, user_id, account_source):
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeUsers, describe_request(models, user_id, account_source, offset)); page = response.UserSet or []
        matches.extend(x._serialize(allow_none=True) for x in page if x.UserId == user_id); offset += len(page)
        if not page or offset >= int(response.TotalCount or 0): break
    if not matches: module.fail_json(msg="DLC user not found", user_id=user_id)
    if len(matches) > 1: module.fail_json(msg="Multiple DLC users returned for exact ID", user_id=user_id)
    return matches[0]
...
def attach_request(models, user_id, account_source, values):
    request = models.AttachUserPolicyRequest(); request.UserId, request.AccountType = user_id, account_source
    request.PolicySet = policy_models(models, values); return request


def detach_request(models, user_id, account_source, values, policy_ids=None):
    request = models.DetachUserPolicyRequest(); request.UserId, request.AccountType = user_id, account_source
    if policy_ids and len(policy_ids) == len(values): request.PolicyIds = policy_ids
    else: request.PolicySet = policy_models(models, values)
    return request
...
def run_module():
    spec = {"user_id": {"required": True}, "account_source": {"choices": ["TencentAccount", "EntraAccount"], "default": "TencentAccount"}, "policies": {"type": "list", "elements": "dict", "required": True}, "allow_empty": {"type": "bool", "default": False}}
    ...
    try:
        ...
        if user.get("UserType") == "ADMIN" and p["policies"]: module.fail_json(msg="ADMIN DLC users cannot have direct policies", user_id=p["user_id"])
        raw_current = user.get("PolicySet") or []; current = normalize_policies(raw_current); target = normalize_policies(p["policies"])
        if not target and not p["allow_empty"]: module.fail_json(msg="set allow_empty=true to authorize removing every direct DLC user policy")
        ...
        if not module.check_mode:
            if added: module.sdk_call(client.AttachUserPolicy, attach_request(models, p["user_id"], p["account_source"], added))
            if removed:
                removed_keys = {json.dumps(value, sort_keys=True, separators=(",", ":")) for value in removed}
                ids = [str(value.get("PolicyId")) for value in raw_current if json.dumps(normalize_policy(value), sort_keys=True, separators=(",", ":")) in removed_keys and value.get("PolicyId")]
                module.sdk_call(client.DetachUserPolicy, detach_request(models, p["user_id"], p["account_source"], removed, ids))
            wait_for_state(module, lambda: "ready" if normalize_policies(current_user(module, client, models, p["user_id"], p["account_source"]).get("PolicySet") or []) == target else "pending", ["ready"], timeout=p["waiter_timeout"], delay=p["waiter_delay"])
