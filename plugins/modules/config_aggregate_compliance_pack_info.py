...
RETURN = r'''
aggregate_compliance_packs:
  description: Matching CONFIG aggregate compliance packs.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.ListAggregateCompliancePacks, request),
        lambda response: response.Items,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    aggregate_compliance_packs = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, aggregate_compliance_packs=aggregate_compliance_packs, total_count=total_count)
