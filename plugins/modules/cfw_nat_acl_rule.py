...
options:
  ...
  protocol:
    type: str
    choices: [ANY, TCP, UDP, ICMP, HTTP, HTTPS, HTTP/HTTPS, SMTP, SMTPS, SMTP/SMTPS, FTP, DNS]
    default: ANY
  ...
  scope:
    description:
      - Optional NAT firewall instance scope.
    type: str
...
def find_rule(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeNatAcRule, describe_request(models, offset))
        ...
        if offset >= int(response.Total or 0) or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple NAT ACL rules matched; specify rule_uuid")
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.RemoveNatAcRule, delete_request(models, p, current["Uuid"]))
