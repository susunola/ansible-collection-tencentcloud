...
def find(module, client, models, p):
    r = models.DescribeGroupsRequest()
    ...
    if p.get("group_id"):
        matches = [x for x in values if x.get("GroupId") == p["group_id"]]
    else:
        matches = [
            x
            for x in values
            if x.get("GroupName") == p["name"]
            and x.get("ApplicationId") == p["application_id"]
            and x.get("NamespaceId") == p["namespace_id"]
            and x.get("ClusterId") == p["cluster_id"]
        ]
    if len(matches) > 1:
        module.fail_json(msg="Multiple TSF VM deployment groups matched", name=p["name"])
...
def comparable(current, target):
    return {k: current.get(k) for k in target}
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                r = models.DeleteGroupRequest()
                r.GroupId = current["GroupId"]
                response = module.sdk_call(client.DeleteGroup, r)
                if response.Result is False:
                    module.fail_json(msg="Tencent Cloud rejected the TSF VM deployment group deletion", request_id=response.RequestId)
            module.exit_json(changed=True, **(diff or {}), deployment_group=None)
        target = desired(p)
        if current:
            require_immutable_unchanged(module, current, target, ["ApplicationId", "NamespaceId", "ClusterId", "GroupResourceType"], "TSF VM deployment group")
        ...
        if not module.check_mode:
            if current:
                ...
                if response.Result is False:
                    module.fail_json(msg="Tencent Cloud rejected the TSF VM deployment group update", request_id=response.RequestId)
