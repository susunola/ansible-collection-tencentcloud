...
def describe(module, client, models, params):
    ...
    request = cls(); request.ProxyId, request.GroupId = params.get("proxy_id"), params.get("group_id")
    ...
    for item in values:
        ...
        if params.get("listener_id") and value.get("ListenerId") == params["listener_id"]: matches.append(value)
        elif not params.get("listener_id") and value.get("ListenerName") == params.get("name") and value.get("Port") == params.get("port"): matches.append(value)
    if len(matches) > 1: module.fail_json(msg="Multiple GAAP listeners matched; specify listener_id")
    return matches[0] if matches else None


def target(params):
    names = {"name": "ListenerName", "port": "Port", "scheduler": "Scheduler", "real_server_type": "RealServerType",
             "delay_loop": "DelayLoop", "connect_timeout": "ConnectTimeout", "healthy_threshold": "HealthyThreshold",
             "unhealthy_threshold": "UnhealthyThreshold", "client_ip_method": "ClientIPMethod",
             "udp_check_type": "CheckType", "udp_check_port": "CheckPort", "send_context": "SendContext",
...
def create(module, client, models, params, wanted):
    payload = dict(wanted); payload["Ports"] = [payload.pop("Port")]
    ...
    request = cls(); request._deserialize(payload)
...
def delete(module, client, models, params, listener_id):
    request = models.DeleteListenersRequest(); request.ListenerIds = [listener_id]
...
def run_module():
    module = TencentCloudModule(argument_spec={
        "state": {"choices": ["present", "absent"], "default": "present"}, "listener_id": {}, "proxy_id": {}, "group_id": {},
        "name": {}, "protocol": {"choices": ["TCP", "UDP"], "required": True}, "port": {"type": "int"},
        "scheduler": {"choices": ["rr", "wrr", "lc", "lrtt"], "default": "rr"},
        "real_server_type": {"choices": ["IP", "DOMAIN"], "default": "IP"}, "health_check": {"type": "bool", "default": False},
        "delay_loop": {"type": "int"}, "connect_timeout": {"type": "int"}, "healthy_threshold": {"type": "int"},
        "unhealthy_threshold": {"type": "int"}, "failover": {"type": "bool", "default": False},
        "client_ip_method": {"type": "int", "choices": [0, 1]}, "udp_check_type": {"choices": ["PORT", "PING"]},
        "udp_check_port": {"type": "int"}, "send_context": {}, "receive_context": {},
    ...
    p = module.params
    if not p.get("listener_id") and p.get("port") is None: module.fail_json(msg="port is required when resolving a listener by name")
    if p["state"] == "present" and (not p.get("name") or p.get("port") is None): module.fail_json(msg="name and port are required to create a listener")
    module.require_sdk(); models, client_module = _load(); client = module.create_client(client_module.GaapClient, "gaap.tencentcloudapi.com")
    try:
        current = describe(module, client, models, p)
        if p["state"] == "absent":
            if current is None: module.exit_json(changed=False, listener=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: delete(module, client, models, p, current["ListenerId"])
        ...
        if current is not None:
            ...
            if not changed(before, wanted): module.exit_json(changed=False, listener=current)
        ...
        if not module.check_mode:
            if current is None: p["listener_id"] = create(module, client, models, p, wanted)
            else: update(module, client, models, p, current, wanted); p["listener_id"] = current["ListenerId"]
            current = describe(module, client, models, p)
