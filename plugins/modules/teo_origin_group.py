...
def describe_request(models, p, offset=0):
    request = models.DescribeOriginGroupRequest(); request.ZoneId, request.Offset, request.Limit = p["zone_id"], offset, 1000
    value = p.get("group_id") or p.get("name")
    if value:
        item = models.AdvancedFilter(); item.Name = "origin-group-id" if p.get("group_id") else "origin-group-name"; item.Values = [value]; request.Filters = [item]
    return request
...
def _records(models, values):
    ...
    for value in values or []:
        item = models.OriginRecord(); item.Record, item.Type = value["record"], value["record_type"]
        if value.get("weight") is not None: item.Weight = value["weight"]
        result.append(item)
    return result


def create_request(models, p):
    request = models.CreateOriginGroupRequest(); request.ZoneId, request.Name, request.Type = p["zone_id"], p["name"], p["group_type"]
    request.Records = _records(models, p["records"])
    if p.get("host_header") is not None: request.HostHeader = p["host_header"]
    return request


def update_request(models, p, group_id):
    request = models.ModifyOriginGroupRequest(); request.ZoneId, request.GroupId = p["zone_id"], group_id
...
def delete_request(models, p, group_id):
    request = models.DeleteOriginGroupRequest(); request.ZoneId, request.GroupId = p["zone_id"], group_id; return request


def find_group(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeOriginGroup, describe_request(models, p, offset)); values = list(response.OriginGroups or [])
        for value in values:
            item = value._serialize(allow_none=True)
            if p.get("group_id") and item.get("GroupId") == p["group_id"]: matches.append(item)
            elif not p.get("group_id") and p.get("name") and item.get("Name") == p["name"]: matches.append(item)
        offset += len(values)
        if offset >= int(response.TotalCount or 0) or not values: break
    if len(matches) > 1: module.fail_json(msg="Multiple EdgeOne origin groups matched; specify group_id")
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "zone_id": {"required": True}, "group_id": {}, "name": {}, "group_type": {"choices": ["GENERAL", "HTTP"], "default": "GENERAL"}, "host_header": {}, "records": {"type": "list", "elements": "dict", "options": {"record": {"required": True}, "record_type": {"choices": ["IP_DOMAIN", "COS", "AWS_S3"], "default": "IP_DOMAIN"}, "weight": {"type": "int"}}}}, required_one_of=[("group_id", "name")], supports_check_mode=True)
    p = module.params
    if p["state"] == "present" and (not p.get("name") or not p.get("records")): module.fail_json(msg="name and at least one record are required when state=present")
    for record in p.get("records") or []:
        if record.get("weight") is not None and not 0 <= record["weight"] <= 100: module.fail_json(msg="record weight must be between 0 and 100")
        if p["group_type"] == "GENERAL" and record["record_type"] != "IP_DOMAIN": module.fail_json(msg="GENERAL origin groups only support IP_DOMAIN records")
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, origin_group=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteOriginGroup, delete_request(models, p, current["GroupId"]))
            module.exit_json(changed=True, **(diff or {}), origin_group=current if module.check_mode else None)
        target = desired(p); before = None
        if current: before = {"Name": current.get("Name"), "Type": current.get("Type"), "HostHeader": current.get("HostHeader") or "", "Records": _normalized_records(current.get("Records"), True)}
        if before == target: module.exit_json(changed=False, origin_group=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if not current:
                p["group_id"] = module.sdk_call(client.CreateOriginGroup, create_request(models, p)).OriginGroupId
            else: module.sdk_call(client.ModifyOriginGroup, update_request(models, p, current["GroupId"]))
            current = find_group(module, client, models, p)
