...
def get_request(models, p):
    request = models.DescribeDataSourceRequest(); request.Id = p["data_source_id"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    return request


def list_request(models, p, offset):
    request = models.DescribeDataSourcesRequest(); request.Offset, request.Limit = offset, 200
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
...
def find(module, client, models, p):
    ...
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeDataSources, list_request(models, p, offset)); items = response.DataSourceInfos or []
    ...
    if len(matches) > 1: module.fail_json(msg="Multiple TIONE data sources matched the exact name", name=p["name"])
    return matches[0] if matches else None


def desired(p, current=None):
    result = dict(current or {}); result["Name"] = p.get("name") or result.get("Name")
    for source, target in (("source_type", "Type"), ("permission", "Permission"), ("storage_id", "StorageId"), ("mount_config", "MountConfigure"), ("tags", "Tags")):
...
def conflict(p, current):
    ...
    for source, key in (("name", "Name"), ("source_type", "Type"), ("permission", "Permission"), ("storage_id", "StorageId"), ("mount_config", "MountConfigure"), ("tags", "Tags")):
...
def create_request(models, p):
    ...
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    if p.get("mount_config") is not None: request.MountConfigure = _model(models.MountConfigureInfo, p["mount_config"])
    if p.get("tags") is not None: request.Tags = [_model(models.Tag, item) for item in p["tags"]]
    return request


def delete_request(models, p):
    request = models.DeleteDataSourceRequest(); request.Id = p["data_source_id"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    return request
...
def run_module():
    spec = {
        "state": {"choices": ["present", "absent"], "default": "present"}, "name": {}, "data_source_id": {}, "project_id": {},
        "source_type": {}, "permission": {"choices": ["RW", "RO"]}, "storage_id": {}, "mount_config": {"type": "dict"},
        "tags": {"type": "list", "elements": "dict"}, "allow_delete": {"type": "bool", "default": False},
    ...
    if p["state"] == "present" and not (p.get("name") or p.get("data_source_id")): module.fail_json(msg="name or data_source_id is required when state=present")
    if p["state"] == "absent" and not p.get("data_source_id"): module.fail_json(msg="data_source_id is required for safe deletion")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TioneClient, "tione.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, data_source=None, data_source_id=p["data_source_id"])
            if not p["allow_delete"]: module.fail_json(msg="allow_delete=true is required to delete a TIONE data source", data_source=current)
        ...
        if current:
            ...
            if changes: module.fail_json(msg="TIONE data sources expose no update API and the existing data source has immutable drift", data_source=current, immutable_drift=changes)
            module.exit_json(changed=False, data_source=current, data_source_id=current.get("Id"))
        if p.get("data_source_id"): module.fail_json(msg="the requested TIONE data_source_id does not exist; omit it to create by name", data_source_id=p["data_source_id"])
        missing = [key for key in ("name", "source_type", "permission", "storage_id") if not p.get(key)]
        if missing: module.fail_json(msg="creation parameters are required for a TIONE data source", missing=missing)
        target, diff_value, data_source_id = desired(p), maybe_diff(module, None, desired(p)), None
        if not module.check_mode:
            response = module.sdk_call(client.CreateDataSource, create_request(models, p)); data_source_id = response.Id
        ...
        module.exit_json(changed=True, **(diff_value or {}), data_source=current if not module.check_mode else target, data_source_id=(current or {}).get("Id") or data_source_id)
    except Exception as exc:
