...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.config_remediation_info
    description: Gather information about Tencent Cloud Config remediations.
...
RETURN = r"""remediation:
  ...
  sample:
    ...
    RemediationId: remediation-2001
    RuleId: cr-222
...
def list_request(models, rule_id):
    ...
    request.Limit, request.RuleIds = 100, [rule_id]
    return request


def create_request(models, p):
    request = models.CreateRemediationRequest()
    request.RuleId, request.RemediationType = p["rule_id"], p["remediation_type"]
    request.RemediationTemplateId, request.InvokeType, request.SourceType = p["remediation_template_id"], p["invoke_type"], p["source_type"]
    return request


def update_request(models, p, remediation_id):
    request = models.UpdateRemediationRequest()
    request.RemediationId, request.RemediationType = remediation_id, p["remediation_type"]
    request.RemediationTemplateId, request.InvokeType, request.SourceType = p["remediation_template_id"], p["invoke_type"], p["source_type"]
    return request
...
def delete_request(models, remediation_id):
    ...
    request.RemediationIds = [remediation_id]
    return request


def find_remediation(module, client, models, p):
    response = module.sdk_call(client.ListRemediations, list_request(models, p["rule_id"]))
    matches = []
    for value in response.Remediations or []:
        item = value._serialize(allow_none=True)
        if p.get("remediation_id") and item.get("RemediationId") == p["remediation_id"]:
            matches.append(item)
        elif not p.get("remediation_id") and item.get("RuleId") == p["rule_id"]:
            matches.append(item)
    if len(matches) > 1:
        module.fail_json(msg="Multiple remediations matched the rule; specify remediation_id")
...
def run_module():
    ...
    p = module.params
    if p["state"] == "present" and any(not p.get(key) for key in ("remediation_type", "remediation_template_id", "invoke_type", "source_type")):
        module.fail_json(msg="remediation_type, remediation_template_id, invoke_type and source_type are required when state=present")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, remediation=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteRemediations, delete_request(models, current["RemediationId"]))
            module.exit_json(changed=True, **(diff or {}), remediation=current if module.check_mode else None)
        desired = {
            "RuleId": p["rule_id"],
            "RemediationType": p["remediation_type"],
            "RemediationTemplateId": p["remediation_template_id"],
            "InvokeType": p["invoke_type"],
            "RemediationSourceType": p["source_type"],
        }
        before = {key: current.get(key) for key in desired} if current else None
        if before == desired:
            module.exit_json(changed=False, remediation=current)
        diff = maybe_diff(module, before, desired)
        if not module.check_mode:
            if current:
                module.sdk_call(client.UpdateRemediation, update_request(models, p, current["RemediationId"]))
            else:
                p["remediation_id"] = module.sdk_call(client.CreateRemediation, create_request(models, p)).RemediationId
            current = find_remediation(module, client, models, p)
