...
options:
  ...
  rules:
    ...
    suboptions:
      ...
      condition:
        description:
          - EdgeOne security expression.
        type: str
        required: true
      action:
        description:
          - Safe parameter-free enforcement action.
        type: str
      ...
      rule_type:
        description:
          - Rule category.
        type: str
        choices: [BasicAccessRule, PreciseMatchRule]
        default: PreciseMatchRule
      priority:
        description:
          - Priority from 0 through 100 for precise-match rules.
        type: int
        default: 0
        ...
        result.append(
            {
                ...
                "rule_type": (value.get("RuleType") or "PreciseMatchRule") if sdk else value["rule_type"],
...
def update_request(models, p, current=None):
    ...
    for value in p["rules"]:
        item = models.CustomRule()
        item.Name, item.Condition = value["name"], value["condition"]
        ...
        rules.append(item)
    custom = models.CustomRules()
    custom.Rules = rules
    policy = models.SecurityPolicy()
    policy.CustomRules = custom
...
def get_rules(module, client, models, p):
    ...
    if not policy or not policy.CustomRules:
        return [], []
    raw = [item._serialize(allow_none=True) for item in policy.CustomRules.Rules or []]
...
def run_module():
        ...
        argument_spec={
            ...
            "rules": {
                ...
                "options": {
                    ...
                    "rule_type": {"choices": ["BasicAccessRule", "PreciseMatchRule"], "default": "PreciseMatchRule"},
    ...
    if len(names) != len(set(names)):
        module.fail_json(msg="rule names must be unique")
    for rule in p["rules"]:
        if not 0 <= rule["priority"] <= 100:
            module.fail_json(msg="rule priority must be between 0 and 100")
        if rule["rule_type"] == "BasicAccessRule" and rule["priority"] != 0:
            module.fail_json(msg="priority is only supported for PreciseMatchRule")
    module.require_sdk()
    models, cm = _load()
