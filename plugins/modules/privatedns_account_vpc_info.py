...
RETURN = r'''
account_vpcs:
  description: Matching PRIVATEDNS account vpcs.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeAccountVpcList, request),
        lambda response: response.VpcSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    account_vpcs = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, account_vpcs=account_vpcs, total_count=total_count)
