...
def _target(models, cls, value):
    x = cls()
    x.TargetIp, x.Port = value["ip"], value["port"]
    if hasattr(x, "Weight"):
        x.Weight = value.get("weight", 10)
    return x


def add_request(models, p, values):
    r = models.AddTargetsToTargetGroupRequest()
    r.TargetGroupId = p["target_group_id"]
    r.Targets = [_target(models, models.TargetToAdd, x) for x in values]
    return r


def update_request(models, p, values):
    r = models.ModifyTargetsInTargetGroupRequest()
    r.TargetGroupId = p["target_group_id"]
    r.Targets = [_target(models, models.TargetToModify, x) for x in values]
    return r


def remove_request(models, p, values):
    r = models.RemoveTargetsFromTargetGroupRequest()
    r.TargetGroupId = p["target_group_id"]
    r.Targets = [_target(models, models.TargetToRemove, x) for x in values]
    return r
...
def run_module():
    spec = {
        ...
        "targets": {
            ...
            "elements": "dict",
            "default": [],
            "options": {"ip": {"required": True}, "port": {"type": "int", "required": True}, "weight": {"type": "int", "default": 10}},
        },
    ...
    try:
        ...
        if not additions and not updates and not removals:
            module.exit_json(changed=False, targets=current)
        diff = maybe_diff(module, current, effective)
        if not module.check_mode:
            if additions:
                module.sdk_call(client.AddTargetsToTargetGroup, add_request(models, p, additions))
            if updates:
