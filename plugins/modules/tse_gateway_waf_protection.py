...
def describe_request(models,p):
    r=models.DescribeWafProtectionRequest(); r.GatewayId,r.Type,r.TypeList=p["gateway_id"],p["scope"],[p["scope"]]; return r
def mutation_request(cls,p,resource_ids):
    r=cls(); r.GatewayId,r.Type,r.List=p["gateway_id"],p["scope"],resource_ids or None; return r
...
def status_map(result,scope):
    if scope=="Global": return {"Global":enabled_value(result.get("GlobalStatus"))}
...
def run_module():
    module=TencentCloudModule(argument_spec={"gateway_id":{"required":True},"scope":{"choices":["Global","Service","Route"],"required":True},"resource_ids":{"type":"list","elements":"str"},"enabled":{"type":"bool","required":True}},supports_check_mode=True); p=module.params
    ids=p.get("resource_ids") or []
    if p["scope"]=="Global" and ids: module.fail_json(msg="resource_ids must be empty for Global WAF protection")
    if p["scope"]!="Global" and not ids: module.fail_json(msg="resource_ids is required for Service or Route WAF protection")
    if len(set(ids))!=len(ids): module.fail_json(msg="resource_ids must not contain duplicates")
    module.require_sdk(); models,cm=_load(); client=module.create_client(cm.TseClient,"tse.tencentcloudapi.com")
    try:
        before=current(module,client,models,p); requested=["Global"] if p["scope"]=="Global" else ids
        missing=[item for item in requested if before.get(item,False)!=p["enabled"]]
        target={item:p["enabled"] for item in requested}
        if not missing: module.exit_json(changed=False,waf_protection={"Scope":p["scope"],"Status":target})
        diff=maybe_diff(module,{item:before.get(item,False) for item in requested},target)
        if not module.check_mode:
            if p["enabled"]: api,cls=client.OpenWafProtection,models.OpenWafProtectionRequest
            else: api,cls=client.CloseWafProtection,models.CloseWafProtectionRequest
            module.sdk_call(api,mutation_request(cls,p,[] if p["scope"]=="Global" else missing)); effective=current(module,client,models,p)
        else: effective=target
        module.exit_json(changed=True,**(diff or {}),waf_protection={"Scope":p["scope"],"Status":{item:effective.get(item,p["enabled"]) for item in requested},"AffectedResourceIds":missing})
