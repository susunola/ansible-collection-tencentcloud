...
RETURN = r'''
runtime_deployed_instances_mcs:
  description: Matching EIS runtime deployed instances mcs.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.eis.v20210601 import models, eis_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-eis package is required.")

    client = eis_client.EisClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "eis.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.ListRuntimeDeployedInstancesMC, request),
        lambda response: response.Instances,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    runtime_deployed_instances_mcs = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, runtime_deployed_instances_mcs=runtime_deployed_instances_mcs, total_count=total_count)
