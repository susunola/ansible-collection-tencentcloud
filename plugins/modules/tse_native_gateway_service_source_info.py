...
RETURN = r'''
native_gateway_service_sources:
  description: Matching TSE native gateway service sources.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeNativeGatewayServiceSources, request),
        lambda response: response.List,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    native_gateway_service_sources = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, native_gateway_service_sources=native_gateway_service_sources,
