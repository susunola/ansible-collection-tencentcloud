...
MUTABLE = {
    ...
    "auto_suspend_time": "AutoSuspendTime",
    "max_concurrency": "MaxConcurrency",
    "tolerable_queue_time": "TolerableQueueTime",
    "crontab_resume_suspend": "CrontabResumeSuspend",
    "crontab_resume_suspend_strategy": "CrontabResumeSuspendStrategy",
    "elastic_switch": "ElasticSwitch",
    "elastic_limit": "ElasticLimit",
    "schedule_elasticity_conf": "ScheduleElasticityConf",
}
IMMUTABLE = {
    ...
    "engine_network_id": "EngineNetworkId",
    "engine_exec_type": "EngineExecType",
...
def find(module, client, models, name):
    ...
    while True:
        ...
        if not page or offset >= int(response.TotalCount or 0):
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple DLC data engines matched the exact name", name=name)
    return matches[0] if matches else None


def create_request(models, p):
    payload = {
        "DataEngineName": p["name"],
    ...
    if p.get("image_version_name") is not None:
        payload["ImageVersionName"] = p["image_version_name"]
...
def resolve_image(module, client, models, current, name):
    ...
    if not matches:
        module.fail_json(msg="online DLC data-engine image version was not found", image_version_name=name, engine_type=engine_type)
    if len(matches) > 1:
        module.fail_json(msg="multiple online DLC data-engine images matched the exact version name", image_version_name=name, engine_type=engine_type)
...
def image_switch_request(models, engine_id, image_id):
    ...
    request.DataEngineId, request.NewImageVersionId = engine_id, image_id
    return request


def standby_request(models, name, enabled):
    request = models.SwitchDataEngineRequest()
    request.DataEngineName, request.StartStandbyCluster = name, enabled
    return request
...
def desired_view(p, current=None):
    ...
    if p.get("image_version_name") is not None:
        result["ImageVersionName"] = p["image_version_name"]
    if p.get("standby_cluster") is not None:
        result["StartStandbyCluster"] = p["standby_cluster"]
    return result


def drift(p, current):
    return {target: (current.get(target), p[source]) for source, target in MUTABLE.items() if p.get(source) is not None and current.get(target) != p[source]}
...
def wait_engine(module, client, models, p, target, desired=None):
    def poll():
        ...
        if target == "running" and current.get("State") != 2:
            return "pending"
        if target == "suspended" and current.get("State") != 1:
            return "pending"
        if target == "ready" and current.get("State") not in (1, 2):
            return "pending"
...
def run_module():
    spec = {
        ...
        "name": {"required": True},
        "engine_type": {"choices": ["spark", "presto", "kyuubi"]},
        "cluster_type": {},
        "mode": {"type": "int", "choices": [0, 1, 2]},
        "size": {"type": "int"},
        "min_clusters": {"type": "int"},
        "max_clusters": {"type": "int"},
        "auto_resume": {"type": "bool"},
        "auto_suspend": {"type": "bool"},
        "auto_suspend_time": {"type": "int"},
        "max_concurrency": {"type": "int"},
        "tolerable_queue_time": {"type": "int"},
        "description": {},
        "crontab_resume_suspend": {"type": "int", "choices": [0, 1]},
        "crontab_resume_suspend_strategy": {"type": "dict"},
        "elastic_switch": {"type": "bool"},
        "elastic_limit": {"type": "int"},
        "schedule_elasticity_conf": {"type": "dict"},
        "cidr_block": {},
        "engine_network_id": {},
        "engine_exec_type": {"choices": ["SQL", "BATCH"]},
        "resource_type": {"choices": ["Standard_CU", "Memory_CU"]},
        "engine_generation": {"choices": ["Native", "SuperSQL"]},
        "image_version_name": {},
        "standby_cluster": {"type": "bool"},
        "pay_mode": {"type": "int", "choices": [0, 1], "default": 0},
        "allow_scale_down": {"type": "bool", "default": False},
        "allow_image_switch": {"type": "bool", "default": False},
    ...
    p = module.params
    if p.get("description") is not None and len(p["description"]) > 250:
        module.fail_json(msg="DLC data-engine description must not exceed 250 characters")
    if p.get("min_clusters") is not None and p.get("max_clusters") is not None and p["min_clusters"] > p["max_clusters"]:
        module.fail_json(msg="min_clusters must not exceed max_clusters")
    if p.get("crontab_resume_suspend") == 1 and p.get("auto_suspend") is True:
        module.fail_json(msg="crontab_resume_suspend and auto_suspend cannot both be enabled")
    if p.get("elastic_limit") is not None and p["elastic_limit"] < 0:
        module.fail_json(msg="elastic_limit must not be negative")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, data_engine=None, data_engine_id=None)
            if not p["allow_delete"]:
                module.fail_json(msg="set allow_delete=true to authorize deleting the DLC data engine", data_engine=current)
            diff_value = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteDataEngine, delete_request(models, p["name"]))
                if p["wait"]:
                    wait_engine(module, client, models, p, "absent")
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a DLC data engine", missing=missing)
            ...
            if not module.check_mode:
                ...
                if p["wait"]:
                    wait_engine(
                        module, client, models, p, "ready", {k: v for k, v in target.items() if k not in ("State", "DataEngineName", "StartStandbyCluster")}
                    )
                current = find(module, client, models, p["name"])
                if p.get("standby_cluster") is not None and (current or {}).get("StartStandbyCluster") != p["standby_cluster"]:
                    ...
                    if p["wait"]:
                        wait_engine(module, client, models, p, "ready", {"StartStandbyCluster": p["standby_cluster"]})
                    current = find(module, client, models, p["name"])
                ...
                if target_state is not None and (current or {}).get("State") != target_state:
                    ...
                    if p["wait"]:
                        wait_engine(module, client, models, p, p["state"])
                    current = find(module, client, models, p["name"])
            else:
                engine_id = None
            module.exit_json(
                changed=True,
                **(diff_value or {}),
                data_engine=current if not module.check_mode else target,
                data_engine_id=(current or {}).get("DataEngineId") if not module.check_mode else engine_id,
            )
        immutable_drift = {
            target: (current.get(target), p[source]) for source, target in IMMUTABLE.items() if p.get(source) is not None and current.get(target) != p[source]
        }
        if immutable_drift:
            module.fail_json(msg="DLC data-engine identity, type, billing, network and generation fields are immutable", immutable_drift=immutable_drift)
        ...
        if p.get("image_version_name") is not None and current.get("ImageVersionName") != p["image_version_name"]:
            ...
            if not p["allow_image_switch"]:
                module.fail_json(
                    msg="set allow_image_switch=true to authorize switching the DLC engine image",
                    image_drift={"ImageVersionName": (current.get("ImageVersionName"), p["image_version_name"])},
        ...
        if p.get("standby_cluster") is not None and current.get("StartStandbyCluster") != p["standby_cluster"]:
            if not p["allow_standby_switch"]:
                module.fail_json(
                    msg="set allow_standby_switch=true to authorize changing the DLC standby cluster",
                    standby_drift={"StartStandbyCluster": (current.get("StartStandbyCluster"), p["standby_cluster"])},
                )
        ...
        if p.get("description") is not None and (current.get("Message") or "") != p["description"]:
            changes["Message"] = (current.get("Message") or "", p["description"])
        target_state = {"running": 2, "suspended": 1}.get(p["state"])
        state_change = target_state is not None and current.get("State") != target_state
        if not changes and not state_change:
            module.exit_json(changed=False, data_engine=current, data_engine_id=current.get("DataEngineId"))
        after = desired_view(p, current)
        if image_target:
            after.update(image_target)
        ...
        if not module.check_mode:
            ...
            if mutable_changes:
                module.sdk_call(client.UpdateDataEngine, update_request(models, p))
            if "Message" in changes:
                module.sdk_call(client.ModifyDataEngineDescription, description_request(models, p["name"], p["description"]))
            if image_target:
                module.sdk_call(client.SwitchDataEngineImage, image_switch_request(models, current["DataEngineId"], image_target["ImageVersionId"]))
            if "StartStandbyCluster" in changes:
                module.sdk_call(client.SwitchDataEngine, standby_request(models, p["name"], p["standby_cluster"]))
            if changes and p["wait"]:
                wait_engine(module, client, models, p, "ready", {k: v[1] for k, v in changes.items()})
            current = find(module, client, models, p["name"])
            if state_change and current.get("State") != target_state:
                ...
                current = find(module, client, models, p["name"])
        module.exit_json(
            changed=True, **(diff_value or {}), data_engine=current if not module.check_mode else after, data_engine_id=current.get("DataEngineId")
        )
