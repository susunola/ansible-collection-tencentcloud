...
def describe_request(models, p):
    ...
    if p["scope"] == "global": request.DbName, request.ObjectType, request.Object, request.ColName = "*", "*", "*", "*"
    elif p["scope"] == "database": request.DbName, request.ObjectType, request.Object, request.ColName = p["database"], "*", "*", "*"
    else: request.DbName, request.ObjectType, request.Object, request.ColName = p["database"], "table", p["table"], "*"
    return request
...
def modify_request(models, p):
    request = models.ModifyUserPrivilegesRequest(); request.InstanceId, request.Users = p["instance_id"], [user(models, p)]; desired = sorted(set(p["privileges"]))
    if p["scope"] == "global": request.GlobalPrivileges = desired
    elif p["scope"] == "database":
        item = models.DatabasePrivileges(); item.Database, item.Privileges = p["database"], desired; request.DatabasePrivileges = [item]
    else:
        item = models.TablePrivileges(); item.Database, item.Table, item.Privileges = p["database"], p["table"], desired; request.TablePrivileges = [item]
    return request
...
def result(p, privileges):
    ...
    if p["scope"] != "global": value["Database"] = p["database"]
    if p["scope"] == "table": value["Table"] = p["table"]
    return value


def run_module():
    spec = {"instance_id": {"required": True}, "username": {"required": True}, "host": {"default": "%"}, "scope": {"required": True, "choices": ["global", "database", "table"]}, "database": {}, "table": {}, "privileges": {"type": "list", "elements": "str", "required": True}}
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if p["scope"] in ("database", "table") and not p.get("database"): module.fail_json(msg="database is required for database and table scope")
    if p["scope"] == "table" and not p.get("table"): module.fail_json(msg="table is required for table scope")
    if len(p["privileges"]) != len(set(p["privileges"])): module.fail_json(msg="privileges must not contain duplicates")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TdmysqlClient, "tdmysql.tencentcloudapi.com")
    try:
        response = module.sdk_call(client.DescribeUserPrivileges, describe_request(models, p)); current = result(p, response.Privileges); target = result(p, p["privileges"])
        if current == target: module.exit_json(changed=False, privilege=current)
        diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            module.sdk_call(client.ModifyUserPrivileges, modify_request(models, p)); response = module.sdk_call(client.DescribeUserPrivileges, describe_request(models, p)); current = result(p, response.Privileges)
