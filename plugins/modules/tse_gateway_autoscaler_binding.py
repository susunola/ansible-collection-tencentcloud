...
def describe_request(models, p, offset=0):
    r = models.DescribeAutoScalerResourceStrategyBindingGroupsRequest()
    r.GatewayId, r.StrategyId, r.Offset, r.Limit = p["gateway_id"], p["strategy_id"], offset, 100
    return r


def mutation_request(cls, p, group_ids):
    r = cls()
    r.GatewayId, r.StrategyId, r.GroupIds = p["gateway_id"], p["strategy_id"], group_ids
    return r


def strategy_request(models, p):
    r = models.DescribeAutoScalerResourceStrategiesRequest()
    r.GatewayId = p["gateway_id"]
    return r
...
def resolve_strategy(module, client, models, p):
    if p.get("strategy_id"):
        return
    result = module.sdk_call(client.DescribeAutoScalerResourceStrategies, strategy_request(models, p)).Result
    matches = []
    for item in (result.StrategyList if result else []) or []:
        value = item._serialize(allow_none=True)
        if value.get("StrategyName") == p["strategy_name"]:
            matches.append(value.get("StrategyId"))
    if not matches:
        module.fail_json(msg="TSE autoscaler strategy name was not found", strategy_name=p["strategy_name"])
    if len(matches) > 1:
        module.fail_json(msg="Multiple TSE autoscaler strategies matched the name", strategy_name=p["strategy_name"])
...
def resolve_groups(module, client, models, p):
    ...
    if not names:
        return
    result = module.sdk_call(client.DescribeNativeGatewayServerGroups, groups_request(models, p)).Result
    by_name = {}
    for item in (result.GatewayGroupList if result else []) or []:
        value = item._serialize(allow_none=True)
        by_name.setdefault(value.get("Name"), []).append(value.get("GroupId"))
    missing = [name for name in names if name not in by_name]
    ambiguous = [name for name in names if len(by_name.get(name, [])) > 1]
    if missing:
        module.fail_json(msg="TSE gateway group names were not found", group_names=missing)
    if ambiguous:
        module.fail_json(msg="Multiple TSE gateway groups matched names", group_names=ambiguous)
...
def current(module, client, models, p):
    ...
    values = []
    while True:
        result = module.sdk_call(client.DescribeAutoScalerResourceStrategyBindingGroups, describe_request(models, p, offset)).Result
        page = result.GroupInfos if result else []
        values.extend(item._serialize(allow_none=True) for item in page or [])
        offset += len(page or [])
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "group_names": {"type": "list", "elements": "str"},
            "purge_unlisted": {"type": "bool", "default": False},
        },
        required_one_of=[("strategy_id", "strategy_name"), ("group_ids", "group_names")],
        mutually_exclusive=[("strategy_id", "strategy_name")],
        supports_check_mode=True,
    )
    p = module.params
    if len(set(p.get("group_names") or [])) != len(p.get("group_names") or []):
        module.fail_json(msg="group_names must not contain duplicates")
    module.require_sdk()
    models, cm = _load()
    ...
    if len(set(p["group_ids"])) != len(p["group_ids"]):
        module.fail_json(msg="group_ids must not contain duplicates")
    if p["state"] == "absent" and p["purge_unlisted"]:
        module.fail_json(msg="purge_unlisted is only valid with state=present")
    try:
        resolve_strategy(module, client, models, p)
        details = current(module, client, models, p)
        existing = {item.get("GroupId") for item in details if item.get("GroupId")}
        requested = set(p["group_ids"])
        add = sorted(requested - existing) if p["state"] == "present" else []
        remove = (
            sorted(existing - requested)
            if p["state"] == "present" and p["purge_unlisted"]
            else (sorted(requested & existing) if p["state"] == "absent" else [])
        )
        target = sorted((existing | requested) - set(remove)) if p["state"] == "present" else sorted(existing - requested)
        if not add and not remove:
            module.exit_json(changed=False, binding={"StrategyId": p["strategy_id"], "GroupIds": sorted(existing), "Groups": details})
        diff = maybe_diff(module, {"GroupIds": sorted(existing)}, {"GroupIds": target})
        if not module.check_mode:
            if add:
                module.sdk_call(client.BindAutoScalerResourceStrategyToGroups, mutation_request(models.BindAutoScalerResourceStrategyToGroupsRequest, p, add))
            if remove:
                module.sdk_call(
                    client.UnbindAutoScalerResourceStrategyFromGroups, mutation_request(models.UnbindAutoScalerResourceStrategyFromGroupsRequest, p, remove)
                )
            details = current(module, client, models, p)
            target = sorted(item.get("GroupId") for item in details if item.get("GroupId"))
        module.exit_json(
            changed=True,
            **(diff or {}),
            binding={
                ...
                "GroupIds": target,
                "Groups": details if not module.check_mode else [],
                "AddedGroupIds": add,
