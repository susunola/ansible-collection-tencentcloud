...
FIELDS = {
    "kind": "Kind", "data_engine_name": "DataEngineName", "dependent_files": "ProgramDependentFiles",
    "dependent_jars": "ProgramDependentJars", "dependent_python": "ProgramDependentPython", "archives": "ProgramArchives",
    "driver_size": "DriverSize", "executor_size": "ExecutorSize", "executor_numbers": "ExecutorNumbers",
    "executor_max_numbers": "ExecutorMaxNumbers", "proxy_user": "ProxyUser", "timeout": "TimeoutInSecond",
}
...
def describe_request(models, session_id):
    request = models.DescribeNotebookSessionRequest(); request.SessionId = session_id; return request


def read_id(module, client, models, session_id):
    try: response = module.sdk_call(client.DescribeNotebookSession, describe_request(models, session_id))
...
def list_request(models, p, offset=0):
    request = models.DescribeNotebookSessionsRequest(); request.Offset, request.Limit = offset, 100
    if p.get("data_engine_name"): request.DataEngineName = p["data_engine_name"]
    item = models.Filter(); item.Name, item.Values = "notebook-keyword", [p["name"]]; request.Filters = [item]
    return request


def find(module, client, models, p):
    if p.get("session_id"): return read_id(module, client, models, p["session_id"])
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeNotebookSessions, list_request(models, p, offset)); page = response.Sessions or []
        for item in page:
            if item.Name == p["name"] and item.State in ACTIVE and (not p.get("data_engine_name") or item.DataEngineName == p["data_engine_name"]):
                ...
                if value: matches.append(value)
        offset += len(page)
        if not page or offset >= int(response.TotalElements or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple active DLC Notebook sessions matched; specify session_id", name=p["name"])
...
def create_request(models, p):
    ...
    for source, target in FIELDS.items():
        if p.get(source) is not None: payload[target] = sorted(p[source]) if source in LIST_FIELDS else p[source]
    if p.get("arguments") is not None: payload["Arguments"] = _pairs(p["arguments"])
    request = models.CreateNotebookSessionRequest(); request.from_json_string(json.dumps(payload)); return request


def delete_request(models, session_id):
    request = models.DeleteNotebookSessionRequest(); request.SessionId = session_id; return request


def desired(p, current=None):
    result = dict(current or {}); result["Name"] = p.get("name") or result.get("Name")
    for source, target in FIELDS.items():
        if p.get(source) is not None: result[target] = sorted(p[source]) if source in LIST_FIELDS else p[source]
    if p.get("arguments") is not None: result["Arguments"] = _pairs(p["arguments"])
    return result


def immutable_drift(p, current):
    target, changes = desired(p, current), {}
    if p.get("name") is not None and current.get("Name") != p["name"]: changes["Name"] = (current.get("Name"), p["name"])
    for source, remote in FIELDS.items():
        if p.get(source) is not None and current.get(remote) != target.get(remote): changes[remote] = (current.get(remote), target.get(remote))
    if p.get("arguments") is not None and _pairs(current.get("Arguments")) != target["Arguments"]: changes["Arguments"] = (_pairs(current.get("Arguments")), target["Arguments"])
    return changes
...
def wait_session(module, client, models, p, session_id, present):
    def poll():
        ...
        if not present: return "absent" if current is None or current.get("State") in TERMINAL else "pending"
        if current is None: return "absent"
        state = current.get("State")
        if state in FAILED: module.fail_json(msg="DLC Notebook session entered a failed state", session=current)
...
def run_module():
    ...
    spec = {
        "state": {"choices": ["present", "absent"], "default": "present"}, "session_id": {}, "name": {},
        "kind": {"choices": ["spark", "pyspark", "sparkr", "sql"]}, "data_engine_name": {},
        "dependent_files": {"type": "list", "elements": "str"}, "dependent_jars": {"type": "list", "elements": "str"},
        "dependent_python": {"type": "list", "elements": "str"}, "archives": {"type": "list", "elements": "str"},
        "driver_size": {"choices": ["small", "medium", "large", "xlarge"]}, "executor_size": {"choices": ["small", "medium", "large", "xlarge"]},
        "executor_numbers": {"type": "int"}, "executor_max_numbers": {"type": "int"}, "arguments": {"type": "list", "elements": "dict", "options": pair},
        "proxy_user": {}, "timeout": {"type": "int"},
        "allow_replace": {"type": "bool", "default": False}, "allow_delete": {"type": "bool", "default": False},
        "wait": {"type": "bool", "default": True}, "waiter_delay": {"type": "int", "default": 5}, "waiter_timeout": {"type": "int", "default": 900},
    }
    module = TencentCloudModule(argument_spec=spec, required_one_of=[("session_id", "name")], supports_check_mode=True); p = module.params
    if p.get("executor_numbers") is not None and p.get("executor_max_numbers") is not None and p["executor_numbers"] > p["executor_max_numbers"]: module.fail_json(msg="executor_numbers must not exceed executor_max_numbers")
    if p.get("arguments") is not None and len({x["key"] for x in p["arguments"]}) != len(p["arguments"]): module.fail_json(msg="arguments contains duplicate keys")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current or current.get("State") in TERMINAL: module.exit_json(changed=False, session=None, session_id=None)
            if not p["allow_delete"]: module.fail_json(msg="set allow_delete=true to authorize deleting the DLC Notebook session", session=current)
            diff_value = maybe_diff(module, current, None); session_id = current["SessionId"]
            if not module.check_mode:
                module.sdk_call(client.DeleteNotebookSession, delete_request(models, session_id))
                if p["wait"]: wait_session(module, client, models, p, session_id, False)
            module.exit_json(changed=True, **(diff_value or {}), session=None, session_id=None)
        if current and current.get("State") in TERMINAL:
            if p.get("session_id"): module.fail_json(msg="the specified DLC Notebook session is terminal and cannot be restarted; omit session_id to create a new named session", session=current)
            current = None
        if not current:
            missing = [key for key in ("name", "kind", "data_engine_name") if not p.get(key)]
            if missing: module.fail_json(msg="creation parameters are required for a DLC Notebook session", missing=missing)
            target = desired(p); diff_value = maybe_diff(module, None, target); session_id = None
            if not module.check_mode:
                session_id = module.sdk_call(client.CreateNotebookSession, create_request(models, p)).SessionId
                if p["wait"]: wait_session(module, client, models, p, session_id, True)
                current = read_id(module, client, models, session_id)
            module.exit_json(changed=True, **(diff_value or {}), session=current if not module.check_mode else target, session_id=session_id)
        if current.get("State") == "shutting_down": module.fail_json(msg="DLC Notebook session is shutting down; wait for it to become terminal before recreating", session=current)
        changes = immutable_drift(p, current)
        if not changes: module.exit_json(changed=False, session=current, session_id=current.get("SessionId"))
        if not p["allow_replace"]: module.fail_json(msg="DLC Notebook session settings are immutable; set allow_replace=true to authorize replacement", immutable_drift=changes, session=current)
        if not p.get("name"): module.fail_json(msg="name is required when replacing a DLC Notebook session")
        target = desired(p); diff_value = maybe_diff(module, current, target); old_id = current["SessionId"]
        if not module.check_mode:
            module.sdk_call(client.DeleteNotebookSession, delete_request(models, old_id))
            if p["wait"]: wait_session(module, client, models, p, old_id, False)
            session_id = module.sdk_call(client.CreateNotebookSession, create_request(models, p)).SessionId
            if p["wait"]: wait_session(module, client, models, p, session_id, True)
            current = read_id(module, client, models, session_id)
