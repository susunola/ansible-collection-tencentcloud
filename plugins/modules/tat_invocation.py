...
def invoke_request(models, p):
    request = models.InvokeCommandRequest(); request.CommandId, request.InstanceIds = p["command_id"], sorted(set(p["instance_ids"])); request.Parameters = json.dumps({str(k): str(v) for k, v in p["parameters"].items()}, sort_keys=True, separators=(",", ":"))
    for source, target in (("username", "Username"), ("working_directory", "WorkingDirectory"), ("timeout", "Timeout"), ("output_cos_bucket_url", "OutputCOSBucketUrl"), ("output_cos_key_prefix", "OutputCOSKeyPrefix")):
        if p.get(source) is not None: setattr(request, target, p[source])
    return request


def cancel_request(models, p):
    request = models.CancelInvocationRequest(); request.InvocationId = p["invocation_id"]
    if p["instance_ids"]: request.InstanceIds = sorted(set(p["instance_ids"]))
    return request


def tasks_request(models, invocation_id, offset, include_output):
    request = models.DescribeInvocationTasksRequest(); request.Offset, request.Limit, request.HideOutput = offset, 100, not include_output
    item = models.Filter(); item.Name, item.Values = "invocation-id", [invocation_id]; request.Filters = [item]; return request


def read_tasks(module, client, models, invocation_id, include_output):
    offset, values = 0, []
    while True:
        response = module.sdk_call(client.DescribeInvocationTasks, tasks_request(models, invocation_id, offset, include_output)); items = list(response.InvocationTaskSet or [])
        values.extend(item._serialize(allow_none=True) for item in items); offset += len(items)
        if not items or offset >= int(response.TotalCount or 0): break
    for value in values:
        value.pop("CommandDocument", None)
        if not include_output and value.get("TaskResult"): value["TaskResult"] = {"Output": "<redacted>"}
    return values
...
def wait_tasks(module, client, models, invocation_id, expected_count, include_output):
    ...
    while True:
        ...
        if len(values) >= expected_count and all(value.get("TaskStatus") in TERMINAL for value in values): return values
        if time.time() >= deadline: module.fail_json(msg="Timed out waiting for TAT invocation tasks", invocation_id=invocation_id, observed_tasks=len(values), expected_tasks=expected_count)
        time.sleep(module.params["waiter_delay"])


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["started", "cancelled"], "default": "started"}, "invocation_id": {}, "command_id": {}, "instance_ids": {"type": "list", "elements": "str", "default": []}, "parameters": {"type": "dict", "default": {}, "no_log": True}, "username": {}, "working_directory": {}, "timeout": {"type": "int"}, "output_cos_bucket_url": {}, "output_cos_key_prefix": {}, "wait": {"type": "bool", "default": True}, "fail_on_task_error": {"type": "bool", "default": True}, "include_output": {"type": "bool", "default": False}}, supports_check_mode=True)
    p = module.params
    if p["state"] == "started" and (not p.get("command_id") or not p["instance_ids"]): module.fail_json(msg="command_id and instance_ids are required when state=started")
    if p["state"] == "cancelled" and not p.get("invocation_id"): module.fail_json(msg="invocation_id is required when state=cancelled")
    if len(p["instance_ids"]) > 200: module.fail_json(msg="instance_ids cannot contain more than 200 targets")
    if p.get("timeout") is not None and not 1 <= p["timeout"] <= 86400: module.fail_json(msg="timeout must be between 1 and 86400 seconds")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TatClient, "tat.tencentcloudapi.com")
    try:
        if module.check_mode: module.exit_json(changed=True, invocation_id=p.get("invocation_id"), tasks=[])
        if p["state"] == "cancelled": module.sdk_call(client.CancelInvocation, cancel_request(models, p)); module.exit_json(changed=True, invocation_id=p["invocation_id"], tasks=[])
        invocation_id = module.sdk_call(client.InvokeCommand, invoke_request(models, p)).InvocationId
        if not p["wait"]: module.exit_json(changed=True, invocation_id=invocation_id, tasks=[])
        values = wait_tasks(module, client, models, invocation_id, len(set(p["instance_ids"])), p["include_output"]); summary = dict(Counter(value.get("TaskStatus") for value in values))
        failures = [value for value in values if value.get("TaskStatus") in FAILED]
        if failures and p["fail_on_task_error"]: module.fail_json(msg="One or more TAT invocation tasks failed", invocation_id=invocation_id, status_summary=summary, failed_tasks=failures)
