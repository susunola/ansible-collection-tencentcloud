...
def describe_request(models, p, offset=0):
    request = models.DescribeCustomWhiteRuleRequest(); request.Domain, request.Offset, request.Limit = p["domain"], offset, 100; return request
...
def create_request(models, p):
    request = _apply(models.AddCustomWhiteRuleRequest(), models, p); request.Name, request.SortId, request.ExpireTime = p["name"], str(p["priority"]), str(p["expire_time"]); return request
def update_request(models, p, rule_id):
    request = _apply(models.ModifyCustomWhiteRuleRequest(), models, p); request.RuleId, request.RuleName = rule_id, p["name"]; return request
def status_request(models, p, rule_id):
    request = models.ModifyCustomWhiteRuleStatusRequest(); request.Domain, request.RuleId, request.Status = p["domain"], rule_id, 1 if p["enabled"] else 0; return request
def delete_request(models, p, rule_id):
    request = models.DeleteCustomWhiteRuleRequest(); request.Domain, request.RuleId = p["domain"], rule_id; return request


def _sorted(values): return sorted(values or [], key=lambda x: (x.get("Field") or "", x.get("Arg") or "", x.get("CompareFunc") or "", x.get("Content") or ""))
def comparable(value): return {"Name": value.get("Name"), "SortId": int(value.get("SortId") or 0), "Bypass": value.get("Bypass") or "", "Strategies": _sorted(value.get("Strategies")), "LogicalOp": value.get("LogicalOp") or "and", "ExpireTime": int(value.get("ExpireTime") or 0), "Status": int(value.get("Status") or 0)}
def desired(p): return {"Name": p["name"], "SortId": p["priority"], "Bypass": p["bypass_modules"], "Strategies": _sorted(p["strategies"]), "LogicalOp": p["logical_operator"], "ExpireTime": p["expire_time"], "Status": 1 if p["enabled"] else 0}
def find(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeCustomWhiteRule, describe_request(models, p, offset)); values = list(response.RuleList or [])
        for item in values:
    ...
    if len(matches) > 1: module.fail_json(msg="Multiple WAF precision allowlist rules matched; specify rule_id")
...
def run_module():
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteCustomWhiteRule, delete_request(models, p, int(current["RuleId"])))
        ...
        if not module.check_mode:
            if current:
                rule_id = int(current["RuleId"]); module.sdk_call(client.ModifyCustomWhiteRule, update_request(models, p, rule_id))
            else:
                rule_id = int(module.sdk_call(client.AddCustomWhiteRule, create_request(models, p)).RuleId); p["rule_id"] = rule_id
            module.sdk_call(client.ModifyCustomWhiteRuleStatus, status_request(models, p, rule_id)); current = find(module, client, models, p)
