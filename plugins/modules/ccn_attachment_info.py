...
options:
  ccn_id:
    description: ID of the CCN instance whose attachments are returned.
    type: str
    required: true
  filters:
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.ccn_attachment
    description: Attach network instances to Tencent Cloud CCN.
...
RETURN = r'''
attachments:
  description: Attachments of the CCN instance.
  returned: always
  type: list
  elements: dict
  sample:
...
def build_request(models, ccn_id, filters, offset, limit):
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda offset, limit: build_request(
            models,
            module.params["ccn_id"],
            module.params["filters"],
            offset,
            limit),
        lambda request: sdk_call(module, client.DescribeCcnAttachedInstances, request),
        lambda response: response.InstanceSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    attachments = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, attachments=attachments,
