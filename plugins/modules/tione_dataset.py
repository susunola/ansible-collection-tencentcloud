...
FIELDS = {
    "dataset_type": ("DatasetType", None), "storage_data_path": ("StorageDataPath", "CosPathInfo"),
    "storage_label_path": ("StorageLabelPath", "CosPathInfo"), "dataset_tags": ("DatasetTags", "Tag"),
    "annotation_status": ("AnnotationStatus", None), "annotation_type": ("AnnotationType", None),
    "annotation_format": ("AnnotationFormat", None), "schema_infos": ("SchemaInfos", "SchemaInfo"),
    "is_schema_existed": ("IsSchemaExisted", None), "content_type": ("ContentType", None),
    "dataset_scene": ("DatasetScene", None), "scene_tags": ("SceneTags", None), "cfs_config": ("CFSConfig", "CFSConfig"),
}
...
def list_request(models, p, offset):
    request = models.DescribeDatasetsRequest(); request.Offset, request.Limit = offset, 200
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    if p.get("dataset_id"): request.DatasetIds = [p["dataset_id"]]
    elif p.get("name"):
        item = models.Filter(); item.Name, item.Values = "DatasetName", [p["name"]]; request.Filters = [item]
    return request
...
def find(module, client, models, p):
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeDatasets, list_request(models, p, offset)); items = response.DatasetGroups or []
        for item in items:
            value = normalize(item._serialize(allow_none=True))
            if p.get("dataset_id") and value.get("DatasetId") == p["dataset_id"]: matches.append(value)
            elif not p.get("dataset_id") and value.get("DatasetName") == p.get("name"): matches.append(value)
        offset += len(items)
...
def desired(p, current=None):
    result = dict(current or {}); result["DatasetName"] = p.get("name") or result.get("DatasetName")
...
def conflict(p, current):
    ...
    for source, (key, _) in FIELDS.items():
        if source == "schema_infos": continue
        if p.get(source) is not None and current.get(key) != target.get(key): changes[key] = (current.get(key), target.get(key))
    if p.get("name") is not None and current.get("DatasetName") != p["name"]: changes["DatasetName"] = (current.get("DatasetName"), p["name"])
    return changes


def create_request(models, p):
    request = models.CreateDatasetRequest(); request.DatasetName = p["name"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
...
def delete_request(models, p):
    request = models.DeleteDatasetRequest(); request.DatasetId, request.DeleteLabelEnable = p["dataset_id"], p["delete_label_files"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    return request
...
def run_module():
    spec = {
        "state": {"choices": ["present", "absent"], "default": "present"}, "name": {}, "dataset_id": {}, "project_id": {},
        "dataset_type": {"choices": ["TYPE_DATASET_IMAGE", "TYPE_DATASET_LLM", "TYPE_DATASET_TABLE", "TYPE_DATASET_OTHER"]},
        "storage_data_path": {"type": "dict"}, "storage_label_path": {"type": "dict"}, "dataset_tags": {"type": "list", "elements": "dict"},
        "annotation_status": {"choices": ["STATUS_NON_ANNOTATED", "STATUS_ANNOTATED"]},
        "annotation_type": {"choices": ["ANNOTATION_TYPE_CLASSIFICATION", "ANNOTATION_TYPE_DETECTION", "ANNOTATION_TYPE_SEGMENTATION", "ANNOTATION_TYPE_TRACKING", "ANNOTATION_TYPE_OCR"]},
        "annotation_format": {"choices": ["ANNOTATION_FORMAT_TI", "ANNOTATION_FORMAT_PASCAL", "ANNOTATION_FORMAT_COCO", "ANNOTATION_FORMAT_FILE"]},
        "schema_infos": {"type": "list", "elements": "dict"}, "is_schema_existed": {"type": "bool"},
        "content_type": {"choices": ["TYPE_TEXT_LINE", "TYPE_TEXT_FILE"]}, "dataset_scene": {"choices": ["LLM", "CV", "STRUCTURE", "OTHER"]},
        "scene_tags": {"type": "list", "elements": "str"}, "cfs_config": {"type": "dict"}, "delete_label_files": {"type": "bool", "default": False},
    ...
    if p["state"] == "absent" and not p.get("dataset_id"): module.fail_json(msg="dataset_id is required for safe deletion")
    if p.get("cfs_config") is not None and p.get("dataset_scene") not in (None, "LLM"): module.fail_json(msg="cfs_config is only valid for the LLM dataset scene")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TioneClient, "tione.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, dataset=None, dataset_id=p["dataset_id"])
            if not p["allow_delete"]: module.fail_json(msg="allow_delete=true is required to delete a TIONE dataset", dataset=current)
            diff_value = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteDataset, delete_request(models, p))
                if p["wait"]: wait_presence(module, client, models, p, False)
            module.exit_json(changed=True, **(diff_value or {}), dataset=None, dataset_id=p["dataset_id"])
        if current:
            changes = conflict(p, current)
            if changes: module.fail_json(msg="TIONE datasets expose no update API and the existing dataset has immutable drift", dataset=current, immutable_drift=changes)
            module.exit_json(changed=False, dataset=current, dataset_id=current.get("DatasetId"))
        if p.get("dataset_id"): module.fail_json(msg="the requested TIONE dataset_id does not exist; omit dataset_id to create by name", dataset_id=p["dataset_id"])
        if not p.get("dataset_type"): module.fail_json(msg="dataset_type is required when creating a TIONE dataset")
        target, diff_value, dataset_id = desired(p), maybe_diff(module, None, desired(p)), None
        if not module.check_mode:
            response = module.sdk_call(client.CreateDataset, create_request(models, p)); dataset_id = response.DatasetId
            lookup = dict(p, dataset_id=dataset_id)
            if p["wait"]: wait_presence(module, client, models, lookup, True)
            current = find(module, client, models, lookup)
        module.exit_json(changed=True, **(diff_value or {}), dataset=current if not module.check_mode else target, dataset_id=(current or {}).get("DatasetId") or dataset_id)
