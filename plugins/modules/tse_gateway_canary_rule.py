...
def list_request(models,p,offset=0):
    r=models.DescribeCloudNativeAPIGatewayCanaryRulesRequest(); r.GatewayId,r.ServiceId,r.RuleType,r.Offset,r.Limit=p["gateway_id"],p["service_id"],p["rule_type"],offset,20; return r
def mutation_request(cls,p,config=None):
    payload={"GatewayId":p["gateway_id"],"ServiceId":p["service_id"]}
    if config is None: payload["Priority"]=p["priority"]
    else:
...
def find(module,client,models,p):
    ...
    while True:
        ...
        for item in values or []:
            ...
            if int(value.get("Priority"))==p["priority"]: matches.append(value)
        offset+=len(values or [])
        total=int(result.TotalCount or 0) if result else 0
        if not result or offset>=total: break
    if len(matches)>1: module.fail_json(msg="Multiple TSE canary rules matched the same service and priority")
    return matches[0] if matches else None
def run_module():
    module=TencentCloudModule(argument_spec={"state":{"choices":["present","absent"],"default":"present"},"gateway_id":{"required":True},"service_id":{"required":True},"priority":{"type":"int","required":True},"rule_type":{"choices":["Standard","Lane"],"default":"Standard"},"config":{"type":"dict"}},required_if=[("state","present",("config",))],supports_check_mode=True); p=module.params
    if not 0<=p["priority"]<=100: module.fail_json(msg="priority must be between 0 and 100")
    module.require_sdk(); models,cm=_load(); client=module.create_client(cm.TseClient,"tse.tencentcloudapi.com")
    try:
        current=find(module,client,models,p)
        if p["state"]=="absent":
            if not current: module.exit_json(changed=False,canary_rule=None)
            diff=maybe_diff(module,current,None)
            if not module.check_mode: module.sdk_call(client.DeleteCloudNativeAPIGatewayCanaryRule,mutation_request(models.DeleteCloudNativeAPIGatewayCanaryRuleRequest,p))
            module.exit_json(changed=True,**(diff or {}),canary_rule=None)
        target=dict(p["config"],Priority=p["priority"])
        if target.get("RuleType") and target["RuleType"]!=p["rule_type"]: module.fail_json(msg="config.RuleType must match rule_type")
        target.setdefault("RuleType",p["rule_type"])
        if current and contains(current,target): module.exit_json(changed=False,canary_rule=current)
        ...
        if not module.check_mode:
            ...
            module.sdk_call(api,mutation_request(cls,p,target)); current=find(module,client,models,p)
