#!/usr/bin/python
# -*- coding: utf-8 -*-
# Generated by scripts/generate_info_modules.py -- regenerate instead of editing.
# Copyright: (c) 2026, Tencent Cloud Ansible Collection Contributors
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)
from __future__ import absolute_import, division, print_function

__metaclass__ = type

DOCUMENTATION = r'''
---
module: ioa_device_info
short_description: Gather information about Tencent Cloud IOA devices
version_added: "0.9.0"
description: Returns IOA devices visible in a Tencent Cloud region.
options:
  page_size:
    description: Number of results requested per API call.
    type: int
    default: 100
extends_documentation_fragment: tencentcloud.cloud.tencentcloud
author: Tencent Cloud Ansible Collection Contributors (@susunola)
'''

EXAMPLES = r'''
- name: List all devices
  tencentcloud.cloud.ioa_device_info:
    region: ap-guangzhou
'''

RETURN = r'''
devices:
  description: Matching IOA devices.
  returned: always
  type: list
  elements: dict
total_count:
  description: Number of devices returned (the API reports no total count).
  returned: always
  type: int
'''

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.tencentcloud.cloud.plugins.module_utils.paging import Paginator
from ansible_collections.tencentcloud.cloud.plugins.module_utils.tencentcloud import (
    create_client_profile, create_credential, sdk_call, serialize_sdk_object,
    tencentcloud_argument_spec,
)


def build_request(models, offset, limit):
    request = models.DescribeDevicesRequest()
    request.PageNum = offset // limit + 1
    request.PageSize = limit
    return request


def run_module():
    argument_spec = tencentcloud_argument_spec()
    argument_spec.update({
        "page_size": {"type": "int", "default": 100},
    })
    module = AnsibleModule(
        argument_spec=argument_spec,
        supports_check_mode=True,
    )
    try:
        from tencentcloud.ioa.v20220601 import models, ioa_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-ioa package is required.")

    client = ioa_client.IoaClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "ioa.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.DescribeDevices, request),
        lambda response: response.Data.Items if response.Data is not None else None,
        lambda response: None,
    )
    item_set, total_count = paginator.fetch_all()
    devices = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, devices=devices, total_count=total_count)


def main():
    run_module()


if __name__ == "__main__":
    main()
