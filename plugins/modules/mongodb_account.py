...
def auth_roles(models, roles):
    ...
    for role in roles:
        item = models.Auth()
        item.NameSpace, item.Mask = role["namespace"], MASKS[role["access"]]
        result.append(item)
    return result
...
def password_request(models, p):
    ...
    request.InstanceId, request.UserName, request.Password = p["instance_id"], p["username"], p["password"]
    return request


def delete_request(models, p):
    request = models.DeleteAccountUserRequest()
    request.InstanceId, request.UserName, request.MongoUserPassword = p["instance_id"], p["username"], p["mongo_user_password"]
    return request
...
def desired(p):
    return {
        "UserName": p["username"],
        "UserDesc": p["description"],
        "AuthRole": normalized_roles([{"NameSpace": x["namespace"], "Mask": MASKS[x["access"]]} for x in p["roles"]]),
    }
...
def find(module, client, models, p):
    ...
    for item in list(response.Users or []):
        ...
        if value.get("UserName") == p["username"]:
            return value
    return None
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "mongo_user_password": {"no_log": True},
            "description": {"default": ""},
            "roles": {
                "type": "list",
                "elements": "dict",
                "options": {"namespace": {"required": True}, "access": {"choices": ["none", "read", "read_write"], "required": True}},
            },
        },
        required_if=[("state", "present", ("roles",))],
    ...
    module.require_sdk()
    models, cm = _load()
    client = module.create_client(cm.MongodbClient, "mongodb.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, account=None)
            if not p["mongo_user_password"]:
                module.fail_json(msg="mongo_user_password is required when deleting a MongoDB account")
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteAccountUser, delete_request(models, p))
        ...
        if current:
            require_immutable_unchanged(module, before, target, ("UserDesc",), "MongoDB account")
        if not module.check_mode:
            if not current:
                if not p["password"] or not p["mongo_user_password"]:
                    module.fail_json(msg="password and mongo_user_password are required when creating a MongoDB account")
                module.sdk_call(client.CreateAccountUser, create_request(models, p))
            else:
                if before["AuthRole"] != target["AuthRole"]:
                    module.sdk_call(client.SetAccountUserPrivilege, privilege_request(models, p))
                if p["rotate_password"]:
                    module.sdk_call(client.ResetDBInstancePassword, password_request(models, p))
            current = find(module, client, models, p)
