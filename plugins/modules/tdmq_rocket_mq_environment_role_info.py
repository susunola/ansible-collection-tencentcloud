...
RETURN = r'''
rocket_mq_environment_roles:
  description: Matching TDMQ rocket mq environment roles.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeRocketMQEnvironmentRoles, request),
        lambda response: response.EnvironmentRoleSets,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    rocket_mq_environment_roles = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, rocket_mq_environment_roles=rocket_mq_environment_roles,
