...
def list_request(models, p):
    r = models.DescribeCloudNativeAPIGatewayServicesRequest()
    r.GatewayId, r.Offset, r.Limit = p["gateway_id"], 0, 100
    f = models.ListFilter()
    f.Key, f.Value = "name", p["name"]
    r.Filters = [f]
    return r


def detail_request(models, p):
    r = models.DescribeOneCloudNativeAPIGatewayServiceRequest()
    r.GatewayId, r.ServiceName = p["gateway_id"], p["service_id"] or p["name"]
    return r
...
def health_detail_request(models, p, name):
    r = models.DescribeUpstreamHealthCheckConfigRequest()
    r.GatewayId, r.Name = p["gateway_id"], name
    return r


def targets_request(models, p, name, targets):
    return write_request(models.UpdateUpstreamTargetsRequest, p, {"GatewayId": p["gateway_id"], "Name": name, "Targets": targets})


def health_update_request(models, p, name, config):
    return write_request(models.UpdateUpstreamHealthCheckConfigRequest, p, {"GatewayId": p["gateway_id"], "Name": name, "HealthCheckConfig": config})


def delete_request(models, p, current):
    r = models.DeleteCloudNativeAPIGatewayServiceRequest()
    r.GatewayId, r.Name, r.DeleteRoutes = p["gateway_id"], current.get("ID") or p["name"], p["delete_routes"]
    return r


def find(module, client, models, p):
    result = module.sdk_call(client.DescribeCloudNativeAPIGatewayServices, list_request(models, p)).Result
    values = result.ServiceList if result else []
    matches = []
    for item in values or []:
        value = item._serialize(allow_none=True)
        if (p.get("service_id") and value.get("ID") == p["service_id"]) or (not p.get("service_id") and value.get("Name") == p["name"]):
            matches.append(value)
    if len(matches) > 1:
        module.fail_json(msg="Multiple TSE gateway services matched; specify service_id")
    if not matches:
        return None
    local = dict(p)
    local["service_id"] = matches[0].get("ID")
    detail = module.sdk_call(client.DescribeOneCloudNativeAPIGatewayService, detail_request(models, local)).Result
...
def auxiliary_state(module, client, models, p, current):
    ...
    if p.get("targets") is not None:
        result["Targets"] = (current.get("UpstreamInfo") or {}).get("Targets") or []
    if p.get("health_check_config") is not None:
        response = module.sdk_call(client.DescribeUpstreamHealthCheckConfig, health_detail_request(models, p, current.get("Name") or p["name"]))
...
def auxiliary_matches(value, p):
    return (p.get("targets") is None or targets_match(value.get("Targets"), p["targets"])) and (
        p.get("health_check_config") is None or contains(value.get("HealthCheckConfig"), p["health_check_config"])
    )
...
def wait_auxiliary(module, client, models, p, current):
    ...
    while True:
        value = find(module, client, models, p) or current
        value.update(auxiliary_state(module, client, models, p, value))
        if auxiliary_matches(value, p):
            return value
        if time.time() >= deadline:
            module.fail_json(msg="Timed out waiting for TSE gateway upstream convergence", service=value)
        time.sleep(p["waiter_delay"])


def require_success(module, response, operation):
    if getattr(response, "Result", None) is not True:
        module.fail_json(msg="TSE gateway upstream operation returned an unsuccessful result", operation=operation)
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "protocol": {},
            "timeout": {"type": "int"},
            "retries_count": {"type": "int"},
            "upstream_type": {},
            "upstream_info": {"type": "dict"},
            "targets": {"type": "list", "elements": "dict"},
            "health_check_config": {"type": "dict"},
            "path": {},
            "delete_routes": {"type": "bool", "default": False},
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteCloudNativeAPIGatewayService, delete_request(models, p, current))
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a TSE gateway service", missing=missing)
        ...
        effective.update(auxiliary)
        expected = dict(target)
        if p.get("targets") is not None:
            expected["Targets"] = p["targets"]
        if p.get("health_check_config") is not None:
            expected["HealthCheckConfig"] = p["health_check_config"]
        if not core_changed and not targets_changed and not health_changed:
            module.exit_json(changed=False, service=effective)
        ...
        if not module.check_mode:
            if core_changed:
                payload = {
                    ...
                    **{
                        k: v
                        for k, v in dict(current or {}, **target).items()
                        if k in ("Name", "Protocol", "Timeout", "Retries", "UpstreamType", "UpstreamInfo", "Path")
                    },
                }
                if existed:
                    payload["ID"] = current["ID"]
                api = client.ModifyCloudNativeAPIGatewayService if existed else client.CreateCloudNativeAPIGatewayService
                cls = models.ModifyCloudNativeAPIGatewayServiceRequest if existed else models.CreateCloudNativeAPIGatewayServiceRequest
                module.sdk_call(api, write_request(cls, p, payload))
                current = find(module, client, models, p)
            resource_name = (current or {}).get("Name") or p["name"]
            if targets_changed:
                require_success(
                    module, module.sdk_call(client.UpdateUpstreamTargets, targets_request(models, p, resource_name, p["targets"])), "UpdateUpstreamTargets"
                )
            if health_changed:
                require_success(
                    module,
                    module.sdk_call(client.UpdateUpstreamHealthCheckConfig, health_update_request(models, p, resource_name, p["health_check_config"])),
