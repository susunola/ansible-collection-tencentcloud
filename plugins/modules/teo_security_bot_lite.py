...
options:
  ...
  challenge_option:
    ...
    description: Challenge type when AI crawler action is Challenge.
...
def update_request(models, p):
    captcha = models.CAPTCHAPageChallenge()
    captcha.Enabled = "on" if p["captcha_page_enabled"] else "off"
    crawler = models.AICrawlerDetection()
    crawler.Enabled = "on" if p["ai_crawler_enabled"] else "off"
    if p["ai_crawler_enabled"]:
        action = models.SecurityAction()
        action.Name = p["ai_crawler_action"]
        if p["ai_crawler_action"] == "Challenge":
            params = models.ChallengeActionParameters()
            params.ChallengeOption = p["challenge_option"]
            action.ChallengeActionParameters = params
        crawler.Action = action
    bot = models.BotManagementLite()
    bot.CAPTCHAPageChallenge, bot.AICrawlerDetection = captcha, crawler
    policy = models.SecurityPolicy()
    policy.BotManagementLite = bot
...
def desired(p):
    return {
        "captcha_page_enabled": p["captcha_page_enabled"],
        "ai_crawler_enabled": p["ai_crawler_enabled"],
        "ai_crawler_action": p["ai_crawler_action"],
        "challenge_option": p["challenge_option"],
    }


def normalize(raw):
    captcha = raw.get("CAPTCHAPageChallenge") or {}
    crawler = raw.get("AICrawlerDetection") or {}
    action = crawler.get("Action") or {}
    return {
        "captcha_page_enabled": captcha.get("Enabled") == "on",
        "ai_crawler_enabled": crawler.get("Enabled") == "on",
        "ai_crawler_action": action.get("Name") or "Monitor",
        "challenge_option": (action.get("ChallengeActionParameters") or {}).get("ChallengeOption") or "ManagedChallenge",
    }
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "template_id": {},
            "host": {},
            "captcha_page_enabled": {"type": "bool", "default": False},
            "ai_crawler_enabled": {"type": "bool", "default": False},
            "ai_crawler_action": {"choices": ["Deny", "Monitor", "Allow", "Challenge"], "default": "Monitor"},
    ...
    try:
        ...
        raw = (
            response.SecurityPolicy.BotManagementLite._serialize(allow_none=True)
            if response.SecurityPolicy and response.SecurityPolicy.BotManagementLite
            else {}
        )
        before, target = normalize(raw), desired(p)
        if before == target:
            module.exit_json(changed=False, bot_lite=before)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            module.sdk_call(client.ModifySecurityPolicy, update_request(models, p))
            response = module.sdk_call(client.DescribeSecurityPolicy, describe_request(models, p))
            before = normalize(response.SecurityPolicy.BotManagementLite._serialize(allow_none=True))
