...
def find(module, client, models, p):
    try:
        ...
        raise
    if response.DatabaseInfo is None: return None
...
def create_request(models, p):
    request = models.CreateMetaDatabaseRequest(); request.DatasourceConnectionName = p["datasource_connection_name"]
    request.MetaDatabaseInfo = _model(models.MetaDatabaseInfo, {"DatabaseName": p["name"], "Comment": p.get("comment")})
    if p.get("govern_policy") is not None: request.GovernPolicy = _model(models.DataGovernPolicy, p["govern_policy"])
    if p.get("smart_policy") is not None: request.SmartPolicy = _model(models.SmartPolicy, p["smart_policy"])
    return request
...
def normalize(value):
    if isinstance(value, dict): return {key: normalize(item) for key, item in value.items() if item is not None}
...
def run_module():
    spec = {
        ...
        "datasource_connection_name": {"default": "DataLakeCatalog"}, "comment": {},
        "govern_policy": {"type": "dict"}, "smart_policy": {"type": "dict"},
        "allow_delete_nonempty": {"type": "bool", "default": False}, "allow_delete": {"type": "bool", "default": False},
        "wait": {"type": "bool", "default": True}, "waiter_delay": {"type": "int", "default": 5},
        "waiter_timeout": {"type": "int", "default": 300},
    }
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if p.get("comment") is not None and len(p["comment"]) > 2048: module.fail_json(msg="DLC database comment must not exceed 2048 characters")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.DlcClient, "dlc.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, database=None, batch_id=None)
            if not p["allow_delete"]: module.fail_json(msg="set allow_delete=true to authorize deleting the DLC database", database=current)
            table_count = int(module.sdk_call(client.DescribeTables, table_count_request(models, p)).TotalCount or 0)
            if table_count and not p["allow_delete_nonempty"]: module.fail_json(msg="DLC database contains tables; set allow_delete_nonempty=true to authorize deletion", table_count=table_count)
            diff_value = maybe_diff(module, current, None); batch_id = None
            if not module.check_mode:
                batch_id = module.sdk_call(client.DeleteMetaDatabase, delete_request(models, p)).BatchId
                if p["wait"]: wait_database(module, client, models, p, False)
        ...
        if not current:
            ...
            if p.get("govern_policy") is not None: target["GovernPolicy"] = normalize(p["govern_policy"])
            diff_value = maybe_diff(module, None, target); batch_id = None
            if not module.check_mode:
                batch_id = module.sdk_call(client.CreateMetaDatabase, create_request(models, p)).BatchId
                if p["wait"]: wait_database(module, client, models, p, True)
                current = find(module, client, models, p)
        ...
        if drift: module.fail_json(msg="DLC database comment and governance policy are immutable; recreate the database to change them", immutable_drift=drift)
