...
RETURN = r'''
native_gateway_server_groups:
  description: Matching TSE native gateway server groups.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeNativeGatewayServerGroups, request),
        lambda response: response.Result.GatewayGroupList if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    native_gateway_server_groups = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, native_gateway_server_groups=native_gateway_server_groups,
