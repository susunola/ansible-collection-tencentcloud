...
def describe_request(models, p):
    ...
    request.InstanceIds, request.TimerAction, request.StatusList = [p["instance_id"]], "TerminateInstances", ["UNDO"]
    if p.get("timer_id"):
        request.ActionTimerIds = [p["timer_id"]]
    return request


def create_request(models, p):
    timer = models.ActionTimer()
    timer.TimerAction, timer.ActionTime = "TerminateInstances", p["action_time"]
    request = models.ImportInstancesActionTimerRequest()
    request.InstanceIds, request.ActionTimer = [p["instance_id"]], timer
    return request


def delete_request(models, timer_id):
    request = models.DeleteInstancesActionTimerRequest()
    request.ActionTimerIds = [timer_id]
    return request


def find(module, client, models, p):
    response = module.sdk_call(client.DescribeInstancesActionTimer, describe_request(models, p))
    values = [item._serialize(allow_none=True) for item in (response.ActionTimers or [])]
    if len(values) > 1:
        module.fail_json(msg="Multiple unexecuted termination timers matched; specify timer_id", timer_ids=[v.get("ActionTimerId") for v in values])
    return values[0] if values else None


def comparable(v):
    return {"InstanceId": v.get("InstanceId"), "TimerAction": v.get("TimerAction"), "ActionTime": v.get("ActionTime"), "Status": v.get("Status")}


def run_module():
    module = TencentCloudModule(
        argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "instance_id": {"required": True}, "action_time": {}, "timer_id": {}},
        required_if=[("state", "present", ["action_time"])],
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, action_timer=None)
            diff = maybe_diff(module, comparable(current), None)
            if not module.check_mode:
                module.sdk_call(client.DeleteInstancesActionTimer, delete_request(models, current["ActionTimerId"]))
            module.exit_json(changed=True, **(diff or {}), action_timer=current if module.check_mode else None)
        target = {"InstanceId": p["instance_id"], "TimerAction": "TerminateInstances", "ActionTime": p["action_time"], "Status": "UNDO"}
        before = comparable(current) if current else None
        if before == target:
            module.exit_json(changed=False, action_timer=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current:
                module.sdk_call(client.DeleteInstancesActionTimer, delete_request(models, current["ActionTimerId"]))
            ids = module.sdk_call(client.ImportInstancesActionTimer, create_request(models, p)).ActionTimerIds or []
            p["timer_id"] = ids[0] if ids else None
            current = find(module, client, models, p)
