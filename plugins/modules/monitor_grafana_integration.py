...
options:
  state:
    description:
      - C(present) creates the integration with V(CreateGrafanaIntegration) when it does not exist and updates
        it with V(UpdateGrafanaIntegration) when it differs. C(absent) deletes it with V(DeleteGrafanaIntegration).
    type: str
