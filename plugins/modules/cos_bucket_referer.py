...
def normalize(value):
    ...
    if root.get("Status") != "Enabled": return None
    domains = (root.get("DomainList") or {}).get("Domain") or []
    if isinstance(domains, str): domains = [domains]
    return {"Status": "Enabled", "RefererType": root.get("RefererType"), "EmptyReferConfiguration": root.get("EmptyReferConfiguration"), "DomainList": {"Domain": sorted(domains)}}


def get_referer(client, bucket):
    try: return normalize(client.get_bucket_referer(Bucket=bucket))
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "referer_type": {"choices": ["White-List", "Black-List"]}, "allow_empty": {"type": "bool", "default": True}, "domains": {"type": "list", "elements": "str"}}, required_if=[("state", "present", ["referer_type", "domains"])], supports_check_mode=True)
    ...
    try:
        current = get_referer(client, bucket); target = normalize({"Status": "Enabled", "RefererType": module.params.get("referer_type"), "EmptyReferConfiguration": "Allow" if module.params["allow_empty"] else "Deny", "DomainList": {"Domain": module.params.get("domains") or []}})
        if module.params["state"] == "absent": target = None
        if current == target: module.exit_json(changed=False, referer=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if target is None: client.delete_bucket_referer(Bucket=bucket)
            else: client.put_bucket_referer(Bucket=bucket, RefererConfiguration=target)
