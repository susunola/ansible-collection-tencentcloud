...
def describe_request(models,p):r=models.DescribeApplicationInfoRequest();r.ApplicationId,r.EnvironmentId,r.SourceChannel=p["application_id"],p["environment_id"],p["source_channel"];return r
def deploy_request(models,p):
    value=dict(p["configuration"]);value["ApplicationId"],value["EnvironmentId"],value["DeployVersion"],value["SourceChannel"]=p["application_id"],p["environment_id"],p["deploy_version"],p["source_channel"]
    r=models.DeployApplicationRequest();r.from_json_string(json.dumps(value));return r
...
def run_module():
    module=TencentCloudModule(argument_spec={"application_id":{"required":True},"environment_id":{"required":True},"deploy_version":{"required":True},"configuration":{"type":"dict","required":True},"source_channel":{"type":"int","default":0},"force_redeploy":{"type":"bool","default":False},"wait":{"type":"bool","default":True},"waiter_timeout":{"type":"int","default":1800}},supports_check_mode=True);p=module.params;module.require_sdk();models,cm=_load();client=module.create_client(cm.TemClient,"tem.tencentcloudapi.com")
    try:
        reserved=set(p["configuration"])&{"ApplicationId","EnvironmentId","DeployVersion","SourceChannel"}
        if reserved:module.fail_json(msg="configuration must not override deployment identity fields",fields=sorted(reserved))
        current=describe(module,client,models,p);target=dict(p["configuration"]);target["DeployVersion"]=p["deploy_version"]
        version=(current or {}).get("DeployVersion") or (current or {}).get("VersionName")
        config_match=contains(current or {},p["configuration"])
        if not p["force_redeploy"] and version==p["deploy_version"] and config_match:module.exit_json(changed=False,deployment=current,version_id=(current or {}).get("VersionId"))
        diff=maybe_diff(module,current,target);version_id=None
        if not module.check_mode:
            version_id=module.sdk_call(client.DeployApplication,deploy_request(models,p)).Result
            if p["wait"]:
                def poll():
                    value=describe(module,client,models,p) or {};active=value.get("DeployVersion") or value.get("VersionName")
            ...
            else:current=describe(module,client,models,p)
