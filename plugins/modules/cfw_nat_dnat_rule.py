...
def describe_request(models, offset=0):
    request = models.DescribeNatFwDnatRuleRequest(); request.Offset, request.Limit = offset, 100; return request
...
def create_request(models, p):
    request = models.CreateNatFwDnatRuleRequest(); request.Mode, request.CfwInstance, request.DnatRules = p["mode"], p["firewall_instance_id"], [dnat_rule(models, p)]; return request


def update_request(models, p, current):
    request = models.SetNatFwDnatRuleRequest(); request.Mode, request.OperationType, request.CfwInstance = p["mode"], "modify", p["firewall_instance_id"]
    original = models.CfwNatDnatRule(); original._deserialize({key: current.get(key) for key in ("IpProtocol", "PublicIpAddress", "PublicPort", "PrivateIpAddress", "PrivatePort", "Description")})
    request.OriginDnat, request.NewDnat = original, dnat_rule(models, p); return request


def delete_request(models, p, current):
    request = models.DeleteNatFwDnatRuleRequest(); request.Mode, request.CfwInstance = p["mode"], p["firewall_instance_id"]
    item = models.CfwNatDnatRule(); item._deserialize({key: current.get(key) for key in ("IpProtocol", "PublicIpAddress", "PublicPort", "PrivateIpAddress", "PrivatePort", "Description")}); request.DnatRules = [item]
    return request
...
def find_rule(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeNatFwDnatRule, describe_request(models, offset)); values = list(response.Data or [])
        for value in values:
            item = value._serialize(allow_none=True)
            if item.get("FwInsId") == p["firewall_instance_id"] and item.get("IpProtocol") == p["protocol"] and item.get("PublicIpAddress") == p["public_ip"] and item.get("PublicPort") == p["public_port"]: matches.append(item)
        offset += len(values)
        if offset >= int(response.Total or 0) or not values: break
    if len(matches) > 1: module.fail_json(msg="Multiple NAT DNAT rules matched the public endpoint")
    return matches[0] if matches else None


def run_module():
    module = TencentCloudModule(
        argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "firewall_instance_id": {"required": True}, "mode": {"type": "int", "choices": [0, 1], "default": 0}, "protocol": {"choices": ["TCP", "UDP"], "required": True}, "public_ip": {"required": True}, "public_port": {"type": "int", "required": True}, "private_ip": {}, "private_port": {"type": "int"}, "description": {"default": ""}}, supports_check_mode=True)
    p = module.params
    if p["state"] == "present" and (not p.get("private_ip") or p.get("private_port") is None): module.fail_json(msg="private_ip and private_port are required when state=present")
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteNatFwDnatRule, delete_request(models, p, current))
            module.exit_json(changed=True, **(diff or {}), rule=current if module.check_mode else None)
        desired = {"IpProtocol": p["protocol"], "PublicIpAddress": p["public_ip"], "PublicPort": p["public_port"], "PrivateIpAddress": p["private_ip"], "PrivatePort": p["private_port"], "Description": p["description"]}
        before = {key: current.get(key) for key in desired} if current else None
        if before == desired: module.exit_json(changed=False, rule=current)
        diff = maybe_diff(module, before, desired)
        if not module.check_mode:
            if current: module.sdk_call(client.SetNatFwDnatRule, update_request(models, p, current))
            else: module.sdk_call(client.CreateNatFwDnatRule, create_request(models, p))
            current = find_rule(module, client, models, p)
