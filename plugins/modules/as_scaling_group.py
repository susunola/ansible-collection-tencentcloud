...
options:
  state:
    description:
      - C(present) creates the scaling group with V(CreateAutoScalingGroup) when it does not exist and updates
        it with V(ModifyAutoScalingGroup) when it differs. C(absent) deletes it with V(DeleteAutoScalingGroup).
