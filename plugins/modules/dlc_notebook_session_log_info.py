...
def build_request(models, session_id, offset, limit):
    request = models.DescribeNotebookSessionLogRequest(); request.SessionId, request.Offset, request.Limit = session_id, offset, limit; return request
...
def read(module, client, models, p):
    ...
    for _ in range(p["max_pages"]):
        ...
        page = response.Logs or []; logs.extend(page); request_id = response.RequestId; offset += len(page)
        if len(page) < p["page_size"]: break
    else: truncated = True
...
def run_module():
    ...
    if not 1 <= p["page_size"] <= 1000: module.fail_json(msg="page_size must be between 1 and 1000")
