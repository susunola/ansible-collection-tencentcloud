...
RETURN = r'''
check_config_assets:
  description: Matching SSA check config assets.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.ssa.v20180608 import models, ssa_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-ssa package is required.")

    client = ssa_client.SsaClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "ssa.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.DescribeCheckConfigAssetList, request),
        lambda response: response.CheckAssetsList,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    check_config_assets = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, check_config_assets=check_config_assets, total_count=total_count)
