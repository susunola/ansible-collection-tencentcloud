...
FIELDS = {
    "name": ("Name", None), "charge_type": ("ChargeType", None), "resource_conf": ("ResourceConf", "ResourceConf"),
    "log_enable": ("LogEnable", None), "root_access": ("RootAccess", None), "auto_stopping": ("AutoStopping", None),
    "direct_internet_access": ("DirectInternetAccess", None), "resource_group_id": ("ResourceGroupId", None),
    "vpc_id": ("VpcId", None), "subnet_id": ("SubnetId", None), "volume_source_type": ("VolumeSourceType", None),
    "volume_size_gb": ("VolumeSizeInGB", None), "volume_source_cfs": ("VolumeSourceCFS", "CFSConfig"),
    "volume_source_goosefs": ("VolumeSourceGooseFS", "GooseFS"), "log_config": ("LogConfig", "LogConfig"),
    "lifecycle_script_id": ("LifecycleScriptId", None), "default_code_repo_id": ("DefaultCodeRepoId", None),
    "additional_code_repo_ids": ("AdditionalCodeRepoIds", None), "automatic_stop_time": ("AutomaticStopTime", None),
    "tags": ("Tags", "Tag"), "data_configs": ("DataConfigs", "DataConfig"), "image_info": ("ImageInfo", "ImageInfo"),
    "image_type": ("ImageType", None), "ssh_config": ("SSHConfig", "SSHConfig"), "description": ("Description", None),
}
...
def detail_request(models, notebook_id, project_id=None):
    request = models.DescribeNotebookRequest(); request.Id = notebook_id
    if project_id is not None: request.TiProjectId = project_id
    return request


def list_request(models, p, offset):
    request = models.DescribeNotebooksRequest(); request.Offset, request.Limit = offset, 200
...
def get(module, client, models, notebook_id, project_id=None):
    try: response = module.sdk_call(client.DescribeNotebook, detail_request(models, notebook_id, project_id))
...
def find(module, client, models, p):
    if p.get("notebook_id"): return get(module, client, models, p["notebook_id"], p.get("project_id"))
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeNotebooks, list_request(models, p, offset)); items = response.NotebookSet or []
        matches.extend(item.Id for item in items if item.Name == p["name"])
        offset += len(items)
        if not items or offset >= int(response.TotalCount or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple TIONE notebooks matched the exact name", name=p["name"])
...
def assign(models, request, key, value, cls_name):
    ...
    elif cls_name and value is not None: value = _model(getattr(models, cls_name), value)
...
def create_request(models, p):
    ...
    for source, (key, cls_name) in FIELDS.items():
        if p.get(source) is not None: assign(models, request, key, p[source], cls_name)
    return request


def modify_request(models, notebook_id, p):
    request = models.ModifyNotebookRequest(); request.Id = notebook_id
    for source, (key, cls_name) in FIELDS.items():
        if key == "VolumeSourceGooseFS": continue
        if p.get(source) is not None: assign(models, request, key, p[source], cls_name)
    return request


def id_request(cls, notebook_id):
    request = cls(); request.Id = notebook_id; return request


def status(value): return str((value or {}).get("Status") or "").lower()
...
def wait_status(module, client, models, p, expected):
    def poll():
        ...
        if actual in ("failed", "submitfailed"): module.fail_json(msg="TIONE notebook entered a failed state", notebook=current)
        return actual
...
def converge_stopped(module, client, models, p, current):
    actual = status(current)
    if actual == "stopped": return current
    if actual in ("failed", "submitfailed"): module.fail_json(msg="TIONE notebook is in a failed state", notebook=current)
    if actual in ("starting", "stopping"):
        if not p["wait"]: module.fail_json(msg="TIONE notebook is transitioning; enable wait before configuration or deletion", notebook=current)
        wait_status(module, client, models, p, "running" if actual == "starting" else "stopped")
        current = get(module, client, models, p["notebook_id"], p.get("project_id")); actual = status(current)
    if actual == "running":
        module.sdk_call(client.StopNotebook, id_request(models.StopNotebookRequest, p["notebook_id"]))
        if p["wait"]: wait_status(module, client, models, p, "stopped")
        current = get(module, client, models, p["notebook_id"], p.get("project_id"))
    elif actual != "stopped":
...
def run_module():
    ...
    spec.update({
        "charge_type": {"choices": ["PREPAID", "POSTPAID_BY_HOUR"]}, "resource_conf": {"type": "dict"}, "log_enable": {"type": "bool"},
        "root_access": {"type": "bool"}, "auto_stopping": {"type": "bool"}, "direct_internet_access": {"type": "bool"},
        "volume_source_type": {"choices": ["FREE", "CLOUD_PREMIUM", "CLOUD_SSD", "CFS", "CFS_TURBO", "GooseFSx"]},
        "volume_size_gb": {"type": "int"}, "volume_source_cfs": {"type": "dict"}, "volume_source_goosefs": {"type": "dict"},
        "log_config": {"type": "dict"}, "additional_code_repo_ids": {"type": "list", "elements": "str"}, "automatic_stop_time": {"type": "int"},
        "tags": {"type": "list", "elements": "dict"}, "data_configs": {"type": "list", "elements": "dict"}, "image_info": {"type": "dict"},
        "image_type": {"choices": ["SYSTEM", "TCR", "CCR"]}, "ssh_config": {"type": "dict"}, "allow_delete": {"type": "bool", "default": False},
    ...
    if p.get("additional_code_repo_ids") is not None and len(p["additional_code_repo_ids"]) > 3: module.fail_json(msg="additional_code_repo_ids accepts at most three repositories")
    if p["state"] == "absent" and not p.get("notebook_id"): module.fail_json(msg="notebook_id is required for safe deletion")
    if p["state"] != "absent" and not (p.get("name") or p.get("notebook_id")): module.fail_json(msg="name or notebook_id is required")
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TioneClient, "tione.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, notebook=None, notebook_id=p["notebook_id"])
            if not p["allow_delete"]: module.fail_json(msg="allow_delete=true is required to delete a TIONE notebook", notebook=current)
            if status(current) != "stopped" and not p["wait"]: module.fail_json(msg="wait=true is required to stop a TIONE notebook before deletion", notebook=current)
            ...
            if not module.check_mode:
                ...
                if p["wait"]: wait_status(module, client, models, p, "absent")
            module.exit_json(changed=True, **(diff_value or {}), notebook=None, notebook_id=p["notebook_id"])
        if not current:
            if p.get("notebook_id"): module.fail_json(msg="the requested TIONE notebook_id does not exist; omit it to create by name", notebook_id=p["notebook_id"])
            missing = [key for key in ("name", "charge_type", "resource_conf") if p.get(key) is None]
            if missing: module.fail_json(msg="creation parameters are required for a TIONE notebook", missing=missing)
            target = desired(p); target["Status"] = p["state"].capitalize(); diff_value = maybe_diff(module, None, target); notebook_id = None
            if not module.check_mode:
                response = module.sdk_call(client.CreateNotebook, create_request(models, p)); notebook_id = response.Id; p = dict(p, notebook_id=notebook_id)
                if p["wait"]: wait_status(module, client, models, p, "running")
                current = get(module, client, models, notebook_id, p.get("project_id"))
                if p["state"] == "stopped": current = converge_stopped(module, client, models, p, current)
            module.exit_json(changed=True, **(diff_value or {}), notebook=current if not module.check_mode else target, notebook_id=(current or {}).get("Id") or notebook_id)
        p = dict(p, notebook_id=current["Id"]); mutable, immutable = drift(p, current)
        if immutable: module.fail_json(msg="TIONE notebook immutable configuration drift requires explicit replacement", notebook=current, immutable_drift=immutable)
        actual = status(current)
        needs_state = actual != p["state"]
        if not mutable and not needs_state: module.exit_json(changed=False, notebook=current, notebook_id=current["Id"])
        target = desired(p, current); target["Status"] = p["state"].capitalize(); diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
            if mutable:
                if status(current) != "stopped" and not p["wait"]: module.fail_json(msg="wait=true is required to stop a TIONE notebook before modifying it", notebook=current)
                current = converge_stopped(module, client, models, p, current)
            ...
            if p["state"] == "running":
                ...
                if status(current) != "running":
                    ...
                    if p["wait"]: wait_status(module, client, models, p, "running")
            else: current = converge_stopped(module, client, models, p, current)
            current = get(module, client, models, p["notebook_id"], p.get("project_id"))
