...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.waf_anti_tamper_rule_info
    description: Gather information about Tencent Cloud WAF anti-tamper rules.
...
def create_request(models, p):
    ...
    request.Domain, request.Name, request.Uri = p["domain"], p["name"], p["uri"]
    return request


def update_request(models, p, rule_id):
    request = models.ModifyAntiFakeUrlRequest()
    request.Domain, request.Name, request.Uri, request.Id = p["domain"], p["name"], p["uri"], rule_id
    return request


def status_request(models, p, rule_id):
    request = models.ModifyAntiFakeUrlStatusRequest()
    request.Domain, request.Status, request.Ids = p["domain"], 1 if p["enabled"] else 0, [rule_id]
    return request


def refresh_request(models, p, rule_id):
    request = models.FreshAntiFakeUrlRequest()
    request.Domain, request.Id = p["domain"], rule_id
    return request
...
def find(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeAntiFakeRules, describe_request(models, p, offset))
        values = list(response.Data or [])
        for item in values:
            value = item._serialize(allow_none=True)
            if (p.get("rule_id") and str(value.get("Id")) == str(p["rule_id"])) or (not p.get("rule_id") and value.get("Name") == p.get("name")):
                matches.append(value)
        offset += len(values)
        if offset >= int(response.Total or 0) or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple WAF anti-tamper rules matched; specify rule_id")
    return matches[0] if matches else None


def comparable(value):
    return {"Name": value.get("Name"), "Uri": value.get("Uri"), "Status": int(value.get("Status") or 0)}


def desired(p):
    return {"Name": p["name"], "Uri": p["uri"], "Status": 1 if p["enabled"] else 0}
...
def run_module():
    ...
    p = module.params
    if p["state"] == "present" and (not p.get("name") or not p.get("uri")):
        module.fail_json(msg="name and uri are required when state=present")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteAntiFakeUrl, delete_request(models, p, int(current["Id"])))
        ...
        if before == target and not p["refresh"]:
            module.exit_json(changed=False, rule=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current:
                rule_id = int(current["Id"])
                module.sdk_call(client.ModifyAntiFakeUrl, update_request(models, p, rule_id))
            else:
                rule_id = int(module.sdk_call(client.AddAntiFakeUrl, create_request(models, p)).Id)
                p["rule_id"] = rule_id
            module.sdk_call(client.ModifyAntiFakeUrlStatus, status_request(models, p, rule_id))
            if p["refresh"]:
                module.sdk_call(client.FreshAntiFakeUrl, refresh_request(models, p, rule_id))
            current = find(module, client, models, p)
