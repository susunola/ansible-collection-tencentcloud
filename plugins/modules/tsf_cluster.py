...
def find(module,client,models,p):
    r=models.DescribeClustersRequest(); r.Offset,r.Limit=0,50
    if p.get("cluster_id"): r.ClusterIdList=[p["cluster_id"]]
    ...
    matches=[x for x in values if x.get("ClusterId")==p.get("cluster_id")] if p.get("cluster_id") else [x for x in values if x.get("ClusterName")==p["name"]]
    if len(matches)>1: module.fail_json(msg="Multiple TSF clusters matched",name=p["name"])
    return matches[0] if matches else None
def desired(p):
    mapping={"name":"ClusterName","cluster_type":"ClusterType","description":"ClusterDesc","remark_name":"ClusterRemarkName","vpc_id":"VpcId","subnet_id":"SubnetId","cluster_cidr":"ClusterCIDR","tsf_region_id":"TsfRegionId","tsf_zone_id":"TsfZoneId","cluster_version":"ClusterVersion","max_node_pods":"MaxNodePodNum","max_cluster_services":"MaxClusterServiceNum","enable_log_collection":"EnableLogCollection"}
    return {b:p[a] for a,b in mapping.items() if p.get(a) is not None}
def comparable(current,target): return {k:current.get(k) for k in target if k not in ("MaxNodePodNum","MaxClusterServiceNum")}
def run_module():
    module=TencentCloudModule(argument_spec={"state":{"choices":["present","absent"],"default":"present"},"cluster_id":{},"name":{"required":True},"cluster_type":{"choices":["V","C","S"]},"description":{},"remark_name":{},"vpc_id":{},"subnet_id":{},"cluster_cidr":{},"tsf_region_id":{},"tsf_zone_id":{},"cluster_version":{},"max_node_pods":{"type":"int"},"max_cluster_services":{"type":"int"},"enable_log_collection":{"type":"bool"},"unbind_only":{"type":"bool","default":False}},supports_check_mode=True)
    ...
    try:
        ...
        if p["state"]=="absent":
            ...
            if not module.check_mode:
                r=models.DeleteClusterRequest(); r.ClusterId=current["ClusterId"]; r.Unbind=p["unbind_only"]
                response=module.sdk_call(client.DeleteCluster,r)
                if response.Result is False: module.fail_json(msg="Tencent Cloud rejected the TSF cluster deletion",request_id=response.RequestId)
            module.exit_json(changed=True,**(diff or {}),cluster=None)
        target=desired(p)
        if not current and not p.get("cluster_type"): module.fail_json(msg="cluster_type is required when creating a TSF cluster")
        if current: require_immutable_unchanged(module,current,target,["ClusterType","VpcId","SubnetId","ClusterCIDR","TsfRegionId","TsfZoneId","ClusterVersion"],"TSF cluster")
        compare_target={k:v for k,v in target.items() if k not in ("MaxNodePodNum","MaxClusterServiceNum")}
        if current and comparable(current,target)==compare_target: module.exit_json(changed=False,cluster=current)
        diff=maybe_diff(module,comparable(current,target) if current else None,compare_target)
        if not module.check_mode:
            if current:
                r=models.ModifyClusterRequest(); r.ClusterId=current["ClusterId"]
                ...
                if response.Result is False: module.fail_json(msg="Tencent Cloud rejected the TSF cluster update",request_id=response.RequestId)
