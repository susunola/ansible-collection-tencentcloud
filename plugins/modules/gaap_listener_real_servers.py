...
def canonical(values):
    ...
    for value in values or []:
        result.append({"RealServerId": value.get("RealServerId") or value.get("real_server_id"),
...
def describe(module, client, models, listener_id):
    request = models.DescribeListenerRealServersRequest(); request.ListenerId = listener_id
...
def bind(module, client, models, listener_id, values):
    request = models.BindListenerRealServersRequest(); request.ListenerId = listener_id
    request.RealServerBindSet = []
    for value in values:
        item = models.RealServerBindSetReq(); item._deserialize(value); request.RealServerBindSet.append(item)
...
def run_module():
    module = TencentCloudModule(argument_spec={
        ...
        "real_servers": {"type": "list", "elements": "dict", "default": [], "options": {
            "real_server_id": {"required": True}, "address": {"required": True}, "port": {"type": "int", "required": True},
            "weight": {"type": "int", "default": 1}, "failover_role": {"choices": ["master", "slave"]},
        }},
    ...
    try:
        ...
        if current == wanted: module.exit_json(changed=False, real_servers=current)
