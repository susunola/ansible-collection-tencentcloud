...
RETURN = r'''
cloud_native_api_gateways:
  description: Matching TSE cloud native api gateways.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeCloudNativeAPIGateways, request),
        lambda response: response.Result.GatewayList if response.Result is not None else None,
        lambda response: response.Result.TotalCount if response.Result is not None else None,
    )
    item_set, total_count = paginator.fetch_all()
    cloud_native_api_gateways = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, cloud_native_api_gateways=cloud_native_api_gateways,
