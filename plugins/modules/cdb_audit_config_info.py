...
options:
  instance_id:
    description: ID of the CDB instance whose audit configuration is returned.
    type: str
    required: true
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cdb_audit_config
    description: Manage TencentDB for MySQL audit configuration.
...
def build_request(models, instance_id, offset, limit):
    # DescribeAuditConfig takes no request arguments and is not
...
def run_module():
    ...
    response = sdk_call(module, client.DescribeAuditConfig, request)
    audit_config = serialize_sdk_object(response)
    audit_config.pop("RequestId", None)
    module.exit_json(changed=False, audit_config=audit_config,
