...
DONE = {
    "start": {"running", "readyComplete", "success"}, "pause": {"pausing", "manualPaused"},
    "resume": {"running", "readyComplete", "success"}, "stop": {"stopping", "success"},
    "complete": {"completing", "success"},
}
VALID_FROM = {
    "start": {"checkPass", "readyRun"}, "pause": {"running", "readyComplete"},
...
def describe(module, client, models, job_id):
    request = models.DescribeMigrationDetailRequest(); request.JobId = job_id
...
def run_module():
    module = TencentCloudModule(argument_spec={
        "job_id": {"required": True}, "action": {"required": True, "choices": list(DONE)},
    ...
    try:
        ...
        if status in DONE[action]: module.exit_json(changed=False, migration_job=current)
        ...
        if status not in VALID_FROM[action]:
            module.fail_json(msg="DTS %s is not valid in state %s" % (action, status),
                             valid_states=sorted(VALID_FROM[action]), migration_job=current)
