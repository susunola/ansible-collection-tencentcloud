...
def config_request(models, resource_group_id, offset=0):
    request = models.DescribeStandardEngineResourceGroupConfigInfoRequest(); request.Offset, request.Limit = offset, 100
    item = models.Filter(); item.Name, item.Values = "engine-resource-group-id", [resource_group_id]
    request.Filters = [item]
    return request
...
def read_config(module, client, models, resource_group_id):
    ...
    while True:
        ...
        page = response.StandardEngineResourceGroupConfigInfos or []; matches.extend(page); offset += len(page)
        if not page or offset >= int(response.Total or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple DLC configuration records matched the resource-group ID", resource_group_id=resource_group_id)
    item = matches[0] if matches else None
    def pairs(value): return {x.ConfigItem: x.ConfigValue for x in (value or []) if x.ConfigItem is not None}
...
def create_request(models, p):
    ...
    def pairs(values):
        ...
        for key, value in sorted((values or {}).items()):
            item = models.EngineResourceGroupConfigPair(); item.ConfigItem, item.ConfigValue = key, str(value); result.append(item)
        return result
    if p.get("static_config") is not None: request.StaticConfigPairs = pairs(p["static_config"])
    if p.get("dynamic_config") is not None: request.DynamicConfigPairs = pairs(p["dynamic_config"])
    return request
...
def config_update_request(models, name, changes, effective_now):
    request = models.UpdateStandardEngineResourceGroupConfigInfoRequest(); request.EngineResourceGroupName = name
    request.IsEffectiveNow = _flag(effective_now); contexts = []
    for config_type, delta in (("StaticConfigType", changes.get("StaticConfig")), ("DynamicConfigType", changes.get("DynamicConfig"))):
        if not delta: continue
        context = models.UpdateConfContext(); context.ConfigType, context.Params = config_type, []
        for key, (old, new) in sorted(delta.items()):
            item = models.Param(); item.ConfigItem, item.ConfigValue = key, "" if new is None else str(new)
            item.Operate = "DELETE" if new is None else ("ADD" if old is None else "MODIFY"); context.Params.append(item)
...
def desired(p, current=None):
    ...
    if p.get("static_config") is not None: result["StaticConfig"] = {k: str(v) for k, v in p["static_config"].items()}
    if p.get("dynamic_config") is not None: result["DynamicConfig"] = {k: str(v) for k, v in p["dynamic_config"].items()}
    return result
...
def drift(p, current):
    ...
    for source, target_name in (("static_config", "StaticConfig"), ("dynamic_config", "DynamicConfig")):
        if p.get(source) is not None:
            ...
            if old != new: changes[target_name] = (old, new)
    return changes
...
def config_delta(changes):
    ...
    for target in ("StaticConfig", "DynamicConfig"):
        if target not in changes: continue
        old, new = changes[target]; keys = set(old) | set(new)
...
def run_module():
    spec = {
        ...
        "network_config_names": {"type": "list", "elements": "str"}, "static_config": {"type": "dict"}, "dynamic_config": {"type": "dict"},
    ...
    try:
        ...
        if not module.check_mode:
            ...
            if "StaticConfig" in changes or "DynamicConfig" in changes: module.sdk_call(client.UpdateStandardEngineResourceGroupConfigInfo, config_update_request(models, p["name"], config_delta(changes), p["effective_now"]))
