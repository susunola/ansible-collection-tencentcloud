...
def describe_request(models, havip_id):
    ...
    request.HaVipIds = [havip_id]
    return request


def _item(models, p):
    item = models.HaVipAssociation()
    item.HaVipId, item.InstanceId, item.InstanceType = p["havip_id"], p["instance_id"], p["instance_type"]
    return item


def associate_request(models, p):
    request = models.AssociateHaVipInstanceRequest()
    request.HaVipAssociationSet = [_item(models, p)]
    return request
...
def find(module, client, models, p):
    response = module.sdk_call(client.DescribeHaVips, describe_request(models, p["havip_id"]))
    values = list(response.HaVipSet or [])
    if not values:
        module.fail_json(msg="HAVIP was not found", havip_id=p["havip_id"])
    for item in values[0].HaVipAssociationSet or []:
        value = item._serialize(allow_none=True)
        if value.get("InstanceId") == p["instance_id"] and value.get("InstanceType") == p["instance_type"]:
            return value
    return None
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "havip_id": {"required": True},
            "instance_id": {"required": True},
            "instance_type": {"choices": ["CVM", "ENI"], "required": True},
        },
        supports_check_mode=True,
    )
    p = module.params
    target = {"HaVipId": p["havip_id"], "InstanceId": p["instance_id"], "InstanceType": p["instance_type"]}
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, association=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DisassociateHaVipInstance, disassociate_request(models, p))
            module.exit_json(changed=True, **(diff or {}), association=current if module.check_mode else None)
        if current:
            module.exit_json(changed=False, association=current)
        diff = maybe_diff(module, None, target)
        if not module.check_mode:
            module.sdk_call(client.AssociateHaVipInstance, associate_request(models, p))
            current = find(module, client, models, p)
