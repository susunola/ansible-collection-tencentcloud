...
RETURN = r'''
environment_roles:
  description: Matching TDMQ environment roles.
  returned: always
  type: list
  elements: dict
total_count:
  description: Number of environment roles reported by the API.
  returned: always
  type: int
request_id:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeEnvironmentRoles, request),
        lambda response: response.EnvironmentRoleSets,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    environment_roles = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, environment_roles=environment_roles,
