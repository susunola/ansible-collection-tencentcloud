  ...
  state:
    description:
      - C(present) creates the instance with V(CreateDBInstances) when it does not exist and updates it with
        V(ModifyDBInstanceName) when it differs. C(absent) deletes it with V(DeleteDBInstance) or V(TerminateDBInstance).
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.sqlserver_instance_info
    description: Gather information about TencentDB for SQL Server instances.
...
def describe_request(models, p, offset=0):
    ...
    r.InstanceNameSet = [p["name"]] if not p.get("instance_id") and p.get("name") else None
    return r


def create_request(models, p):
    r = models.CreateDBInstancesRequest()
    r.Zone, r.Memory, r.Storage = p["zone"], p["memory"], p["storage"]
    ...
    r.HAType, r.MultiZones, r.DrZones = p.get("ha_type"), bool(p.get("secondary_zones")), p.get("secondary_zones")
    return r
...
def resize_request(models, p, instance_id):
    r = models.UpgradeDBInstanceRequest()
    r.InstanceId, r.Memory, r.Storage, r.Cpu = instance_id, p.get("memory"), p.get("storage"), p.get("cpu")
    r.DBVersion, r.HAType, r.MultiZones, r.DrZones = (
        p.get("db_version"),
        p.get("ha_type"),
        "true" if p.get("secondary_zones") else "false",
        p.get("secondary_zones"),
    )
    return r
...
def security_groups_request(models, instance_id, groups):
    ...
    r.InstanceId, r.SecurityGroupIdSet = instance_id, groups
    return r


def renew_request(models, instance_id, enabled):
    item = models.InstanceRenewInfo()
    item.InstanceId, item.RenewFlag = instance_id, 1 if enabled else 0
    r = models.ModifyDBInstanceRenewFlagRequest()
    r.RenewFlags = [item]
    return r


def isolate_request(models, instance_id):
    r = models.TerminateDBInstanceRequest()
    r.InstanceIdSet = [instance_id]
    return r


def destroy_request(models, instance_id):
    r = models.DeleteDBInstanceRequest()
...
def find(module, client, models, p):
    ...
    def describe(filters):
        ...
        return resolver.records(response.DBInstances)

    return resolver.resolve_one(
        module, describe, resource="SQL Server instance",
        id_value=p.get("instance_id"), name_value=p.get("name"),
        id_keys=("InstanceId",), name_keys=("Name",),
    )
...
def run_module():
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if p["purge"]:
                if status not in (5, -1):
                    module.fail_json(msg="purge requires an already isolated SQL Server instance", current_status=status)
                diff = maybe_diff(module, current, None)
                if not module.check_mode:
                    module.sdk_call(client.DeleteDBInstance, destroy_request(models, instance_id))
                module.exit_json(changed=True, **(diff or {}), instance=None)
            if status in (5, -1):
                module.exit_json(changed=False, instance=current)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.TerminateDBInstance, isolate_request(models, instance_id))
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a new SQL Server instance", missing=missing)
            target = {
                "Name": p["name"],
                "Zone": p["zone"],
                "UniqVpcId": p["vpc_id"],
                "UniqSubnetId": p["subnet_id"],
                "Memory": p["memory"],
                "Storage": p["storage"],
            ...
            if not module.check_mode:
                module.sdk_call(client.CreateDBInstances, create_request(models, p))
        ...
        immutable = {"Zone": p.get("zone"), "UniqVpcId": p.get("vpc_id"), "UniqSubnetId": p.get("subnet_id")}
        drift = {k: (current.get(k), v) for k, v in immutable.items() if v is not None and current.get(k) != v}
        if drift:
            module.fail_json(msg="SQL Server network placement is immutable", immutable_drift=drift)
        desired = {
            ...
            "Version": p.get("db_version") if p.get("db_version") is not None else current.get("Version"),
        }
        ...
        if not module.check_mode:
            if before["Name"] != desired["Name"]:
                module.sdk_call(client.ModifyDBInstanceName, rename_request(models, instance_id, desired["Name"]))
            if any(before[k] != desired[k] for k in ("Memory", "Storage", "Cpu", "Version")):
                module.sdk_call(client.UpgradeDBInstance, resize_request(models, p, instance_id))
