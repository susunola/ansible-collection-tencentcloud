    ...
    for value in values:
        ...
        for key, field_value in value.items():
            if field_value is not None:
                setattr(item, field_map[key], field_value)
        result.append(item)
    return result
...
def create_request(models, p):
    ...
    return _request(
        ...
        Domains=p["domains"],
        PrivilegesList=_objects(models, "ProductPrivilegeUnit", p["privileges"]),
        Description=p["description"],
        KmsKeyId=p.get("kms_key_id"),
        Tags=_objects(models, "Tag", tags),
        RotationBeginTime=p.get("rotation_begin_time"),
        EnableRotation=p["rotation_enabled"],
        RotationFrequency=p["rotation_frequency"],
        KmsHsmClusterId=p.get("kms_hsm_cluster_id"),
        AccountRemark=p.get("account_remark"),
...
def wait_task(module, client, models, flow_id):
    if flow_id is None:
        module.fail_json(msg="SSM mutation returned no FlowID; asynchronous completion cannot be verified")
    ...
    return wait_for_task(
        module, poll, timeout=module.params["waiter_timeout"], delay=module.params["waiter_delay"], success_statuses=(1,), failure_statuses=(2,)
    )
...
def comparable(value):
    return {
        ...
        "ResourceID": value.get("ResourceID"),
        "Enabled": value.get("Status") == "Enabled",
        "RotationStatus": int(value.get("RotationStatus") or 0),
        "RotationFrequency": int(value.get("RotationFrequency") or 0),
    }


def run_module():
    privilege = {
        key: {"type": "list", "elements": "str"} if key == "privileges" else {"type": "str"}
        for key in (
            "privilege_name",
            "privileges",
            "database",
    ...
    module = TencentCloudModule(
        argument_spec={
            ...
            "privileges": {"type": "list", "elements": "dict", "options": privilege, "default": []},
            ...
            "rotation_enabled": {"type": "bool", "default": False},
            "rotation_frequency": {"type": "int", "default": 30},
            "rotation_begin_time": {},
            "account_remark": {},
    ...
    p = module.params
    if p["state"] == "present" and any(not p.get(key) for key in ("product_name", "instance_id", "username_prefix")):
        module.fail_json(msg="product_name, instance_id and username_prefix are required when state=present")
    if p.get("username_prefix") and len(p["username_prefix"]) > 8:
        module.fail_json(msg="username_prefix must be at most eight characters")
    if not 0 <= p["recovery_window_days"] <= 30:
        module.fail_json(msg="recovery_window_days must be between 0 and 30")
    if p["rotation_enabled"] and not p.get("rotation_begin_time"):
        module.fail_json(msg="rotation_begin_time is required when rotation_enabled=true")
    if p["rotation_enabled"] and not 30 <= p["rotation_frequency"] <= 365:
        module.fail_json(msg="rotation_frequency must be between 30 and 365 days")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(
                    client.DeleteSecret, _request(models, "DeleteSecretRequest", SecretName=p["secret_name"], RecoveryWindowInDays=p["recovery_window_days"])
                )
                current = find(module, client, models, p["secret_name"])
        ...
        if restored and not module.check_mode:
            module.sdk_call(client.RestoreSecret, _request(models, "RestoreSecretRequest", SecretName=p["secret_name"]))
            current = find(module, client, models, p["secret_name"])
        if current:
            before = comparable(current)
            if before["ProductName"] != p["product_name"] or before["ResourceID"] != p["instance_id"]:
                module.fail_json(msg="product_name and instance_id are immutable after creation", current=before)
            target = dict(before)
            target.update(
                {
                    "Description": p["description"],
                    "Enabled": p["enabled"],
                    "RotationStatus": int(p["rotation_enabled"]),
                    "RotationFrequency": p["rotation_frequency"],
                }
            )
            ...
            if not module.check_mode:
                if before["Description"] != target["Description"]:
                    module.sdk_call(
                        client.UpdateDescription, _request(models, "UpdateDescriptionRequest", SecretName=p["secret_name"], Description=p["description"])
                    )
                if before["Enabled"] != target["Enabled"]:
                    module.sdk_call(
                        client.EnableSecret if p["enabled"] else client.DisableSecret,
                        _request(models, "EnableSecretRequest" if p["enabled"] else "DisableSecretRequest", SecretName=p["secret_name"]),
                    )
                if before["RotationStatus"] != target["RotationStatus"] or before["RotationFrequency"] != target["RotationFrequency"]:
                    module.sdk_call(
                        client.UpdateRotationStatus,
                        _request(
                            models,
                            "UpdateRotationStatusRequest",
                            SecretName=p["secret_name"],
                            EnableRotation=p["rotation_enabled"],
                            Frequency=p["rotation_frequency"],
                            RotationBeginTime=p.get("rotation_begin_time"),
                        ),
                    )
                current = find(module, client, models, p["secret_name"])
        ...
        if not module.check_mode:
            response = module.sdk_call(client.CreateProductSecret, create_request(models, p))
            wait_task(module, client, models, response.FlowID)
            current = find(module, client, models, p["secret_name"])
            if not p["enabled"]:
                module.sdk_call(client.DisableSecret, _request(models, "DisableSecretRequest", SecretName=p["secret_name"]))
                current = find(module, client, models, p["secret_name"])
        module.exit_json(changed=True, **(diff or {}), secret=current if not module.check_mode else target)
    except Exception as exc:
