...
options:
  state:
    description:
      - C(present) creates the channel with V(CreateGrafanaNotificationChannel) when it does not exist and updates
        it with V(UpdateGrafanaNotificationChannel) when it differs. C(absent) deletes it with V(DeleteGrafanaNotificationChannel).
    type: str
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.monitor_grafana_notification_channel_info
    description: Gather information about Tencent Cloud Monitor Grafana notification channels.
...
RETURN = r"""channel:
  description:
    - Grafana notification-channel metadata.
  returned: always
  type: dict
  sample:
    # shape captured from this module's unit tests -- scripts/add_return_samples.py
    ChannelId: channel-new-001
    ChannelName: metrics-alerts
    Receivers:
      - notice-1
      - notice-2
    OrganizationIds:
