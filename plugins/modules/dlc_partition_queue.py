...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.dlc_partition_queue_info
    description: Gather information about Tencent Cloud DLC partition queues.
...
def normalize(value):
    ...
    result["ResourceUsage"] = _usage(result.get("ResourceUsage"))
    return result
...
def find(module, client, models, partition_code, name):
    ...
    while True:
        ...
        if not items or page * 200 >= int(response.Total or 0):
            break
        page += 1
    if len(matches) > 1:
        module.fail_json(msg="Multiple DLC partition queues matched the exact name", partition_code=partition_code, name=name)
...
def payload(p, queue_id=None):
    ...
    if p.get("queue_type") is not None:
        result["QueueType"] = p["queue_type"]
    if p.get("resource_usages") is not None:
        result["ResourceUsages"] = _usage(p["resource_usages"])
    return result
...
def make_request(models, p, update=False, queue_id=None):
    ...
    request.from_json_string(json.dumps(payload(p, queue_id)))
    return request
...
def desired(p, current=None):
    ...
    result["QueueName"] = p["name"]
    if p.get("description") is not None:
        result["Description"] = p["description"]
    if p.get("queue_type") is not None:
        result["QueueType"] = p["queue_type"]
    if p.get("resource_usages") is not None:
        result["ResourceUsage"] = _usage(p["resource_usages"])
    return result
...
def run_module():
    ...
    spec = {
        ...
        "partition_code": {"required": True},
        "name": {"required": True},
        "queue_type": {"type": "int", "choices": [1, 2]},
        "description": {},
        ...
        "allow_delete_default": {"type": "bool", "default": False},
    ...
    for item in p.get("resource_usages") or []:
        if item["min"] > item["max"]:
            module.fail_json(msg="resource usage min must not exceed max", resource_usage=item)
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, queue=None, queue_id=None)
            if not p["allow_delete"]:
                module.fail_json(msg="set allow_delete=true to authorize deleting the DLC partition queue", queue=current)
            if int(current.get("IsDefault") or 0) and not p["allow_delete_default"]:
                module.fail_json(msg="set allow_delete_default=true to authorize deleting a default DLC partition queue", queue=current)
            ...
            if not module.check_mode:
                ...
                if p["wait"]:
                    wait_queue(module, client, models, p, absent=True)
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a DLC partition queue", missing=missing)
            ...
            if not module.check_mode:
                ...
                if p["wait"]:
                    wait_queue(module, client, models, p, expected={k: v for k, v in after.items() if k != "QueueName"})
                current = find(module, client, models, p["partition_code"], p["name"])
            module.exit_json(
                changed=True, **(diff_value or {}), queue=current if not module.check_mode else after, queue_id=(current or {}).get("Id") or queue_id
            )
        ...
        if not changes:
            module.exit_json(changed=False, queue=current, queue_id=current.get("Id"))
        ...
        if not module.check_mode:
            ...
            if p["wait"]:
                wait_queue(module, client, models, p, expected={k: v[1] for k, v in changes.items()})
            current = find(module, client, models, p["partition_code"], p["name"])
