...
def describe_request(models, p, offset=0):
    r = models.DescribeRabbitMQVipInstancesRequest()
    r.Offset, r.Limit = offset, 100
    if p.get("instance_id") or p.get("name"):
        f = models.Filter()
        f.Name, f.Values = ("instanceId", [p["instance_id"]]) if p.get("instance_id") else ("instanceName", [p["name"]])
        r.Filters = [f]
    return r
...
def create_request(models, p):
    r = models.CreateRabbitMQVipInstanceRequest()
    r.ZoneIds, r.VpcId, r.SubnetId, r.ClusterName = p["zone_ids"], p["vpc_id"], p["subnet_id"], p["name"]
    r.NodeSpec, r.NodeNum, r.StorageSize, r.ClusterVersion = (
        p["node_spec"],
        p.get("node_count") or (3 if len(p["zone_ids"]) > 1 else 1),
        p["storage_size"],
        p.get("cluster_version") or "3.11.8",
    )
    r.EnableCreateDefaultHaMirrorQueue, r.AutoRenewFlag, r.TimeSpan, r.PayMode = (
        p["default_ha_mirror_queue"],
        p["auto_renew"],
        p["period_months"],
        p["pay_mode"],
    )
    r.Bandwidth, r.EnablePublicAccess = p.get("bandwidth"), p["public_access"]
    r.EnableDeletionProtection = bool(p.get("deletion_protection"))
    r.IsIntl, r.ResourceTags = p["international"], _tags(models, p.get("tags"))
    return r


def modify_request(models, p, instance_id):
    r = models.ModifyRabbitMQVipInstanceRequest()
    r.InstanceId, r.ClusterName, r.Remark = instance_id, p.get("name"), p.get("remark")
    r.EnableDeletionProtection = p.get("deletion_protection")
    if p.get("tags") is not None:
        r.Tags, r.RemoveAllTags = _tags(models, p["tags"]), not bool(p["tags"])
    return r


def delete_request(models, instance_id, international=False):
    r = models.DeleteRabbitMQVipInstanceRequest()
    r.InstanceId, r.IsIntl = instance_id, international
    return r


def find(module, client, models, p):
    response = module.sdk_call(client.DescribeRabbitMQVipInstances, describe_request(models, p))
    matches = []
    ...
    if len(matches) > 1:
        module.fail_json(msg="Multiple RabbitMQ instances matched; specify instance_id")
...
def _tag_map(current):
    return {x.get("TagKey"): x.get("TagValue") for x in current.get("Tags") or []}
...
def run_module():
    spec = {
        ...
        "vpc_id": {},
        "subnet_id": {},
        "node_spec": {"default": "rabbit-vip-basic-1"},
        "node_count": {"type": "int"},
        "storage_size": {"type": "int", "default": 200},
        "cluster_version": {"choices": ["3.8.30", "3.11.8", "3.13.7"]},
        "pay_mode": {"type": "int", "choices": [0, 1], "default": 0},
        "period_months": {"type": "int", "default": 1},
        "auto_renew": {"type": "bool", "default": True},
        "default_ha_mirror_queue": {"type": "bool", "default": False},
        "bandwidth": {"type": "int"},
        "public_access": {"type": "bool", "default": False},
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if current.get("EnableDeletionProtection") and p.get("deletion_protection") is not False:
                module.fail_json(msg="set deletion_protection=false to authorize disabling protection before deletion")
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                if current.get("EnableDeletionProtection"):
                    module.sdk_call(client.ModifyRabbitMQVipInstance, modify_request(models, p, current["InstanceId"]))
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a new RabbitMQ instance", missing=missing)
            target = {
                "InstanceName": p["name"],
                "InstanceVersion": p.get("cluster_version") or "3.11.8",
                "NodeCount": p.get("node_count") or (3 if len(p["zone_ids"]) > 1 else 1),
                "MaxStorage": p["storage_size"],
                "EnableDeletionProtection": bool(p.get("deletion_protection")),
            }
            diff = maybe_diff(module, None, target)
            if not module.check_mode:
                p["instance_id"] = module.sdk_call(client.CreateRabbitMQVipInstance, create_request(models, p)).InstanceId
        ...
        vpcs = current.get("Vpcs") or []
        actual_vpcs = {(x.get("VpcId"), x.get("SubnetId")) for x in vpcs}
        drift = {}
        if p.get("vpc_id") and (p["vpc_id"], p.get("subnet_id")) not in actual_vpcs:
            drift["Vpc"] = (sorted(actual_vpcs), (p["vpc_id"], p.get("subnet_id")))
        if p.get("cluster_version") and current.get("InstanceVersion") != p["cluster_version"]:
            drift["InstanceVersion"] = (current.get("InstanceVersion"), p["cluster_version"])
        if p.get("node_count") is not None and current.get("NodeCount") != p["node_count"]:
            drift["NodeCount"] = (current.get("NodeCount"), p["node_count"])
        if drift:
            module.fail_json(msg="RabbitMQ network, version and node topology are immutable", immutable_drift=drift)
        desired = {
            ...
            "EnableDeletionProtection": p.get("deletion_protection") if p.get("deletion_protection") is not None else current.get("EnableDeletionProtection"),
            "Tags": p.get("tags") if p.get("tags") is not None else _tag_map(current),
        }
        before = {
            "InstanceName": current.get("InstanceName"),
            "Remark": current.get("Remark"),
            "EnableDeletionProtection": current.get("EnableDeletionProtection"),
            "Tags": _tag_map(current),
        }
        if before == desired:
