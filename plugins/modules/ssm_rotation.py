...
def describe_request(models, secret_name):
    request = models.DescribeRotationDetailRequest(); request.SecretName = secret_name; return request
def update_request(models, p):
    request = models.UpdateRotationStatusRequest(); request.SecretName, request.EnableRotation, request.Frequency = p["secret_name"], p["enabled"], p["frequency"]
    if p.get("begin_time"): request.RotationBeginTime = p["begin_time"]
    return request
def comparable(response): return {"EnableRotation": bool(response.EnableRotation), "Frequency": int(response.Frequency or 0)}
def result(response):
    value = comparable(response); value["LatestRotateTime"], value["NextRotateBeginTime"] = response.LatestRotateTime, response.NextRotateBeginTime; return value
...
def run_module():
    ...
    try:
        ...
        if current == target: module.exit_json(changed=False, rotation=result(response))
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            module.sdk_call(client.UpdateRotationStatus, update_request(models, p)); response = module.sdk_call(client.DescribeRotationDetail, describe_request(models, p["secret_name"])); current = result(response)
