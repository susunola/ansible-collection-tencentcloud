...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.tse_sre_instance_info
    description: Gather information about Tencent Cloud TSE sre instances.
...
def _wait(module, client, models, p, states=None, expected=None, absent=False):
    ...
    while True:
        ...
        if not absent and value is not None and (not states or status in states) and matches:
            return value
        if status in ("failed", "create_failed", "update_failed", "delete_failed"):
            module.fail_json(msg="TSE engine operation failed", instance=value)
        if time.time() >= deadline:
            module.fail_json(msg="Timed out waiting for TSE engine convergence", instance=value, expected=expected)
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                ...
                _wait(module, client, models, p, absent=True)
        ...
        if not current:
            ...
            if not module.check_mode:
                ...
                current = _wait(module, client, models, p, ["running"])
            else:
                current = target
            changed = True
        else:
        ...
        if p.get("internet_access") is not None and bool(current.get("EnableInternet")) != p["internet_access"]:
            ...
            if not module.check_mode:
                ...
                p["instance_id"] = current["InstanceId"]
                current = _wait(module, client, models, p, ["running"], {"EnableInternet": p["internet_access"]})
