...
def describe_request(models, p, offset=0):
    r = models.DescribeSREInstancesRequest(); r.Offset, r.Limit = offset, 100
    if p.get("instance_id") or p.get("name"):
        f = models.Filter(); f.Name, f.Values = ("InstanceId", [p["instance_id"]]) if p.get("instance_id") else ("Name", [p["name"]]); r.Filters = [f]
    return r
def _tags(models, values):
    result = []
    for key, value in sorted((values or {}).items()):
        item = models.InstanceTagInfo(); item.TagKey, item.TagValue = key, value; result.append(item)
    return result
def _apollo(models, values):
    result = []
    for value in values or []:
        item = models.ApolloEnvParam(); item.Name, item.EngineResourceSpec, item.EngineNodeNum = value["name"], value["resource_spec"], value["node_count"]
        item.StorageCapacity, item.VpcId, item.SubnetId, item.EnvDesc = value["storage_capacity"], value["vpc_id"], value["subnet_id"], value.get("description"); result.append(item)
    return result
def create_request(models, p):
    r = models.CreateEngineRequest(); r.EngineType, r.EngineVersion, r.EngineProductVersion = p["engine_type"], p["engine_version"], p["product_version"]
    r.EngineRegion, r.EngineName, r.TradeType = p["region"], p["name"], 0; r.EngineResourceSpec, r.EngineNodeNum = p.get("resource_spec"), p.get("node_count")
    r.VpcId, r.SubnetId, r.ZoneIds = p.get("vpc_id"), p.get("subnet_id"), p.get("zone_ids"); r.StorageType, r.StorageCapacity, r.StorageOption = p.get("storage_type"), p.get("storage_capacity"), p.get("storage_option")
    if p.get("admin_name") or p.get("admin_password") or p.get("admin_token"):
        admin = models.EngineAdmin(); admin.Name, admin.Password, admin.Token = p.get("admin_name"), p.get("admin_password"), p.get("admin_token"); r.EngineAdmin = admin
    r.ApolloEnvParams, r.EngineTags = _apollo(models, p.get("apollo_environments")), _tags(models, p.get("tags")); return r
def internet_request(models, instance_id, engine_type, enabled):
    r = models.UpdateEngineInternetAccessRequest(); r.InstanceId, r.EngineType, r.EnableClientInternetAccess = instance_id, engine_type, enabled; return r
def delete_request(models, instance_id):
    r = models.DeleteEngineRequest(); r.InstanceId = instance_id; return r
def find(module, client, models, p):
    response = module.sdk_call(client.DescribeSREInstances, describe_request(models, p)); matches = []
...
def run_module():
    env = {"type": "list", "elements": "dict", "options": {"name": {"required": True}, "resource_spec": {"required": True}, "node_count": {"type": "int", "required": True}, "storage_capacity": {"type": "int", "required": True}, "vpc_id": {"required": True}, "subnet_id": {"required": True}, "description": {}}}
    spec = {"state": {"choices": ["present", "absent"], "default": "present"}, "instance_id": {}, "name": {}, "engine_type": {"choices": ["zookeeper", "nacos", "consul", "apollo", "eureka", "polarismesh"]}, "engine_version": {}, "product_version": {"choices": ["STANDARD", "PROFESSIONAL"]}, "resource_spec": {}, "node_count": {"type": "int"}, "vpc_id": {}, "subnet_id": {}, "zone_ids": {"type": "list", "elements": "int"}, "storage_type": {}, "storage_capacity": {"type": "int"}, "storage_option": {"type": "list", "elements": "int"}, "admin_name": {}, "admin_password": {"no_log": True}, "admin_token": {"no_log": True}, "apollo_environments": env, "tags": {"type": "dict"}, "internet_access": {"type": "bool"}}
    module = TencentCloudModule(argument_spec=spec, required_one_of=[("instance_id", "name")], supports_check_mode=True); p = module.params; module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TseClient, "tse.tencentcloudapi.com")
    try:
        ...
        immutable = {"Name": p.get("name"), "Type": p.get("engine_type"), "Edition": p.get("product_version"), "SpecId": p.get("resource_spec"), "Replica": p.get("node_count"), "VpcId": p.get("vpc_id"), "StorageType": p.get("storage_type"), "StorageCapacity": p.get("storage_capacity")}; drift = {k: (current.get(k), v) for k, v in immutable.items() if v is not None and current.get(k) != v}
