...
def describe_request(models, name):
    request = models.DescribeBackupStorageLocationsRequest(); request.Names = [name]; return request
...
def delete_request(models, name):
    request = models.DeleteBackupStorageLocationRequest(); request.Name = name; return request
...
def find(module, client, models, name):
    ...
    for item in response.BackupStorageLocationSet or []:
        ...
        if value.get("Name") == name: return value
    return None


def comparable(value):
    return {"Name": value.get("Name"), "StorageRegion": value.get("StorageRegion"), "Bucket": value.get("Bucket"), "Provider": value.get("Provider") or "tencentcloud", "Path": value.get("Path") or ""}
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "storage_region": {}, "bucket": {}, "provider": {"default": "tencentcloud"}, "path": {"default": ""}, "force_replace": {"type": "bool", "default": False}}, required_if=[("state", "present", ["storage_region", "bucket"])], supports_check_mode=True)
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, backup_storage_location=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteBackupStorageLocation, delete_request(models, p["name"]))
        ...
        if before == target: module.exit_json(changed=False, backup_storage_location=current)
        diff = maybe_diff(module, before, target)
        if current and not p["force_replace"]: module.fail_json(msg="TKE backup storage location configuration is immutable; set force_replace=true to recreate it", current=before, desired=target)
        if not module.check_mode:
            if current: module.sdk_call(client.DeleteBackupStorageLocation, delete_request(models, p["name"]))
            module.sdk_call(client.CreateBackupStorageLocation, create_request(models, p)); current = find(module, client, models, p["name"])
