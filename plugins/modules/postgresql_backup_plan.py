...
def build_describe(models, instance_id):
    request = models.DescribeBackupPlansRequest(); request.DBInstanceId = instance_id; return request
...
def build_create(models, p): return apply_request(models.CreateBackupPlanRequest(), p)
def build_update(models, p, plan_id): return apply_request(models.ModifyBackupPlanRequest(), p, plan_id)
def build_delete(models, instance_id, plan_id):
    request = models.DeleteBackupPlanRequest(); request.DBInstanceId, request.PlanId = instance_id, plan_id; return request


def find(module, client, models, p):
    response = module.sdk_call(client.DescribeBackupPlans, build_describe(models, p["instance_id"]))
    matches = [x._serialize(allow_none=True) for x in list(response.Plans or []) if (p.get("plan_id") and x.PlanId == p["plan_id"]) or (not p.get("plan_id") and x.PlanName == p.get("name"))]
    return matches[0] if matches else None


def desired(p):
    return {"PlanName": p["name"], "BackupPeriodType": p["period_type"], "BackupPeriod": sorted(p["periods"]), "MinBackupStartTime": p["min_start_time"], "MaxBackupStartTime": p["max_start_time"], "BaseBackupRetentionPeriod": p["retention_days"], "LogBackupRetentionPeriod": p.get("log_retention_days")}


def comparable(value):
    result = {k: value.get(k) for k in ("PlanName", "BackupPeriodType", "BackupPeriod", "MinBackupStartTime", "MaxBackupStartTime", "BaseBackupRetentionPeriod", "LogBackupRetentionPeriod")}
...
def run_module():
    module = TencentCloudModule(argument_spec={
        "state": {"choices": ["present", "absent"], "default": "present"}, "instance_id": {"required": True}, "plan_id": {}, "name": {},
        "period_type": {"default": "week"}, "periods": {"type": "list", "elements": "str", "default": []}, "min_start_time": {},
        "max_start_time": {}, "retention_days": {"type": "int"}, "log_retention_days": {"type": "int"},
    }, required_one_of=[("plan_id", "name")], required_if=[("state", "present", ("name", "min_start_time", "max_start_time", "retention_days"))], supports_check_mode=True)
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, backup_plan=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteBackupPlan, build_delete(models, p["instance_id"], current["PlanId"]))
        ...
        if before == target: module.exit_json(changed=False, backup_plan=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current:
                require_immutable_unchanged(module, before, target, ("BackupPeriodType",), "PostgreSQL backup plan")
                module.sdk_call(client.ModifyBackupPlan, build_update(models, p, current["PlanId"])); p["plan_id"] = current["PlanId"]
            else:
