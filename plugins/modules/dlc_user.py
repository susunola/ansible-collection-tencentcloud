...
def find(module, client, models, p):
    offset, matches = 0, []
    while True:
        response = module.sdk_call(client.DescribeUsers, describe_request(models, p, offset))
        ...
        if not page or offset >= int(response.TotalCount or 0):
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple DLC users matched the exact user_id", user_id=p["user_id"])
...
def create_request(models, p):
    payload = {
        "UserId": p["user_id"],
        "UserDescription": p.get("description"),
        "UserType": p.get("user_type") or "COMMON",
        "UserAlias": p.get("alias"),
        "AccountType": p["principal_type"],
        "PolicySet": p.get("initial_policies"),
        "WorkGroupIds": p.get("initial_work_group_ids"),
    }
...
def modify_request(models, p):
    ...
    request.UserId, request.UserDescription = p["user_id"], p["description"]
    request.AccountType = p["account_source"]
    return request


def type_request(models, p):
    request = models.ModifyUserTypeRequest()
    request.UserId, request.UserType = p["user_id"], p["user_type"]
    request.AccountType = p["account_source"]
    return request
...
def delete_request(models, p):
    ...
    request.UserIds, request.AccountType = [p["user_id"]], p["account_source"]
    return request
...
def wait_user(module, client, models, p, present, desired=None):
    def poll():
        ...
        if current is None:
            return "absent"
        if desired and any((current.get(key) or "") != value for key, value in desired.items()):
            return "pending"
        return "present"
...
def run_module():
    spec = {
        ...
        "description": {},
        "user_type": {"choices": ["ADMIN", "COMMON"]},
        "alias": {},
        ...
        "initial_work_group_ids": {"type": "list", "elements": "int"},
        "allow_delete_bound": {"type": "bool", "default": False},
    ...
    p = module.params
    if p.get("alias") is not None and len(p["alias"]) >= 50:
        module.fail_json(msg="DLC user alias must be shorter than 50 characters")
    if p.get("user_type") == "ADMIN" and (p.get("initial_policies") or p.get("initial_work_group_ids")):
        module.fail_json(msg="ADMIN users cannot declare initial policies or work groups")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, user=None, user_id=p["user_id"])
            if current.get("IsOwner"):
                module.fail_json(msg="The DLC owner account cannot be deleted", user=current)
            if not p["allow_delete"]:
                module.fail_json(msg="set allow_delete=true to authorize deleting the DLC user", user=current)
            if (current.get("PolicySet") or current.get("WorkGroupSet")) and not p["allow_delete_bound"]:
                module.fail_json(msg="DLC user still has policies or work groups; set allow_delete_bound=true to authorize deletion", user=current)
            diff_value = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteUser, delete_request(models, p))
                if p["wait"]:
                    wait_user(module, client, models, p, False)
            module.exit_json(changed=True, **(diff_value or {}), user=None, user_id=p["user_id"])
        if not current:
            target = {
                "UserId": p["user_id"],
                "UserDescription": p.get("description") or "",
                "UserType": p.get("user_type") or "COMMON",
                "UserAlias": p.get("alias"),
                "AccountType": p["principal_type"],
            }
            ...
            if not module.check_mode:
                ...
                if p["wait"]:
                    wait_user(module, client, models, p, True)
                current = find(module, client, models, p)
        ...
        if drift:
            module.fail_json(msg="DLC user alias and account type are immutable", immutable_drift=drift)
        changes = {}
        if p.get("description") is not None and (current.get("UserDescription") or "") != p["description"]:
            changes["UserDescription"] = ((current.get("UserDescription") or ""), p["description"])
        if p.get("user_type") is not None and current.get("UserType") != p["user_type"]:
            changes["UserType"] = (current.get("UserType"), p["user_type"])
        if not changes:
            module.exit_json(changed=False, user=current, user_id=p["user_id"])
        after = dict(current)
        after.update({key: value[1] for key, value in changes.items()})
        diff_value = maybe_diff(module, current, after)
        if not module.check_mode:
            if "UserDescription" in changes:
                module.sdk_call(client.ModifyUser, modify_request(models, p))
            if "UserType" in changes:
                module.sdk_call(client.ModifyUserType, type_request(models, p))
            if p["wait"]:
                wait_user(module, client, models, p, True, {key: value[1] for key, value in changes.items()})
            current = find(module, client, models, p)
