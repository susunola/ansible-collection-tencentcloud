...
def normalize(value):
    ...
    return {
        "backup_method": value.get("BackupMethod"),
        "backup_hour": value.get("BackupTime"),
        "frequency_hours": value.get("BackupFrequency"),
        "active_weekdays": sorted(int(x) for x in weekdays.split(",") if x != ""),
        "retention_days": value.get("BackupSaveTime"),
        "oplog_retention_days": value.get("OplogExpiredDays"),
        "backup_version": value.get("BackupVersion"),
        "alert_threshold": value.get("AlertThreshold"),
    }


def desired(p):
    return {
        "backup_method": p["backup_method"],
        "backup_hour": p["backup_hour"],
        "frequency_hours": p["frequency_hours"],
        "active_weekdays": sorted(set(p["active_weekdays"])),
        "retention_days": p["retention_days"],
        "oplog_retention_days": p["oplog_retention_days"],
        "backup_version": p["backup_version"],
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "backup_method": {"type": "int", "choices": [0, 1, 3], "default": 1},
            "backup_hour": {"type": "int", "default": 2},
            "frequency_hours": {"type": "int", "choices": [12, 24], "default": 24},
            "active_weekdays": {"type": "list", "elements": "int", "default": [0, 1, 2, 3, 4, 5, 6]},
            "retention_days": {"type": "int", "default": 7},
            "oplog_retention_days": {"type": "int", "default": 7},
            "backup_version": {"type": "int", "choices": [0, 1], "default": 1},
            "alert_threshold": {"type": "int", "default": 100},
        },
        supports_check_mode=True,
    )
    p = module.params
    if any(day < 0 or day > 6 for day in p["active_weekdays"]):
        module.fail_json(msg="active_weekdays entries must be between 0 and 6")
    if p["backup_hour"] < 0 or p["backup_hour"] > 23:
    ...
    try:
        ...
        if not module.check_mode:
            module.sdk_call(client.SetBackupRules, set_request(models, p))
            current = normalize(module.sdk_call(client.DescribeBackupRules, describe_request(models, p["instance_id"])))
