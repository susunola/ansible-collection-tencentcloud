...
def _action(models, name):
    result = models.SecurityAction()
    result.Name = name
    return result


def update_request(models, p):
    managed = models.ManagedRules()
    managed.Enabled = "on" if p["enabled"] else "off"
    managed.DetectionOnly, managed.SemanticAnalysis = "on" if p["detection_only"] else "off", "on" if p["semantic_analysis"] else "off"
    auto = models.ManagedRuleAutoUpdate()
    auto.AutoUpdateToLatestVersion = "on" if p["auto_update"] else "off"
    managed.AutoUpdate = auto
    managed.ManagedRuleGroups = []
    for value in p["groups"]:
        group = models.ManagedRuleGroup()
        group.GroupId, group.SensitivityLevel = value["group_id"], value["sensitivity"]
        if value["sensitivity"] == "custom":
            group.RuleActions = []
            for override in value["rule_actions"]:
                item = models.ManagedRuleAction()
                item.RuleId, item.Action = override["rule_id"], _action(models, override["action"])
                group.RuleActions.append(item)
        else:
            group.Action = _action(models, value["action"])
        managed.ManagedRuleGroups.append(group)
    scan_value = p["frequent_scanning"]
    scan = models.FrequentScanningProtection()
    scan.Enabled = "on" if scan_value["enabled"] else "off"
        ...
        groups.append(
            {
                ...
                "sensitivity": value.get("SensitivityLevel"),
                "action": (value.get("Action") or {}).get("Name") or "Deny",
                "rule_actions": sorted(actions, key=lambda item: item["rule_id"] or ""),
            }
        )
    scan = raw.get("FrequentScanningProtection") or {}
    scan_enabled = scan.get("Enabled") == "on"
    return {
        "enabled": raw.get("Enabled") == "on",
        "detection_only": raw.get("DetectionOnly") == "on",
        "semantic_analysis": raw.get("SemanticAnalysis") == "on",
        "auto_update": (raw.get("AutoUpdate") or {}).get("AutoUpdateToLatestVersion") == "on",
        "groups": sorted(groups, key=lambda item: item["group_id"] or ""),
        "frequent_scanning": {
            "enabled": scan_enabled,
            "action": (scan.get("Action") or {}).get("Name") or "Deny",
            "count_by": scan.get("CountBy") or "http.request.ip",
            "block_threshold": scan.get("BlockThreshold") or 100,
            "counting_period": int((scan.get("CountingPeriod") or "60s")[:-1]),
            "action_duration": int((scan.get("ActionDuration") or "600s")[:-1]),
        },
    }
    ...
    groups = [
        {
            ...
            "action": item["action"],
            "rule_actions": sorted(item["rule_actions"], key=lambda rule: rule["rule_id"]),
        }
        for item in p["groups"]
    ]
    return {
        "enabled": p["enabled"],
        "detection_only": p["detection_only"],
        "semantic_analysis": p["semantic_analysis"],
        "auto_update": p["auto_update"],
        "groups": sorted(groups, key=lambda item: item["group_id"]),
        "frequent_scanning": p["frequent_scanning"],
    }
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "detection_only": {"type": "bool", "default": False},
            "semantic_analysis": {"type": "bool", "default": False},
            ...
            "groups": {
                ...
                "options": {
                    ...
                    "sensitivity": {"choices": ["loose", "normal", "strict", "extreme", "custom"], "default": "normal"},
                    "action": {"choices": ["Deny", "Monitor", "Disabled"], "default": "Deny"},
                    "rule_actions": {
                        "type": "list",
                        "elements": "dict",
                        "default": [],
                        "options": {"rule_id": {"required": True}, "action": {"choices": ["Deny", "Monitor", "Disabled"], "required": True}},
                    },
                },
            },
            "frequent_scanning": {
                ...
                "options": {
                    ...
                    "action": {"choices": ["Deny", "Monitor"], "default": "Deny"},
                    "count_by": {"choices": ["http.request.ip", "http.request.xff_header_ip"], "default": "http.request.ip"},
                    "block_threshold": {"type": "int", "default": 100},
                    "counting_period": {"type": "int", "default": 60},
                    "action_duration": {"type": "int", "default": 600},
                },
            },
        },
    ...
    if len(ids) != len(set(ids)):
        module.fail_json(msg="managed rule group IDs must be unique")
    for group in p["groups"]:
        if group["sensitivity"] == "custom" and not group["rule_actions"]:
            module.fail_json(msg="custom sensitivity requires rule_actions")
        rule_ids = [item["rule_id"] for item in group["rule_actions"]]
        if len(rule_ids) != len(set(rule_ids)):
            module.fail_json(msg="managed rule IDs must be unique within a group")
    scan = p["frequent_scanning"]
    if not 1 <= scan["block_threshold"] <= 4294967294 or not 5 <= scan["counting_period"] <= 1800 or not 60 <= scan["action_duration"] <= 86400:
        module.fail_json(msg="frequent_scanning thresholds or durations are outside supported ranges")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        raw = response.SecurityPolicy.ManagedRules._serialize(allow_none=True) if response.SecurityPolicy and response.SecurityPolicy.ManagedRules else {}
        before, target = _normalize(raw), desired(p)
        if before == target:
            module.exit_json(changed=False, managed_rules=before)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            module.sdk_call(client.ModifySecurityPolicy, update_request(models, p))
            response = module.sdk_call(client.DescribeSecurityPolicy, describe_request(models, p))
            before = _normalize(response.SecurityPolicy.ManagedRules._serialize(allow_none=True))
