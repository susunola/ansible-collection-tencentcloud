...
def find(module,client,models,p):
    r=models.DescribeMicroservicesRequest(); r.NamespaceId=p["namespace_id"]; r.Offset,r.Limit=0,50
    if p.get("microservice_id"): r.MicroserviceIdList=[p["microservice_id"]]
    else: r.MicroserviceNameList=[p["name"]]
    result=module.sdk_call(client.DescribeMicroservices,r).Result
    values=[_ser(x) for x in ((result.Content if result else None) or [])]
    matches=[x for x in values if x.get("MicroserviceId")==p.get("microservice_id")] if p.get("microservice_id") else [x for x in values if x.get("MicroserviceName")==p["name"] and x.get("NamespaceId")==p["namespace_id"]]
    if len(matches)>1: module.fail_json(msg="Multiple TSF microservices matched",namespace_id=p["namespace_id"],name=p["name"])
...
def comparable(current,target): return {k:current.get(k) for k in target if k in ("NamespaceId","MicroserviceName","MicroserviceDesc")}
def run_module():
    module=TencentCloudModule(argument_spec={"state":{"choices":["present","absent"],"default":"present"},"microservice_id":{},"namespace_id":{"required":True},"name":{"required":True},"description":{}},supports_check_mode=True)
    ...
    try:
        ...
        if p["state"]=="absent":
            if not current: module.exit_json(changed=False,microservice=None)
            diff=maybe_diff(module,current,None)
            if not module.check_mode:
                r=models.DeleteMicroserviceRequest(); r.MicroserviceId=current["MicroserviceId"]
                response=module.sdk_call(client.DeleteMicroservice,r)
                if response.Result is False: module.fail_json(msg="Tencent Cloud rejected the TSF microservice deletion",request_id=response.RequestId)
            module.exit_json(changed=True,**(diff or {}),microservice=None)
        target=desired(p)
        if current: require_immutable_unchanged(module,current,target,["NamespaceId","MicroserviceName"],"TSF microservice")
        compare_target={k:v for k,v in target.items() if k in ("NamespaceId","MicroserviceName","MicroserviceDesc")}
        if current and comparable(current,target)==compare_target: module.exit_json(changed=False,microservice=current)
        diff=maybe_diff(module,comparable(current,target) if current else None,compare_target)
        if not module.check_mode:
            if current:
                r=models.ModifyMicroserviceRequest(); r.MicroserviceId=current["MicroserviceId"]; r.MicroserviceDesc=target.get("MicroserviceDesc")
                response=module.sdk_call(client.ModifyMicroservice,r)
                if response.Result is False: module.fail_json(msg="Tencent Cloud rejected the TSF microservice update",request_id=response.RequestId)
