...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.organization_member_identity_info
    description: Gather information about Tencent Cloud ORGANIZATION member auth identities.
...
def fetch(module, client, models, member_uin):
    offset, result = 0, []
    while True:
        response = module.sdk_call(client.DescribeOrganizationMemberAuthIdentities, describe_request(models, member_uin, offset))
        items = list(response.Items or [])
        result.extend(int(item.IdentityId) for item in items)
        offset += len(items)
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "identity_ids": {"type": "list", "elements": "int", "required": True},
    ...
    try:
        ...
        requested = sorted(set(p["identity_ids"]))
        additions = sorted(set(requested) - set(current))
        removals = sorted(set(current) - set(requested)) if p["purge"] else []
        target = sorted((set(current) | set(additions)) - set(removals))
        if current == target:
            module.exit_json(changed=False, identity_ids=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if additions:
                module.sdk_call(client.CreateOrganizationMemberAuthIdentity, create_request(models, p["member_uin"], additions))
            for identity_id in removals:
                module.sdk_call(client.DeleteOrganizationMemberAuthIdentity, delete_request(models, p["member_uin"], identity_id))
            current = fetch(module, client, models, p["member_uin"])
