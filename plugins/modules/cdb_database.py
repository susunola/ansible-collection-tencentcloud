...
options:
  ...
  character_set:
    ...
    type: str
    choices: [utf8, gbk, latin1, utf8mb4]
    default: utf8mb4
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cdb_database_info
    description: Gather information about Tencent Cloud CDB databases.
...
def describe_request(models, instance_id, offset=0):
    ...
    request.InstanceId, request.Offset, request.Limit = instance_id, offset, 5000
    return request


def create_request(models, p):
    request = models.CreateDatabaseRequest()
    request.InstanceId, request.DBName, request.CharacterSetName = p["instance_id"], p["name"], p["character_set"]
    return request


def delete_request(models, p):
    request = models.DeleteDatabaseRequest()
    request.InstanceId, request.DBName = p["instance_id"], p["name"]
    return request
...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeDatabases, describe_request(models, p["instance_id"], offset))
        items = list(response.DatabaseList or [])
        for item in items:
            value = item._serialize(allow_none=True)
            if value.get("DatabaseName") == p["name"]:
                return value
        offset += len(items)
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "name": {"required": True},
            "character_set": {"choices": ["utf8", "gbk", "latin1", "utf8mb4"], "default": "utf8mb4"},
        },
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, database=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteDatabase, delete_request(models, p))
        ...
        if before == target:
            module.exit_json(changed=False, database=current)
        diff = maybe_diff(module, before, target)
        if current:
            require_immutable_unchanged(module, before, target, ("CharacterSet",), "CDB database")
        if not module.check_mode:
            module.sdk_call(client.CreateDatabase, create_request(models, p))
            current = find(module, client, models, p)
