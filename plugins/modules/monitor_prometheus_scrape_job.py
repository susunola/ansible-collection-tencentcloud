...
options:
  ...
  name:
    description:
      - Scrape job name used for discovery.
    type: str
  config:
    description:
...
def build_describe(models, p):
    ...
    request.InstanceId, request.AgentId, request.Name, request.JobIds, request.Offset, request.Limit = (
        p["instance_id"],
        p["agent_id"],
        p.get("name"),
        [p["job_id"]] if p.get("job_id") else None,
        0,
        100,
    )
    return request
...
def build_create(models, p):
    ...
    request.InstanceId, request.AgentId, request.Config = p["instance_id"], p["agent_id"], p["config"]
    return request


def build_update(models, p, job_id):
    request = models.UpdatePrometheusScrapeJobRequest()
    request.InstanceId, request.AgentId, request.JobId, request.Config = p["instance_id"], p["agent_id"], job_id, p["config"]
    return request


def build_delete(models, p, job_id):
    request = models.DeletePrometheusScrapeJobsRequest()
    request.InstanceId, request.AgentId, request.JobIds = p["instance_id"], p["agent_id"], [job_id]
    return request


def find(module, client, models, p):
    response = module.sdk_call(client.DescribePrometheusScrapeJobs, build_describe(models, p))
    matches = []
    ...
    if len(matches) > 1:
        module.fail_json(msg="Multiple Prometheus scrape jobs have the requested name", name=p.get("name"))
...
def run_module():
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, scrape_job=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeletePrometheusScrapeJobs, build_delete(models, p, current["JobId"]))
            module.exit_json(changed=True, **(diff or {}), scrape_job=current if module.check_mode else None)
        target = {"Name": p.get("name"), "Config": p["config"]}
        before = {"Name": current.get("Name"), "Config": current.get("Config")} if current else None
        if before == target:
            module.exit_json(changed=False, scrape_job=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current:
                module.sdk_call(client.UpdatePrometheusScrapeJob, build_update(models, p, current["JobId"]))
                p["job_id"] = current["JobId"]
            else:
                p["job_id"] = module.sdk_call(client.CreatePrometheusScrapeJob, build_create(models, p)).JobId
            current = find(module, client, models, p)
