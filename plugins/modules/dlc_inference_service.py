...
CREATE_FIELDS = {
    "model_uid": "ModelUid", "engine": "Engine", "replicas": "Replicas", "resource_partition_id": "ResourcePartitionId", "image": "Image",
    "model_identifier": "ModelIdentifier", "queue": "Queue", "deployment_name": "DeploymentName", "model_version": "ModelVersion",
    "head_high_availability_enabled": "HeadHighAvailabilityEnabled", "advanced_params": "AdvancedParams", "image_pull_policy": "ImagePullPolicy",
    "autoscaling_enabled": "AutoscalingEnabled", "min_replicas": "MinReplicas", "max_replicas": "MaxReplicas", "autoscaler_options": "AutoscalerOptions",
    "api_key_ids": "ApiKeyIds", "advanced_options": "AdvancedOptions", "is_custom": "IsCustom", "runtime_env": "RuntimeEnv",
}
...
def normalize(value):
    result = dict(value or {}); result["ResourceTags"] = tags(result.get("ResourceTags"))
    for key in ("ResourceConfig", "AdvancedOptions"):
        if result.get(key) is not None: result[key] = canonical(result[key])
    return result


def list_request(models, page=1):
    request = models.ListInferenceServicesRequest(); request.Page, request.PageSize = page, 200; return request


def get_request(models, service_id):
    request = models.GetInferenceServiceRequest(); request.ServiceId = service_id; return request


def find(module, client, models, name):
    page, matches = 1, []
    while True:
        response = module.sdk_call(client.ListInferenceServices, list_request(models, page)); items = response.Items or []
        ...
        page += 1
    if len(matches) > 1: module.fail_json(msg="Multiple DLC inference services matched the exact name", name=name)
    if not matches: return None
    response = module.sdk_call(client.GetInferenceService, get_request(models, matches[0]["ServiceId"])); return normalize(response._serialize(allow_none=True))
...
def readable_conflict(p, current):
    ...
    for source, key in (("model_uid", "ModelUid"), ("model_version", "ModelVersion"), ("model_identifier", "ModelIdentifier"), ("is_custom", "IsCustom")):
        if p.get(source) is not None and current.get(key) != p[source]: changes[key] = (current.get(key), p[source])
    if p.get("resource_tags") is not None and current.get("ResourceTags") != tags(p["resource_tags"]): changes["ResourceTags"] = (current.get("ResourceTags"), tags(p["resource_tags"]))
    return changes


def create_request(models, p):
    request = models.CreateInferenceServiceRequest(); request.Name = p["name"]
    for source, target in CREATE_FIELDS.items():
        if p.get(source) is not None: setattr(request, target, canonical(p[source]) if source in JSON_FIELDS else p[source])
    if p.get("resource_tags") is not None:
        request.ResourceTags = []
        for value in tags(p["resource_tags"]):
            item = models.Tag(); item.from_json_string(json.dumps(value)); request.ResourceTags.append(item)
    return request


def state_request(cls, service_id):
    request = cls(); request.ServiceId = service_id; return request
...
def wait_service(module, client, models, p, expected):
    def poll():
        ...
        if status in ("failed", "error"): module.fail_json(msg="DLC inference service entered a failed state", inference_service=current)
        return status
...
def run_module():
    ...
    for key in CREATE_FIELDS: spec[key] = {}
    spec.update({
        "engine": {"choices": ["vllm", "xgboost"]}, "replicas": {"type": "int"}, "head_high_availability_enabled": {"type": "bool"},
        "image_pull_policy": {"choices": ["Always", "IfNotPresent", "Never"]}, "autoscaling_enabled": {"type": "bool"},
        "min_replicas": {"type": "int"}, "max_replicas": {"type": "int"}, "api_key_ids": {"type": "list", "elements": "str"},
        "is_custom": {"type": "bool"}, "resource_tags": {"type": "list", "elements": "dict", "options": tag},
    ...
    if p.get("min_replicas") is not None and p.get("max_replicas") is not None and p["min_replicas"] > p["max_replicas"]: module.fail_json(msg="min_replicas must not exceed max_replicas")
    ...
    try:
        ...
        if not current:
            ...
            if missing: module.fail_json(msg="creation parameters are required for a DLC inference service", missing=missing)
            if p["state"] == "stopped" and not p["wait"]: module.fail_json(msg="wait=true is required when creating an inference service directly into stopped state")
            target = {"Name": p["name"], "Status": p["state"].capitalize()}; diff_value = maybe_diff(module, None, target); service_id = None
            if not module.check_mode:
                response = module.sdk_call(client.CreateInferenceService, create_request(models, p)); service_id = response.ServiceId
                if p["wait"]: wait_service(module, client, models, p, "running")
                current = find(module, client, models, p["name"])
                if p["state"] == "stopped":
                    module.sdk_call(client.StopInferenceService, state_request(models.StopInferenceServiceRequest, service_id))
                    if p["wait"]: wait_service(module, client, models, p, "stopped")
                    current = find(module, client, models, p["name"])
            module.exit_json(changed=True, **(diff_value or {}), inference_service=current if not module.check_mode else target, service_id=(current or {}).get("ServiceId") or service_id)
        fixed = readable_conflict(p, current)
        if fixed: module.fail_json(msg="DLC inference-service creation fields are immutable because the service exposes no update API", inference_service=current, immutable_drift=fixed)
        status = str(current.get("Status") or "").lower()
        if status in ("failed", "error"): module.fail_json(msg="DLC inference service is in a failed state", inference_service=current)
        if status in ("deploying", "starting", "restarting"):
            if not p["wait"]: module.fail_json(msg="DLC inference service is still transitioning; enable wait to converge before another state action", inference_service=current)
            wait_service(module, client, models, p, "running"); current = find(module, client, models, p["name"]); status = "running"
        elif status == "stopping":
            if not p["wait"]: module.fail_json(msg="DLC inference service is still stopping; enable wait to converge", inference_service=current)
            wait_service(module, client, models, p, "stopped"); current = find(module, client, models, p["name"]); status = "stopped"
        ...
        if status == p["state"]: module.exit_json(changed=False, inference_service=current, service_id=current.get("ServiceId"))
        ...
        if not module.check_mode:
            ...
            if p["wait"]: wait_service(module, client, models, p, p["state"])
            current = find(module, client, models, p["name"])
        module.exit_json(changed=True, **(diff_value or {}), inference_service=current if not module.check_mode else dict(current, Status=p["state"].capitalize()), service_id=current.get("ServiceId"))
