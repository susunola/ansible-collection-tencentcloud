...
def detail_request(models, p):
    ...
    r.GatewayId, r.ModelAPIId = p["gateway_id"], p["model_api_id"]
    return r


def mutation_request(cls, p, group_ids):
    r = cls()
    r.GatewayId, r.ResourceType, r.ResourceId, r.ConsumerGroupIds = p["gateway_id"], "ModelAPI", p["model_api_id"], group_ids
    return r
...
def resolve_names(module, client, models, p):
    if not p.get("model_api_id"):
        ...
        if missing:
            module.fail_json(msg="TSE gateway Model API name was not found", model_api_name=missing[0])
        p["model_api_id"] = ids[0]
    if not p.get("consumer_group_ids"):
        values = list_values(
            module, client.DescribeCloudNativeAPIGatewayConsumerGroupList, lambda offset: _group_list_request(models, p, offset), "ConsumerGroups"
        )
        ids, missing = resolve_ids(values, p["consumer_group_names"], "ConsumerGroupId")
        if missing:
            module.fail_json(msg="TSE gateway consumer group names were not found", consumer_group_names=missing)
...
def current(module, client, models, p):
    ...
    if not result:
        module.fail_json(msg="TSE gateway Model API was not found", model_api_id=p["model_api_id"])
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "model_api_id": {},
            "model_api_name": {},
            "consumer_group_ids": {"type": "list", "elements": "str"},
            "consumer_group_names": {"type": "list", "elements": "str"},
        },
        required_one_of=[("model_api_id", "model_api_name"), ("consumer_group_ids", "consumer_group_names")],
        mutually_exclusive=[("model_api_id", "model_api_name"), ("consumer_group_ids", "consumer_group_names")],
        supports_check_mode=True,
    )
    p = module.params
    supplied = p.get("consumer_group_ids") or p.get("consumer_group_names") or []
    if not 1 <= len(supplied) <= 10:
        module.fail_json(msg="consumer_group_ids or consumer_group_names must contain between 1 and 10 entries")
    if len(set(supplied)) != len(supplied):
        module.fail_json(msg="consumer_group_ids or consumer_group_names must not contain duplicates")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        requested = set(p["consumer_group_ids"])
        existing = set(before)
        ...
        if not affected:
            module.exit_json(changed=False, authorization={"ModelAPIId": p["model_api_id"], "ConsumerGroupIds": before})
        diff = maybe_diff(module, {"ConsumerGroupIds": before}, {"ConsumerGroupIds": target})
        if not module.check_mode:
            if p["state"] == "present":
                api, cls = client.AddCloudNativeAPIGatewayConsumerGroupAuth, models.AddCloudNativeAPIGatewayConsumerGroupAuthRequest
            else:
                api, cls = client.RemoveCloudNativeAPIGatewayConsumerGroupAuth, models.RemoveCloudNativeAPIGatewayConsumerGroupAuthRequest
            module.sdk_call(api, mutation_request(cls, p, affected))
            target = current(module, client, models, p)
        module.exit_json(
            changed=True, **(diff or {}), authorization={"ModelAPIId": p["model_api_id"], "ConsumerGroupIds": target, "AffectedConsumerGroupIds": affected}
        )
