...
FIELDS = {
    "backup_start_time": "BackupStartTime", "backup_end_time": "BackupEndTime", "backup_method": "BackupMethod",
    "enable_full": "EnableFull", "enable_log": "EnableLog", "full_retention_days": "FullRetentionPeriod",
    "log_retention_days": "LogRetentionPeriod", "period_time": "PeriodTime",
}
...
def describe_request(models, instance_id): request = models.DescribeDBSBackupPolicyRequest(); request.InstanceId = instance_id; return request


def normalize(value):
    result = dict(value or {})
    for key in ("EnableFull", "EnableLog"):
        if result.get(key) is not None: result[key] = bool(result[key])
    return result


def get(module, client, models, instance_id):
    response = module.sdk_call(client.DescribeDBSBackupPolicy, describe_request(models, instance_id)); items = response.Items or []
    if len(items) != 1: module.fail_json(msg="expected exactly one TDSQL MySQL backup policy", instance_id=instance_id, policy_count=len(items))
...
def modify_request(models, p, target):
    value = {key: target.get(key) for key in FIELDS.values()}; value["InstanceId"] = p["instance_id"]
    ...
    request = models.ModifyDBSBackupPolicyRequest(); request.InstanceId = p["instance_id"]
    request.BackupPolicy = models.BackupPolicyModelInput(); request.BackupPolicy.from_json_string(json.dumps(value)); return request


def run_module():
    spec = {"instance_id": {"required": True}, "backup_start_time": {}, "backup_end_time": {}, "backup_method": {"choices": ["physical", "snapshot"]}, "enable_full": {"type": "bool"}, "enable_log": {"type": "bool"}, "full_retention_days": {"type": "int"}, "log_retention_days": {"type": "int"}, "period_time": {}}
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True); p = module.params
    if not any(p.get(key) is not None for key in FIELDS): module.fail_json(msg="at least one backup policy field is required")
    for key in ("backup_start_time", "backup_end_time"):
        if p.get(key) is not None and not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", p[key]): module.fail_json(msg="%s must use HH:MM format" % key)
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TdmysqlClient, "tdmysql.tencentcloudapi.com")
    try:
        current = get(module, client, models, p["instance_id"]); target = desired(p, current)
        before = {key: current.get(key) for key in FIELDS.values()}; after = {key: target.get(key) for key in FIELDS.values()}
        if before == after: module.exit_json(changed=False, backup_policy=current)
        ...
        if not module.check_mode:
            ...
            if response.IsSuccess is False: module.fail_json(msg="TDSQL MySQL backup policy update failed", detail=response.Msg, request_id=response.RequestId)
