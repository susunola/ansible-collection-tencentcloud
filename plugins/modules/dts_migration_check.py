...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.dts_migration_check_info
    description: Gather information about Tencent Cloud DTS migration check jobs.
...
def run_module():
    module = TencentCloudModule(
        argument_spec={"job_id": {"required": True}, "wait": {"type": "bool", "default": True}, "fail_on_check_error": {"type": "bool", "default": True}},
    ...
    try:
        ...
        if should_start and not module.check_mode:
            ...
            result = (
                wait_check(module, client, models, module.params["job_id"])
                if module.params["wait"]
                else describe(module, client, models, module.params["job_id"])
            )
        ...
        if (
            module.params["fail_on_check_error"]
            and not module.check_mode
            and result.get("Status") in ("success", "failed")
            and result.get("CheckFlag") != "checkPass"
        ):
