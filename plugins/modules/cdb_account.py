...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cdb_account_info
    description: Gather information about Tencent Cloud CDB accounts.
...
def account(models, user, host):
    ...
    value.User, value.Host = user, host
    return value


def describe(models, p, offset=0):
    request = models.DescribeAccountsRequest()
    request.InstanceId, request.Offset, request.Limit = p["instance_id"], offset, 100
    return request
...
def create(models, p):
    ...
    request.Description, request.MaxUserConnections = p["description"], p["max_user_connections"]
    return request


def simple(models, kind, p):
    request = getattr(models, kind + "Request")()
    request.InstanceId = p["instance_id"]
    request.Accounts = [account(models, p["username"], p["host"])]
    return request
...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeAccounts, describe(models, p, offset))
        items = list(response.Items or [])
        for item in items:
            value = item._serialize(allow_none=True)
            if value.get("User") == p["username"] and value.get("Host") == p["host"]:
                return value
        offset += len(items)
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteAccounts, simple(models, "DeleteAccounts", p))
        ...
        if not module.check_mode:
            if not current:
                if not p["password"]:
                    module.fail_json(msg="password is required when creating a CDB account")
                module.sdk_call(client.CreateAccounts, create(models, p))
            else:
                if before["Notes"] != target["Notes"]:
                    request = simple(models, "ModifyAccountDescription", p)
                ...
                if before["MaxUserConnections"] != target["MaxUserConnections"]:
                    request = simple(models, "ModifyAccountMaxUserConnections", p)
                    request.MaxUserConnections = p["max_user_connections"]
                    module.sdk_call(client.ModifyAccountMaxUserConnections, request)
                if p["rotate_password"]:
                    request = simple(models, "ModifyAccountPassword", p)
                    request.NewPassword = p["password"]
