...
def describe_request(models, p, offset=0):
    r = models.DescribeJobsRequest(); r.Offset, r.Limit, r.WorkSpaceId = offset, 100, p["workspace_id"]
    if p.get("job_id"): r.JobIds = [p["job_id"]]
    elif p.get("name"):
        f = models.Filter(); f.Name, f.Values = "Name", [p["name"]]; r.Filters = [f]
    return r
...
def create_request(models, p):
    r = models.CreateJobRequest(); r.Name, r.JobType, r.WorkSpaceId = p["name"], p["job_type"], p["workspace_id"]
    ...
    r.FlinkVersion, r.JdkVersion, r.OpenJobDefaultAlarm, r.Tags = p.get("flink_version"), p.get("jdk_version"), 1 if p["default_alarm"] else 0, _tags(models, p.get("tags")); return r
def modify_request(models, p, job_id, name, remark, description, folder_id):
    r = models.ModifyJobRequest(); r.JobId, r.WorkSpaceId, r.Name = job_id, p["workspace_id"], name; r.Remark, r.Description, r.TargetFolderId = remark, description, folder_id
    r.ContinueAlarm = None if p.get("continue_alarm") is None else int(p["continue_alarm"]); return r
def run_request(models, p, job_id, resume=False):
    item = models.RunJobDescription(); item.JobId, item.RunType, item.StartMode = job_id, 2 if resume else 1, p["start_mode"]
    ...
    r = models.RunJobsRequest(); r.WorkSpaceId, r.RunJobDescriptions = p["workspace_id"], [item]; return r
def stop_request(models, p, job_id, pause=False):
    item = models.StopJobDescription(); item.JobId, item.StopType = job_id, 2 if pause else 1
...
def run_module():
    spec = {"state": {"choices": ["present", "absent"], "default": "present"}, "job_id": {}, "name": {}, "workspace_id": {"required": True}, "job_type": {"type": "int", "choices": [1, 2]}, "cluster_type": {"type": "int", "choices": [1, 2]}, "cluster_id": {}, "cu_memory": {"type": "int", "choices": [2, 4, 8, 16]}, "folder_id": {}, "flink_version": {}, "jdk_version": {}, "remark": {}, "description": {}, "default_alarm": {"type": "bool", "default": False}, "continue_alarm": {"type": "bool"}, "tags": {"type": "dict"}, "desired_status": {"choices": ["running", "stopped", "paused"]}, "job_config_version": {"type": "int"}, "start_mode": {"default": "LATEST"}, "savepoint_id": {}, "savepoint_path": {}}
    module = TencentCloudModule(argument_spec=spec, required_one_of=[("job_id", "name")], supports_check_mode=True); p = module.params; module.require_sdk(); models, cm = _load(); client = module.create_client(cm.OceanusClient, "oceanus.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, job=None)
        ...
        if not current:
            ...
            if p.get("cluster_type") == 2 and not p.get("cluster_id"): module.fail_json(msg="cluster_id is required for a dedicated Oceanus job")
            target = {"Name": p["name"], "JobType": p["job_type"], "ClusterType": p.get("cluster_type") or 1, "ClusterId": p.get("cluster_id"), "CuMem": p.get("cu_memory") or 4, "WorkSpaceId": p["workspace_id"], "Status": 5}; diff = maybe_diff(module, None, target)
            if not module.check_mode: p["job_id"] = module.sdk_call(client.CreateJob, create_request(models, p)).JobId; current = find(module, client, models, p)
            else: current = target
            changed = True
        ...
        if status_target is not None and current.get("Status") != status_target:
            ...
            if not module.check_mode:
                if status_target == 4: module.sdk_call(client.RunJobs, run_request(models, p, job_id, current.get("Status") == 6))
                else: module.sdk_call(client.StopJobs, stop_request(models, p, job_id, status_target == 6))
                p["job_id"] = job_id; _wait(module, client, models, p, [status_target]); current = find(module, client, models, p)
