...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.oceanus_job_info
    description: Gather information about Tencent Cloud Oceanus jobs.
...
def _folder_for_job(node, job_id):
    if not isinstance(node, dict):
        return None
    if any(item.get("JobId") == job_id for item in node.get("JobSet") or []):
        return node.get("Id")
...
def find(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeJobs, describe_request(models, p, offset))
        page = response.JobSet or []
        for item in page:
            value = item._serialize(allow_none=True)
            if (p.get("job_id") and value.get("JobId") == p["job_id"]) or (not p.get("job_id") and value.get("Name") == p.get("name")):
                if p.get("folder_id") is not None:
                    value["FolderId"] = job_folder(module, client, models, p, value["JobId"])
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                ...
                _wait_gone(module, client, models, p)
        ...
        if before != desired:
            ...
            if not module.check_mode:
                module.sdk_call(
                    client.ModifyJob,
                    modify_request(models, p, current["JobId"], desired["Name"], desired["Remark"], desired["Description"], desired.get("FolderId")),
                )
