...
RETURN = r'''
cluster_nat_ccn_fw_switches:
  description: Matching CFW cluster nat ccn fw switches.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeClusterNatCcnFwSwitchList, request),
        lambda response: response.Data,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    cluster_nat_ccn_fw_switches = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, cluster_nat_ccn_fw_switches=cluster_nat_ccn_fw_switches, total_count=total_count)
