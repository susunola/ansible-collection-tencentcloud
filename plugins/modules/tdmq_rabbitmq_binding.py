...
def describe_request(models, p, offset=0):
    ...
    request.InstanceId, request.VirtualHost = p["instance_id"], p["virtual_host"]
    request.Offset, request.Limit, request.SourceExchange = offset, 100, p["source_exchange"]
    if p["destination_type"] == "queue":
        request.QueueName = p["destination"]
    else:
        request.DestinationExchange = p["destination"]
    return request
...
def create_request(models, p):
    ...
    request.InstanceId, request.VirtualHost = p["instance_id"], p["virtual_host"]
    request.Source, request.DestinationType, request.Destination, request.RoutingKey = (
        p["source_exchange"],
        p["destination_type"],
        p["destination"],
        p["routing_key"],
...
def delete_request(models, p, binding_id):
    ...
    request.InstanceId, request.VirtualHost, request.BindingId = p["instance_id"], p["virtual_host"], binding_id
    return request
...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRabbitMQBindings, describe_request(models, p, offset))
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteRabbitMQBinding, delete_request(models, p, current["BindingId"]))
        ...
        if not module.check_mode:
            module.sdk_call(client.CreateRabbitMQBinding, create_request(models, p))
            current = find(module, client, models, p)
