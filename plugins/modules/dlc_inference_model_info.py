...
def build_request(models, p, page):
    request = models.ListInferenceModelsRequest(); request.Page, request.PageSize = page, p["page_size"]
    for source, target in (("start_time", "StartTime"), ("end_time", "EndTime"), ("parameter_size_min", "ParameterSizeMin"), ("parameter_size_max", "ParameterSizeMax")):
...
def read(module, client, models, p):
    ...
    for page in range(1, p["max_pages"] + 1):
        response = module.sdk_call(client.ListInferenceModels, build_request(models, p, page)); items = response.Items or []
        values.extend(x._serialize(allow_none=True) for x in items); total, request_id = int(response.Total or 0), response.RequestId
...
def run_module():
    ...
    spec = {
        "start_time": {"type": "int"}, "end_time": {"type": "int"}, "parameter_size_min": {"type": "float"},
        "parameter_size_max": {"type": "float"}, "filters": {"type": "dict", "default": {}},
    ...
    if p.get("parameter_size_min") is not None and p.get("parameter_size_max") is not None and p["parameter_size_min"] > p["parameter_size_max"]: module.fail_json(msg="parameter_size_min must not exceed parameter_size_max")
