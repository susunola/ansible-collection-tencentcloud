...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.tcb_environment_info
    description: Gather information about Tencent Cloud TCB envs.
...
def describe_request(models, env_id=None, offset=0):
    r = models.DescribeEnvsRequest()
    r.EnvId = env_id
...
def create_request(models, p):
    r = models.CreateEnvRequest()
    r.Alias, r.PackageId, r.Resources = p["alias"], p["package_id"], p["resources"]
    r.Period, r.AutoVoucher = p["period"], p.get("auto_voucher")
    r.Tags = _tags(models, p.get("tags"))
    r.RenewFlag = p.get("renew_flag")
    r.ExternalStorage = _model(models.ExternalStorage, p.get("external_storage"))
    r.EnableOverrun = p.get("enable_overrun")
    return r


def update_request(models, env_id, alias):
    r = models.ModifyEnvRequest()
    r.EnvId, r.Alias = env_id, alias
    return r


def delete_request(models, p, env_id):
    r = models.DestroyEnvRequest()
    r.EnvId, r.IsForce, r.BypassCheck = env_id, p["force_destroy"], p["bypass_destroy_check"]
    return r
...
def find(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeEnvs, describe_request(models, p.get("env_id"), offset))
        page = response.EnvList or []
        for item in page:
            value = item._serialize(allow_none=True)
            if (p.get("env_id") and value.get("EnvId") == p["env_id"]) or (not p.get("env_id") and value.get("Alias") == p.get("alias")):
                matches.append(value)
        offset += len(page)
        if not page or offset >= int(response.Total or 0):
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple CloudBase environments matched; specify env_id")
...
def run_module():
    spec = {
        ...
        "auto_voucher": {"type": "bool"},
        "tags": {"type": "dict"},
        "renew_flag": {"choices": ["NOTIFY_AND_AUTO_RENEW", "NOTIFY_AND_MANUAL_RENEW"]},
        "external_storage": {"type": "dict"},
        "enable_overrun": {"choices": ["TRUE", "FALSE"]},
        "force_destroy": {"type": "bool", "default": False},
        "bypass_destroy_check": {"type": "bool", "default": False},
    }
    module = TencentCloudModule(argument_spec=spec, required_one_of=[("env_id", "alias")], supports_check_mode=True)
    p = module.params
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DestroyEnv, delete_request(models, p, current["EnvId"]))
            module.exit_json(changed=True, **(diff or {}), environment=None)
        if not current:
            missing = [x for x in ("alias", "package_id", "resources") if not p.get(x)]
            if missing:
                module.fail_json(msg="creation parameters are required for a CloudBase environment", missing=missing)
            target = {"Alias": p["alias"], "PackageId": p["package_id"], "Resources": p["resources"]}
            diff = maybe_diff(module, None, target)
            if not module.check_mode:
                p["env_id"] = module.sdk_call(client.CreateEnv, create_request(models, p)).EnvId
                current = find(module, client, models, p)
        ...
        if alias == current.get("Alias"):
            module.exit_json(changed=False, environment=current)
        diff = maybe_diff(module, {"Alias": current.get("Alias")}, {"Alias": alias})
        if not module.check_mode:
            module.sdk_call(client.ModifyEnv, update_request(models, current["EnvId"], alias))
            p["env_id"] = current["EnvId"]
            current = find(module, client, models, p)
