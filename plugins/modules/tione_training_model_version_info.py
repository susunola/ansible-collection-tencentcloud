...
def run_module():
    ...
    module = TencentCloudModule(argument_spec=spec, supports_check_mode=True, mutually_exclusive=[("version_id", "filters")])
    p = module.params
    if not p.get("version_id") and not p.get("model_id"):
        module.fail_json(msg="model_id is required in list mode")
    if len(p["filters"]) > 10:
        module.fail_json(msg="filters accepts at most ten filter names")
    if any(len(value if isinstance(value, list) else [value]) > 100 for value in p["filters"].values()):
        module.fail_json(msg="each filter accepts at most 100 values")
    module.require_sdk()
    models, cm = _load()
