...
def create_request(models, p):
    ...
    request.SecretName, request.VersionId, request.Description = p["secret_name"], p["initial_version_id"], p["description"]
    request.SecretString, request.SecretBinary = p.get("initial_secret_string"), p.get("initial_secret_binary")
    request.KmsKeyId, request.KmsHsmClusterId, request.EncryptType, request.SecretType = p.get("kms_key_id"), p.get("kms_hsm_cluster_id"), p["encrypt_type"], 0
    return request


def description_request(models, p):
    request = models.UpdateDescriptionRequest()
    request.SecretName, request.Description = p["secret_name"], p["description"]
    return request


def state_request(models, name, enabled):
    request = models.EnableSecretRequest() if enabled else models.DisableSecretRequest()
    request.SecretName = name
    return request
...
def delete_request(models, p):
    ...
    request.SecretName, request.RecoveryWindowInDays = p["secret_name"], p["recovery_window_days"]
    return request


def find(module, client, models, name):
    try:
        return module.sdk_call(client.DescribeSecret, describe_request(models, name))._serialize(allow_none=True)
...
def comparable(v):
    return {
        "SecretName": v.get("SecretName"),
        "Description": v.get("Description") or "",
        "KmsKeyId": v.get("KmsKeyId"),
        "SecretType": int(v.get("SecretType") or 0),
        "EncryptType": int(v.get("EncryptType") or 0),
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "initial_version_id": {"default": "SSM_Current"},
            "initial_secret_string": {"no_log": True},
            "initial_secret_binary": {"no_log": True},
            "kms_key_id": {},
            "kms_hsm_cluster_id": {},
            "encrypt_type": {"type": "int", "choices": [0, 1], "default": 0},
            "recovery_window_days": {"type": "int", "default": 7},
        },
        mutually_exclusive=[("initial_secret_string", "initial_secret_binary")],
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteSecret, delete_request(models, p))
                current = find(module, client, models, p["secret_name"])
            module.exit_json(changed=True, **(diff or {}), secret=current)
        if not current and p.get("initial_secret_string") is None and p.get("initial_secret_binary") is None:
            module.fail_json(msg="initial_secret_string or initial_secret_binary is required when creating a secret")
        restored = bool(current and current.get("Status") == "PendingDelete")
        if restored:
            if not module.check_mode:
                module.sdk_call(client.RestoreSecret, restore_request(models, p["secret_name"]))
                current = find(module, client, models, p["secret_name"])
        if current:
            before = comparable(current)
            target = dict(before)
            target["Description"], target["Enabled"] = p["description"], p["enabled"]
            if p.get("kms_key_id") is not None and before["KmsKeyId"] != p["kms_key_id"]:
                module.fail_json(msg="kms_key_id is immutable after secret creation", current_kms_key_id=before["KmsKeyId"])
            if before["EncryptType"] != p["encrypt_type"]:
                module.fail_json(msg="encrypt_type is immutable after secret creation", current_encrypt_type=before["EncryptType"])
            if before == target and not restored:
                module.exit_json(changed=False, secret=current)
            diff = maybe_diff(module, before, target)
            if not module.check_mode:
                if before["Description"] != target["Description"]:
                    module.sdk_call(client.UpdateDescription, description_request(models, p))
                if before["Enabled"] != target["Enabled"]:
                    module.sdk_call(client.EnableSecret if p["enabled"] else client.DisableSecret, state_request(models, p["secret_name"], p["enabled"]))
                current = find(module, client, models, p["secret_name"])
            module.exit_json(changed=True, **(diff or {}), secret=current)
        target = {
        ...
        if not module.check_mode:
            module.sdk_call(client.CreateSecret, create_request(models, p))
            current = find(module, client, models, p["secret_name"])
            if not p["enabled"]:
                module.sdk_call(client.DisableSecret, state_request(models, p["secret_name"], False))
                current = find(module, client, models, p["secret_name"])
