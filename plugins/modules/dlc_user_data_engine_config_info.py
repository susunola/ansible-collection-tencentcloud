...
RETURN = r'''
user_data_engine_configs:
  description: Matching DLC user data engine configs.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeUserDataEngineConfig, request),
        lambda response: response.DataEngineConfigInstanceInfos,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    user_data_engine_configs = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, user_data_engine_configs=user_data_engine_configs,
