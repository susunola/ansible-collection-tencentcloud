...
def _port_models(models, values):
    result = []
    for value in values or []:
        item = models.ProtocolPort()
        item.from_json_string(
            json.dumps(
                {
                    "Protocol": value["protocol"],
                    "Port": value["port"],
                    "TargetPort": value["target_port"],
                    "NodePort": value.get("node_port"),
                    "Name": value.get("name"),
                }
            )
        )
...
def find(module, client, models, p):
    r = models.DescribeContainerGroupsRequest()
    r.ApplicationId = p["application_id"]
    r.NamespaceId = p["namespace_id"]
    r.ClusterId = p["cluster_id"]
    r.Offset, r.Limit = 0, 50
    if p.get("group_id"):
        r.GroupIdList = [p["group_id"]]
    else:
        r.SearchWord = p["name"]
    result = module.sdk_call(client.DescribeContainerGroups, r).Result
    values = (result.Content if result else None) or []
    matches = (
        [x for x in values if x.GroupId == p.get("group_id")]
        if p.get("group_id")
        else [x for x in values if x.GroupName == p["name"] and x.NamespaceId == p["namespace_id"] and x.ClusterId == p["cluster_id"]]
    )
    if len(matches) > 1:
        module.fail_json(msg="Multiple TSF container deployment groups matched", name=p["name"])
    if not matches:
        return None
    detail = models.DescribeContainerGroupDetailRequest()
    detail.GroupId = matches[0].GroupId
...
def desired(p):
    mapping = {
        ...
        "update_interval": "UpdateIvl",
        "subnet_id": "SubnetId",
        "alias": "Alias",
        "resource_type": "GroupResourceType",
    }
    target = {b: p[a] for a, b in mapping.items() if p.get(a) is not None}
    if p.get("protocol_ports") is not None:
        target["ProtocolPorts"] = _ports(
            [
                {"Protocol": x["protocol"], "Port": x["port"], "TargetPort": x["target_port"], "NodePort": x.get("node_port"), "Name": x.get("name")}
                for x in p["protocol_ports"]
            ]
        )
    return target
...
def comparable(current, target):
    ...
    if "ProtocolPorts" in result:
        result["ProtocolPorts"] = _ports(result["ProtocolPorts"])
    return result
...
def run_module():
    ...
    module = TencentCloudModule(
        argument_spec={
            ...
            "memory_request": {},
            "memory_limit": {},
            "access_type": {"type": "int", "choices": [0, 1, 2], "default": 1},
            "protocol_ports": {"type": "list", "elements": "dict", "options": port},
            "update_type": {"type": "int", "choices": [0, 1], "default": 0},
            "update_interval": {"type": "int"},
            "subnet_id": {},
            "alias": {},
            "resource_type": {"choices": ["DEF"], "default": "DEF"},
        },
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, deployment_group=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                r = models.DeleteContainerGroupRequest()
                r.GroupId = current["GroupId"]
                response = module.sdk_call(client.DeleteContainerGroup, r)
                if response.Result is False:
                    module.fail_json(msg="Tencent Cloud rejected the TSF container deployment group deletion", request_id=response.RequestId)
            module.exit_json(changed=True, **(diff or {}), deployment_group=None)
        if p.get("replicas") is None:
            module.fail_json(msg="replicas is required when state is present")
        if p["update_type"] == 1 and p.get("update_interval") is None:
            module.fail_json(msg="update_interval is required for rolling updates")
        target = desired(p)
        immutable = ["GroupName", "ApplicationId", "NamespaceId", "ClusterId", "CpuRequest", "CpuLimit", "MemRequest", "MemLimit", "GroupResourceType"]
        if current:
            require_immutable_unchanged(module, current, target, immutable, "TSF container deployment group")
        if current and comparable(current, target) == target:
            module.exit_json(changed=False, deployment_group=current)
        ...
        if not module.check_mode:
            if current:
                ...
                if any(comparable(current, target).get(k) != target.get(k) for k in mutable if k in target):
                    r = models.ModifyContainerGroupRequest()
                    r.GroupId = current["GroupId"]
                    for key in mutable:
                        if key in target:
                            setattr(r, key, _port_models(models, p["protocol_ports"]) if key == "ProtocolPorts" else target[key])
                    response = module.sdk_call(client.ModifyContainerGroup, r)
                    if response.Result is False:
                        module.fail_json(msg="Tencent Cloud rejected the TSF container group update", request_id=response.RequestId)
                if current.get("InstanceNum") != target["InstanceNum"]:
                    r = models.ModifyContainerReplicasRequest()
                    r.GroupId = current["GroupId"]
                    r.InstanceNum = target["InstanceNum"]
                    response = module.sdk_call(client.ModifyContainerReplicas, r)
                    if response.Result is False:
                        module.fail_json(msg="Tencent Cloud rejected the TSF replica change", request_id=response.RequestId)
                ...
                for source, dest in create_map.items():
                    if source in target:
                        setattr(r, dest, target[source])
                if p.get("protocol_ports") is not None:
                    r.ProtocolPorts = _port_models(models, p["protocol_ports"])
