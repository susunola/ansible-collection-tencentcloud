...
def run_module():
    ...
    p = module.params
    if not p["domains"]:
        module.fail_json(msg="domains must contain at least one entry")
    if len(set(p["domains"])) != len(p["domains"]):
    ...
    try:
        ...
        if not add and not remove:
            module.exit_json(changed=False, waf_domains=before)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if add:
                module.sdk_call(client.CreateWafDomains, request(models.CreateWafDomainsRequest, p, add))
            if remove:
                module.sdk_call(client.DeleteWafDomains, request(models.DeleteWafDomainsRequest, p, remove))
