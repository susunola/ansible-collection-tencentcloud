...
def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeRocketMQGroups, describe_request(models, p, offset)); items = list(response.Groups or [])
        for item in items:
...
def comparable(value):
    return {"Name": value.get("Name"), "GroupType": value.get("GroupType"), "ReadEnabled": bool(value.get("ReadEnabled")), "BroadcastEnabled": bool(value.get("BroadcastEnabled")), "RetryMaxTimes": int(value.get("RetryMaxTimes") or 0), "Remark": value.get("Remark") or ""}


def desired(p):
    return {"Name": p["name"], "GroupType": p["group_type"], "ReadEnabled": p["read_enabled"], "BroadcastEnabled": p["broadcast_enabled"], "RetryMaxTimes": p["retry_max_times"], "Remark": p["remark"]}


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "cluster_id": {"required": True}, "namespace": {"required": True}, "name": {"required": True}, "group_type": {"choices": ["TCP", "HTTP"], "default": "TCP"}, "read_enabled": {"type": "bool", "default": True}, "broadcast_enabled": {"type": "bool", "default": False}, "retry_max_times": {"type": "int", "default": 16}, "remark": {"default": ""}}, supports_check_mode=True)
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode: module.sdk_call(client.DeleteRocketMQGroup, delete_request(models, p))
        ...
        if before == target: module.exit_json(changed=False, group=current)
        diff = maybe_diff(module, before, target)
        if current: require_immutable_unchanged(module, before, target, ("GroupType",), "RocketMQ consumer group")
