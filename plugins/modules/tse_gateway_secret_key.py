...
CONFIG_FIELDS = {
    "jwt_credential_config":"JWTCredentialConfig", "oauth_credential_config":"OAuthCredentialConfig",
    "oidc_credential_config":"OIDCCredentialConfig", "aksk_credential_config":"AKSKCredentialConfig",
    "cam_credential_config":"CAMCredentialConfig", "bearer_token_credential_config":"BearerTokenCredentialConfig",
    "basic_credential_config":"BasicCredentialConfig", "custom_header_credential_config":"CustomHeaderCredentialConfig",
    "query_param_credential_config":"QueryParamCredentialConfig",
}
IMMUTABLE_FIELDS = ["Name", "SecretType", "GenerateType", "ResourceType", "KmsKeyName", "KmsKeyVersion", "Description", "Provider"] + list(CONFIG_FIELDS.values())
...
def scrub_secret(value):
    ...
    if isinstance(value, dict): return {k:scrub_secret(v) for k,v in value.items() if k not in sensitive}
    if isinstance(value, list): return [scrub_secret(item) for item in value]
    return value


def create_payload(p):
    payload = {"GatewayId":p["gateway_id"], "SecretType":p["secret_type"], "Name":p["name"], "GenerateType":p["generate_type"], "ResourceType":p["resource_type"]}
    mapping = {"kms_key_name":"KmsKeyName", "kms_key_version":"KmsKeyVersion", "secret_value":"SecretValue", "description":"Description", "provider":"Provider"}
...
def create_request(models, p):
    r = models.CreateCloudNativeAPIGatewaySecretKeyRequest(); r.from_json_string(json.dumps(create_payload(p))); return r
def list_request(models, p, offset=0):
    r = models.DescribeCloudNativeAPIGatewaySecretKeyListRequest(); r.GatewayId, r.Offset, r.Limit, r.ResourceType = p["gateway_id"], offset, 20, p.get("resource_type"); return r
def detail_request(models, p, secret_id):
    r = models.DescribeCloudNativeAPIGatewaySecretKeyRequest(); r.GatewayId, r.SecretKeyId = p["gateway_id"], secret_id; return r
def delete_request(models, p, current):
    r = models.DeleteCloudNativeAPIGatewaySecretKeyRequest(); r.GatewayId, r.SecretKeyId = p["gateway_id"], current["SecretKeyId"]; return r
def status_request(models, p, current, status):
    r = models.ModifyCloudNativeAPIGatewaySecretKeyStatusRequest(); r.GatewayId, r.SecretKeyId, r.Status = p["gateway_id"], current["SecretKeyId"], status; return r
def value_request(models, p, current):
    r = models.DescribeCloudNativeAPIGatewaySecretKeyValueRequest(); r.GatewayId, r.SecretKeyId = p["gateway_id"], current["SecretKeyId"]; return r
...
def find(module, client, models, p):
    ...
    while True:
        ...
        for item in values or []:
            ...
            if (p.get("secret_key_id") and value.get("SecretKeyId") == p["secret_key_id"]) or (not p.get("secret_key_id") and value.get("Name") == p["name"]): matches.append(value)
        offset += len(values or [])
        if not result or offset >= int(result.TotalCount or 0): break
    if len(matches) > 1: module.fail_json(msg="Multiple TSE gateway secret keys matched; specify secret_key_id")
...
def validate_creation(module, p):
    ...
    if missing: module.fail_json(msg="Required for secret key creation: %s" % ", ".join(missing))
    if p["generate_type"] == "Custom" and not p.get("secret_value"): module.fail_json(msg="secret_value is required when generate_type=Custom")
    if p["generate_type"] == "KMS" and (not p.get("kms_key_name") or not p.get("kms_key_version")): module.fail_json(msg="kms_key_name and kms_key_version are required when generate_type=KMS")


def desired_immutable(p, current):
    target = {}
    reverse = {value:key for key,value in CONFIG_FIELDS.items()}
    standard = {"Name":"name", "SecretType":"secret_type", "GenerateType":"generate_type", "ResourceType":"resource_type", "KmsKeyName":"kms_key_name", "KmsKeyVersion":"kms_key_version", "Description":"description", "Provider":"provider"}
...
def run_module():
    spec = {"state":{"choices":["present","absent"],"default":"present"}, "gateway_id":{"required":True}, "secret_key_id":{}, "name":{}, "secret_type":{"choices":["ApiKey","Basic","Hmac","OAuth2","JWT"]}, "generate_type":{"choices":["System","Custom","KMS"]}, "resource_type":{"choices":["Consumer","ModelService"]}, "status":{"choices":["Enable","Disable"]}, "secret_value":{"no_log":True}, "kms_key_name":{}, "kms_key_version":{}, "description":{}, "provider":{}, "rotate_secret":{"type":"bool","default":False}, "allow_recreate":{"type":"bool","default":False}, "reveal_secret_value":{"type":"bool","default":False}}
    ...
    p = module.params; module.require_sdk(); models, cm = _load(); client = module.create_client(cm.TseClient, "tse.tencentcloudapi.com")
    try:
        current = find(module, client, models, p)
        if p["state"] == "absent":
            if not current: module.exit_json(changed=False, secret_key=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteCloudNativeAPIGatewaySecretKey, delete_request(models, p, current))
            module.exit_json(changed=True, **(diff or {}), secret_key=None)
        if p["rotate_secret"] and not p["allow_recreate"]: module.fail_json(msg="rotate_secret=true requires allow_recreate=true because rotation deletes the existing credential")
        if not current or p["rotate_secret"]: validate_creation(module, p)
        ...
        if drift and not p["rotate_secret"]: module.fail_json(msg="Immutable secret key configuration differs; use rotate_secret=true and allow_recreate=true to replace it", immutable_before=before, immutable_after=immutable)
        ...
        if not changed:
            ...
            if not module.check_mode:
                if current and p["rotate_secret"]: module.sdk_call(client.DeleteCloudNativeAPIGatewaySecretKey, delete_request(models, p, current)); current = None
                if not current:
                    response = module.sdk_call(client.CreateCloudNativeAPIGatewaySecretKey, create_request(models, p)); p["secret_key_id"] = response.Result.ID
                    current = find(module, client, models, p)
                if current.get("Status") != desired_status:
                    module.sdk_call(client.ModifyCloudNativeAPIGatewaySecretKeyStatus, status_request(models, p, current, desired_status)); current = find(module, client, models, p)
            result = {"changed":True, "secret_key":current if not module.check_mode else dict(immutable, Status=desired_status)}; result.update(diff or {})
