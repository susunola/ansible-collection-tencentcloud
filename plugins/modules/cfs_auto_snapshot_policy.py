...
def describe_request(models, p, offset=0):
    ...
    if p.get("policy_id"): request.AutoSnapshotPolicyId = p["policy_id"]
    return request
def create_request(models, p):
    request = models.CreateAutoSnapshotPolicyRequest(); request.PolicyName, request.Hour, request.DayOfWeek, request.AliveDays, request.DayOfMonth, request.IntervalDays = p["name"], p["hour"], p["day_of_week"], p["alive_days"], p["day_of_month"], p["interval_days"]; return request
def update_request(models, p, policy_id):
    request = models.UpdateAutoSnapshotPolicyRequest(); request.AutoSnapshotPolicyId, request.PolicyName, request.Hour, request.DayOfWeek = policy_id, p["name"], p["hour"], p["day_of_week"]
    request.AliveDays, request.IsActivated, request.DayOfMonth, request.IntervalDays = p["alive_days"], 1 if p["enabled"] else 0, p["day_of_month"], p["interval_days"]; return request
def delete_request(models, policy_id):
    request = models.DeleteAutoSnapshotPolicyRequest(); request.AutoSnapshotPolicyId = policy_id; return request
def bind_request(models, policy_id, ids):
    request = models.BindAutoSnapshotPolicyRequest(); request.AutoSnapshotPolicyId, request.FileSystemIds = policy_id, ",".join(sorted(ids)); return request
def unbind_request(models, policy_id, ids):
    request = models.UnbindAutoSnapshotPolicyRequest(); request.AutoSnapshotPolicyId, request.FileSystemIds = policy_id, ",".join(sorted(ids)); return request
def find(module, client, models, p):
    offset = 0; matches = []
    while True:
        response = module.sdk_call(client.DescribeAutoSnapshotPolicies, describe_request(models, p, offset)); values = list(response.AutoSnapshotPolicies or [])
        for item in values:
            value = item._serialize(allow_none=True)
            if (p.get("policy_id") and value.get("AutoSnapshotPolicyId") == p["policy_id"]) or (not p.get("policy_id") and value.get("PolicyName") == p.get("name")): matches.append(value)
        offset += len(values)
        if offset >= int(response.TotalCount or 0) or not values: break
    if len(matches) > 1: module.fail_json(msg="Multiple CFS auto-snapshot policies matched; specify policy_id")
    return matches[0] if matches else None
def _bound(value): return sorted(item.get("FileSystemId") for item in (value.get("FileSystems") or []) if item.get("FileSystemId"))
def comparable(value): return {"PolicyName": value.get("PolicyName"), "Hour": value.get("Hour") or "", "DayOfWeek": value.get("DayOfWeek") or "", "DayOfMonth": value.get("DayOfMonth") or "", "IntervalDays": int(value.get("IntervalDays") or 0), "AliveDays": int(value.get("AliveDays") or 0), "IsActivated": int(value.get("IsActivated") or 0), "FileSystemIds": _bound(value)}
def desired(p): return {"PolicyName": p["name"], "Hour": p["hour"], "DayOfWeek": p["day_of_week"], "DayOfMonth": p["day_of_month"], "IntervalDays": p["interval_days"], "AliveDays": p["alive_days"], "IsActivated": 1 if p["enabled"] else 0, "FileSystemIds": sorted(p["file_system_ids"])}


def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "policy_id": {}, "name": {}, "hour": {"default": "00"}, "day_of_week": {"default": ""}, "day_of_month": {"default": ""}, "interval_days": {"type": "int", "default": 0}, "alive_days": {"type": "int", "default": 0}, "enabled": {"type": "bool", "default": True}, "file_system_ids": {"type": "list", "elements": "str", "default": []}, "force_delete": {"type": "bool", "default": False}}, required_one_of=[("policy_id", "name")], supports_check_mode=True)
    p = module.params
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if bound and not p["force_delete"]: module.fail_json(msg="policy is still bound to file systems; set force_delete=true to unbind and delete", file_system_ids=bound)
        ...
        if not module.check_mode:
            if current: policy_id = current["AutoSnapshotPolicyId"]; module.sdk_call(client.UpdateAutoSnapshotPolicy, update_request(models, p, policy_id))
            ...
            old_ids = set(_bound(current)) if current else set(); new_ids = set(p["file_system_ids"])
