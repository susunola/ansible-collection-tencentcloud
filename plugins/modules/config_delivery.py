...
def find_delivery(module, client, models):
    response = module.sdk_call(client.DescribeConfigDeliver, describe_request(models)); value = response._serialize(allow_none=True); value.pop("RequestId", None); return value


def run_module():
    module = TencentCloudModule(argument_spec={"enabled": {"type": "bool", "default": True}, "name": {"required": True}, "target_arn": {"required": True}, "prefix": {"default": "config"}, "delivery_type": {"required": True}, "content_type": {"type": "int", "choices": [1, 2, 3], "default": 3}}, supports_check_mode=True)
    p = module.params
    ...
    try:
        ...
        desired = {"Status": int(p["enabled"]), "DeliverName": p["name"], "TargetArn": p["target_arn"], "DeliverPrefix": p["prefix"], "DeliverType": p["delivery_type"], "DeliverContentType": p["content_type"]}
        ...
        if not module.check_mode:
            module.sdk_call(client.UpdateConfigDeliver, update_request(models, p)); current = find_delivery(module, client, models)
