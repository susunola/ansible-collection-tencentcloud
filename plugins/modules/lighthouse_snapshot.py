...
def create_request(models, p):
    ...
    request.InstanceId, request.SnapshotName = p["instance_id"], p["name"]
    return request


def update_request(models, snapshot_id, name):
    request = models.ModifySnapshotAttributeRequest()
    request.SnapshotId, request.SnapshotName = snapshot_id, name
    return request
...
def wait_normal(module, client, models, p):
    ...
    while True:
        ...
        state = current and current.get("SnapshotState")
        if state == "NORMAL":
            return current
        if current and current.get("LatestOperationState") == "FAILED":
            module.fail_json(msg="Lighthouse snapshot creation failed", snapshot=current)
        if time.time() >= deadline:
            module.fail_json(msg="Timed out waiting for Lighthouse snapshot to reach NORMAL", snapshot=current)
...
def run_module():
    ...
    p = module.params
    if p["state"] == "present" and (not p["name"] or (not p["snapshot_id"] and not p["instance_id"])):
        module.fail_json(msg="name and either snapshot_id or instance_id are required when state=present")
    if p["state"] == "absent" and not p["snapshot_id"] and not p["instance_id"]:
        module.fail_json(msg="instance_id is required with name when state=absent")
    module.require_sdk()
    models, cm = _load()
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteSnapshots, delete_request(models, current["SnapshotId"]))
            module.exit_json(changed=True, **(diff or {}), snapshot=current if module.check_mode else None)
        before = {"SnapshotName": current.get("SnapshotName")} if current else None
        target = {"SnapshotName": p["name"]}
        if before == target:
            module.exit_json(changed=False, snapshot=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            if current:
                module.sdk_call(client.ModifySnapshotAttribute, update_request(models, current["SnapshotId"], p["name"]))
            else:
                p["snapshot_id"] = module.sdk_call(client.CreateInstanceSnapshot, create_request(models, p)).SnapshotId
