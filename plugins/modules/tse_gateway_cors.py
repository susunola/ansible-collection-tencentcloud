...
FIELDS = {"enabled":"Enabled", "origins":"Origins", "headers":"Headers", "methods":"Methods", "exposed_headers":"ExposedHeaders", "max_age":"MaxAge", "credentials":"Credentials", "preflight_continue":"PreFlightContinue"}
...
def request(cls,p,target=None):
    r=cls(); r.GatewayId,r.SourceType,r.SourceId=p["gateway_id"],p["scope"],p["resource_id"]
    for key,value in (target or {}).items(): setattr(r,key,value)
    return r
def get_current(module,client,models,p):
    response=module.sdk_call(client.DescribeCloudNativeAPIGatewayCORS,request(models.DescribeCloudNativeAPIGatewayCORSRequest,p)); result=response.Result
    return result._serialize(allow_none=True) if result else None
def target_config(p,current):
    return {field:(p[source] if p.get(source) is not None else ((current or {}).get(field,default))) for source,field in FIELDS.items() for default in [DEFAULTS[field]]}
def run_module():
    spec={"state":{"choices":["present","absent"],"default":"present"},"gateway_id":{"required":True},"scope":{"choices":["service","route"],"required":True},"resource_id":{"required":True},"enabled":{"type":"bool"},"origins":{"type":"list","elements":"str"},"headers":{"type":"list","elements":"str"},"methods":{"type":"list","elements":"str"},"exposed_headers":{"type":"list","elements":"str"},"max_age":{"type":"int"},"credentials":{"type":"bool"},"preflight_continue":{"type":"bool"}}
    ...
    try:
        ...
        if p["state"]=="absent":
            if not current: module.exit_json(changed=False,cors=None)
            diff=maybe_diff(module,current,None)
            if not module.check_mode: module.sdk_call(client.DeleteCloudNativeAPIGatewayCORS,request(models.DeleteCloudNativeAPIGatewayCORSRequest,p))
            module.exit_json(changed=True,**(diff or {}),cors=None)
        target=target_config(p,current); before={key:(current or {}).get(key) for key in target}
        if current and before==target: module.exit_json(changed=False,cors=current)
        diff=maybe_diff(module,before if current else None,target)
        if not module.check_mode: module.sdk_call(client.CreateOrModifyCloudNativeAPIGatewayCORS,request(models.CreateOrModifyCloudNativeAPIGatewayCORSRequest,p,target)); current=get_current(module,client,models,p)
