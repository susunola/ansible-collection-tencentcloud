...
def request(models, name, params, privileges=None):
    ...
    value.InstanceId, value.UserName, value.Host = params["instance_id"], params["username"], params["host"]
    ...
    if privileges is not None:
        value.Privileges = sorted(set(privileges))
    return value
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "host": {"default": "%"},
            "database": {"default": "*"},
            "object_type": {"choices": ["*", "table"], "default": "*"},
            "object_name": {"default": "*"},
            "column": {"default": "*"},
    ...
    p = module.params
    if p["database"] == "*" and any(p[key] != "*" for key in ("object_type", "object_name", "column")):
