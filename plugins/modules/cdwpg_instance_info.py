...
short_description: Gather information about Tencent Cloud CDW PostgreSQL instances
version_added: "1.4.0"
description:
  - Returns CDW PostgreSQL instances visible in a Tencent Cloud region.
  - Optionally attaches the service-reported instance state for each matched instance.
options:
  instance_id:
    description: CDW PostgreSQL instance ID to return.
    type: str
  name:
...
EXAMPLES = r'''
- name: List CDW PostgreSQL instances
  susunola.tencentcloud.cdwpg_instance_info:
    region: ap-guangzhou

- name: Find a CDW PostgreSQL instance by name
  susunola.tencentcloud.cdwpg_instance_info:
    region: ap-guangzhou
    name: analytics-pg
'''
...
RETURN = r'''
instances:
  description: Matching CDW PostgreSQL instances.
  returned: always
  type: list
  elements: dict
instance:
...
def _instance_id(item):
    return item.get("InstanceId") or item.get("InstanceID")


def _matches(item, instance_id, name):
    if instance_id and _instance_id(item) != instance_id:
        return False
