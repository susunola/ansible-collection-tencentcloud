...
def preflight_request(models, p):
    ...
    for source, target, cls_name in (
        ("image_info", "ImageInfo", "ImageInfo"),
        ("model_info", "ModelInfo", "ModelInfo"),
        ("volume_mount", "VolumeMount", "VolumeMount"),
    ):
        if p.get(source) is not None:
            setattr(request, target, _model(getattr(models, cls_name), p[source]))
    return request
...
def run_module():
    ...
    if bool(p.get("service_group_id")) == preflight:
        module.fail_json(msg="provide service_group_id for call-info mode or hot-update inputs for preflight mode, but not both")
    if p.get("project_id") is not None and not p.get("service_group_id"):
        module.fail_json(msg="project_id is only valid with service_group_id")
    module.require_sdk()
    models, cm = _load()
