...
options:
  state:
    description:
      - C(present) creates the logset with V(CreateLogset) when it does not exist and updates it with V(ModifyLogset)
        when it differs. C(absent) deletes it with V(DeleteLogset).
