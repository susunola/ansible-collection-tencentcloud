...
RETURN = r'''
chunks:
  description: Matching DATAAGENT chunks.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.dataagent.v20250513 import models, dataagent_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-dataagent package is required.")

    client = dataagent_client.DataagentClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "dataagent.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, offset, limit),
        lambda request: sdk_call(module, client.QueryChunkList, request),
        lambda response: response.Chunks,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    chunks = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, chunks=chunks, total_count=total_count)
