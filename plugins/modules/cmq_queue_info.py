...
RETURN = r'''
queues:
  description: Matching CMQ queues.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    try:
        from tencentcloud.cmq.v20190304 import models, cmq_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-cmq package is required.")

    client = cmq_client.CmqClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "cmq.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, module.params["filters"], offset, limit),
        lambda request: sdk_call(module, client.DescribeQueueDetail, request),
        lambda response: response.QueueSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    queues = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, queues=queues, total_count=total_count)
