...
def build_request(models, p, offset):
    request = models.DescribeDataSourcesRequest(); request.Offset, request.Limit = offset, p["page_size"]
    ...
    if p.get("order_field") is not None: request.OrderField = p["order_field"]
...
def read(module, client, models, p):
    ...
    for _ in range(p["max_pages"]):
        response = module.sdk_call(client.DescribeDataSources, build_request(models, p, offset)); items = response.DataSourceInfos or []
...
def run_module():
    spec = {
        ...
        "order_field": {}, "order": {"choices": ["ASC", "DESC"], "default": "DESC"},
        "page_size": {"type": "int", "default": 200}, "max_pages": {"type": "int", "default": 1000},
    }
