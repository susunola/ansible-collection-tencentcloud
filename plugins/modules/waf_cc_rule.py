...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.waf_cc_rule_info
    description: Gather information about Tencent Cloud WAF CC rules.
...
def describe_request(models, p, offset=0):
    ...
    request.Domain, request.Offset, request.Limit, request.By, request.Order = p["domain"], offset, 100, "ts_version", "asc"
    return request
...
def upsert_request(models, p, rule_id=0):
    ...
    request.Advance, request.Limit, request.Interval, request.ActionType = (
...
def delete_request(models, p, rule_id):
    ...
    request.Domain, request.Name, request.Edition, request.RuleId = p["domain"], p["name"], p["edition"], rule_id
    return request
...
def comparable(value):
    return {
        ...
        "Advance": str(value.get("Advance") or "0"),
        "Limit": str(value.get("Limit") or "0"),
        "Interval": str(value.get("Interval") or "0"),
        "ActionType": str(value.get("ActionType") or ""),
        "Priority": int(value.get("Priority") or 0),
        "ValidTime": int(value.get("ValidTime") or 0),
        "Url": value.get("Url") or "",
        "MatchFunc": int(value.get("MatchFunc") or 0),
        "Options": canonical_options(value.get("Options") or value.get("OptionsArr")),
        "SessionApplied": sorted(value.get("SessionApplied") or []),
        "LimitMethod": value.get("LimitMethod") or "only_limit",
        "LogicalOp": value.get("LogicalOp") or "and",
        "ActionRatio": int(value.get("ActionRatio") or 100),
    }
...
def desired(p):
    return {
        ...
        "Advance": "1" if p["advanced"] else "0",
        "Limit": str(p["threshold"]),
        "Interval": str(p["interval"]),
        "ActionType": str(p["action"]),
        "Priority": p["priority"],
        "ValidTime": p["valid_time"],
        "Url": p["url"],
        "MatchFunc": p["match_function"],
        "Options": canonical_options(p["options"]),
        "SessionApplied": sorted(p["session_ids"]),
        "LimitMethod": p["limit_method"],
...
def find(module, client, models, p):
    ...
    matches = []
    while True:
        response = module.sdk_call(client.DescribeCCRuleList, describe_request(models, p, offset))
        data = response.Data
        values = list(data.Res or []) if data else []
        for item in values:
        ...
        offset += len(values)
        total = int(data.TotalCount or 0) if data else 0
        if offset >= total or not values:
            break
    if len(matches) > 1:
        module.fail_json(msg="Multiple WAF CC rules matched; specify rule_id")
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "action": {"type": "int", "choices": [20, 21, 22, 23, 26, 27], "default": 22},
            "priority": {"type": "int", "default": 50},
            "valid_time": {"type": "int", "default": 600},
            "url": {"default": ""},
            "match_function": {"type": "int", "choices": [0, 1, 2, 3, 6, 7], "default": 0},
            "advanced": {"type": "bool", "default": False},
            "options": {"type": "list", "elements": "dict", "default": []},
            "session_ids": {"type": "list", "elements": "int", "default": []},
            "limit_method": {"default": "only_limit"},
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteCCRule, delete_request(models, p, int(current["RuleId"])))
        ...
        if not module.check_mode:
            response = module.sdk_call(client.UpsertCCRule, upsert_request(models, p, int(current["RuleId"]) if current else 0))
