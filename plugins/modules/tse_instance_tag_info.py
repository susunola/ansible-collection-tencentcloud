...
DOCUMENTATION = r'''
---
module: tse_instance_tag_info
short_description: Gather Tencent Cloud TSE instance tag metadata
...
'''
EXAMPLES = r'''
- susunola.tencentcloud.tse_instance_tag_info:
...
def request(models, instance_id):
    value = models.DescribeInstanceTagInfosRequest()
...
def run_module():
    ...
    try:
        response = module.sdk_call(client.DescribeInstanceTagInfos, request(models, module.params["instance_id"]))
        tags = [item._serialize(allow_none=True) for item in (response.TagInfos or [])]
