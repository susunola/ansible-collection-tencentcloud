...
FIELDS = {
    "service_group_name": ("ServiceGroupName", None, False), "service_description": ("ServiceDescription", None, True),
    "charge_type": ("ChargeType", None, False), "resource_group_id": ("ResourceGroupId", None, True),
    "model_info": ("ModelInfo", "ModelInfo", True), "image_info": ("ImageInfo", "ImageInfo", True),
    "env": ("Env", "EnvVar", True), "resources": ("Resources", "ResourceInfo", True), "instance_type": ("InstanceType", None, True),
    "scale_mode": ("ScaleMode", None, True), "replicas": ("Replicas", None, True),
    "horizontal_pod_autoscaler": ("HorizontalPodAutoscaler", "HorizontalPodAutoscaler", True),
    "log_enable": ("LogEnable", None, True), "log_config": ("LogConfig", "LogConfig", True),
    "authorization_enable": ("AuthorizationEnable", None, False), "tags": ("Tags", "Tag", False),
    "scale_strategy": ("ScaleStrategy", None, True), "cron_scale_jobs": ("CronScaleJobs", "CronScaleJob", True),
    "hybrid_billing_prepaid_replicas": ("HybridBillingPrepaidReplicas", None, True), "create_source": ("CreateSource", None, False),
    "model_hot_update_enable": ("ModelHotUpdateEnable", None, True), "scheduled_action": ("ScheduledAction", "ScheduledAction", True),
    "volume_mount": ("VolumeMount", "VolumeMount", True), "service_limit": ("ServiceLimit", "ServiceLimit", True),
    "model_turbo_enable": ("ModelTurboEnable", None, True), "command": ("Command", None, True),
    "service_eip": ("ServiceEIP", "ServiceEIP", True), "service_port": ("ServicePort", None, True),
    "deploy_type": ("DeployType", None, False), "instance_per_replicas": ("InstancePerReplicas", None, True),
    "termination_grace_period_seconds": ("TerminationGracePeriodSeconds", None, True), "pre_stop_command": ("PreStopCommand", None, True),
    "grpc_enable": ("GrpcEnable", None, True), "health_probe": ("HealthProbe", "HealthProbe", True),
    "rolling_update": ("RollingUpdate", "RollingUpdate", True), "volume_mounts": ("VolumeMounts", "VolumeMount", True),
    "scheduling_strategy": ("SchedulingStrategy", None, True),
    "resource_supply_attribute": ("ResourceSupplyAttribute", "ResourceSupplyAttribute", False), "infer_template_id": ("InferTemplateId", None, True),
}
...
def normalize(value):
    result = dict(value or {}); result.update(result.pop("ServiceInfo", None) or {})
    for key in ("Tags", "Env", "CronScaleJobs", "VolumeMounts"):
        if result.get(key) is not None: result[key] = sorted(result[key], key=lambda x: json.dumps(x, sort_keys=True))
    return result
...
def create_request(models, p):
    ...
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    if p.get("service_group_id") is not None: request.ServiceGroupId, request.NewVersion = p["service_group_id"], True
    for source, (key, cls_name, _) in FIELDS.items():
        if p.get(source) is not None: assign(models, request, key, cls_name, p[source])
    return request


def modify_request(models, p):
    request = models.ModifyModelServiceRequest(); request.ServiceId = p["service_id"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    for source, (key, cls_name, mutable) in FIELDS.items():
        if mutable and p.get(source) is not None: assign(models, request, key, cls_name, p[source])
    return request


def delete_request(models, p):
    request = models.DeleteModelServiceRequest(); request.ServiceId = p["service_id"]
    if p.get("project_id") is not None: request.TiProjectId = p["project_id"]
    return request
...
def wait_service(module, client, models, p, accepted):
    def poll():
        ...
        if status in FAILED: module.fail_json(msg="TIONE model service entered a failed state", service=value)
        return status
...
def run_module():
    ...
    spec.update({
        "charge_type": {"choices": ["PREPAID", "POSTPAID_BY_HOUR", "HYBRID_PAID"]}, "scale_mode": {"choices": ["AUTO", "MANUAL"]},
        "deploy_type": {"choices": ["STANDARD", "DIST", "ROLE_SET"]}, "scheduling_strategy": {"choices": ["binpack", "spread"]},
        "model_info": {"type": "dict"}, "image_info": {"type": "dict"}, "env": {"type": "list", "elements": "dict"}, "resources": {"type": "dict"},
        "replicas": {"type": "int"}, "horizontal_pod_autoscaler": {"type": "dict"}, "log_enable": {"type": "bool"}, "log_config": {"type": "dict"},
        "authorization_enable": {"type": "bool"}, "tags": {"type": "list", "elements": "dict"}, "cron_scale_jobs": {"type": "list", "elements": "dict"},
        "hybrid_billing_prepaid_replicas": {"type": "int"}, "model_hot_update_enable": {"type": "bool"}, "scheduled_action": {"type": "dict"},
        "volume_mount": {"type": "dict"}, "service_limit": {"type": "dict"}, "model_turbo_enable": {"type": "bool"}, "service_eip": {"type": "dict"},
        "service_port": {"type": "int"}, "instance_per_replicas": {"type": "int"}, "termination_grace_period_seconds": {"type": "int"},
        "pre_stop_command": {"type": "list", "elements": "str"}, "grpc_enable": {"type": "bool"}, "health_probe": {"type": "dict"}, "rolling_update": {"type": "dict"},
        "volume_mounts": {"type": "list", "elements": "dict"}, "resource_supply_attribute": {"type": "dict"}, "allow_delete": {"type": "bool", "default": False},
    ...
    if p["state"] == "absent" and not p.get("service_id"): module.fail_json(msg="service_id is required for safe deletion")
    if p.get("service_group_id") and p.get("service_id"): module.fail_json(msg="service_group_id creates a new version and cannot be combined with service_id")
    ...
    try:
        ...
        if current is None:
            if p.get("service_id"): module.fail_json(msg="requested service_id does not exist", service_id=p["service_id"])
            missing = [key for key in ("charge_type", "replicas") if p.get(key) is None]
            if p.get("image_info") is None and p.get("infer_template_id") is None: missing.append("image_info or infer_template_id")
            if not (p.get("service_group_id") or p.get("service_group_name")): missing.append("service_group_id or service_group_name")
            if missing: module.fail_json(msg="creation parameters are required for a TIONE model service", missing=missing)
            target = desired(p); diff_value = maybe_diff(module, None, target); service_id = None
            if not module.check_mode:
                response = module.sdk_call(client.CreateModelService, create_request(models, p)); value = response.Service
                service_id = value.ServiceId if value else None
                if not service_id: module.fail_json(msg="CreateModelService returned no service identity", request_id=response.RequestId)
                p = dict(p, service_id=service_id)
                if p["wait"]: wait_service(module, client, models, p, ["normal", "stopped"])
        ...
        if immutable: module.fail_json(msg="TIONE model service create-only configuration drift requires a new service version", service=current, immutable_drift=immutable)
        if not mutable: module.exit_json(changed=False, service=current, service_id=p["service_id"])
        target = desired(p, current); diff_value = maybe_diff(module, current, target)
        if not module.check_mode:
