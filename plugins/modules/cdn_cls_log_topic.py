...
def list_topics_request(models, channel):
    request = models.ListClsLogTopicsRequest(); request.Channel = channel; return request
...
def _domain_configs(models, values):
    ...
    for value in sorted(values or [], key=lambda item: item["domain"]):
        config = models.DomainAreaConfig(); config.Domain, config.Area = value["domain"], sorted(value["areas"]); result.append(config)
    return result
...
def _topic_candidates(response):
    ...
    for topic in response.Topics or []:
        item = topic._serialize(allow_none=True); item["LogsetId"] = default_logset.get("LogsetId"); values.append(item)
    for extra in response.ExtraLogset or []:
        ...
        for topic in extra.Topics or []:
            item = topic._serialize(allow_none=True); item["LogsetId"] = logset.get("LogsetId"); values.append(item)
    return values
...
def find_topic(module, client, models, p):
    ...
    for item in _topic_candidates(response):
        if p.get("topic_id") and item.get("TopicId") == p["topic_id"]: matches.append(item)
        elif not p.get("topic_id") and p.get("topic_name") and item.get("TopicName") == p["topic_name"]: matches.append(item)
    if p.get("logset_id"): matches = [item for item in matches if item.get("LogsetId") == p["logset_id"]]
    if len(matches) > 1: module.fail_json(msg="Multiple CDN CLS topics matched; specify topic_id and logset_id")
...
def _normalized(values):
    return sorted(({"Domain": item.get("Domain") or item.get("domain"), "Area": sorted(item.get("Area") or item.get("areas") or [])} for item in values or []), key=lambda item: item["Domain"])


def run_module():
    module = TencentCloudModule(
        argument_spec={
            "state": {"choices": ["present", "absent"], "default": "present"}, "topic_id": {}, "topic_name": {}, "logset_id": {},
            "channel": {"choices": ["cdn", "ecdn"], "default": "cdn"}, "enabled": {"type": "bool", "default": True},
            "domain_area_configs": {"type": "list", "elements": "dict", "default": [], "options": {"domain": {"required": True}, "areas": {"type": "list", "elements": "str", "required": True, "choices": ["mainland", "overseas"]}}},
            "inherit_domain_tags": {"type": "bool", "default": False}, "force_replace": {"type": "bool", "default": False},
        }, required_one_of=[("topic_id", "topic_name")], supports_check_mode=True)
    p = module.params
    module.require_sdk(); models, cm = _load(); client = module.create_client(cm.CdnClient, "cdn.tencentcloudapi.com")
    try:
        ...
        if p["state"] == "absent":
            ...
            if not logset_id: module.fail_json(msg="logset_id is required to delete this topic")
            diff = maybe_diff(module, current, None)
            if not module.check_mode: module.sdk_call(client.DeleteClsLogTopic, delete_request(models, p, current["TopicId"], logset_id))
        ...
        immutable = current and ((p.get("topic_name") and current.get("TopicName") != p["topic_name"]) or (p.get("logset_id") and current.get("LogsetId") != p["logset_id"]))
        if immutable and not p["force_replace"]: module.fail_json(msg="Topic name or logset is immutable; set force_replace=true to recreate", topic=current)
        desired = {"TopicName": p.get("topic_name") or (current and current.get("TopicName")), "LogsetId": p.get("logset_id") or (current and current.get("LogsetId")), "Enabled": p["enabled"], "DomainAreaConfigs": _normalized(p["domain_area_configs"]), "InheritDomainTags": p["inherit_domain_tags"]}
        before = None if not current else {"TopicName": current.get("TopicName"), "LogsetId": current.get("LogsetId"), "Enabled": bool(current.get("Enabled")), "DomainAreaConfigs": _normalized(current.get("DomainAreaConfigs")), "InheritDomainTags": bool(current.get("InheritDomainTags"))}
        if before == desired: module.exit_json(changed=False, topic=current)
        diff = maybe_diff(module, before, desired)
        if not module.check_mode:
            if immutable:
                module.sdk_call(client.DeleteClsLogTopic, delete_request(models, p, current["TopicId"], current["LogsetId"])); current = None
            if not current:
                ...
                if domains_changed: module.sdk_call(client.ManageClsTopicDomains, manage_domains_request(models, p, current["TopicId"], current["LogsetId"]))
