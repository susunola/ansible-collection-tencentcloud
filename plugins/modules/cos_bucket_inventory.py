...
def normalize(value, inventory_id):
    ...
    if optional and isinstance(optional.get("Field"), list): optional["Field"] = sorted(optional["Field"])
    return result


def get_inventory(client, bucket, inventory_id):
    try: return normalize(client.get_bucket_inventory(Bucket=bucket, Id=inventory_id), inventory_id)
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "name": {"required": True}, "appid": {}, "inventory_id": {"required": True}, "configuration": {"type": "dict"}}, required_if=[("state", "present", ["configuration"])], supports_check_mode=True)
    cos.require_cos_sdk(module); bucket = cos.bucket_full_name(module.params["name"], cos.resolve_appid(module)); client = cos.create_cos_client(module); inventory_id = module.params["inventory_id"]
    try:
        current = get_inventory(client, bucket, inventory_id); target = normalize(module.params.get("configuration"), inventory_id)
        if module.params["state"] == "absent":
            if current is None: module.exit_json(changed=False, inventory=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode: client.delete_bucket_inventory(Bucket=bucket, Id=inventory_id)
            module.exit_json(changed=True, **(diff or {}), inventory=current if module.check_mode else None)
        if current == target: module.exit_json(changed=False, inventory=current)
        diff = maybe_diff(module, current, target)
        if not module.check_mode: client.put_bucket_inventory(Bucket=bucket, Id=inventory_id, InventoryConfiguration=copy.deepcopy(target))
