...
options:
  amqp_cluster_ids:
    description: Amqp cluster IDs to return. Mutually exclusive with O(filters).
    type: list
    elements: str
  filters:
...
RETURN = r'''
amqp_clusters:
  description: Matching TDMQ amqp clusters.
  returned: always
  type: list
  elements: dict
total_count:
...
def build_request(models, amqp_cluster_ids, filters, offset, limit):
    request = models.DescribeAMQPClustersRequest()
    request.Offset = offset
    request.Limit = limit
    if amqp_cluster_ids:
        request.ClusterIdList = amqp_cluster_ids
    if filters:
        request.Filters = []
...
def run_module():
    ...
    argument_spec.update({
        "amqp_cluster_ids": {"type": "list", "elements": "str"},
        "filters": {"type": "dict", "default": {}},
        "page_size": {"type": "int", "default": 100},
    })
    module = AnsibleModule(
        argument_spec=argument_spec,
        mutually_exclusive=[("amqp_cluster_ids", "filters")],
    ...
    paginator = Paginator(
        ...
        lambda offset, limit: build_request(
            models,
            module.params["amqp_cluster_ids"],
            module.params["filters"],
            offset,
            limit),
        lambda request: sdk_call(module, client.DescribeAMQPClusters, request),
        lambda response: response.ClusterList,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    amqp_clusters = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, amqp_clusters=amqp_clusters, total_count=total_count)
