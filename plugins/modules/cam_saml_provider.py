...
def create_request(models, p):
    ...
    request.Name, request.Description, request.SAMLMetadataDocument = p["name"], p["description"], p["metadata_document"]
    return request
...
def find(module, client, models, name):
    try:
        response = module.sdk_call(client.GetSAMLProvider, get_request(models, name))
...
def comparable(value):
    return {"Name": value.get("Name"), "Description": value.get("Description") or "", "SAMLMetadata": canonical_metadata(value.get("SAMLMetadata"))}


def desired(p):
    return {"Name": p["name"], "Description": p["description"], "SAMLMetadata": canonical_metadata(p["metadata_document"])}
...
def run_module():
    module = TencentCloudModule(
        ...
        required_if=[("state", "present", ["metadata_document"])],
    ...
    try:
        ...
        if p["state"] == "absent":
            if not current:
                module.exit_json(changed=False, saml_provider=None)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.DeleteSAMLProvider, delete_request(models, p["name"]))
        ...
        if before == target:
            module.exit_json(changed=False, saml_provider=current)
        diff = maybe_diff(module, before, target)
        if not module.check_mode:
            module.sdk_call(
                client.UpdateSAMLProvider if current else client.CreateSAMLProvider, update_request(models, p) if current else create_request(models, p)
            )
            current = find(module, client, models, p["name"])
