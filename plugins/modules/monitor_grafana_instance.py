...
options:
  state:
    description:
      - C(present) creates the instance with V(CreateGrafanaInstance) when it does not exist and updates it
        with V(ModifyGrafanaInstance) when it differs. C(absent) deletes it with V(DeleteGrafanaInstance).
    type: str
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.monitor_grafana_instance_info
    description: Gather information about Tencent Cloud Grafana instances.
...
def build_describe(models, iid=None, name=None):
    request = models.DescribeGrafanaInstancesRequest()
    request.InstanceIds = [iid] if iid else None
...
def build_create(models, p):
    ...
    request.InstanceName, request.VpcId, request.SubnetIds = p["name"], p["vpc_id"], p["subnet_ids"]
    request.EnableInternet, request.GrafanaInitPassword = p["enable_internet"], p.get("initial_password")
    request.TagSpecification = _tags(models, p["tags"])
    return request


def build_update(models, iid, name):
    request = models.ModifyGrafanaInstanceRequest()
    request.InstanceId, request.InstanceName = iid, name
    return request


def build_delete(models, iid):
...
def find(module, client, models, iid, name):
    response = module.sdk_call(client.DescribeGrafanaInstances, build_describe(models, iid, name))
    items = list(response.InstanceSet or response.Instances or [])
    matches = [x._serialize(allow_none=True) for x in items if (iid and x.InstanceId == iid) or (not iid and x.InstanceName == name)]
    if len(matches) > 1:
        module.fail_json(msg="Multiple Grafana instances have the requested name", name=name)
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "name": {},
            "vpc_id": {},
            "subnet_ids": {"type": "list", "elements": "str", "default": []},
            "enable_internet": {"type": "bool", "default": False},
            "initial_password": {"no_log": True},
    ...
    try:
        ...
        if p["state"] == "absent":
            ...
            if not module.check_mode:
                module.sdk_call(client.DeleteGrafanaInstance, build_delete(models, current["InstanceId"]))
            module.exit_json(changed=True, **(diff or {}), instance=current if module.check_mode else None)
        if current and current.get("InstanceName") == p["name"]:
            module.exit_json(changed=False, instance=current)
        if not current and (not p["vpc_id"] or not p["subnet_ids"]):
            module.fail_json(msg="vpc_id and subnet_ids are required when creating")
        target = {"InstanceName": p["name"]}
        diff = maybe_diff(module, {"InstanceName": current.get("InstanceName")} if current else None, target)
        if not module.check_mode:
            if current:
                module.sdk_call(client.ModifyGrafanaInstance, build_update(models, current["InstanceId"], p["name"]))
                iid = current["InstanceId"]
            else:
                iid = module.sdk_call(client.CreateGrafanaInstance, build_create(models, p)).InstanceId
