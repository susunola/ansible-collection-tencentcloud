...
def current_users(module, client, models, work_group_id):
    ...
    matches = [x for x in response.WorkGroupSet or [] if x.WorkGroupId == work_group_id]
    if not matches:
...
def membership_request(models, work_group_id, user_ids, add=True):
    info = models.UserIdSetOfWorkGroupId()
    info.WorkGroupId, info.UserIds = work_group_id, user_ids
    r = models.AddUsersToWorkGroupRequest() if add else models.DeleteUsersFromWorkGroupRequest()
    r.AddInfo = info
    return r
...
def run_module():
    module = TencentCloudModule(
        argument_spec={
            ...
            "user_ids": {"type": "list", "elements": "str", "required": True},
    ...
    try:
        target = sorted(set(p["user_ids"]))
        current = current_users(module, client, models, p["work_group_id"])
        if not target and not p["allow_empty"]:
            module.fail_json(msg="set allow_empty=true to authorize removing every DLC work-group member")
        added, removed = delta(current, target)
        if not added and not removed:
            module.exit_json(changed=False, user_ids=current, added=[], removed=[])
        diff = maybe_diff(module, current, target)
        if not module.check_mode:
            if added:
                module.sdk_call(client.AddUsersToWorkGroup, membership_request(models, p["work_group_id"], added, True))
            if removed:
                module.sdk_call(client.DeleteUsersFromWorkGroup, membership_request(models, p["work_group_id"], removed, False))
            wait_for_state(
                module,
                lambda: "ready" if current_users(module, client, models, p["work_group_id"]) == target else "pending",
                ["ready"],
