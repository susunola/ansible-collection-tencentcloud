...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.dcdb_instance_info
    description: Gather information about Tencent Cloud DCDB instances.
...
def describe_request(models, p, offset=0):
    r = models.DescribeDCDBInstancesRequest()
    r.Offset, r.Limit = offset, 100
    r.InstanceIds = [p["instance_id"]] if p.get("instance_id") else None
    r.SearchName = p.get("name") if not p.get("instance_id") else None
    return r
...
def _create_fields(r, p):
    ...
    r.Count, r.InstanceName, r.SecurityGroupIds, r.Ipv6Flag = 1, p["name"], p["security_group_ids"], 1 if p["ipv6"] else 0
    return r


def create_prepaid_request(models, p):
    r = _create_fields(models.CreateDCDBInstanceRequest(), p)
    r.Period, r.AutoRenewFlag = p["period_months"], 1 if p["auto_renew"] else 0
    return r


def create_hour_request(models, p):
    r = _create_fields(models.CreateHourDCDBInstanceRequest(), p)
    r.ShardCpu = p.get("shard_cpu")
    return r


def rename_request(models, instance_id, name):
    r = models.ModifyDBInstanceNameRequest()
    r.InstanceId, r.InstanceName = instance_id, name
    return r


def upgrade_request(models, p, current, add_count=0, hourly=True):
    r = models.UpgradeHourDCDBInstanceRequest() if hourly else models.UpgradeDCDBInstanceRequest()
    r.InstanceId = current["InstanceId"]
    first = (current.get("ShardDetail") or [{}])[0]
    memory, storage = p.get("shard_memory") or first.get("Memory") or current.get("Memory"), p.get("shard_storage") or first.get("Storage") or current.get(
        "Storage"
    )
    if add_count:
        cfg = models.AddShardConfig()
        cfg.ShardCount, cfg.ShardMemory, cfg.ShardStorage = add_count, memory, storage
        r.UpgradeType, r.AddShardConfig = "ADD", cfg
    else:
        cfg = models.ExpandShardConfig()
        cfg.ShardInstanceIds = [x["ShardInstanceId"] for x in current.get("ShardDetail") or []]
        cfg.ShardMemory, cfg.ShardStorage = memory, storage
        cfg.ShardNodeCount = p.get("shard_node_count") or first.get("NodeCount") or current.get("NodeCount")
        r.UpgradeType, r.ExpandShardConfig = "EXPAND", cfg
    return r


def isolate_request(models, instance_id, hourly=True):
    r = models.IsolateHourDCDBInstanceRequest() if hourly else models.IsolateDCDBInstanceRequest()
    r.InstanceIds = [instance_id]
    return r


def destroy_request(models, instance_id, hourly=True):
    r = models.DestroyHourDCDBInstanceRequest() if hourly else models.DestroyDCDBInstanceRequest()
...
def _hourly(current):
    return str(current.get("Paymode", "")).lower() in ("0", "postpaid", "postpaid_by_hour", "hour")
...
def run_module():
    spec = {
        ...
        "subnet_id": {},
        "db_version": {},
        "shard_memory": {"type": "int"},
        "shard_storage": {"type": "int"},
        "shard_node_count": {"type": "int", "choices": [2, 3]},
    ...
    module.require_sdk()
    models, cm = _load()
    client = module.create_client(cm.DcdbClient, "dcdb.tencentcloudapi.com")
    try:
        ...
        if p["state"] == "absent":
            ...
            if p["purge"]:
                if status not in (-1, 5):
                    module.fail_json(msg="purge requires an already isolated DCDB instance", current_status=status)
                diff = maybe_diff(module, current, None)
                if not module.check_mode:
                    module.sdk_call(client.DestroyHourDCDBInstance if hourly else client.DestroyDCDBInstance, destroy_request(models, instance_id, hourly))
                module.exit_json(changed=True, **(diff or {}), instance=None)
            if status in (-1, 5):
                module.exit_json(changed=False, instance=current)
            diff = maybe_diff(module, current, None)
            if not module.check_mode:
                module.sdk_call(client.IsolateHourDCDBInstance if hourly else client.IsolateDCDBInstance, isolate_request(models, instance_id, hourly))
        ...
        if not current:
            ...
            if missing:
                module.fail_json(msg="creation parameters are required for a new DCDB instance", missing=missing)
            target = {
                "InstanceName": p["name"],
                "VpcId": p["vpc_id"],
                "SubnetId": p["subnet_id"],
                "DbVersionId": p["db_version"],
                "Memory": p["shard_memory"],
                "Storage": p["shard_storage"],
                "ShardCount": p.get("shard_count") or 2,
                "NodeCount": p.get("shard_node_count") or 2,
            }
            diff = maybe_diff(module, None, target)
            if not module.check_mode:
                hourly = p["charge_type"] == "POSTPAID_BY_HOUR"
                response = module.sdk_call(
                    client.CreateHourDCDBInstance if hourly else client.CreateDCDBInstance,
                    create_hour_request(models, p) if hourly else create_prepaid_request(models, p),
                )
                ids = response.InstanceIds or []
                p["instance_id"] = ids[0]
                _wait(module, client, models, p, [2])
                current = find(module, client, models, p)
            module.exit_json(changed=True, **(diff or {}), instance=current if not module.check_mode else target)
        immutable = {"VpcId": p.get("vpc_id"), "SubnetId": p.get("subnet_id"), "DbVersionId": p.get("db_version")}
        drift = {k: (current.get(k), v) for k, v in immutable.items() if v is not None and current.get(k) != v}
        if drift:
            module.fail_json(msg="DCDB network placement and database version are immutable", immutable_drift=drift)
        desired = {
            "InstanceName": p.get("name") or current.get("InstanceName"),
            "Memory": p.get("shard_memory") if p.get("shard_memory") is not None else current.get("Memory"),
            "Storage": p.get("shard_storage") if p.get("shard_storage") is not None else current.get("Storage"),
            "ShardCount": p.get("shard_count") if p.get("shard_count") is not None else current.get("ShardCount"),
            "NodeCount": p.get("shard_node_count") if p.get("shard_node_count") is not None else current.get("NodeCount"),
        }
        before = {k: current.get(k) for k in desired}
        if desired["ShardCount"] < before["ShardCount"] or desired["Memory"] < before["Memory"] or desired["Storage"] < before["Storage"]:
            module.fail_json(msg="DCDB shard count, memory and storage cannot be reduced")
        if before == desired:
            module.exit_json(changed=False, instance=current)
        diff = maybe_diff(module, before, desired)
        instance_id, hourly = current["InstanceId"], _hourly(current)
        if not module.check_mode:
            if before["InstanceName"] != desired["InstanceName"]:
                module.sdk_call(client.ModifyDBInstanceName, rename_request(models, instance_id, desired["InstanceName"]))
            if any(before[k] != desired[k] for k in ("Memory", "Storage", "NodeCount")):
                module.sdk_call(client.UpgradeHourDCDBInstance if hourly else client.UpgradeDCDBInstance, upgrade_request(models, p, current, hourly=hourly))
            ...
            if add_count:
                module.sdk_call(
                    client.UpgradeHourDCDBInstance if hourly else client.UpgradeDCDBInstance, upgrade_request(models, p, current, add_count, hourly)
                )
                _wait(module, client, models, p, [2])
            current = find(module, client, models, p)
        module.exit_json(changed=True, **(diff or {}), instance=current)
