...
def describe_request(models, p, offset=0):
    request = models.DescribeUserRequest(); request.InstanceId, request.SearchWord, request.Offset, request.Limit = p["instance_id"], p["name"], offset, 100; return request


def create_request(models, p):
    request = models.CreateUserRequest(); request.InstanceId, request.Name, request.Password = p["instance_id"], p["name"], p["password"]; return request


def password_request(models, p):
    request = models.ModifyPasswordRequest(); request.InstanceId, request.Name = p["instance_id"], p["name"]
    request.Password, request.PasswordNew = p["current_password"], p["password"]; return request


def delete_request(models, p):
    request = models.DeleteUserRequest(); request.InstanceId, request.Name = p["instance_id"], p["name"]; return request


def find(module, client, models, p):
    offset = 0
    while True:
        response = module.sdk_call(client.DescribeUser, describe_request(models, p, offset)); result = response.Result; items = list(result.Users or [])
        for item in items:
...
def run_module():
    module = TencentCloudModule(argument_spec={"state": {"choices": ["present", "absent"], "default": "present"}, "instance_id": {"required": True}, "name": {"required": True}, "password": {"no_log": True}, "rotate_password": {"type": "bool", "default": False}, "current_password": {"no_log": True}}, supports_check_mode=True)
    p = module.params
    if p["rotate_password"] and (not p["password"] or not p["current_password"]): module.fail_json(msg="password and current_password are required when rotate_password=true")
    ...
    try:
        ...
        if current and not p["rotate_password"]: module.exit_json(changed=False, user=current)
        diff = maybe_diff(module, None if not current else current, {"Name": p["name"]})
        if not module.check_mode:
            if not current:
                if not p["password"]: module.fail_json(msg="password is required when creating a CKafka user")
                module.sdk_call(client.CreateUser, create_request(models, p))
            else: module.sdk_call(client.ModifyPassword, password_request(models, p))
            current = find(module, client, models, p)
