...
def describe_request(models,p,offset=0):
    r=models.DescribeJobSavepointRequest(); r.JobId,r.WorkSpaceId=p["job_id"],p["workspace_id"]; r.Offset,r.Limit,r.RecordTypes=offset,100,[1]; return r
def list_savepoints(module,client,models,p):
    offset=0; values=[]; seen=set()
    while True:
        response=module.sdk_call(client.DescribeJobSavepoint,describe_request(models,p,offset)); page=response.Savepoint or []
        for item in list(response.RunningSavepoint or [])+list(page):
            value=item._serialize(allow_none=True); identity=value.get("SerialId") or value.get("Id")
            if identity not in seen: seen.add(identity); values.append(value)
        offset+=len(page)
        if not page or offset>=int(response.TotalNumber or 0): break
    return values
...
def trigger_request(models,p): r=models.TriggerJobSavepointRequest(); r.JobId,r.WorkSpaceId,r.Description=p["job_id"],p["workspace_id"],p["description"]; return r
def run_module():
    spec={"job_id":{"required":True},"workspace_id":{"required":True},"description":{"required":True},"force":{"type":"bool","default":False},"wait":{"type":"bool","default":True}}
    ...
    try:
        ...
        if current and not p["force"]: module.exit_json(changed=False,savepoint=current,savepoint_id=current.get("SerialId"),savepoint_path=current.get("Path"))
        target={"Description":p["description"],"Status":3,"RecordType":1}
        if module.check_mode: module.exit_json(changed=True,savepoint=target,savepoint_id=None,savepoint_path=None)
        response=module.sdk_call(client.TriggerJobSavepoint,trigger_request(models,p))
        if not response.SavepointTrigger: module.fail_json(msg="Oceanus rejected the savepoint trigger",error=response.ErrorMsg)
        savepoint_id=response.SavepointId
        def poll():
            values=list_savepoints(module,client,models,p); value=find_by_id(values,savepoint_id) or matching(values,p["description"],(1,3,4,5))
            return ((value or {}).get("Status"),(value or {}).get("Description"),value)
        current=wait_for_task(module,poll,timeout=p["waiter_timeout"],delay=p["waiter_delay"],success_statuses=(1,),failure_statuses=(4,5)) if p["wait"] else find_by_id(list_savepoints(module,client,models,p),savepoint_id)
        module.exit_json(changed=True,savepoint=current or target,savepoint_id=savepoint_id,savepoint_path=(current or {}).get("Path") or response.FinalSavepointPath)
