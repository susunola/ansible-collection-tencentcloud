...
RETURN = r'''
standard_engine_resource_groups:
  description: Matching DLC standard engine resource groups.
  returned: always
  type: list
  elements: dict
total_count:
...
def run_module():
    ...
    paginator = Paginator(
        ...
        lambda request: sdk_call(module, client.DescribeStandardEngineResourceGroups, request),
        lambda response: response.UserEngineResourceGroupInfos,
        lambda response: response.Total,
    )
    item_set, total_count = paginator.fetch_all()
    standard_engine_resource_groups = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, standard_engine_resource_groups=standard_engine_resource_groups,
