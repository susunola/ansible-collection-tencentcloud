#!/usr/bin/python
# -*- coding: utf-8 -*-
# Generated by scripts/generate_info_modules.py -- regenerate instead of editing.
# Copyright: (c) 2026, Tencent Cloud Ansible Collection Contributors
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)
from __future__ import absolute_import, division, print_function

__metaclass__ = type

DOCUMENTATION = r'''
---
module: ame_ktv_robot_info
short_description: Gather information about Tencent Cloud AME ktv robots
version_added: "0.8.0"
description: Returns AME ktv robots visible in a Tencent Cloud region.
options:
  ktv_robot_ids:
    description: Ktv robot IDs to return.
    type: list
    elements: str
  page_size:
    description: Number of results requested per API call.
    type: int
    default: 100
extends_documentation_fragment: tencentcloud.cloud.tencentcloud
author: Tencent Cloud Ansible Collection Contributors (@susunola)
'''

EXAMPLES = r'''
- name: List all ktv robots
  tencentcloud.cloud.ame_ktv_robot_info:
    region: ap-guangzhou

- name: Find ktv robots by ID
  tencentcloud.cloud.ame_ktv_robot_info:
    region: ap-guangzhou
    ktv_robot_ids: [x-xxxxxxxx]
'''

RETURN = r'''
ktv_robots:
  description: Matching AME ktv robots.
  returned: always
  type: list
  elements: dict
total_count:
  description: Number of ktv robots reported by the API.
  returned: always
  type: int
'''

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.tencentcloud.cloud.plugins.module_utils.paging import Paginator
from ansible_collections.tencentcloud.cloud.plugins.module_utils.tencentcloud import (
    create_client_profile, create_credential, sdk_call, serialize_sdk_object,
    tencentcloud_argument_spec,
)


def build_request(models, ktv_robot_ids, offset, limit):
    request = models.DescribeKTVRobotsRequest()
    request.Offset = offset
    request.Limit = limit
    if ktv_robot_ids:
        request.RobotIds = ktv_robot_ids
    return request


def run_module():
    argument_spec = tencentcloud_argument_spec()
    argument_spec.update({
        "ktv_robot_ids": {"type": "list", "elements": "str"},
        "page_size": {"type": "int", "default": 100},
    })
    module = AnsibleModule(
        argument_spec=argument_spec,
        supports_check_mode=True,
    )
    try:
        from tencentcloud.ame.v20190916 import models, ame_client
    except ImportError:
        module.fail_json(msg="The tencentcloud-sdk-python-ame package is required.")

    client = ame_client.AmeClient(
        create_credential(module), module.params["region"],
        create_client_profile(module, "ame.tencentcloudapi.com"),
    )
    paginator = Paginator(
        module.params["page_size"],
        lambda offset, limit: build_request(models, module.params["ktv_robot_ids"], offset, limit),
        lambda request: sdk_call(module, client.DescribeKTVRobots, request),
        lambda response: response.KTVRobotInfoSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    ktv_robots = [serialize_sdk_object(item) for item in item_set]
    module.exit_json(changed=False, ktv_robots=ktv_robots, total_count=total_count)


def main():
    run_module()


if __name__ == "__main__":
    main()
