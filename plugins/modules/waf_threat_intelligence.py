...
def describe_request(models): return models.DescribeWafThreatenIntelligenceRequest()
def update_request(models, p):
    detail = models.WafThreatenIntelligenceDetails(); detail.Tags, detail.DefenseStatus = sorted(p["tags"]), 1 if p["enabled"] else 0
    request = models.ModifyWafThreatenIntelligenceRequest(); request.WafThreatenIntelligenceDetails = detail; return request
def normalize(item):
...
def run_module():
    module = TencentCloudModule(argument_spec={"enabled": {"type": "bool", "default": True}, "tags": {"type": "list", "elements": "str", "default": []}}, supports_check_mode=True)
    p = module.params; module.require_sdk(); models, cm = _load(); client = module.create_client(cm.WafClient, "waf.tencentcloudapi.com")
    try:
        response = module.sdk_call(client.DescribeWafThreatenIntelligence, describe_request(models)); before = normalize(response.WafThreatenIntelligenceDetails)
        target = {"Tags": sorted(p["tags"]), "DefenseStatus": 1 if p["enabled"] else 0}
        if before == target: module.exit_json(changed=False, threat_intelligence=before)
        diff = maybe_diff(module, before, target)
        if not module.check_mode: module.sdk_call(client.ModifyWafThreatenIntelligence, update_request(models, p))
