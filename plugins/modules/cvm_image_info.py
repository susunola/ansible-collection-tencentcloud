...
options:
  image_ids:
    description: Image IDs to return. Mutually exclusive with O(filters).
    type: list
    elements: str
  filters:
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cvm_image
    description: Manage Tencent Cloud CVM custom images.
...
def build_request(models, image_ids, filters, offset, limit):
...
def run_module():
    ...
    argument_spec.update({
        "image_ids": {"type": "list", "elements": "str"},
        "filters": {"type": "dict", "default": {}},
        "page_size": {"type": "int", "default": 100},
    })
    module = AnsibleModule(
        argument_spec=argument_spec,
        mutually_exclusive=[("image_ids", "filters")],
    ...
    paginator = Paginator(
        ...
        lambda offset, limit: build_request(
            models,
            module.params["image_ids"],
            module.params["filters"],
            offset,
            limit),
        lambda request: sdk_call(module, client.DescribeImages, request),
        lambda response: response.ImageSet,
        lambda response: response.TotalCount,
    )
    item_set, total_count = paginator.fetch_all()
    images = [serialize_sdk_object(item) for item in item_set]
