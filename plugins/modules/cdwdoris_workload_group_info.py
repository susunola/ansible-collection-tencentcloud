...
options:
  instance_id:
    description: Sets the C(InstanceId) field of V(DescribeWorkloadGroup); omit it to leave that field unset.
    type: str
...
attributes:
  ...
  idempotent:
    ...
    support: full
seealso:
  - module: susunola.tencentcloud.cdwdoris_workload_group
    description: Manage Tencent Cloud CDW Doris workload groups.
...
RETURN = r'''
workload_groups:
  description: Matching CDWDORIS workload groups.
  returned: always
  type: list
  elements: dict
  sample:
...
def build_request(models, instance_id, offset, limit):
    # DescribeWorkloadGroup returns the full list in one call and is not
...
def run_module():
    ...
    response = sdk_call(module, client.DescribeWorkloadGroup, request)
    items = response.WorkloadGroups or []
    workload_groups = [serialize_sdk_object(item) for item in items]
    module.exit_json(changed=False, workload_groups=workload_groups,
                     total_count=len(workload_groups), request_id=response.RequestId)
