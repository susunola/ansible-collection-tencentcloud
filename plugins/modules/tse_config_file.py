...
def describe_request(models,p): r=models.DescribeConfigFileRequest(); r.InstanceId,r.Namespace,r.Group,r.Name=p["instance_id"],p["namespace"],p["group"],p["name"]; return r
def _model(models,value): x=models.ConfigFile(); x.from_json_string(json.dumps(value)); return x
def request(cls,models,p,value): r=cls(); r.InstanceId=p["instance_id"]; r.ConfigFile=_model(models,value); return r
def delete_request(models,p,current): r=models.DeleteConfigFilesRequest(); r.InstanceId,r.Namespace,r.Group,r.Name,r.Id=p["instance_id"],p["namespace"],p["group"],p["name"],current.get("Id"); return r
...
def desired(p):
    mapping={"content":"Content","format":"Format","comment":"Comment","tags":"Tags","supported_client":"ConfigFileSupportedClient","persistent":"ConfigFilePersistent","encrypted":"Encrypted","encrypt_algo":"EncryptAlgo"}; value={"Name":p["name"],"Namespace":p["namespace"],"Group":p["group"]}
...
def writable(value):
    keys=("Name","Namespace","Group","Content","Format","Comment","Tags","ConfigFileSupportedClient","ConfigFilePersistent","Encrypted","EncryptAlgo")
    return {key:value.get(key) for key in keys if key in value}


def run_module():
    module=TencentCloudModule(argument_spec={"state":{"choices":["present","absent"],"default":"present"},"instance_id":{"required":True},"namespace":{"required":True},"group":{"required":True},"name":{"required":True},"content":{},"format":{},"comment":{},"tags":{"type":"list","elements":"dict"},"supported_client":{"type":"int"},"persistent":{"type":"dict"},"encrypted":{"type":"bool"},"encrypt_algo":{}},supports_check_mode=True); p=module.params; module.require_sdk(); models,cm=_load(); client=module.create_client(cm.TseClient,"tse.tencentcloudapi.com")
    try:
        current=find(module,client,models,p)
        if p["state"]=="absent":
            if not current: module.exit_json(changed=False,config_file=None)
            diff=maybe_diff(module,current,None)
            if not module.check_mode: module.sdk_call(client.DeleteConfigFiles,delete_request(models,p,current))
            module.exit_json(changed=True,**(diff or {}),config_file=None)
        target=desired(p)
        if not current:
            missing=[key for key in ("content","format") if p.get(key) is None]
            if missing: module.fail_json(msg="content and format are required for a new TSE configuration file",missing=missing)
        if current and contains(current,target): module.exit_json(changed=False,config_file=current)
        diff=maybe_diff(module,current,target)
        if not module.check_mode:
            payload=writable(dict(current or {},**target)); api=client.ModifyConfigFiles if current else client.CreateConfigFile; cls=models.ModifyConfigFilesRequest if current else models.CreateConfigFileRequest
