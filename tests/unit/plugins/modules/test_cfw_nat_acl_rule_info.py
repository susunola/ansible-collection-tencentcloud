...
def test_build_request_sets_pagination():
    request = cfw_nat_acl_rule_info.build_request(FakeModels, "sample", "sample", "sample", "sample", "sample", {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = cfw_nat_acl_rule_info.build_request(FakeModels, "sample", "sample", "sample", "sample", "sample", {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cfw_nat_acl_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cfw_nat_acl_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cfw_nat_acl_rule_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cfw_nat_acl_rule_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["nat_ac_rules"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cfw_nat_acl_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cfw_nat_acl_rule_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cfw_nat_acl_rule_info, "sdk_call", failing_sdk_call)
