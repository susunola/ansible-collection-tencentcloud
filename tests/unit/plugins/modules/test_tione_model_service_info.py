...
class Object: pass
class Models: DescribeModelServiceRequest = DescribeModelServiceGroupRequest = DescribeModelServiceGroupsRequest = Filter = TagFilter = Object


def params():
    return {"service_id": "s1", "service_group_id": "g1", "project_id": "p1", "filters": {"Status": "Normal", "ModelVersionId": ["mv1"]}, "tag_filters": {"env": "prod"}, "order_field": "UpdateTime", "order": "DESC", "page_size": 2, "max_pages": 5}
...
class Item:
    ...
    def _serialize(self, allow_none=True): return {"ServiceGroupId": self.value}
class Response:
    def __init__(self, values, total, global_total, request_id): self.ServiceGroups, self.TotalCount, self.GlobalTotalCount, self.RequestId = [Item(x) for x in values], total, global_total, request_id
class Client:
    def __init__(self): self.responses = [Response(["g1", "g2"], 3, 8, "r1"), Response(["g3"], 3, 8, "r2")]
    def DescribeModelServiceGroups(self, request): return self.responses.pop(0)
class Module:
