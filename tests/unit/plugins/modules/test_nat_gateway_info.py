...
from ansible_collections.susunola.tencentcloud.plugins.modules import nat_gateway_info
...
def test_build_request_maps_ids_and_int_pagination():
    request = nat_gateway_info.build_request(FakeModels, ["nat-123"], {}, 20, 100)
...
def test_build_request_sorts_filters():
    request = nat_gateway_info.build_request(
        FakeModels, [], {"vpc-id": ["vpc-123"], "nat-gateway-name": ["gw"]}, 0, 100)
    assert [(item.Name, item.Values) for item in request.Filters] == [
        ("nat-gateway-name", ["gw"]), ("vpc-id", ["vpc-123"]),
    ]
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(nat_gateway_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(nat_gateway_info, "create_credential", lambda module: object())
    monkeypatch.setattr(nat_gateway_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        nat_gateway_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["nat_gateways"]] == ["a", "b", "c"]
