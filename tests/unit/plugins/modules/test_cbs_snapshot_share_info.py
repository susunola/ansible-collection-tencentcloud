...
from ansible_collections.susunola.tencentcloud.plugins.modules import cbs_snapshot_share_info
...
class FakeModels:
    DescribeSnapshotSharePermissionRequest = FakeRequest
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cbs_snapshot_share_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cbs_snapshot_share_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cbs_snapshot_share_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cbs_snapshot_share_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({"region": "ap-guangzhou", "snapshot_id": "sample"})
    monkeypatch.setattr(cbs_snapshot_share_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cbs_snapshot_share_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cbs_snapshot_share_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cbs_snapshot_share_info, "sdk_call", failing_sdk_call)
