...
from ansible_collections.susunola.tencentcloud.plugins.modules import svp_saving_plan_coverage_info
...
class FakeModels:
    DescribeSavingPlanCoverageRequest = FakeRequest
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.svp.v20240125")
    service.models = FakeModels
    service.svp_client = types.SimpleNamespace(SvpClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.svp",
                        types.ModuleType("tencentcloud.svp"))
    monkeypatch.setitem(sys.modules, "tencentcloud.svp.v20240125", service)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(svp_saving_plan_coverage_info, "create_credential", lambda module: object())
    monkeypatch.setattr(svp_saving_plan_coverage_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        svp_saving_plan_coverage_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(svp_saving_plan_coverage_info, "create_credential", lambda module: object())
    monkeypatch.setattr(svp_saving_plan_coverage_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(svp_saving_plan_coverage_info, "sdk_call", failing_sdk_call)
