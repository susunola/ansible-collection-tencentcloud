...
from ansible_collections.susunola.tencentcloud.plugins.modules import waf_anti_info_leak_rule_info
...
class FakeModels:
    DescribeAntiInfoLeakageRulesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = waf_anti_info_leak_rule_info.build_request(FakeModels, "sample", 200, 100)
...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.waf",
                        types.ModuleType("tencentcloud.waf"))
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(waf_anti_info_leak_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(waf_anti_info_leak_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(waf_anti_info_leak_rule_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        waf_anti_info_leak_rule_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                domain="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["anti_info_leak_rules"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(waf_anti_info_leak_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(waf_anti_info_leak_rule_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(waf_anti_info_leak_rule_info, "sdk_call", failing_sdk_call)
