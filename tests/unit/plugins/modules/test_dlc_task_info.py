...
from ansible_collections.susunola.tencentcloud.plugins.modules import dlc_task_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeTaskListRequest = FakeRequest


def test_build_request_sets_pagination():
    request = dlc_task_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = dlc_task_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.HouseIds == ["x-1"]


def test_build_request_sorts_filters():
    request = dlc_task_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(dlc_task_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(dlc_task_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dlc_task_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        dlc_task_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                task_ids=None, filters={}, page_size=2)
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(dlc_task_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dlc_task_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(dlc_task_info, "sdk_call", failing_sdk_call)
