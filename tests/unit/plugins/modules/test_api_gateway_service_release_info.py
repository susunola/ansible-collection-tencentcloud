...
class FakeModels:
    DescribeServiceEnvironmentReleaseHistoryRequest = FakeRequest


def test_build_request_sets_pagination():
    request = api_gateway_service_release_info.build_request(FakeModels, "sample", "sample", 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(TotalCount=total_count, VersionList=items)
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                service_id="sample", environment_name="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["service_environment_release_histories"]] == ["a", "b", "c"]
