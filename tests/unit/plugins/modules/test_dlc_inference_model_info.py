...
class Object: pass
class Models: ListInferenceModelsRequest = Filter = SortField = Object


def params(): return {"start_time": 10, "end_time": 20, "parameter_size_min": 7.0, "parameter_size_max": 70.0, "filters": {"name": "llm"}, "sort_fields": [{"field": "CreateTime", "order": "DESC"}], "page_size": 2, "max_pages": 5}
...
class Response:
    def __init__(self, values, pages, request_id): self.Items, self.TotalPages, self.Total, self.RequestId = [Item(x) for x in values], pages, 3, request_id
class Client:
    def __init__(self): self.responses = [Response(["a", "b"], 2, "r1"), Response(["c"], 2, "r2")]
    def ListInferenceModels(self, request): return self.responses.pop(0)
class Module:
