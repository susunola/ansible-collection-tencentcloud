...
from ansible_collections.susunola.tencentcloud.plugins.modules import dnspod_domain_info
...
def test_build_request_sets_pagination():
    request = dnspod_domain_info.build_request(FakeModels, "sample", 1, "sample", 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(dnspod_domain_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(dnspod_domain_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dnspod_domain_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        dnspod_domain_info.run_module()
    return fake
...
def test_run_module_paginates_until_short_page(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                type="sample", group_id=1, keyword="sample", page_size=2)
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(dnspod_domain_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dnspod_domain_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(dnspod_domain_info, "sdk_call", failing_sdk_call)
