...
class Models:
    DescribeDataSourcesRequest = Filter = TagFilter = Object
...
class Response:
    def __init__(self, values, total, request_id):
        self.DataSourceInfos, self.TotalCount, self.RequestId = [Item(x) for x in values], total, request_id


class Client:
    def __init__(self):
        self.responses = [Response(["d1", "d2"], 3, "r1"), Response(["d3"], 3, "r2")]
