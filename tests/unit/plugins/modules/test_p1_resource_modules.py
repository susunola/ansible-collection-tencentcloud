...
from ansible_collections.susunola.tencentcloud.plugins.module_utils.monitor import _policy_converged
...
def test_tcr_repository_create_request(models):
    request = tcr_repository.build_create_request(
        models,
        {
            "registry_id": "tcr-x",
            "namespace": "prod",
...
def test_cam_detach_role_can_use_name(models):
    request = cam_policy_attachment.build_mutation_request(
        models,
        {
            "target_type": "role",
            "target_id": None,
...
def test_cam_attachment_detection():
    ...
    client = SimpleNamespace(ListAttachedUserPolicies=lambda request: SimpleNamespace(List=[SimpleNamespace(PolicyId=9)]))
...
def test_cam_group_membership_requests(models, present, request_name):
    request = cam_group_membership.build_mutation_request(
        models,
        {
            "group_id": 7,
            "sub_uin": 1001,
...
def test_kms_alias_lookup_uses_exact_match_and_paginates(models):
    ...
    def list_keys(request):
        ...
        items = [Metadata(Alias="production-copy", KeyId="key-copy")] if request.Offset == 0 else [Metadata(Alias="production", KeyId="key-exact")]
...
@pytest.mark.parametrize(
    "enabled,request_name,rotate_days",
    [
...
def test_kms_waiter_normalizes_pending_delete(monkeypatch, models):
    ...
    result = kms_key.wait_for_key_state(module, None, models, "key-x", ("PendingDelete",))
    assert result["KeyState"] == "Pending_Delete"


@pytest.mark.parametrize(
    "enabled,request_name",
    [
...
def test_kms_rotation_waiter(monkeypatch, models):
    ...
    assert kms_key_rotation.wait_for_rotation(module, None, models, "key-x", True, 90)["enabled"]


def test_monitor_create_request_maps_conditions(models):
    request = monitor_alarm_policy.build_create_request(
        models,
        {
            "module": "monitor",
            "name": "cpu-high",
...
def test_monitor_condition_update_preserves_unmanaged_event(models):
    request = monitor_alarm_policy.build_condition_request(
        models,
        {
            "module": "monitor",
            "name": None,
...
def test_monitor_policy_convergence():
    ...
    assert _policy_converged(current, desired)
...
def test_private_dns_zone_create_request(models):
    request = private_dns_zone.build_create_request(
        models,
        {
...
def test_private_dns_waiters(monkeypatch, models):
    ...
    assert private_dns_zone.wait_for_zone(module, None, models, "zone-x", {"Remark": "ready", "VpcSet": []}) == zone
    assert private_dns_record.wait_for_record(module, None, models, "zone-x", "record-x", {"RecordValue": "10.0.0.8"}) == record
...
def test_tke_install_request(models):
    request = tke_addon.build_install_request(
        models,
        {
            "cluster_id": "cls-x",
            "name": "cbs",
...
def test_tke_waiter_surfaces_failed_phase(monkeypatch, models):
    ...
    module = SimpleNamespace(
        params={"waiter_timeout": 10, "waiter_delay": 0},
        fail_json=lambda **kwargs: (_ignored for _ignored in ()).throw(RuntimeError(kwargs["reason"])),
    )
