...
from ansible_collections.susunola.tencentcloud.plugins.modules import network_acl_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeNetworkAclsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = network_acl_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = network_acl_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.NetworkAclIds == ["x-1"]


def test_build_request_sorts_filters():
    request = network_acl_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(network_acl_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(network_acl_info, "create_credential", lambda module: object())
    monkeypatch.setattr(network_acl_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        network_acl_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                network_acl_ids=None, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["network_acls"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(network_acl_info, "create_credential", lambda module: object())
    monkeypatch.setattr(network_acl_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(network_acl_info, "sdk_call", failing_sdk_call)
