...
def test_build_request_sets_pagination():
    request = ssm_secret_info.build_request(FakeModels, 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(ssm_secret_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(ssm_secret_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ssm_secret_info, "create_client_profile", lambda module, endpoint: object())
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["secrets"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(ssm_secret_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ssm_secret_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(ssm_secret_info, "sdk_call", failing_sdk_call)
