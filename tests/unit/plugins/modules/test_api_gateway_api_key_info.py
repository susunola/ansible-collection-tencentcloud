...
from ansible_collections.susunola.tencentcloud.plugins.modules import api_gateway_api_key_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeApiKeysStatusRequest = FakeRequest


def test_build_request_sets_pagination():
    request = api_gateway_api_key_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = api_gateway_api_key_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(ApiKeySet=items, TotalCount=total_count)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(api_gateway_api_key_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(api_gateway_api_key_info, "create_credential", lambda module: object())
    monkeypatch.setattr(api_gateway_api_key_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        api_gateway_api_key_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["api_keys"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(api_gateway_api_key_info, "create_credential", lambda module: object())
    monkeypatch.setattr(api_gateway_api_key_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(api_gateway_api_key_info, "sdk_call", failing_sdk_call)
