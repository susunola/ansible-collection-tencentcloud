...
class Client:
    ...
    def GetModelConfig(self, request):
        self.calls.append(("config", request.ModelUid, request.ModelVersion))
        return Response({"ModelName": "m", "ConfigJson": '{"layers": 12}', "RequestId": "rc"})

    def GetModelFiles(self, request):
        self.calls.append(("files", request.ModelUid, request.ModelVersion))
        return Response({"Files": [{"Name": "weights"}], "RequestId": "rf"})

    def GetModelReadme(self, request):
        self.calls.append(("readme", request.ModelUid, request.ModelVersion))
        return Response({"Readme": "# Model", "RequestId": "rr"})
...
def test_read_combines_artifacts_and_parses_valid_config_json():
    client = Client()
    values, request_ids = read(Module(), client, Models, params())
...
def test_read_honours_independent_artifact_selection_and_invalid_json():
    p = params()
    p.update(include_files=False, include_readme=False)
    client = Client()
