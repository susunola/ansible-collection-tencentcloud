...
from ansible_collections.susunola.tencentcloud.plugins.modules import scf_alias_info
...
class FakeModels:
    ListAliasesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = scf_alias_info.build_request(FakeModels, "sample", "sample", "sample", 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(scf_alias_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(scf_alias_info, "create_credential", lambda module: object())
    monkeypatch.setattr(scf_alias_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        scf_alias_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                function_name="sample", namespace="sample", function_version="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["aliases"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({
        ...
        "namespace": "sample",
        "function_version": "sample",
        "page_size": 2,
    })
    monkeypatch.setattr(scf_alias_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(scf_alias_info, "create_credential", lambda module: object())
    monkeypatch.setattr(scf_alias_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(scf_alias_info, "sdk_call", failing_sdk_call)
