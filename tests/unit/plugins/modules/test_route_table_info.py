...
def test_build_request_maps_ids_and_string_pagination():
    request = route_table_info.build_request(FakeModels, ["rtb-123"], {}, 20, 100)
...
def test_build_request_sorts_filters():
    request = route_table_info.build_request(FakeModels, [], {"vpc-id": ["vpc-1"], "route-table-name": ["app"]}, 0, 100)
...
def test_build_request_wraps_scalar_filter_values():
    request = route_table_info.build_request(FakeModels, [], {"association.main": "true"}, 0, 100)
