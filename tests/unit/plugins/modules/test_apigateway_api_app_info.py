...
from ansible_collections.susunola.tencentcloud.plugins.modules import apigateway_api_app_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeApiAppsStatusRequest = FakeRequest


def test_build_request_sets_pagination():
    request = apigateway_api_app_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = apigateway_api_app_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(ApiAppSet=items, TotalCount=total_count)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(apigateway_api_app_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(apigateway_api_app_info, "create_credential", lambda module: object())
    monkeypatch.setattr(apigateway_api_app_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        apigateway_api_app_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["api_apps"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(apigateway_api_app_info, "create_credential", lambda module: object())
    monkeypatch.setattr(apigateway_api_app_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(apigateway_api_app_info, "sdk_call", failing_sdk_call)
