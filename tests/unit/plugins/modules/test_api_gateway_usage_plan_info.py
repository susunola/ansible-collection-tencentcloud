...
from ansible_collections.susunola.tencentcloud.plugins.modules import api_gateway_usage_plan_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeUsagePlansStatusRequest = FakeRequest


def test_build_request_sets_pagination():
    request = api_gateway_usage_plan_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = api_gateway_usage_plan_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(TotalCount=total_count, UsagePlanStatusSet=items)
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["usage_plans"]] == ["a", "b", "c"]
