...
from ansible_collections.susunola.tencentcloud.plugins.modules import config_compliance_pack_info
...
def test_build_request_sets_pagination():
    request = config_compliance_pack_info.build_request(FakeModels, 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(config_compliance_pack_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(config_compliance_pack_info, "create_credential", lambda module: object())
    monkeypatch.setattr(config_compliance_pack_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        config_compliance_pack_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["compliance_packs"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(config_compliance_pack_info, "create_credential", lambda module: object())
    monkeypatch.setattr(config_compliance_pack_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(config_compliance_pack_info, "sdk_call", failing_sdk_call)
