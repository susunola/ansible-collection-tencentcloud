...
# ---------------------------------------------------------------------------
# legacy helper regression tests (folded from test_dlc_spark_job.py)
# ---------------------------------------------------------------------------
...
class LegacyModels(object):
    DescribeSparkAppJobRequest = LegacyObject
    CreateSparkAppRequest = LegacyObject
    ModifySparkAppRequest = LegacyObject
    DeleteSparkAppRequest = LegacyObject
...
def test_describe_prefers_stable_id_over_name():
    assert mod.describe_request(LegacyModels, "daily-etl").JobName == "daily-etl"
    request = mod.describe_request(LegacyModels, "ignored", "job-1")
...
def test_modify_includes_stable_job_id():
    request = mod.make_request(LegacyModels, legacy_params(), update=True, job_id="job-1")
...
def test_drift_maps_api_read_names_and_ignores_omitted_values():
    ...
    assert mod.drift(legacy_params(), current) == {"JobDriverSize": ("small", "medium")}


def test_delete_uses_exact_job_name():
    assert mod.delete_request(LegacyModels, "daily-etl").AppName == "daily-etl"
