...
from ansible_collections.susunola.tencentcloud.plugins.modules import cvm_disaster_recover_group_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeDisasterRecoverGroupsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = cvm_disaster_recover_group_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = cvm_disaster_recover_group_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.DisasterRecoverGroupIds == ["x-1"]


def test_build_request_sorts_filters():
    request = cvm_disaster_recover_group_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.DisasterRecoverGroupSet = items
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cvm_disaster_recover_group_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_disaster_recover_group_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_disaster_recover_group_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cvm_disaster_recover_group_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                disaster_recover_group_ids=None, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["disaster_recover_groups"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({
        "region": "ap-guangzhou",
        "disaster_recover_group_ids": None,
        "filters": {},
        "page_size": 2,
    })
    monkeypatch.setattr(cvm_disaster_recover_group_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_disaster_recover_group_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_disaster_recover_group_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cvm_disaster_recover_group_info, "sdk_call", failing_sdk_call)
