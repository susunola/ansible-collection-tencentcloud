...
from ansible_collections.susunola.tencentcloud.plugins.modules import cvm_image_share_info
...
class FakeModels:
    DescribeImageSharePermissionRequest = FakeRequest
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cvm_image_share_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_image_share_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_image_share_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cvm_image_share_info.run_module()
    return fake


def test_run_module_returns_full_list(monkeypatch):
    client = FakeClient([FakeResponse([FakeItem("a"), FakeItem("b")])])
    fake = _run(monkeypatch, client, region="ap-guangzhou", image_id="sample")
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({"region": "ap-guangzhou", "image_id": "sample"})
    monkeypatch.setattr(cvm_image_share_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_image_share_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_image_share_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cvm_image_share_info, "sdk_call", failing_sdk_call)
