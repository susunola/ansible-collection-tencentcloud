...
def params():
    return {"data_engine_name": "spark-prod", "states": ["idle", "busy"], "keyword": "analyst", "engine_generation": "supersql",
            "sort_fields": ["create_time"], "ascending": True, "page_size": 2}
...
class Response:
    def __init__(self, values, total, request_id): self.Sessions, self.TotalElements, self.RequestId = values, total, request_id
...
class Client:
    def __init__(self): self.pages = [Response([Item("a"), Item("b")], 3, "r1"), Response([Item("c")], 3, "r2")]; self.offsets = []
    def DescribeNotebookSessions(self, request): self.offsets.append(request.Offset); return self.pages.pop(0)


def test_read_fetches_all_pages_and_returns_final_request_id():
    client = Client(); sessions, total, request_id = read(Module(), client, Models, params())
