...
class FakeModels:
    ListDeviceSnapshotsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = iss_device_snapshot_info.build_request(FakeModels, 200, 100)
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.iss.v20230517")
    service.models = FakeModels
    service.iss_client = types.SimpleNamespace(IssClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.iss",
                        types.ModuleType("tencentcloud.iss"))
    monkeypatch.setitem(sys.modules, "tencentcloud.iss.v20230517", service)


def _run(monkeypatch, client, **params):
    _inject_sdk(monkeypatch, client)
    fake = FakeModule(params)
    monkeypatch.setattr(iss_device_snapshot_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(iss_device_snapshot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(iss_device_snapshot_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        iss_device_snapshot_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["device_snapshots"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(iss_device_snapshot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(iss_device_snapshot_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(iss_device_snapshot_info, "sdk_call", failing_sdk_call)
