...
class Object: pass
class Models: CreateUsersRequest = DeleteUsersRequest = ModifyUserPrivilegesRequest = ResetUsersPasswordRequest = ResetUserPasswordInfo = DescribeUsersRequest = DescribeUserPrivilegesRequest = User = Object
P = {"instance_id": "db1", "username": "report", "host": "10.%", "password": "secret", "encrypted_password": None, "description": "reporting", "global_privileges": ["SELECT"]}
...
class Response:
    def __init__(self, users=None, privileges=None, request_id="r1"): self.Users, self.Privileges, self.RequestId = users, privileges, request_id
class Client:
    def DescribeUsers(self, request): return Response([Item({"UserName": "report", "Host": "10.%"}), Item({"UserName": "report", "Host": "%"})])
    def DescribeUserPrivileges(self, request): return Response(privileges=["UPDATE", "SELECT"], request_id="r2")
class Module:
