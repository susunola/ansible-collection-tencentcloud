...
from ansible_collections.susunola.tencentcloud.plugins.modules import clb_target_group_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeTargetGroupListRequest = FakeRequest


def test_build_request_sets_pagination():
    request = clb_target_group_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = clb_target_group_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.TargetGroupIds == ["x-1"]


def test_build_request_sorts_filters():
    request = clb_target_group_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.clb",
                        types.ModuleType("tencentcloud.clb"))
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(clb_target_group_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(clb_target_group_info, "create_credential", lambda module: object())
    monkeypatch.setattr(clb_target_group_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        clb_target_group_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(clb_target_group_info, "create_credential", lambda module: object())
    monkeypatch.setattr(clb_target_group_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(clb_target_group_info, "sdk_call", failing_sdk_call)
