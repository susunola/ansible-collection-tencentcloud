...
from ansible_collections.susunola.tencentcloud.plugins.modules import chdfs_file_system_info
...
class FakeModels:
    DescribeFileSystemsRequest = FakeRequest
...
class FakeResponse:
    ...
    def __init__(self, items, total_count, next_token, done):
        ...
        self.NextFileSystemIdMarker = next_token
        self.IsOver = done
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(chdfs_file_system_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(chdfs_file_system_info, "create_credential", lambda module: object())
    monkeypatch.setattr(chdfs_file_system_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        chdfs_file_system_info.run_module()
    return fake


def test_build_request_maps_parameters():
    request = chdfs_file_system_info.build_request(FakeModels, None, 100)
    assert isinstance(request, FakeRequest)
    # The cursor stays off the first request; it is only set once a page returns one.
    assert not hasattr(request, "FileSystemIdMarker")
...
def test_run_module_follows_the_cursor_to_the_last_page(monkeypatch):
    ...
    assert [getattr(request, "FileSystemIdMarker", None)
...
def test_run_module_stops_when_the_cursor_is_empty(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou")
    payload = fake.exit_payload
    assert payload["file_systems"] == [{"Marker": "a"}]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(chdfs_file_system_info, "create_credential", lambda module: object())
    monkeypatch.setattr(chdfs_file_system_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(chdfs_file_system_info, "sdk_call", failing_sdk_call)
