...
class Request(object):
    def __init__(self): self.JobId = None
...
class Client(object):
    ...
    def CreateMigrateCheckJob(self, request): self.calls.append(("check", request)); return object()
    def StartMigrateJob(self, request): self.calls.append(("start", request)); return object()
    def PauseMigrateJob(self, request): self.calls.append(("pause", request)); return object()
    def ResumeMigrateJob(self, request): self.calls.append(("resume", request)); return object()
    def StopMigrateJob(self, request): self.calls.append(("stop", request)); return object()
    def CompleteMigrateJob(self, request): self.calls.append(("complete", request)); return object()
...
def test_desired_configuration_maps_and_sorts_tags():
    value = desired({"job_id": "dts-1", "run_mode": "immediate", "source": {"Region": "a"},
                     "destination": {"Region": "b"}, "migration_options": {"MigrateType": "full"},
                     "auto_retry_minutes": 0, "name": None, "expected_run_time": None,
                     "tags": {"z": "2", "a": "1"}})
...
def test_check_start_uses_new_api_name():
    client = Client(); start(Module(), client, Models, "dts-1")
...
def test_actions_build_sdk_requests():
    for action in DONE:
        client = Client()
        execute(Module(), client, Models, {"job_id": "dts-1", "action": action,
                                           "resume_option": "normal", "complete_mode": "waitForSync"})
