...
from ansible_collections.susunola.tencentcloud.plugins.modules import organization_member_policy_info
...
class FakeModels:
    DescribeOrganizationMemberPoliciesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = organization_member_policy_info.build_request(FakeModels, 1, "sample", 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(organization_member_policy_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(organization_member_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(organization_member_policy_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        organization_member_policy_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                member_uin=1, search_key="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["member_policies"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(organization_member_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(organization_member_policy_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(organization_member_policy_info, "sdk_call", failing_sdk_call)
