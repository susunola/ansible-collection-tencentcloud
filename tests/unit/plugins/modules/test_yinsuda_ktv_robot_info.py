...
from ansible_collections.susunola.tencentcloud.plugins.modules import yinsuda_ktv_robot_info
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.yinsuda.v20220527")
    service.models = FakeModels
    service.yinsuda_client = types.SimpleNamespace(YinsudaClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.yinsuda",
                        types.ModuleType("tencentcloud.yinsuda"))
    monkeypatch.setitem(sys.modules, "tencentcloud.yinsuda.v20220527", service)


def _run(monkeypatch, client, **params):
    _inject_sdk(monkeypatch, client)
    fake = FakeModule(params)
    monkeypatch.setattr(yinsuda_ktv_robot_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(yinsuda_ktv_robot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(yinsuda_ktv_robot_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        yinsuda_ktv_robot_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(yinsuda_ktv_robot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(yinsuda_ktv_robot_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(yinsuda_ktv_robot_info, "sdk_call", failing_sdk_call)
