...
from ansible_collections.susunola.tencentcloud.plugins.modules import cfs_auto_snapshot_policy_info
...
def test_build_request_sets_pagination():
    request = cfs_auto_snapshot_policy_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = cfs_auto_snapshot_policy_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.AutoSnapshotPolicies = items
...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.cfs",
                        types.ModuleType("tencentcloud.cfs"))
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cfs_auto_snapshot_policy_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cfs_auto_snapshot_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cfs_auto_snapshot_policy_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cfs_auto_snapshot_policy_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cfs_auto_snapshot_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cfs_auto_snapshot_policy_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cfs_auto_snapshot_policy_info, "sdk_call", failing_sdk_call)
