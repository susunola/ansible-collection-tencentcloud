...
class Object: pass
class Models: ListSecretsRequest = TagFilter = GetSecretValueRequest = DescribeRotationDetailRequest = Object


def params(): return {"page_size": 2, "max_pages": 5, "order": "ascending", "state_filter": 1, "search_name": "prod", "tag_filters": {"env": "prod"}, "secret_type": 0, "product_name": None, "encrypt_type": 1, "instance_id": None}
...
class Item:
    ...
    def _serialize(self, allow_none=True): return {"SecretName": self.name}
class Response:
    def __init__(self, names, total, request_id): self.SecretMetadatas, self.TotalCount, self.RequestId = [Item(x) for x in names], total, request_id
class Client:
    def __init__(self): self.values = [Response(["a", "b"], 3, "r1"), Response(["c"], 3, "r2")]
    def ListSecrets(self, request): return self.values.pop(0)
class Module:
...
def test_sensitive_value_and_rotation_requests_keep_exact_identity():
    ...
    value = value_request(Models, p); rotation = rotation_request(Models.DescribeRotationDetailRequest, "prod/db")
