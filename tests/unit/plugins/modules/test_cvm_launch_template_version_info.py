...
from ansible_collections.susunola.tencentcloud.plugins.modules import cvm_launch_template_version_info
...
class FakeModels:
    DescribeLaunchTemplateVersionsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = cvm_launch_template_version_info.build_request(FakeModels, "sample", 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.LaunchTemplateVersionSet = items
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cvm_launch_template_version_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_launch_template_version_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_launch_template_version_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cvm_launch_template_version_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                launch_template_id="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["launch_template_versions"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({
        "region": "ap-guangzhou",
        "launch_template_id": "sample",
        "page_size": 2,
    })
    monkeypatch.setattr(cvm_launch_template_version_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_launch_template_version_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_launch_template_version_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cvm_launch_template_version_info, "sdk_call", failing_sdk_call)
