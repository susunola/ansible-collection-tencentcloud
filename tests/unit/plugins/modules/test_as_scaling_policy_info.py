...
from ansible_collections.susunola.tencentcloud.plugins.modules import as_scaling_policy_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeScalingPoliciesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = as_scaling_policy_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = as_scaling_policy_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.AutoScalingPolicyIds == ["x-1"]


def test_build_request_sorts_filters():
    request = as_scaling_policy_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(as_scaling_policy_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(as_scaling_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(as_scaling_policy_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        as_scaling_policy_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(as_scaling_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(as_scaling_policy_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(as_scaling_policy_info, "sdk_call", failing_sdk_call)
