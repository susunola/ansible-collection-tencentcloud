...
from ansible_collections.susunola.tencentcloud.plugins.modules.tdmysql_database_object_info import (
    database_request,
    object_request,
    read_databases,
    read_objects,
...
class Models:
    DescribeUserPrivilegesRequest = ModifyUserPrivilegesRequest = User = DatabasePrivileges = TablePrivileges = DescribeDatabasesRequest = (
...
class Client:
    def __init__(self):
        self.db = [Response([Item({"DbName": "a"}), Item({"DbName": "b"})], 3, request_id="r1"), Response([Item({"DbName": "c"})], 3, request_id="r2")]

    def DescribeDatabases(self, request):
        return self.db.pop(0)

    def DescribeDatabaseObjects(self, request):
        return Response(tables=[Item({"Table": "orders"})], request_id="ro")
