...
from ansible_collections.susunola.tencentcloud.plugins.modules import monitor_prometheus_grafana_binding_info
...
class FakeModels:
    DescribePrometheusInstancesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = monitor_prometheus_grafana_binding_info.build_request(FakeModels, ["sample"], 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(monitor_prometheus_grafana_binding_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(monitor_prometheus_grafana_binding_info, "create_credential", lambda module: object())
    monkeypatch.setattr(monitor_prometheus_grafana_binding_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        monitor_prometheus_grafana_binding_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                instance_ids=["sample"], page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["prometheus_instances"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(monitor_prometheus_grafana_binding_info, "create_credential", lambda module: object())
    monkeypatch.setattr(monitor_prometheus_grafana_binding_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(monitor_prometheus_grafana_binding_info, "sdk_call", failing_sdk_call)
