...
from ansible_collections.susunola.tencentcloud.plugins.modules import tse_governance_service_info
...
class FakeModels:
    DescribeGovernanceServicesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = tse_governance_service_info.build_request(FakeModels, 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(tse_governance_service_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(tse_governance_service_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tse_governance_service_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        tse_governance_service_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["governance_services"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(tse_governance_service_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tse_governance_service_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(tse_governance_service_info, "sdk_call", failing_sdk_call)
