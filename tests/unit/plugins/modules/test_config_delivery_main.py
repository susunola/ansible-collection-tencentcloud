"""Unit tests for the config_delivery write module (run_module flows).

Drives ``run_module()`` against an in-memory fake Config client whose
update operation mutates a delivery store so post-write describes
converge immediately.

There is no delete: ``enabled=false`` simply flips the delivery
``Status`` to 0 via the same update call (disable, not destroy).

Scenario matrix:

* matching delivery configuration is idempotent
* name/prefix/type/content-type drift triggers an update
* disabling (enabled=false) flips Status to 0 via UpdateConfigDeliver
...
from ansible_collections.susunola.tencentcloud.plugins.modules import config_delivery as mod
...
class FakeConfigClient(object):
    """In-memory Config client holding one delivery configuration."""
...
def test_no_drift_is_idempotent(monkeypatch):
    ...
    assert result["delivery"]["DeliverName"] == "compliance-archive"
    assert [c for c, unused in fake.calls] == ["DescribeConfigDeliver"]
...
def test_name_drift_updates(monkeypatch):
    ...
    assert ops == ["DescribeConfigDeliver", "UpdateConfigDeliver", "DescribeConfigDeliver"]


def test_prefix_drift_updates(monkeypatch):
    fake = FakeConfigClient(delivery=_delivery())
    _make_module(monkeypatch, fake)
    _args(prefix="audit")
    result = run(mod.run_module)
    assert result["changed"] is True
    assert result["delivery"]["DeliverPrefix"] == "audit"


def test_content_type_drift_updates(monkeypatch):
    fake = FakeConfigClient(delivery=_delivery())
    _make_module(monkeypatch, fake)
    _args(content_type=1)
    result = run(mod.run_module)
    assert result["changed"] is True
    assert result["delivery"]["DeliverContentType"] == 1
...
def test_disable_flips_status_to_zero(monkeypatch):
    ...
    update_call = [r for c, r in fake.calls if c == "UpdateConfigDeliver"][0]
...
def test_check_mode_is_dry_run(monkeypatch):
    ...
    result = run(mod.run_module)
    assert result["changed"] is True
    assert fake.delivery["DeliverName"] == "compliance-archive"
    assert [c for c, unused in fake.calls] == ["DescribeConfigDeliver"]
