...
def test_build_request_sets_pagination():
    request = config_rule_info.build_request(FakeModels, 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(config_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(config_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(config_rule_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        config_rule_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["config_rules"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(config_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(config_rule_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(config_rule_info, "sdk_call", failing_sdk_call)
