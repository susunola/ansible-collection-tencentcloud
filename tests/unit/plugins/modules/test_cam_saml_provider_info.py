...
from ansible_collections.susunola.tencentcloud.plugins.modules import cam_saml_provider_info
...
class FakeModels:
    ListSAMLProvidersRequest = FakeRequest
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cam_saml_provider_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cam_saml_provider_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cam_saml_provider_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cam_saml_provider_info.run_module()
    return fake
...
def test_run_module_returns_full_list(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["saml_providers"]] == ["a", "b"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cam_saml_provider_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cam_saml_provider_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cam_saml_provider_info, "sdk_call", failing_sdk_call)
