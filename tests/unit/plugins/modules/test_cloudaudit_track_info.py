...
from ansible_collections.susunola.tencentcloud.plugins.modules import cloudaudit_track_info
...
def test_build_request_sets_pagination():
    request = cloudaudit_track_info.build_request(FakeModels, 200, 100)
...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.cloudaudit",
                        types.ModuleType("tencentcloud.cloudaudit"))
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cloudaudit_track_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cloudaudit_track_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cloudaudit_track_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cloudaudit_track_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["audit_tracks"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cloudaudit_track_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cloudaudit_track_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cloudaudit_track_info, "sdk_call", failing_sdk_call)
