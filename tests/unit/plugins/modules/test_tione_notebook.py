...
def params():
    return {"name": "nb", "charge_type": "POSTPAID_BY_HOUR", "resource_conf": {"Cpu": 4}, "log_enable": True, "root_access": False, "auto_stopping": True, "direct_internet_access": False, "resource_group_id": None, "vpc_id": "vpc-1", "subnet_id": "subnet-1", "volume_source_type": "CLOUD_PREMIUM", "volume_size_gb": 100, "volume_source_cfs": None, "volume_source_goosefs": None, "log_config": None, "lifecycle_script_id": None, "default_code_repo_id": None, "additional_code_repo_ids": ["r2", "r1"], "automatic_stop_time": 4, "tags": [{"TagKey": "b", "TagValue": "2"}], "data_configs": None, "image_info": {"ImageId": "img-1"}, "image_type": "SYSTEM", "ssh_config": None, "description": "workbench"}
...
def test_modify_excludes_unmodifiable_goosefs_but_maps_mutable_fields():
    p = params(); p["volume_source_goosefs"] = {"Id": "g1"}
...
def test_drift_separates_mutable_and_immutable_fields_and_normalizes_tags():
    p = params(); current = {key: value for key, value in normalize({"Name": "nb", "ChargeType": "PREPAID", "ResourceConf": {"Cpu": 2}, "Tags": p["tags"]}).items()}
