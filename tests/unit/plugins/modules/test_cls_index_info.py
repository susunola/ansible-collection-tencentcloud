...
from ansible_collections.susunola.tencentcloud.plugins.modules import cls_index_info
...
class FakeModels:
    DescribeIndexRequest = FakeRequest


def test_build_request_maps_parameters():
    request = cls_index_info.build_request(FakeModels, "sample", 0, 0)
    assert request.TopicId == "sample"
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cls_index_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cls_index_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cls_index_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cls_index_info.run_module()
    return fake


def test_run_module_returns_the_single_result(monkeypatch):
    client = FakeClient(FakeResponse({"TopicId": "sample"}))
    fake = _run(monkeypatch, client, region="ap-guangzhou", topic_id="sample")
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert payload["index"] == {"TopicId": "sample"}
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({"region": "ap-guangzhou", "topic_id": "sample"})
    monkeypatch.setattr(cls_index_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cls_index_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cls_index_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cls_index_info, "sdk_call", failing_sdk_call)
