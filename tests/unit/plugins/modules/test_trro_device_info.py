...
from ansible_collections.susunola.tencentcloud.plugins.modules import trro_device_info
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.trro.v20220325")
    service.models = FakeModels
    service.trro_client = types.SimpleNamespace(TrroClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.trro",
                        types.ModuleType("tencentcloud.trro"))
    monkeypatch.setitem(sys.modules, "tencentcloud.trro.v20220325", service)


def _run(monkeypatch, client, **params):
    _inject_sdk(monkeypatch, client)
    fake = FakeModule(params)
    monkeypatch.setattr(trro_device_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(trro_device_info, "create_credential", lambda module: object())
    monkeypatch.setattr(trro_device_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        trro_device_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(trro_device_info, "create_credential", lambda module: object())
    monkeypatch.setattr(trro_device_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(trro_device_info, "sdk_call", failing_sdk_call)
