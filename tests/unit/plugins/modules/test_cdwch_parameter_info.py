...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cdwch_parameter_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cdwch_parameter_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cdwch_parameter_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cdwch_parameter_info.run_module()
    return fake


def test_run_module_returns_full_list(monkeypatch):
    client = FakeClient([FakeResponse([FakeItem("a"), FakeItem("b")])])
    fake = _run(monkeypatch, client, region="ap-guangzhou", instance_id="sample", search_config_name="sample")
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["parameters"]] == ["a", "b"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({"region": "ap-guangzhou", "instance_id": "sample", "search_config_name": "sample"})
    monkeypatch.setattr(cdwch_parameter_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cdwch_parameter_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cdwch_parameter_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cdwch_parameter_info, "sdk_call", failing_sdk_call)
