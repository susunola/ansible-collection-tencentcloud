...
from ansible_collections.susunola.tencentcloud.plugins.modules import gaap_proxy_info
...
def test_build_request_maps_ids_and_int_pagination():
    request = gaap_proxy_info.build_request(FakeModels, ["link-123"], {}, 20, 100)
    assert request.ProxyIds == ["link-123"]
    assert request.Offset == 20
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = gaap_proxy_info.build_request(
        FakeModels, [], {"ProjectId": ["0"], "ProxyName": ["proxy"]}, 0, 100)
    assert [(item.Name, item.Values) for item in request.Filters] == [
        ("ProjectId", ["0"]), ("ProxyName", ["proxy"]),
    ]
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(gaap_proxy_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(gaap_proxy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(gaap_proxy_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        gaap_proxy_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                proxy_ids=None, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["proxies"]] == ["a", "b", "c"]
