...
from ansible_collections.susunola.tencentcloud.plugins.modules import api_gateway_usage_plan_key_binding_info
...
class FakeModels:
    DescribeUsagePlanSecretIdsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = api_gateway_usage_plan_key_binding_info.build_request(FakeModels, "sample", 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(AccessKeyList=items, TotalCount=total_count)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(api_gateway_usage_plan_key_binding_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(api_gateway_usage_plan_key_binding_info, "create_credential", lambda module: object())
    monkeypatch.setattr(api_gateway_usage_plan_key_binding_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        api_gateway_usage_plan_key_binding_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(api_gateway_usage_plan_key_binding_info, "create_credential", lambda module: object())
    monkeypatch.setattr(api_gateway_usage_plan_key_binding_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(api_gateway_usage_plan_key_binding_info, "sdk_call", failing_sdk_call)
