"""Unit tests for the cfw_internet_acl_rule write module (helpers + run_module).

Covers the create / drift-update / delete flows of
``plugins/modules/cfw_internet_acl_rule.py`` with an in-memory fake Cloud
Firewall client whose write operations mutate the rule store, so the module's
post-write ``find_rule`` refetch converges immediately. Rules are matched by
``rule_uuid`` or by ``description`` (both across the paged DescribeAclRule
...
from ansible_collections.susunola.tencentcloud.plugins.modules import cfw_internet_acl_rule as mod
...
class FakeCfwClient(object):
    ...
    Stores API-shaped rule dicts. DescribeAclRule pages over the store
    ...
    def AddAclRule(self, request):
        self._record("AddAclRule", request)
        uuids = []
    ...
    def RemoveAclRule(self, request):
        self._record("RemoveAclRule", request)
...
def test_find_rule_multiple_matches_fails(monkeypatch):
    ...
    assert "Multiple internet ACL rules matched" in exc.value.args[0]["msg"]
...
def test_find_rule_paginates_until_match(monkeypatch):
    ...
    list_calls = [c for c in fake.calls if c[0] == "DescribeAclRule"]
...
def test_present_creates_rule(monkeypatch):
    ...
    assert names.count("AddAclRule") == 1
    add = [c for c in fake.calls if c[0] == "AddAclRule"][0][1]
...
def test_present_noop_returns_unchanged(monkeypatch):
    ...
    names = [c[0] for c in fake.calls]
    assert "AddAclRule" not in names
...
def test_present_drift_triggers_update(monkeypatch):
    ...
    names = [c[0] for c in fake.calls]
    assert names.count("ModifyAclRule") == 1
    update = [c for c in fake.calls if c[0] == "ModifyAclRule"][0][1]
...
def test_present_rule_uuid_referencing_missing_is_create(monkeypatch):
    ...
    assert "AddAclRule" in [c[0] for c in fake.calls]
...
def test_check_mode_create_is_dry_run(monkeypatch):
    ...
    assert not any("AddAclRule" == c[0] for c in fake.calls)
...
def test_absent_removes_rule(monkeypatch):
    ...
    names = [c[0] for c in fake.calls]
    assert names.count("RemoveAclRule") == 1
    remove = [c for c in fake.calls if c[0] == "RemoveAclRule"][0][1]
...
def test_absent_not_found_is_noop(monkeypatch):
    ...
    assert result["changed"] is False
    assert result["rule"] is None
    assert not any("RemoveAclRule" == c[0] for c in fake.calls)
