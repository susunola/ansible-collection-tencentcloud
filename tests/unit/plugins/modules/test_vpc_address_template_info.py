...
from ansible_collections.susunola.tencentcloud.plugins.modules import vpc_address_template_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeAddressTemplatesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = vpc_address_template_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == "200"
    assert request.Limit == "100"


def test_build_request_sorts_filters():
    request = vpc_address_template_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(vpc_address_template_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(vpc_address_template_info, "create_credential", lambda module: object())
    monkeypatch.setattr(vpc_address_template_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        vpc_address_template_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["address_templates"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(vpc_address_template_info, "create_credential", lambda module: object())
    monkeypatch.setattr(vpc_address_template_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(vpc_address_template_info, "sdk_call", failing_sdk_call)
