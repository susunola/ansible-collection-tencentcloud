...
from ansible_collections.susunola.tencentcloud.plugins.modules import organization_member_identity_info
...
class FakeModels:
    DescribeOrganizationMemberAuthIdentitiesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = organization_member_identity_info.build_request(FakeModels, 1, 1, 200, 100)
...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.organization",
                        types.ModuleType("tencentcloud.organization"))
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(organization_member_identity_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(organization_member_identity_info, "create_credential", lambda module: object())
    monkeypatch.setattr(organization_member_identity_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        organization_member_identity_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                member_uin=1, identity_id=1, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["member_auth_identities"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(organization_member_identity_info, "create_credential", lambda module: object())
    monkeypatch.setattr(organization_member_identity_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(organization_member_identity_info, "sdk_call", failing_sdk_call)
