...
class Object: pass
class Models: DescribeNotebooksRequest = Filter = TagFilter = Object


def params():
    return {"project_id": "p1", "filters": {"Status": ["Running"], "Name": "nb"}, "tag_filters": {"env": "prod"}, "order_field": "UpdateTime", "order": "DESC", "page_size": 2, "max_pages": 5}
...
class Response:
    def __init__(self, values, total, request_id): self.NotebookSet, self.TotalCount, self.RequestId = [Item(x) for x in values], total, request_id
class Client:
    def __init__(self): self.responses = [Response(["n1", "n2"], 3, "r1"), Response(["n3"], 3, "r2")]
    def DescribeNotebooks(self, request): return self.responses.pop(0)
class Module:
