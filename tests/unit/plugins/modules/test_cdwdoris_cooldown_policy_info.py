...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cdwdoris_cooldown_policy_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cdwdoris_cooldown_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cdwdoris_cooldown_policy_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cdwdoris_cooldown_policy_info.run_module()
    return fake
...
def test_run_module_returns_full_list(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["cool_down_policies"]] == ["a", "b"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cdwdoris_cooldown_policy_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cdwdoris_cooldown_policy_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cdwdoris_cooldown_policy_info, "sdk_call", failing_sdk_call)
