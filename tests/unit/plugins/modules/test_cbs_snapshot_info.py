...
from ansible_collections.susunola.tencentcloud.plugins.modules import cbs_snapshot_info
...
def test_build_request_sets_pagination():
    request = cbs_snapshot_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = cbs_snapshot_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.SnapshotIds == ["x-1"]


def test_build_request_sorts_filters():
    request = cbs_snapshot_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cbs_snapshot_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cbs_snapshot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cbs_snapshot_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cbs_snapshot_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cbs_snapshot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cbs_snapshot_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cbs_snapshot_info, "sdk_call", failing_sdk_call)
