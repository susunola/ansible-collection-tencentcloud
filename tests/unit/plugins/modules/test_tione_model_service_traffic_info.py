...
from ansible_collections.susunola.tencentcloud.plugins.modules import tione_model_service_traffic_info
...
def test_build_request_sets_pagination():
    request = tione_model_service_traffic_info.build_request(FakeModels, "sample", "sample", "sample", {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = tione_model_service_traffic_info.build_request(FakeModels, "sample", "sample", "sample", {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _inject_sdk(monkeypatch, client):
    ...
    service.tione_client = types.SimpleNamespace(TioneClient=lambda *args: client)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(tione_model_service_traffic_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(tione_model_service_traffic_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tione_model_service_traffic_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        tione_model_service_traffic_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                ti_project_id="sample", order="sample", order_field="sample", filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["model_service_groups"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(tione_model_service_traffic_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tione_model_service_traffic_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(tione_model_service_traffic_info, "sdk_call", failing_sdk_call)
