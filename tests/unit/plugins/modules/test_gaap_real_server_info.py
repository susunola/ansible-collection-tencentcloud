...
from ansible_collections.susunola.tencentcloud.plugins.modules import gaap_real_server_info
...
def test_build_request_sets_pagination():
    request = gaap_real_server_info.build_request(FakeModels, 1, "sample", {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = gaap_real_server_info.build_request(FakeModels, 1, "sample", {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(gaap_real_server_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(gaap_real_server_info, "create_credential", lambda module: object())
    monkeypatch.setattr(gaap_real_server_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        gaap_real_server_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                project_id=1, search_value="sample", filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["real_servers"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(gaap_real_server_info, "create_credential", lambda module: object())
    monkeypatch.setattr(gaap_real_server_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(gaap_real_server_info, "sdk_call", failing_sdk_call)
