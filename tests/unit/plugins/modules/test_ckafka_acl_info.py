...
from ansible_collections.susunola.tencentcloud.plugins.modules import ckafka_acl_info
...
def test_build_request_sets_pagination():
    request = ckafka_acl_info.build_request(FakeModels, "sample", 1, "sample", 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(AclList=items, TotalCount=total_count)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(ckafka_acl_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(ckafka_acl_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ckafka_acl_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        ckafka_acl_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                instance_id="sample", resource_type=1, resource_name="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["acls"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(ckafka_acl_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ckafka_acl_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(ckafka_acl_info, "sdk_call", failing_sdk_call)
