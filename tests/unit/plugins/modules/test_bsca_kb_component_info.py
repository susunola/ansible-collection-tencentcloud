...
from ansible_collections.susunola.tencentcloud.plugins.modules import bsca_kb_component_info
...
class FakeModels:
    SearchKBComponentRequest = FakeRequest
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.bsca.v20210811")
    service.models = FakeModels
    service.bsca_client = types.SimpleNamespace(BscaClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.bsca",
                        types.ModuleType("tencentcloud.bsca"))
    monkeypatch.setitem(sys.modules, "tencentcloud.bsca.v20210811", service)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(bsca_kb_component_info, "create_credential", lambda module: object())
    monkeypatch.setattr(bsca_kb_component_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        bsca_kb_component_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(bsca_kb_component_info, "create_credential", lambda module: object())
    monkeypatch.setattr(bsca_kb_component_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(bsca_kb_component_info, "sdk_call", failing_sdk_call)
