...
from ansible_collections.susunola.tencentcloud.plugins.modules import tione_dataset_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeDatasetsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = tione_dataset_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = tione_dataset_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.DatasetIds == ["x-1"]


def test_build_request_sorts_filters():
    request = tione_dataset_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(tione_dataset_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(tione_dataset_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tione_dataset_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        tione_dataset_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                dataset_ids=None, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["datasets"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(tione_dataset_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tione_dataset_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(tione_dataset_info, "sdk_call", failing_sdk_call)
