from ansible_collections.susunola.tencentcloud.plugins.modules.dlc_inference_service import (
    create_request,
    get_request,
    list_request,
    readable_conflict,
...
class Models:
    CreateInferenceServiceRequest = GetInferenceServiceRequest = ListInferenceServicesRequest = StopInferenceServiceRequest = RestartInferenceServiceRequest = (
        Tag
    ) = Model
...
def test_create_payload_normalizes_json_and_tags():
    p = params()
    request = create_request(Models, p)
