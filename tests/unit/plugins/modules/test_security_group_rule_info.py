...
from ansible_collections.susunola.tencentcloud.plugins.modules import security_group_rule_info
...
def test_build_request_maps_parameters():
    request = security_group_rule_info.build_request(FakeModels, "sample", {}, 0, 0)
    assert request.SecurityGroupId == "sample"


def test_build_request_sorts_filters():
    request = security_group_rule_info.build_request(FakeModels, "sample", {"b-name": ["v1"], "a-name": ["v2"]}, 0, 0)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(security_group_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(security_group_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(security_group_rule_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        security_group_rule_info.run_module()
    return fake


def test_run_module_returns_the_single_result(monkeypatch):
    client = FakeClient(FakeResponse({"SecurityGroupId": "sample"}))
    fake = _run(monkeypatch, client, region="ap-guangzhou", security_group_id="sample", filters={})
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert payload["policies"] == {"SecurityGroupId": "sample"}
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({"region": "ap-guangzhou", "security_group_id": "sample", "filters": {}})
    monkeypatch.setattr(security_group_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(security_group_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(security_group_rule_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(security_group_rule_info, "sdk_call", failing_sdk_call)
