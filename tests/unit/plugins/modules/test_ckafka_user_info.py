...
from ansible_collections.susunola.tencentcloud.plugins.modules import ckafka_user_info
...
class FakeModels:
    DescribeUserRequest = FakeRequest


def test_build_request_sets_pagination():
    request = ckafka_user_info.build_request(FakeModels, "sample", 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.Result = types.SimpleNamespace(TotalCount=total_count, Users=items)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(ckafka_user_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(ckafka_user_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ckafka_user_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        ckafka_user_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(ckafka_user_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ckafka_user_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(ckafka_user_info, "sdk_call", failing_sdk_call)
