...
from ansible_collections.susunola.tencentcloud.plugins.modules import tse_cloud_native_api_gateway_llm_model_api_info
...
def test_build_request_sets_pagination():
    request = tse_cloud_native_api_gateway_llm_model_api_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = tse_cloud_native_api_gateway_llm_model_api_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(tse_cloud_native_api_gateway_llm_model_api_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(tse_cloud_native_api_gateway_llm_model_api_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tse_cloud_native_api_gateway_llm_model_api_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        tse_cloud_native_api_gateway_llm_model_api_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(tse_cloud_native_api_gateway_llm_model_api_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tse_cloud_native_api_gateway_llm_model_api_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(tse_cloud_native_api_gateway_llm_model_api_info, "sdk_call", failing_sdk_call)
