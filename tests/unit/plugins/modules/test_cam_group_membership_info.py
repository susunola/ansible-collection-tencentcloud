...
from ansible_collections.susunola.tencentcloud.plugins.modules import cam_group_membership_info
...
class FakeModels:
    ListGroupsForUserRequest = FakeRequest


def test_build_request_sets_pagination():
    request = cam_group_membership_info.build_request(FakeModels, 1, 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cam_group_membership_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cam_group_membership_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cam_group_membership_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cam_group_membership_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cam_group_membership_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cam_group_membership_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cam_group_membership_info, "sdk_call", failing_sdk_call)
