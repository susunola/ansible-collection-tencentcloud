...
class FakeSsmClient(object):
    ...
    def UpdateSecret(self, request):
        ...
        for secret in self.secrets:
            if secret["SecretName"] == request.SecretName:
                if hasattr(request, "SecretString"):
                    secret["SecretString"] = request.SecretString
                if hasattr(request, "SecretBinary"):
                    secret["SecretBinary"] = request.SecretBinary
        return SimpleNamespace()

    def GetSecretValue(self, request):
        self._record("GetSecretValue", request)
        secret = next(item for item in self.secrets if item["SecretName"] == request.SecretName)
        return SimpleNamespace(SecretString=secret.get("SecretString"), SecretBinary=secret.get("SecretBinary"))
...
def test_present_value_and_description_drift_updates_both(client):
    ...
    assert any(name == "UpdateDescription" for name, request in client.calls)


def test_present_description_only_drift_updates_description(client):
    client.secrets = [_secret(Description="stale")]
