...
from ansible_collections.susunola.tencentcloud.plugins.modules import dlc_user_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeUsersRequest = FakeRequest


def test_build_request_sets_pagination():
    request = dlc_user_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = dlc_user_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(dlc_user_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(dlc_user_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dlc_user_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        dlc_user_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(dlc_user_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dlc_user_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(dlc_user_info, "sdk_call", failing_sdk_call)
