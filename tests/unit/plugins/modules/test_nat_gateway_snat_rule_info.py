...
from ansible_collections.susunola.tencentcloud.plugins.modules import nat_gateway_snat_rule_info
...
def test_build_request_sets_pagination():
    request = nat_gateway_snat_rule_info.build_request(FakeModels, "sample", {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = nat_gateway_snat_rule_info.build_request(FakeModels, "sample", {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.SourceIpTranslationNatRuleSet = items
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(nat_gateway_snat_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(nat_gateway_snat_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(nat_gateway_snat_rule_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        nat_gateway_snat_rule_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                nat_gateway_id="sample", filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["snat_rules"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(nat_gateway_snat_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(nat_gateway_snat_rule_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(nat_gateway_snat_rule_info, "sdk_call", failing_sdk_call)
