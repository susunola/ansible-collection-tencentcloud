...
``cos_bucket_object_lock_info``, ``cos_bucket_origin_info``,
``cos_bucket_referer_info``, ``cos_bucket_replication_info`` and
``cos_bucket_response_control_info`` were named by no test file and no
...
than by five copies of the same file.
...
@pytest.mark.parametrize("name,plural,singular", CASES)
