...
from ansible_collections.susunola.tencentcloud.plugins.modules import cam_policy_attachment_info
...
class FakeModels:
    ListEntitiesForPolicyRequest = FakeRequest


def test_build_request_sets_pagination():
    request = cam_policy_attachment_info.build_request(FakeModels, 1, "sample", 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cam_policy_attachment_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cam_policy_attachment_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cam_policy_attachment_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cam_policy_attachment_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                policy_id=1, entity_filter="sample", page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["entities"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({
        "region": "ap-guangzhou",
        "policy_id": 1,
        "entity_filter": "sample",
        "page_size": 2,
    })
    monkeypatch.setattr(cam_policy_attachment_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cam_policy_attachment_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cam_policy_attachment_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cam_policy_attachment_info, "sdk_call", failing_sdk_call)
