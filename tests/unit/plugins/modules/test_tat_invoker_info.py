...
from ansible_collections.susunola.tencentcloud.plugins.modules import tat_invoker_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeInvokersRequest = FakeRequest


def test_build_request_sets_pagination():
    request = tat_invoker_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = tat_invoker_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.InvokerIds == ["x-1"]


def test_build_request_sorts_filters():
    request = tat_invoker_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(tat_invoker_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(tat_invoker_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tat_invoker_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        tat_invoker_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                invoker_ids=None, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["invokers"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(tat_invoker_info, "create_credential", lambda module: object())
    monkeypatch.setattr(tat_invoker_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(tat_invoker_info, "sdk_call", failing_sdk_call)
