...
from ansible_collections.susunola.tencentcloud.plugins.modules import cvm_chc_info
...
def test_build_request_sets_pagination():
    request = cvm_chc_info.build_request(FakeModels, None, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_maps_ids():
    request = cvm_chc_info.build_request(FakeModels, ["x-1"], {}, 0, 100)
    assert request.ChcIds == ["x-1"]


def test_build_request_sorts_filters():
    request = cvm_chc_info.build_request(FakeModels, None, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cvm_chc_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cvm_chc_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_chc_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cvm_chc_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                chc_ids=None, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["chc_hosts"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cvm_chc_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cvm_chc_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cvm_chc_info, "sdk_call", failing_sdk_call)
