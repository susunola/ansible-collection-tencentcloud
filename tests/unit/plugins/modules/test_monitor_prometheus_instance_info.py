...
from ansible_collections.susunola.tencentcloud.plugins.modules import monitor_prometheus_instance_info
...
class FakeModels:
    Filter = FakeFilter
    DescribePrometheusInstancesOverviewRequest = FakeRequest


def test_build_request_sets_pagination():
    request = monitor_prometheus_instance_info.build_request(FakeModels, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = monitor_prometheus_instance_info.build_request(FakeModels, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(monitor_prometheus_instance_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(monitor_prometheus_instance_info, "create_credential", lambda module: object())
    monkeypatch.setattr(monitor_prometheus_instance_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        monitor_prometheus_instance_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(monitor_prometheus_instance_info, "create_credential", lambda module: object())
    monkeypatch.setattr(monitor_prometheus_instance_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(monitor_prometheus_instance_info, "sdk_call", failing_sdk_call)
