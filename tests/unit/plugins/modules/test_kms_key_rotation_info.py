# Generated by scripts/generate_info_modules.py -- regenerate instead of editing.
from __future__ import absolute_import, division, print_function

__metaclass__ = type

import sys
import types

import pytest

from ansible_collections.susunola.tencentcloud.plugins.modules import kms_key_rotation_info


class FakeRequest:
    pass


class FakeModels:
    GetKeyRotationStatusRequest = FakeRequest


def test_build_request_maps_parameters():
    request = kms_key_rotation_info.build_request(FakeModels, "sample", 0, 0)
    assert request.KeyId == "sample"


class FakeResponse:
    """Whole-response stand-in: unpaginated modules serialize the response itself."""

    def __init__(self, payload):
        self._payload = payload
        self.RequestId = "req-single"

    def _serialize(self, allow_none=True):
        return dict(self._payload)


class FakeClient:
    def __init__(self, response):
        self._response = response
        self.requests = []

    def GetKeyRotationStatus(self, request):
        self.requests.append(request)
        return self._response


class ModuleExit(Exception):
    pass


class ModuleFail(Exception):
    def __init__(self, payload):
        self.payload = payload
        super(ModuleFail, self).__init__("module failed: %r" % (payload,))


class FakeModule:
    def __init__(self, params):
        self.params = params
        self.exit_payload = None
        self.fail_payload = None

    def exit_json(self, **kwargs):
        self.exit_payload = kwargs
        raise ModuleExit()

    def fail_json(self, **kwargs):
        self.fail_payload = kwargs
        raise ModuleFail(kwargs)


def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.kms.v20190118")
    service.models = FakeModels
    service.kms_client = types.SimpleNamespace(KmsClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.kms", types.ModuleType("tencentcloud.kms"))
    monkeypatch.setitem(sys.modules, "tencentcloud.kms.v20190118", service)


def _run(monkeypatch, client, **params):
    _inject_sdk(monkeypatch, client)
    fake = FakeModule(params)
    monkeypatch.setattr(kms_key_rotation_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(kms_key_rotation_info, "create_credential", lambda module: object())
    monkeypatch.setattr(kms_key_rotation_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        kms_key_rotation_info.run_module()
    return fake


def test_run_module_returns_the_single_result(monkeypatch):
    client = FakeClient(FakeResponse({"KeyId": "sample"}))
    fake = _run(monkeypatch, client, region="ap-guangzhou", key_id="sample")
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert payload["rotation"] == {"KeyId": "sample"}
    assert payload["request_id"] == "req-single"
    # Unpaginated: exactly one call, no paging fields sent.
    assert len(client.requests) == 1


def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    class FailingClient:
        def GetKeyRotationStatus(self, request):
            raise RuntimeError("api exploded")

    def failing_sdk_call(module, function, request):
        # Mirrors the real sdk_call failure contract pinned in
        # tests/unit/plugins/module_utils/test_tencentcloud.py.
        try:
            return function(request)
        except RuntimeError as exc:
            module.fail_json(
                msg="Tencent Cloud API request failed",
                error=str(exc),
                error_code="UnauthorizedOperation",
                request_id="req-err",
            )

    _inject_sdk(monkeypatch, FailingClient())
    fake = FakeModule({"region": "ap-guangzhou", "key_id": "sample"})
    monkeypatch.setattr(kms_key_rotation_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(kms_key_rotation_info, "create_credential", lambda module: object())
    monkeypatch.setattr(kms_key_rotation_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(kms_key_rotation_info, "sdk_call", failing_sdk_call)
    with pytest.raises(ModuleFail) as excinfo:
        kms_key_rotation_info.run_module()
    payload = excinfo.value.payload
    assert payload["msg"] == "Tencent Cloud API request failed"
    assert payload["error_code"] == "UnauthorizedOperation"
    assert payload["request_id"] == "req-err"
