...
from ansible_collections.susunola.tencentcloud.plugins.modules import cfs_snapshot_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeCfsSnapshotsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = cfs_snapshot_info.build_request(FakeModels, "sample", "sample", {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = cfs_snapshot_info.build_request(FakeModels, "sample", "sample", {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cfs_snapshot_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cfs_snapshot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cfs_snapshot_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cfs_snapshot_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                file_system_id="sample", snapshot_id="sample", filters={}, page_size=2)
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cfs_snapshot_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cfs_snapshot_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cfs_snapshot_info, "sdk_call", failing_sdk_call)
