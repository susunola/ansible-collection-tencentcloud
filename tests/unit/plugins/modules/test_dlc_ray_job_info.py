from ansible_collections.susunola.tencentcloud.plugins.modules.dlc_ray_job_info import (
    event_read,
    event_request,
    history_request,
    id_request,
...
def params():
    return {"ray_job_id": "job-1", "page_size": 2, "max_pages": 3, "start_time": 10, "end_time": 20, "event_type": "Warning"}
...
def test_requests_keep_job_identity_and_diagnostic_bounds():
    ...
    pods = pods_request(Models, params(), 3)
    assert pods.Page == 3 and pods.StartTime == 10
    event = event_request(Models, params(), "ctx")
    assert event.Context == "ctx" and event.EventType == "Warning"
...
class Response:
    def __init__(self, values, pages):
        self.Items, self.TotalPages = [Item(x) for x in values], pages
...
class EventResponse:
    def __init__(self, values, context, over):
        self.Events, self.Context, self.ListOver = [Item(x) for x in values], context, over


class Client:
    def __init__(self):
        self.responses = [EventResponse(["a"], "c2", False), EventResponse(["b"], None, True)]
