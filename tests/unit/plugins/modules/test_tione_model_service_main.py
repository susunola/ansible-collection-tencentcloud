...
# ---------------------------------------------------------------------------
# legacy helper regression tests (folded from test_tione_model_service.py)
# ---------------------------------------------------------------------------
...
class LegacyModels(object):
    CreateModelServiceRequest = ModifyModelServiceRequest = ModelInfo = ImageInfo = EnvVar = ResourceInfo = HorizontalPodAutoscaler = LogConfig = Tag = (
        CronScaleJob
    ) = ScheduledAction = VolumeMount = ServiceLimit = ServiceEIP = HealthProbe = RollingUpdate = ResourceSupplyAttribute = LegacyObject


def legacy_params():
    result = {key: None for key in mod.FIELDS}
    result.update(
        project_id="p1",
        service_id="s1",
        service_group_id="g1",
...
def test_normalize_flattens_service_info_and_sorts_tags():
    value = mod.normalize({"ServiceId": "s1", "Tags": [{"TagKey": "z"}, {"TagKey": "a"}], "ServiceInfo": {"Replicas": 2}})
...
def test_modify_request_excludes_create_only_fields():
    request = mod.modify_request(LegacyModels, legacy_params())
