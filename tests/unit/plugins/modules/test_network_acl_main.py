...
# ---------------------------------------------------------------------------
# legacy helper regression tests (folded from test_network_acl.py)
# ---------------------------------------------------------------------------
...
def test_build_requests_and_normalization():
    ...
    request = mod.build_entries_request(models, "acl-1", values, [])
...
def test_rule_and_subnet_canonicalization():
    ...
    assert mod._rules(rules)[0]["cidr"] == "0.0.0.0/0"
    assert mod._subnets([{"SubnetId": "subnet-b"}, {"SubnetId": "subnet-a"}]) == ["subnet-a", "subnet-b"]
