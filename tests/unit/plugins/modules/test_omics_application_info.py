...
from ansible_collections.susunola.tencentcloud.plugins.modules import omics_application_info
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.omics.v20221128")
    service.models = FakeModels
    service.omics_client = types.SimpleNamespace(OmicsClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.omics",
                        types.ModuleType("tencentcloud.omics"))
    monkeypatch.setitem(sys.modules, "tencentcloud.omics.v20221128", service)


def _run(monkeypatch, client, **params):
    _inject_sdk(monkeypatch, client)
    fake = FakeModule(params)
    monkeypatch.setattr(omics_application_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(omics_application_info, "create_credential", lambda module: object())
    monkeypatch.setattr(omics_application_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        omics_application_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(omics_application_info, "create_credential", lambda module: object())
    monkeypatch.setattr(omics_application_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(omics_application_info, "sdk_call", failing_sdk_call)
