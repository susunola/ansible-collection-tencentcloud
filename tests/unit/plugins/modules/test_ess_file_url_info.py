...
from ansible_collections.susunola.tencentcloud.plugins.modules import ess_file_url_info
...
def _inject_sdk(monkeypatch, client):
    service = types.ModuleType("tencentcloud.ess.v20201111")
    service.models = FakeModels
    service.ess_client = types.SimpleNamespace(EssClient=lambda *args: client)
    monkeypatch.setitem(sys.modules, "tencentcloud", types.ModuleType("tencentcloud"))
    monkeypatch.setitem(sys.modules, "tencentcloud.ess",
                        types.ModuleType("tencentcloud.ess"))
    monkeypatch.setitem(sys.modules, "tencentcloud.ess.v20201111", service)


def _run(monkeypatch, client, **params):
    _inject_sdk(monkeypatch, client)
    fake = FakeModule(params)
    monkeypatch.setattr(ess_file_url_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(ess_file_url_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ess_file_url_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        ess_file_url_info.run_module()
    return fake
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(ess_file_url_info, "create_credential", lambda module: object())
    monkeypatch.setattr(ess_file_url_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(ess_file_url_info, "sdk_call", failing_sdk_call)
