...
from ansible_collections.susunola.tencentcloud.plugins.modules import sms_template_info
...
class FakeModels:
    DescribeSmsTemplateListRequest = FakeRequest


def test_build_request_sets_pagination():
    request = sms_template_info.build_request(FakeModels, ["sample"], 1, 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.DescribeTemplateStatusSet = items
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(sms_template_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(sms_template_info, "create_credential", lambda module: object())
    monkeypatch.setattr(sms_template_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        sms_template_info.run_module()
    return fake
...
def test_run_module_paginates_until_short_page(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                template_ids=["sample"], international=1, page_size=2)
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(sms_template_info, "create_credential", lambda module: object())
    monkeypatch.setattr(sms_template_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(sms_template_info, "sdk_call", failing_sdk_call)
