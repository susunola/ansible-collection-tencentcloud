...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.cdn",
                        types.ModuleType("tencentcloud.cdn"))
...
def test_run_module_returns_full_list(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou", channel="sample")
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["cls_log_topics"]] == ["a", "b"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    fake = FakeModule({"region": "ap-guangzhou", "channel": "sample"})
    monkeypatch.setattr(cdn_cls_log_topic_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cdn_cls_log_topic_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cdn_cls_log_topic_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cdn_cls_log_topic_info, "sdk_call", failing_sdk_call)
