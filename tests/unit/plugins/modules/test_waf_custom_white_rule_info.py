...
from ansible_collections.susunola.tencentcloud.plugins.modules import waf_custom_white_rule_info
...
class FakeModels:
    DescribeCustomWhiteRuleRequest = FakeRequest


def test_build_request_sets_pagination():
    request = waf_custom_white_rule_info.build_request(FakeModels, "sample", 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(waf_custom_white_rule_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(waf_custom_white_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(waf_custom_white_rule_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        waf_custom_white_rule_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["custom_white_rules"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(waf_custom_white_rule_info, "create_credential", lambda module: object())
    monkeypatch.setattr(waf_custom_white_rule_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(waf_custom_white_rule_info, "sdk_call", failing_sdk_call)
