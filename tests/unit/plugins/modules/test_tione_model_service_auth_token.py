from ansible_collections.susunola.tencentcloud.plugins.modules.tione_model_service_auth_token import (
    create_request,
    delete_request,
    desired,
    find,
...
class Models:
    CreateModelServiceAuthTokenRequest = ModifyModelServiceAuthTokenRequest = DeleteModelServiceAuthTokenRequest = AuthToken = Object
...
def test_find_prefers_stable_id_and_rejects_duplicate_names():
    ...
    try:
        find(Module(), TOKENS * 2, dict(P, token_id=None))
    except ValueError:
        pass
    else:
...
def test_requests_keep_group_workspace_and_secret_identity():
    ...
    modify = modify_request(Models, P, TOKENS[0], True)
    delete = delete_request(Models, P, TOKENS[0])
