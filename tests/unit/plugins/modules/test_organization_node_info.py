...
from ansible_collections.susunola.tencentcloud.plugins.modules import organization_node_info
...
class FakeModels:
    DescribeOrganizationNodesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = organization_node_info.build_request(FakeModels, 200, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(organization_node_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(organization_node_info, "create_credential", lambda module: object())
    monkeypatch.setattr(organization_node_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        organization_node_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["nodes"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(organization_node_info, "create_credential", lambda module: object())
    monkeypatch.setattr(organization_node_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(organization_node_info, "sdk_call", failing_sdk_call)
