...
from ansible_collections.susunola.tencentcloud.plugins.modules import dlc_network_connection_info
...
class FakeModels:
    DescribeNetworkConnectionsRequest = FakeRequest


def test_build_request_sets_pagination():
    request = dlc_network_connection_info.build_request(FakeModels, 200, 100)
...
class FakeResponse:
    def __init__(self, items, total_count):
        self.NetworkConnectionSet = items
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(dlc_network_connection_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(dlc_network_connection_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dlc_network_connection_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        dlc_network_connection_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    assert [item["Marker"] for item in payload["network_connections"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(dlc_network_connection_info, "create_credential", lambda module: object())
    monkeypatch.setattr(dlc_network_connection_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(dlc_network_connection_info, "sdk_call", failing_sdk_call)
