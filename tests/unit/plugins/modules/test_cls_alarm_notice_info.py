...
from ansible_collections.susunola.tencentcloud.plugins.modules import cls_alarm_notice_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeAlarmNoticesRequest = FakeRequest


def test_build_request_sets_pagination():
    request = cls_alarm_notice_info.build_request(FakeModels, True, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = cls_alarm_notice_info.build_request(FakeModels, True, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _inject_sdk(monkeypatch, client):
    ...
    monkeypatch.setitem(sys.modules, "tencentcloud.cls",
                        types.ModuleType("tencentcloud.cls"))
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(cls_alarm_notice_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(cls_alarm_notice_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cls_alarm_notice_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        cls_alarm_notice_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                has_alarm_shield_count=True, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["alarm_notices"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(cls_alarm_notice_info, "create_credential", lambda module: object())
    monkeypatch.setattr(cls_alarm_notice_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(cls_alarm_notice_info, "sdk_call", failing_sdk_call)
