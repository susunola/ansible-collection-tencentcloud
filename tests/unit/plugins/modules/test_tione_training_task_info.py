...
class Models:
    DescribeTrainingTaskRequest = DescribeTrainingTasksRequest = Filter = TagFilter = Object
...
class Response:
    def __init__(self, values, total, request_id):
        self.TrainingTaskSet, self.TotalCount, self.RequestId = [Item(x) for x in values], total, request_id


class Client:
    def __init__(self):
        self.responses = [Response(["t1", "t2"], 3, "r1"), Response(["t3"], 3, "r2")]
