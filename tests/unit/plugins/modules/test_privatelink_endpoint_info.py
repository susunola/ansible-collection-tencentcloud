...
from ansible_collections.susunola.tencentcloud.plugins.modules import privatelink_endpoint_info
...
class FakeModels:
    Filter = FakeFilter
    DescribeVpcEndPointRequest = FakeRequest


def test_build_request_sets_pagination():
    request = privatelink_endpoint_info.build_request(FakeModels, ["sample"], "sample", 1, {}, 200, 100)
    assert request.Offset == 200
    assert request.Limit == 100


def test_build_request_sorts_filters():
    request = privatelink_endpoint_info.build_request(FakeModels, ["sample"], "sample", 1, {"b-name": ["v1"], "a-name": ["v2"]}, 0, 100)
...
def _run(monkeypatch, client, **params):
    ...
    monkeypatch.setattr(privatelink_endpoint_info, "AnsibleModule", lambda **kwargs: fake)
    monkeypatch.setattr(privatelink_endpoint_info, "create_credential", lambda module: object())
    monkeypatch.setattr(privatelink_endpoint_info, "create_client_profile", lambda module, endpoint: object())
    with pytest.raises(ModuleExit):
        privatelink_endpoint_info.run_module()
    return fake
...
def test_run_module_paginates_until_total_count(monkeypatch):
    ...
    fake = _run(monkeypatch, client, region="ap-guangzhou",
                end_point_id=["sample"], ip_address_type="sample", max_results=1, filters={}, page_size=2)
    payload = fake.exit_payload
    assert payload["changed"] is False
    assert [item["Marker"] for item in payload["end_points"]] == ["a", "b", "c"]
...
def test_run_module_fails_cleanly_on_sdk_error(monkeypatch):
    ...
    monkeypatch.setattr(privatelink_endpoint_info, "create_credential", lambda module: object())
    monkeypatch.setattr(privatelink_endpoint_info, "create_client_profile", lambda module, endpoint: object())
    monkeypatch.setattr(privatelink_endpoint_info, "sdk_call", failing_sdk_call)
