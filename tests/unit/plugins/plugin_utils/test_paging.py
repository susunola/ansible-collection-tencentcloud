...
The paginator lives in ``plugins/plugin_utils/paging.py`` because generated
``_info`` modules, hand-written modules and the inventory plugins all page the
same list APIs. ``tests/unit/plugins/module_utils/test_paging.py`` separately
guards the ``module_utils.paging`` re-export.
"""
...
def test_short_page_without_reported_total_terminates():
    ...
    p = Paginator(3, lambda o, lim: {"offset": o}, call_api, lambda r: r.items, lambda r: r.total)
    items, total = p.fetch_all()
