...
These tests are the guard's non-vacuity proof. Completed items must satisfy
their criteria, and each badge must agree with the measurement in both
directions (so a false claim and a stale claim both fail).
"""
...
def test_the_real_repo_passes_every_completed_criterion(measured, marked):
    failures = {item: detail for item, (ok, detail) in measured.items()
                if marked[item] and not ok}
    assert failures == {}, "completed items with unmet acceptance: %s" % failures
